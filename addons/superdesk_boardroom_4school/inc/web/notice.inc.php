...
$total = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename('superdesk_boardroom_4school_s_feedback') . $where, $paras);
$list = pdo_fetchall("SELECT * FROM " . tablename('superdesk_boardroom_4school_s_feedback') . $where . " ORDER BY id DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize, $paras);
...
if (!empty($transaction_ids)) {
    $sql = "SELECT * FROM " . tablename('superdesk_boardroom_4school_s_order') . " WHERE uniacid='{$_W['uniacid']}' AND transaction_id IN ( '" . implode("','", $transaction_ids) . "' )";
