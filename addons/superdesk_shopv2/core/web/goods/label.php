...
class Label_SuperdeskShopV2Page extends WebPage
{
    public function main()
    {
        ...
        $label = pdo_fetchall(
            'SELECT id,uniacid,label,labelname,status,displayorder ' .
    ...
    protected function post()
    {
        ...
        if ($_W['ispost']) {
            ...
            $data = array(
                ...
                'labelname' => json_encode(array_filter($_GPC['labelname'])),
                'status'    => intval($_GPC['status'])
            );

            if (!empty($item)) {

                pdo_update('superdesk_shop_goods_label', $data, array('id' => $item['id']));
                plog('goods.label.edit', '修改标签组 ID: ' . $id);

            } else {

                $data['uniacid'] = $uniacid;
                pdo_insert('superdesk_shop_goods_label', $data);
                $id = pdo_insertid();
            ...
            show_json(1, array(
                'url' => webUrl('goods/label/edit', array('id' => $id))
            ));
        }
    ...
    public function delete()
    {
        ...
        foreach ($items as $item) {
            pdo_delete('superdesk_shop_goods_label', array('id' => $item['id']));
