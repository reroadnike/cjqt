...
class Index_SuperdeskShopV2Page extends PluginWebPage
{
    public function main()
    {
        ...
        $articles = pdo_fetchall(
            'SELECT ' .
            '       a.id,a.displayorder, a.article_title,a.article_category,a.article_keyword2,a.article_date,a.article_readnum,a.article_likenum,a.article_state,' .
            '       c.category_name ' .
            ...
            $select_title .
            $select_category .
            ' order by displayorder desc,article_date desc ' .
        ...
        $articlenum = pdo_fetchcolumn(
            'SELECT COUNT(*) ' .
            ' FROM ' . tablename('superdesk_shop_article') .
            ' WHERE ' .
            '       uniacid= :uniacid ',
            array(
                ':uniacid' => $_W['uniacid']
        ...
        $categorys = pdo_fetchall(
            'SELECT * ' .
            ' FROM ' . tablename('superdesk_shop_article_category') .
            ' WHERE ' .
            '       uniacid=:uniacid ',
            array(
                ':uniacid' => $_W['uniacid']
    ...
    protected function post()
    {
        ...
        if ($_W['ispost']) {
            ...
            if (empty($aid)) {
                ...
                pdo_insert('superdesk_shop_article', $data);
                ...
                $articlekeyword = pdo_fetchcolumn(
                    'SELECT article_keyword2 ' .
                    ' FROM ' . tablename('superdesk_shop_article') .
                    ' WHERE ' .
                    '       id=:aid ' .
                    '       and uniacid=:uniacid ' .
            ...
            $rule = pdo_fetch(
                ...
                array(
                    ...
                    ':name'    => 'superdesk_shopv2:article:' . $aid
                )
            );
            ...
            show_json(1, array(
                'url' => webUrl('article/edit', array('aid' => $aid, 'tab' => str_replace('#tab_', '', $_GPC['tab'])))
            ));
        }
    ...
    public function delete()
    {
        ...
        foreach ($items as $item) {
            ...
            pdo_delete('superdesk_shop_article_log', array('aid' => $item['id']));
            pdo_delete('superdesk_shop_article_share', array('aid' => $item['id']));
    ...
    public function state()
    {
        ...
        foreach ($items as $item) {
            ...
            pdo_update(
                'superdesk_shop_article',
                array(
                    'article_state' => intval($_GPC['state'])
                ),
                array(
                    'id' => $item['id']
                )
