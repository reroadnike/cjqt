...
class Batch_SuperdeskShopV2Page extends PluginWebPage
{
    private $_orderModel;
        ...
        if ($_W['ispost']) {
            ...
            foreach ($orders as $i => $order) {
                ...
                $order_goods = pdo_fetchall(
                    ...
                    ' left join ' . tablename('superdesk_shop_goods') . ' g on g.id=og.goodsid ' .
                    ' where og.uniacid=:uniacid and og.merchid=0 and og.orderid=:orderid ',
                    array(
                        ':uniacid' => $_W['uniacid'],
                        ':orderid' => $order['id']
                    )
    ...
    public function getorderinfo()
    {
        ...
        if ($_W['ispost']) {
            ...
            $printTemp = pdo_fetch('SELECT id,type,expressname,express,expresscom FROM ' . tablename('superdesk_shop_exhelper_express') . ' WHERE id=:id and type=:type and uniacid=:uniacid and merchid=0 limit 1', array(':id' => $temp_express, ':type' => 1, ':uniacid' => $_W['uniacid']));
