...
class Orders_SuperdeskShopV2Page extends PluginMobileLoginPage
{
    ...
    public function detail()
    {
        ...
        if (!empty($order['isverify'])) {
            ...
            if (empty($storeids)) {
                if (0 < $merchid) {
                    $stores = pdo_fetchall('select * from ' . tablename('superdesk_shop_merch_store') . ' where  uniacid=:uniacid and merchid=:merchid and status=1 and type in(2,3)', array(':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
                } else {
                    $stores = pdo_fetchall('select * from ' . tablename('superdesk_shop_store') . ' where  uniacid=:uniacid and status=1 and type in(2,3)', array(':uniacid' => $_W['uniacid']));
                }
            } else if (0 < $merchid) {
                $stores = pdo_fetchall('select * from ' . tablename('superdesk_shop_merch_store') . ' where id in (' . implode(',', $storeids) . ') and uniacid=:uniacid and merchid=:merchid and status=1 and type in(2,3)', array(':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
            } else {
                $stores = pdo_fetchall('select * from ' . tablename('superdesk_shop_store') . ' where id in (' . implode(',', $storeids) . ') and uniacid=:uniacid and status=1 and type in(2,3)', array(':uniacid' => $_W['uniacid']));
            }
            $verifytotal = pdo_fetchcolumn('select count(1) from ' . tablename('superdesk_shop_groups_verify') . ' where orderid = :orderid and openid = :openid and uniacid = :uniacid and verifycode = :verifycode ', array(':orderid' => $order['id'], ':openid' => $order['openid'], ':uniacid' => $order['uniacid'], ':verifycode' => $order['verifycode']));
            if ($order['verifytype'] == 0) {
                $verify = pdo_fetch('select isverify from ' . tablename('superdesk_shop_groups_verify') . ' where orderid = :orderid and openid = :openid and uniacid = :uniacid and verifycode = :verifycode ', array(':orderid' => $order['id'], ':openid' => $order['openid'], ':uniacid' => $order['uniacid'], ':verifycode' => $order['verifycode']));
            }
    ...
    public function express()
    {
        ...
        $order = pdo_fetch('select * from ' . tablename('superdesk_shop_groups_order') . ' where id=:id and uniacid=:uniacid and openid=:openid limit 1', array(':id' => $orderid, ':uniacid' => $uniacid, ':openid' => $openid));
        if (empty($order)) {
        ...
        $goods       = pdo_fetch('select *  from ' . tablename('superdesk_shop_groups_goods') . '  where id=:id and uniacid=:uniacid ', array(':uniacid' => $uniacid, ':id' => $order['goodid']));
    ...
    public function cancel()
    {
        ...
        try {
            $orderid = intval($_GPC['id']);
            $order   = pdo_fetch('select id,orderno,openid,status,credit,teamid,groupnum,creditmoney,price,freight,pay_type,discount,success from ' . tablename('superdesk_shop_groups_order') . ' where id=:id and uniacid=:uniacid and openid=:openid limit 1', array(':id' => $orderid, ':uniacid' => $_W['uniacid'], ':openid' => $_W['openid']));
            $total   = pdo_fetchcolumn('select count(1) from ' . tablename('superdesk_shop_groups_order') . '  where teamid = :teamid  ', array(':teamid' => $order['teamid']));
            if (empty($order)) {
                show_json(0, '订单未找到');
            }
            if ($order['status'] != 0) {
            ...
            pdo_update('superdesk_shop_groups_order', array('status' => -1, 'canceltime' => time()), array('id' => $order['id'], 'uniacid' => $_W['uniacid']));
    ...
    public function delete()
    {
        ...
        global $_GPC;
        $orderid = intval($_GPC['id']);
        $order   = pdo_fetch('select id,status from ' . tablename('superdesk_shop_groups_order') . ' where id=:id and uniacid=:uniacid and openid=:openid limit 1', array(':id' => $orderid, ':uniacid' => $_W['uniacid'], ':openid' => $_W['openid']));
        if (empty($order)) {
            show_json(0, '订单未找到!');
        }
        ...
        pdo_update('superdesk_shop_groups_order', array('deleted' => 1), array('id' => $order['id'], 'uniacid' => $_W['uniacid']));
        show_json(1);
    }
    ...
    public function get_list()
    {
        ...
        $total  = pdo_fetchcolumn('select count(1) from ' . tablename('superdesk_shop_groups_order') . ' as o where 1 ' . $condition, $params);
        foreach ($orders as $key => $value) {
            $verifytotal = pdo_fetchcolumn('select count(1) from ' . tablename('superdesk_shop_groups_verify') . ' where orderid = :orderid and openid = :openid and uniacid = :uniacid and verifycode = :verifycode ', array(':orderid' => $value['id'], ':openid' => $value['openid'], ':uniacid' => $value['uniacid'], ':verifycode' => $value['verifycode']));
            if (!$verifytotal) {
                $verifytotal = 0;
            }
    ...
    public function confirm()
    {
        ...
        try {
            ...
            $ordernum = pdo_fetchcolumn('select count(1) from ' . tablename('superdesk_shop_groups_order') . ' as o' . "\n\t\t\t" . 'where openid = :openid and status >= :status and goodid = :goodid and uniacid = :uniacid ', array(':openid' => $_W['openid'], ':status' => 0, ':goodid' => $goodid, ':uniacid' => $uniacid));
            if (!empty($goods['purchaselimit']) && ($goods['purchaselimit'] <= $ordernum)) {
                throw new Exception('您已到达此商品购买上限，请浏览其他商品或联系商家！');
            }
            $order = pdo_fetch('select * from ' . tablename('superdesk_shop_groups_order') . "\n\t\t\t\t\t" . 'where goodid = :goodid and status >= 0 and is_team = 1 and openid = :openid and uniacid = :uniacid and success = 0 and deleted = 0 ', array(':goodid' => $goodid, ':openid' => $_W['openid'], ':uniacid' => $uniacid));
            ...
            if (!empty($teamid)) {
                ...
                $num = pdo_fetchcolumn('select count(1) from ' . tablename('superdesk_shop_groups_order') . ' as o where teamid = :teamid and status > :status and goodid = :goodid and uniacid = :uniacid ', array(':teamid' => $teamid, ':status' => 0, ':goodid' => $goods['id'], ':uniacid' => $uniacid));
            ...
            $set = pdo_fetch('select discount,headstype,headsmoney,headsdiscount from ' . tablename('superdesk_shop_groups_set') . "\n\t\t\t\t\t" . 'where uniacid = :uniacid ', array(':uniacid' => $uniacid));
            ...
            if (!empty($goods['isverify'])) {
                ...
                while (1) {
                    $count = pdo_fetchcolumn('select count(*) from ' . tablename('superdesk_shop_groups_order') . ' where verifycode=:verifycode and uniacid=:uniacid limit 1', array(':verifycode' => $verifycode, ':uniacid' => $_W['uniacid']));
                    if ($count <= 0) {
                        break;
                    }
                    $verifycode = 'PT' . random(8, true);
            ...
            $creditdeduct = pdo_fetch(
                'SELECT '.
                '       creditdeduct,groupsdeduct,credit,groupsmoney '.
    ...
    public function finish()
    {
        ...
        global $_GPC;
        $orderid = intval($_GPC['id']);
        $order   = pdo_fetch('select * from ' . tablename('superdesk_shop_groups_order') . ' where id=:id and uniacid=:uniacid and openid=:openid limit 1', array(':id' => $orderid, ':uniacid' => $_W['uniacid'], ':openid' => $_W['openid']));
        if (empty($order)) {
            show_json(0, '订单未找到');
        }
        if ($order['status'] != 2) {
        ...
        if ((0 < $order['refundstate']) && !empty($order['refundid'])) {
            ...
            pdo_update('superdesk_shop_groups_order_refund', $change_refund, array('id' => $order['refundid'], 'uniacid' => $_W['uniacid']));
        }
        pdo_update('superdesk_shop_groups_order', array('status' => 3, 'finishtime' => time(), 'refundstate' => 0), array('id' => $order['id'], 'uniacid' => $_W['uniacid']));
        p('groups')->sendTeamMessage($orderid);
        show_json(1);
    }
}
