...
class Set_SuperdeskShopV2Page extends PluginWebPage
{
    public function main()
    {
        ...
        $set     = pdo_fetch('SELECT * FROM ' . tablename('superdesk_shop_groups_set') . ' WHERE uniacid = :uniacid ', array(':uniacid' => $uniacid));
        ...
        if ($_W['ispost']) {
            ...
            if (!empty($keyword)) {
                if ($keyword['name'] != 'superdesk_shopv2:groups') {
                    show_json(0, '关键字已存在!');
                }
            ...
            $rule = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name  limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'superdesk_shopv2', ':name' => 'superdesk_shopv2:groups'));

            if (empty($rule)) {
                $rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'superdesk_shopv2:groups', 'module' => 'superdesk_shopv2', 'displayorder' => 0, 'status' => 1);
                pdo_insert('rule', $rule_data);
                $rid          = pdo_insertid();
                $keyword_data = array('uniacid' => $_W['uniacid'], 'rid' => $rid, 'module' => 'superdesk_shopv2', 'content' => trim($exchangekeyword), 'type' => 1, 'displayorder' => 0, 'status' => 1);
            ...
            if (!empty($set)) {
                $set_update = pdo_update('superdesk_shop_groups_set', $data, array('id' => $set['id'], 'uniacid' => $uniacid));
            } else {
                $set_insert = pdo_insert('superdesk_shop_groups_set', $data);
            }

            pdo_update('superdesk_shop_groups_goods', array('rights' => 1), array('uniacid' => $uniacid));
            $goodsid = explode(',', $data['goodsid']);

            foreach ($goodsid as $value) {
                $goods_update = pdo_update('superdesk_shop_groups_goods', array('rights' => 0), array('id' => intval($value), 'uniacid' => $uniacid));
            }
        ...
        $data2    = $this->set;
        $data     = pdo_fetch('SELECT * FROM ' . tablename('superdesk_shop_groups_set') . ' WHERE uniacid = :uniacid ', array(':uniacid' => $uniacid));

        if ($data['goodsid']) {
            $goods = pdo_fetchall('SELECT id,title,thumb FROM ' . tablename('superdesk_shop_groups_goods') . "\r\n" . '                    WHERE uniacid = :uniacid and id in (' . $data['goodsid'] . ') ', array(':uniacid' => $uniacid));
        }
    ...
    public function query()
    {
        ...
        $ds = pdo_fetchall('SELECT id,title,rights,thumb FROM ' . tablename('superdesk_shop_groups_goods') . ' WHERE 1 ' . $condition . ' order by createtime desc', $params);
