...
class Exchange_SuperdeskShopV2Page extends PluginMobileLoginPage
{
    ...
    public function check()
    {
        ...
        $id      = intval($_GPC['id']);
        $log     = pdo_fetch(
            'select id,status from ' . tablename('superdesk_shop_creditshop_log') . // TODO 标志 楼宇之窗 openid shop_creditshop_log 已处理
    ...
    public function qrcode()
    {
        ...
        $id      = intval($_GPC['id']);
        $log     = pdo_fetch(
            'select id,eno from ' . tablename('superdesk_shop_creditshop_log') . // TODO 标志 楼宇之窗 openid shop_creditshop_log 已处理
            ' where id=:id and uniacid=:uniacid and openid=:openid and core_user=:core_user limit 1',
            array(
                ':id' => $id,
                ':uniacid' => $uniacid,
