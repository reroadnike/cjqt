...
class Refund_SuperdeskShopV2Page extends WebPage
{
    private $_orderModel;
    ...
    public function submit()
    {
        ...
        if ($_W['ispost']) {
            ...
            if ($refundstatus == 0) {
                ...
                $goods     = pdo_fetchall(
                    ' SELECT g.id,g.credit, o.total,o.realprice ' .
...
//                paytype	支付类型 0为未支付 1为余额支付 2在线支付 3 企业月结 11 后台付款 21 微信支付 22 支付宝支付 23 银联支付
                ...
                if ($item['paytype'] == 1) { // 1为余额支付 如有余额抵扣： 会返回金额到商城用户余额
                ...
                } else if ($item['paytype'] == 21) { // 21 微信支付 微信支付方式： 会返回到相应的的支付渠道(如零钱或银行卡）
                ...
                if (0 < $credits) {
                    ...
                    m('member')->setCredit($item['openid'], $item['core_user'],
                        ...
                        array(
                            0,
                            $shopset['name'] . '退款扣除购物赠送积分: ' . $credits . ' 订单号: ' . $item['ordersn']
                        )
                    );
                }
                ...
                pdo_update('superdesk_shop_order_refund', $change_refund, array('id' => $item['refundid']));
                m('order')->setGiveBalance($item['id'], 2);
    ...
    public function main()
    {
        ...
        $goods = pdo_fetchall(
            'SELECT ' .
            '   g.*, ' .
            '   o.goodssn as option_goodssn, o.productsn as option_productsn,o.total,g.type,o.optionname,o.optionid,' .
            '   o.price as orderprice,o.realprice,o.changeprice,o.oldprice,o.commission1,o.commission2,o.commission3,o.commissions' .
            $diyformfields .
            ' FROM ' . tablename('superdesk_shop_order_goods') . ' o ' .// TODO 标志 楼宇之窗 openid shop_order_goods 不处理
            '   left join ' . tablename('superdesk_shop_goods') . ' g on o.goodsid=g.id ' .
            ' WHERE o.orderid=:orderid and o.uniacid=:uniacid',
            array(
                ':orderid' => $id,
                ':uniacid' => $_W['uniacid']
            )
