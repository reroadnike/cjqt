...
if (!class_exists('CommissionModel')) {
    class CommissionModel extends PluginModel
    {
        ...
        public function getSet($uniacid = 0)
        {
            ...
            $set['texts'] = array('agent' => (empty($set['texts']['agent']) ? '营销商' : $set['texts']['agent']), 'shop' => (empty($set['texts']['shop']) ? '小店' : $set['texts']['shop']), 'myshop' => (empty($set['texts']['myshop']) ? '我的小店' : $set['texts']['myshop']), 'center' => (empty($set['texts']['center']) ? '营销中心' : $set['texts']['center']), 'become' => (empty($set['texts']['become']) ? '成为营销商' : $set['texts']['become']), 'withdraw' => (empty($set['texts']['withdraw']) ? '提现' : $set['texts']['withdraw']), 'commission' => (empty($set['texts']['commission']) ? '佣金' : $set['texts']['commission']), 'commission1' => (empty($set['texts']['commission1']) ? '营销佣金' : $set['texts']['commission1']), 'commission_total' => (empty($set['texts']['commission_total']) ? '累计佣金' : $set['texts']['commission_total']), 'commission_ok' => (empty($set['texts']['commission_ok']) ? '可提现佣金' : $set['texts']['commission_ok']), 'commission_apply' => (empty($set['texts']['commission_apply']) ? '已申请佣金' : $set['texts']['commission_apply']), 'commission_check' => (empty($set['texts']['commission_check']) ? '待打款佣金' : $set['texts']['commission_check']), 'commission_lock' => (empty($set['texts']['commission_lock']) ? '未结算佣金' : $set['texts']['commission_lock']), 'commission_detail' => (empty($set['texts']['commission_detail']) ? '提现明细' : (($set['texts']['commission_detail'] == '佣金明细' ? '提现明细' : $set['texts']['commission_detail']))), 'commission_pay' => (empty($set['texts']['commission_pay']) ? '成功提现佣金' : $set['texts']['commission_pay']), 'commission_wait' => (empty($set['texts']['commission_wait']) ? '待收货佣金' : $set['texts']['commission_wait']), 'commission_fail' => (empty($set['texts']['commission_fail']) ? '无效佣金' : $set['texts']['commission_fail']), 'commission_charge' => (empty($set['texts']['commission_charge']) ? '扣除个人所得税' : $set['texts']['commission_charge']), 'order' => (empty($set['texts']['order']) ? '营销订单' : $set['texts']['order']), 'c1' => (empty($set['texts']['c1']) ? '一级' : $set['texts']['c1']), 'c2' => (empty($set['texts']['c2']) ? '二级' : $set['texts']['c2']), 'c3' => (empty($set['texts']['c3']) ? '三级' : $set['texts']['c3']), 'mydown' => (empty($set['texts']['mydown']) ? '我的下线' : $set['texts']['mydown']), 'down' => (empty($set['texts']['down']) ? '下线' : $set['texts']['down']), 'up' => (empty($set['texts']['up']) ? '推荐人' : $set['texts']['up']), 'yuan' => (empty($set['texts']['yuan']) ? '元' : $set['texts']['yuan']));
            return $set;
        }
        ...
        public function calculate($orderid = 0, $update = true)
        {
            ...
            $order  = pdo_fetch(
                'select agentid,price,goodsprice,deductcredit2,discountprice,isdiscountprice,dispatchprice,changeprice ' .
                ' from ' . tablename('superdesk_shop_order') . ' where id=:id limit 1',// TODO 标志 楼宇之窗 openid shop_order 不处理
                array(
                    ':id' => $orderid
                )
            );
            ...
            $goods   = pdo_fetchall(
                'select og.id,og.realprice,og.total,g.hasoption,og.optionid,g.hascommission,g.nocommission, ' .
                '       g.commission1_rate,g.commission1_pay,g.commission2_rate,g.commission2_pay,g.commission3_rate,g.commission3_pay, ' .
                '       g.commission,og.commissions ' .
        ...
        public function getOrderCommissions($orderid = 0, $ogid = 0)
        {
            ...
            $goods = pdo_fetch(
                'select commission1,commission2,commission3 from ' . tablename('superdesk_shop_order_goods') . // TODO 标志 楼宇之窗 openid shop_order_goods 不处理
                ' where id=:id and orderid=:orderid and uniacid=:uniacid and nocommission=0 limit 1',
                array(
                    ':id'      => $ogid,
                    ':orderid' => $orderid,
                    ':uniacid' => $_W['uniacid']
        ...
        public function getInfo($openid, $options = NULL)
        {
            ...
            if (1 <= $level) {
                if (in_array('ordercount0', $options)) {
                    $level1_ordercount = pdo_fetch(
                        'select sum(og.realprice) as ordermoney,count(distinct o.id) as ordercount ' .
                        ' from ' . tablename('superdesk_shop_order') . ' o ' . // TODO 标志 楼宇之窗 openid shop_order 不处理
                        ' left join  ' . tablename('superdesk_shop_order_goods') . ' og on og.orderid=o.id ' .// TODO 标志 楼宇之窗 openid shop_order_goods 不处理
                        ' where o.agentid=:agentid and o.status>=0 and og.status1>=0 and og.nocommission=0 and o.uniacid=:uniacid  limit 1',
                        array(
                            ':uniacid' => $_W['uniacid'],
                            ':agentid' => $member['id']
                        )
                ...
                if (in_array('ordercount', $options)) {
                    $level1_ordercount = pdo_fetch(
                        ...
                        ' from ' . tablename('superdesk_shop_order') . ' o ' . // TODO 标志 楼宇之窗 openid shop_order 不处理
                        ' left join  ' . tablename('superdesk_shop_order_goods') . ' og on og.orderid=o.id ' .// TODO 标志 楼宇之窗 openid shop_order_goods 不处理
                        ' where o.agentid=:agentid and o.status>=1 and og.status1>=0 and og.nocommission=0 and o.uniacid=:uniacid  limit 1',
                        array(
                            ':uniacid' => $_W['uniacid'],
                            ':agentid' => $member['id']
                        )
                ...
                if (in_array('ordercount3', $options)) {
                    $level1_ordercount3 = pdo_fetch(
                        'select sum(og.realprice) as ordermoney,count(distinct o.id) as ordercount ' .
                        ' from ' . tablename('superdesk_shop_order') . ' o ' . // TODO 标志 楼宇之窗 openid shop_order 不处理
                        ' left join  ' . tablename('superdesk_shop_order_goods') . ' og on og.orderid=o.id ' .// TODO 标志 楼宇之窗 openid shop_order_goods 不处理
                        ' where o.agentid=:agentid and o.status>=3 and og.status1>=0 and og.nocommission=0 and o.uniacid=:uniacid  limit 1',
                        array(
                            ':uniacid' => $_W['uniacid'],
                            ':agentid' => $member['id']
                        )
                ...
                if (in_array('total', $options)) {
                    $level1_commissions = pdo_fetchall(
                        'select og.commission1,og.commissions  from ' . tablename('superdesk_shop_order_goods') . ' og ' .// TODO 标志 楼宇之窗 openid shop_order_goods 不处理
                        ' left join  ' . tablename('superdesk_shop_order') . ' o on o.id = og.orderid' . // TODO 标志 楼宇之窗 openid shop_order 不处理
                        ' where o.agentid=:agentid and o.status>=1 and og.nocommission=0 and o.uniacid=:uniacid',
                        array(
                            ':uniacid' => $_W['uniacid'],
                            ':agentid' => $member['id']
                        )
                ...
                if (in_array('ok', $options)) {
                    $level1_commissions = pdo_fetchall(
                        'select og.commission1,og.commissions  from ' . tablename('superdesk_shop_order_goods') . ' og ' .// TODO 标志 楼宇之窗 openid shop_order_goods 不处理
                        ' left join  ' . tablename('superdesk_shop_order') . ' o on o.id = og.orderid' . // TODO 标志 楼宇之窗 openid shop_order 不处理
                        ' where o.agentid=:agentid and o.status>=3 and og.nocommission=0 and (' . $time . ' - o.finishtime > ' . $day_times . ') and og.status1=0  and o.uniacid=:uniacid',
                        array(
                            ':uniacid' => $_W['uniacid'],
                            ':agentid' => $member['id']
                        )
                ...
                if (in_array('lock', $options)) {
                    $level1_commissions1 = pdo_fetchall(
                        'select og.commission1,og.commissions  from ' . tablename('superdesk_shop_order_goods') . ' og ' .// TODO 标志 楼宇之窗 openid shop_order_goods 不处理
                        ' left join  ' . tablename('superdesk_shop_order') . ' o on o.id = og.orderid' . // TODO 标志 楼宇之窗 openid shop_order 不处理
                        ' where o.agentid=:agentid and o.status>=3 and og.nocommission=0 and (' . $time . ' - o.finishtime <= ' . $day_times . ')  and og.status1=0  and o.uniacid=:uniacid',
                        array(
                ...
                if (in_array('apply', $options)) {
                    $level1_commissions2 = pdo_fetchall(
                        'select og.commission1,og.commissions  from ' . tablename('superdesk_shop_order_goods') . ' og ' .// TODO 标志 楼宇之窗 openid shop_order_goods 不处理
                        ' left join  ' . tablename('superdesk_shop_order') . ' o on o.id = og.orderid' . // TODO 标志 楼宇之窗 openid shop_order 不处理
                        ' where o.agentid=:agentid and o.status>=3 and og.status1=1 and og.nocommission=0 and o.uniacid=:uniacid',
                        array(
                            ':uniacid' => $_W['uniacid'],
                            ':agentid' => $member['id']
                        )
                ...
                if (in_array('check', $options)) {
                    $level1_commissions2 = pdo_fetchall(
                        'select og.commission1,og.commissions  from ' . tablename('superdesk_shop_order_goods') . ' og ' .// TODO 标志 楼宇之窗 openid shop_order_goods 不处理
                        ' left join  ' . tablename('superdesk_shop_order') . ' o on o.id = og.orderid' . // TODO 标志 楼宇之窗 openid shop_order 不处理
                        ' where o.agentid=:agentid and o.status>=3 and og.status1=2 and og.nocommission=0 and o.uniacid=:uniacid ',
                        array(
                            ':uniacid' => $_W['uniacid'],
                            ':agentid' => $member['id']
                        )
                ...
                if (in_array('pay', $options)) {
                    $level1_commissions2 = pdo_fetchall(
                        'select og.commission1,og.commissions  from ' . tablename('superdesk_shop_order_goods') . ' og ' .// TODO 标志 楼宇之窗 openid shop_order_goods 不处理
                        ' left join  ' . tablename('superdesk_shop_order') . ' o on o.id = og.orderid' . // TODO 标志 楼宇之窗 openid shop_order 不处理
                        ' where o.agentid=:agentid and o.status>=3 and og.status1=3 and og.nocommission=0 and o.uniacid=:uniacid ',
                        array(
                            ':uniacid' => $_W['uniacid'],
                            ':agentid' => $member['id']
                        )
                ...
                if (in_array('wait', $options)) {
                    $level1_commissions2 = pdo_fetchall(
                        'select og.commission1,og.commissions  from ' . tablename('superdesk_shop_order_goods') . ' og ' .// TODO 标志 楼宇之窗 openid shop_order_goods 不处理
                        ' left join  ' . tablename('superdesk_shop_order') . ' o on o.id = og.orderid' . // TODO 标志 楼宇之窗 openid shop_order 不处理
                        ' where o.agentid=:agentid and (o.status=2 or o.status=1) and og.status1=0  and og.nocommission=0 and o.uniacid=:uniacid ',
                        array(
                            ':uniacid' => $_W['uniacid'],
                            ':agentid' => $member['id']
                        )
                ...
                $level1_agentids = pdo_fetchall('select id from ' . tablename('superdesk_shop_member') . ' where agentid=:agentid and isagent=1 and status=1 and uniacid=:uniacid ', array(':uniacid' => $_W['uniacid'], ':agentid' => $member['id']), 'id');
            ...
            if (2 <= $level) {
                if (0 < $level1) {
                    if (in_array('ordercount0', $options)) {
                        $level2_ordercount = pdo_fetch(
                            'select sum(og.realprice) as ordermoney,count(distinct o.id) as ordercount ' .
                            ' from ' . tablename('superdesk_shop_order') . ' o ' . // TODO 标志 楼宇之窗 openid shop_order 不处理
                            ' left join  ' . tablename('superdesk_shop_order_goods') . ' og on og.orderid=o.id ' .// TODO 标志 楼宇之窗 openid shop_order_goods 不处理
                            ' where o.agentid in( ' . implode(',', array_keys($level1_agentids)) . ')  and o.status>=0 and og.status2>=0 and og.nocommission=0 and o.uniacid=:uniacid limit 1',
                            array(
                                ':uniacid' => $_W['uniacid']
                            )
                        );
                    ...
                    if (in_array('ordercount', $options)) {
                        $level2_ordercount = pdo_fetch(
                            ...
                            ' from ' . tablename('superdesk_shop_order') . ' o ' . // TODO 标志 楼宇之窗 openid shop_order 不处理
                            ' left join  ' . tablename('superdesk_shop_order_goods') . ' og on og.orderid=o.id ' .// TODO 标志 楼宇之窗 openid shop_order_goods 不处理
                            ' where o.agentid in( ' . implode(',', array_keys($level1_agentids)) . ')  and o.status>=1 and og.status2>=0 and og.nocommission=0 and o.uniacid=:uniacid limit 1',
                            array(
                                ':uniacid' => $_W['uniacid']
                            )
                        );
                    ...
                    if (in_array('ordercount3', $options)) {
                        $level2_ordercount3 = pdo_fetch(
                            'select sum(og.realprice) as ordermoney,count(distinct o.id) as ordercount ' .
                            ' from ' . tablename('superdesk_shop_order') . ' o ' . // TODO 标志 楼宇之窗 openid shop_order 不处理
                            ' left join  ' . tablename('superdesk_shop_order_goods') . ' og on og.orderid=o.id ' .// TODO 标志 楼宇之窗 openid shop_order_goods 不处理
                            ' where o.agentid in( ' . implode(',', array_keys($level1_agentids)) . ')  and o.status>=3 and og.status2>=0 and og.nocommission=0 and o.uniacid=:uniacid limit 1',
                            array(
                                ':uniacid' => $_W['uniacid']
                            )
                        );
                    ...
                    if (in_array('total', $options)) {
                        $level2_commissions = pdo_fetchall(
                            'select og.commission2,og.commissions from ' . tablename('superdesk_shop_order_goods') . ' og ' .// TODO 标志 楼宇之窗 openid shop_order_goods 不处理
                            ' left join  ' . tablename('superdesk_shop_order') . ' o on o.id = og.orderid ' . // TODO 标志 楼宇之窗 openid shop_order 不处理
                            ' where o.agentid in( ' . implode(',', array_keys($level1_agentids)) . ')  and o.status>=1 and og.nocommission=0 and o.uniacid=:uniacid',
                            array(
                                ':uniacid' => $_W['uniacid']
                            )
                        );
                    ...
                    if (in_array('ok', $options)) {
                        $level2_commissions = pdo_fetchall(
                            'select og.commission2,og.commissions  from ' . tablename('superdesk_shop_order_goods') . ' og ' .// TODO 标志 楼宇之窗 openid shop_order_goods 不处理
                            ' left join  ' . tablename('superdesk_shop_order') . ' o on o.id = og.orderid ' . // TODO 标志 楼宇之窗 openid shop_order 不处理
                            ' where o.agentid in( ' . implode(',', array_keys($level1_agentids)) . ')  and (' . $time . ' - o.finishtime > ' . $day_times . ') and o.status>=3 and og.status2=0 and og.nocommission=0  and o.uniacid=:uniacid',
                            array(
                                ':uniacid' => $_W['uniacid']
                            )
                        );
                    ...
                    if (in_array('lock', $options)) {
                        $level2_commissions1 = pdo_fetchall(
                            'select og.commission2,og.commissions ' .
                            ' from ' . tablename('superdesk_shop_order_goods') . ' og ' .// TODO 标志 楼宇之窗 openid shop_order_goods 不处理
                            ' left join  ' . tablename('superdesk_shop_order') . ' o on o.id = og.orderid ' . // TODO 标志 楼宇之窗 openid shop_order 不处理
                            ' where o.agentid in( ' . implode(',', array_keys($level1_agentids)) . ')  and (' . $time . ' - o.finishtime <= ' . $day_times . ') and og.status2=0 and o.status>=3 and og.nocommission=0 and o.uniacid=:uniacid',
                            array(
                                ':uniacid' => $_W['uniacid']
                            )
                        );
                    ...
                    if (in_array('apply', $options)) {
                        $level2_commissions2 = pdo_fetchall(
                            'select og.commission2,og.commissions ' .
                            ' from ' . tablename('superdesk_shop_order_goods') . ' og ' .// TODO 标志 楼宇之窗 openid shop_order_goods 不处理
                            ' left join  ' . tablename('superdesk_shop_order') . ' o on o.id = og.orderid ' . // TODO 标志 楼宇之窗 openid shop_order 不处理
                            ' where o.agentid in( ' . implode(',', array_keys($level1_agentids)) . ')  and o.status>=3 and og.status2=1 and og.nocommission=0 and o.uniacid=:uniacid',
                            array(
                                ':uniacid' => $_W['uniacid']
                            )
                        );
                    ...
                    if (in_array('check', $options)) {
                        $level2_commissions3 = pdo_fetchall(
                            'select og.commission2,og.commissions ' .
                            ' from ' . tablename('superdesk_shop_order_goods') . ' og ' .// TODO 标志 楼宇之窗 openid shop_order_goods 不处理
                            ' left join  ' . tablename('superdesk_shop_order') . ' o on o.id = og.orderid ' . // TODO 标志 楼宇之窗 openid shop_order 不处理
                            ' where o.agentid in( ' . implode(',', array_keys($level1_agentids)) . ')  and o.status>=3 and og.status2=2 and og.nocommission=0 and o.uniacid=:uniacid',
                            array(
                                ':uniacid' => $_W['uniacid']
                            )
                        );
                    ...
                    if (in_array('pay', $options)) {
                        $level2_commissions3 = pdo_fetchall(
                            ...
                            ' from ' . tablename('superdesk_shop_order_goods') . ' og ' .// TODO 标志 楼宇之窗 openid shop_order_goods 不处理
                            ' left join  ' . tablename('superdesk_shop_order') . ' o on o.id = og.orderid ' . // TODO 标志 楼宇之窗 openid shop_order 不处理
                            ' where o.agentid in( ' . implode(',', array_keys($level1_agentids)) . ')  and o.status>=3 and og.status2=3 and og.nocommission=0 and o.uniacid=:uniacid',
                            array(
                                ':uniacid' => $_W['uniacid']
                            )
                        );
                    ...
                    if (in_array('wait', $options)) {
                        $level2_commissions3 = pdo_fetchall(
                            'select og.commission2,og.commissions  from ' . tablename('superdesk_shop_order_goods') . ' og ' .// TODO 标志 楼宇之窗 openid shop_order_goods 不处理
                            ' left join  ' . tablename('superdesk_shop_order') . ' o on o.id = og.orderid ' . // TODO 标志 楼宇之窗 openid shop_order 不处理
                            ' where o.agentid in( ' . implode(',', array_keys($level1_agentids)) . ')  and (o.status=1 or o.status=2) and og.status2=0 and og.nocommission=0 and o.uniacid=:uniacid',
                            array(
                                ':uniacid' => $_W['uniacid']
                            )
                        );
                    ...
                    $level2_agentids = pdo_fetchall('select id from ' . tablename('superdesk_shop_member') . ' where agentid in( ' . implode(',', array_keys($level1_agentids)) . ') and isagent=1 and status=1 and uniacid=:uniacid', array(':uniacid' => $_W['uniacid']), 'id');
            ...
            if (3 <= $level) {
                if (0 < $level2) {
                    if (in_array('ordercount0', $options)) {
                        $level3_ordercount = pdo_fetch(
                            'select sum(og.realprice) as ordermoney,count(distinct og.orderid) as ordercount ' .
                            ' from ' . tablename('superdesk_shop_order') . ' o ' . // TODO 标志 楼宇之窗 openid shop_order 不处理
                            ' left join  ' . tablename('superdesk_shop_order_goods') . ' og on og.orderid=o.id ' .// TODO 标志 楼宇之窗 openid shop_order_goods 不处理
                            ' where o.agentid in( ' . implode(',', array_keys($level2_agentids)) . ')  and o.status>=0 and og.status3>=0 and og.nocommission=0 and o.uniacid=:uniacid limit 1',
                            array(
                                ':uniacid' => $_W['uniacid']
                            )
                        );
                    ...
                    if (in_array('ordercount', $options)) {
                        $level3_ordercount = pdo_fetch(
                            ...
                            ' from ' . tablename('superdesk_shop_order') . ' o ' . // TODO 标志 楼宇之窗 openid shop_order 不处理
                            ' left join  ' . tablename('superdesk_shop_order_goods') . ' og on og.orderid=o.id ' .// TODO 标志 楼宇之窗 openid shop_order_goods 不处理
                            ' where o.agentid in( ' . implode(',', array_keys($level2_agentids)) . ')  and o.status>=1 and og.status3>=0 and og.nocommission=0 and o.uniacid=:uniacid limit 1',
                            array(
                                ':uniacid' => $_W['uniacid']
                            )
                        );
                    ...
                    if (in_array('ordercount3', $options)) {
                        $level3_ordercount3 = pdo_fetch(
                            'select sum(og.realprice) as ordermoney,count(distinct og.orderid) as ordercount ' .
                            ' from ' . tablename('superdesk_shop_order') . ' o ' .// TODO 标志 楼宇之窗 openid shop_order 不处理
                            ' left join  ' . tablename('superdesk_shop_order_goods') . ' og on og.orderid=o.id ' .// TODO 标志 楼宇之窗 openid shop_order_goods 不处理
                            ' where o.agentid in( ' . implode(',', array_keys($level2_agentids)) . ')  and o.status>=3 and og.status3>=0 and og.nocommission=0 and o.uniacid=:uniacid limit 1',
                            array(
                                ':uniacid' => $_W['uniacid']
                            )
                        );
                    ...
                    if (in_array('total', $options)) {
                        $level3_commissions = pdo_fetchall(
                            'select og.commission3,og.commissions  from ' . tablename('superdesk_shop_order_goods') . ' og ' .// TODO 标志 楼宇之窗 openid shop_order_goods 不处理
                            ' left join  ' . tablename('superdesk_shop_order') . ' o on o.id = og.orderid' . // TODO 标志 楼宇之窗 openid shop_order 不处理
                            ' where o.agentid in( ' . implode(',', array_keys($level2_agentids)) . ')  and o.status>=1 and og.nocommission=0 and o.uniacid=:uniacid',
                            array(
                                ':uniacid' => $_W['uniacid']
                            )
                        );
                    ...
                    if (in_array('ok', $options)) {
                        $level3_commissions = pdo_fetchall(
                            'select og.commission3,og.commissions  from ' . tablename('superdesk_shop_order_goods') . ' og ' .// TODO 标志 楼宇之窗 openid shop_order_goods 不处理
                            ' left join  ' . tablename('superdesk_shop_order') . ' o on o.id = og.orderid' . // TODO 标志 楼宇之窗 openid shop_order 不处理
                            ' where o.agentid in( ' . implode(',', array_keys($level2_agentids)) . ')  and (' . $time . ' - o.finishtime > ' . $day_times . ') and o.status>=3 and og.status3=0  and og.nocommission=0 and o.uniacid=:uniacid',
                            array(
                                ':uniacid' => $_W['uniacid']
                            )
                        );
                    ...
                    if (in_array('lock', $options)) {
                        $level3_commissions1 = pdo_fetchall(
                            'select og.commission3,og.commissions  from ' . tablename('superdesk_shop_order_goods') . ' og ' .// TODO 标志 楼宇之窗 openid shop_order_goods 不处理
                            ' left join  ' . tablename('superdesk_shop_order') . ' o on o.id = og.orderid' . // TODO 标志 楼宇之窗 openid shop_order 不处理
                            ' where o.agentid in( ' . implode(',', array_keys($level2_agentids)) . ')  and o.status>=3 and (' . $time . ' - o.finishtime <= ' . $day_times . ') and og.status3=0  and og.nocommission=0 and o.uniacid=:uniacid',
                            array(
                                ':uniacid' => $_W['uniacid']
                            )
                        );
                    ...
                    if (in_array('apply', $options)) {
                        $level3_commissions2 = pdo_fetchall(
                            'select og.commission3,og.commissions  from ' . tablename('superdesk_shop_order_goods') . ' og ' .// TODO 标志 楼宇之窗 openid shop_order_goods 不处理
                            ' left join  ' . tablename('superdesk_shop_order') . ' o on o.id = og.orderid' . // TODO 标志 楼宇之窗 openid shop_order 不处理
                            ' where o.agentid in( ' . implode(',', array_keys($level2_agentids)) . ')  and o.status>=3 and og.status3=1 and og.nocommission=0 and o.uniacid=:uniacid',
                            array(
                                ':uniacid' => $_W['uniacid']
                            )
                        );
                    ...
                    if (in_array('check', $options)) {
                        $level3_commissions3 = pdo_fetchall(
                            'select og.commission3,og.commissions  from ' . tablename('superdesk_shop_order_goods') . ' og ' .// TODO 标志 楼宇之窗 openid shop_order_goods 不处理
                            ' left join  ' . tablename('superdesk_shop_order') . ' o on o.id = og.orderid' . // TODO 标志 楼宇之窗 openid shop_order 不处理
                            ' where o.agentid in( ' . implode(',', array_keys($level2_agentids)) . ')  and o.status>=3 and og.status3=2 and og.nocommission=0 and o.uniacid=:uniacid',
                            array(
                                ':uniacid' => $_W['uniacid']
                            )
                        );
                    ...
                    if (in_array('pay', $options)) {
                        $level3_commissions3 = pdo_fetchall(
                            'select og.commission3,og.commissions  from ' . tablename('superdesk_shop_order_goods') . ' og ' .// TODO 标志 楼宇之窗 openid shop_order_goods 不处理
                            ' left join  ' . tablename('superdesk_shop_order') . ' o on o.id = og.orderid' . // TODO 标志 楼宇之窗 openid shop_order 不处理
                            ' where o.agentid in( ' . implode(',', array_keys($level2_agentids)) . ')  and o.status>=3 and og.status3=3 and og.nocommission=0 and o.uniacid=:uniacid',
                            array(
                                ':uniacid' => $_W['uniacid']
                            )
                        );
                    ...
                    if (in_array('wait', $options)) {
                        $level3_commissions3 = pdo_fetchall(
                            'select og.commission3,og.commissions  from ' . tablename('superdesk_shop_order_goods') . ' og ' .// TODO 标志 楼宇之窗 openid shop_order_goods 不处理
                            ' left join  ' . tablename('superdesk_shop_order') . ' o on o.id = og.orderid' . // TODO 标志 楼宇之窗 openid shop_order 不处理
                            ' where o.agentid in( ' . implode(',', array_keys($level2_agentids)) . ')  and ( o.status=1 or o.status=2) and og.status3=0 and og.nocommission=0 and o.uniacid=:uniacid',
                            array(
                                ':uniacid' => $_W['uniacid']
                            )
                        );
                    ...
                    $level3_agentids = pdo_fetchall('select id from ' . tablename('superdesk_shop_member') . ' where uniacid=:uniacid and agentid in( ' . implode(',', array_keys($level2_agentids)) . ') and isagent=1 and status=1', array(':uniacid' => $_W['uniacid']), 'id');
        ...
        public function getAgents($orderid = 0)
        {
            ...
            $agents = array();
            $order  = pdo_fetch(
                'select id,agentid,openid,core_user from ' . tablename('superdesk_shop_order') . // TODO 标志 楼宇之窗 openid shop_order 已处理
        ...
        public function getAgentsDownNum($openid = NULL)
        {
            ...
            $level1      = (int)pdo_fetchcolumn('select count(*) from ' . tablename('superdesk_shop_member') . ' where agentid=:agentid and uniacid=:uniacid limit 1', array(':agentid' => $member['id'], ':uniacid' => $_W['uniacid']));

            if ((2 <= $set['level']) && (0 < $levelcount1)) {
                $level2 = (int)pdo_fetchcolumn('select count(*) from ' . tablename('superdesk_shop_member') . ' where agentid in( ' . implode(',', array_keys($member['level1_agentids'])) . ') and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid']));
            }


            if ((3 <= $set['level']) && (0 < $levelcount2)) {
                $level3 = (int)pdo_fetchcolumn('select count(*) from ' . tablename('superdesk_shop_member') . ' where agentid in( ' . implode(',', array_keys($member['level2_agentids'])) . ') and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid']));
            }
        ...
        public function createGoodsImage($goods)
        {
            ...
            $path = IA_ROOT . '/addons/superdesk_shopv2/data/poster/' . $_W['uniacid'] . '/';
            ...
            if (!is_file($path . $file)) {
                ...
                $bg     = imagecreatefromjpeg(IA_ROOT . '/addons/superdesk_shopv2/plugin/commission/static/images/poster.jpg');
            ...
            return $_W['siteroot'] . 'addons/superdesk_shopv2/data/poster/' . $_W['uniacid'] . '/' . $file;
        }
        ...
        public function checkAgent($openid = '', $core_user = '')// TODO 标志 楼宇之窗 openid shop_order 已处理
        {
            global $_W;
            global $_GPC;
            $set = $this->getSet();
            ...
            if ($parent_is_agent) {
                if ($parent['openid'] != $openid) {
                    $clickcount = pdo_fetchcolumn(
                        'select count(*) '.
                        ' from ' . tablename('superdesk_shop_commission_clickcount') .// TODO 标志 楼宇之窗 openid shop_commission_clickcount 已处理
                        ' where '.
                        '       uniacid=:uniacid and openid=:openid and core_user=:core_user and from_openid=:from_openid limit 1',
                        array(
                            ':uniacid'     => $_W['uniacid'],
                            ':openid'      => $openid,
                            ':core_user'   => $core_user,
                            ':from_openid' => $parent['openid']// TODO 标志 楼宇之窗 openid shop_commission_clickcount 待处理
                        )
                    );
                    ...
                    if ($clickcount <= 0) {
                        ...
                        pdo_insert('superdesk_shop_commission_clickcount', $click);// TODO 标志 楼宇之窗 openid shop_commission_clickcount 已处理
                        pdo_update('superdesk_shop_member', array('clickcount' => $parent['clickcount'] + 1), array('uniacid' => $_W['uniacid'], 'id' => $parent['id']));
                    }
            ...
            $first = pdo_fetchcolumn('select count(*) from ' . tablename('superdesk_shop_member') . ' where id<>:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $member['id']));

            if ($first <= 0) {
                pdo_update('superdesk_shop_member', array('isagent' => 1, 'status' => 1, 'agenttime' => time(), 'agentblack' => 0), array('uniacid' => $_W['uniacid'], 'id' => $member['id']));
                return NULL;
            }
                    ...
                    if (empty($become_child)) {
                        if (empty($member['fixagentid'])) {
                            ...
                            if (p('author')) {
                                ...
                                pdo_update('superdesk_shop_member', array('agentid' => $parent['id'], 'childtime' => $time, 'authorid' => $authorid), array('uniacid' => $_W['uniacid'], 'id' => $member['id']));
                            } else {
                                pdo_update('superdesk_shop_member', array('agentid' => $parent['id'], 'childtime' => $time), array('uniacid' => $_W['uniacid'], 'id' => $member['id']));
                            }
                                ...
                                if (!empty($author_set['become']) && (($author_set['become'] == '2') || ($author_set['become'] == '5'))) {
                                    ...
                                    if ($can_author) {
                                        ...
                                        if (empty($member['authorblack'])) {
                                            pdo_update('superdesk_shop_member', array('authorstatus' => $become_check, 'isauthor' => 1, 'authortime' => $time), array('uniacid' => $_W['uniacid'], 'id' => $parent['id']));
                        ...
                        pdo_update('superdesk_shop_member', array('inviter' => $parent['id']), array('uniacid' => $_W['uniacid'], 'id' => $member['id']));
                    }
                }
            ...
            if ($to_check_agent) {
                pdo_update('superdesk_shop_member', array('isagent' => 1, 'status' => $become_check, 'agenttime' => ($become_check == 1 ? $time : 0)), array('uniacid' => $_W['uniacid'], 'id' => $member['id']));
        ...
        public function checkOrderConfirm($orderid = '0')
        {
            ...
            $order = pdo_fetch(
                'select id,openid,core_user,ordersn,goodsprice,agentid,paytime from ' . tablename('superdesk_shop_order') . // TODO 标志 楼宇之窗 openid shop_order 已处理
        ...
        public function checkOrderPay($orderid = '0')
        {
                ...
                if ((intval($set['become']) == 4) && !empty($set['become_goodsid'])) {
                    if (empty($set['become_order'])) {
                        $order_goods = pdo_fetchall(
                            'select goodsid from ' . tablename('superdesk_shop_order_goods') . // TODO 标志 楼宇之窗 openid shop_order_goods 不处理
                        ...
                        if (in_array($set['become_goodsid'], array_keys($order_goods))) {
                            if (empty($member['agentblack'])) {
                                pdo_update('superdesk_shop_member', array('status' => $become_check, 'isagent' => 1, 'agenttime' => ($become_check == 1 ? $time : 0)), array('uniacid' => $_W['uniacid'], 'id' => $member['id']));
                    ...
                    if (($set['become'] == 2) || ($set['become'] == 3)) {
                        if (empty($set['become_order'])) {
                            ...
                            if ($set['become'] == '2') {
                                $ordercount = pdo_fetchcolumn(
                                    'select count(*) from ' . tablename('superdesk_shop_order') . // TODO 标志 楼宇之窗 openid shop_order 已处理
                                    ' where openid=:openid and core_user=:core_user and status>=1 and uniacid=:uniacid limit 1',
                                    array(
                                        ':uniacid'   => $_W['uniacid'],
                                        ':openid'    => $openid,
                                        ':core_user' => $core_user
                            ...
                            if ($can) {
                                if (empty($member['agentblack'])) {
                                    pdo_update('superdesk_shop_member', array('status' => $become_check, 'isagent' => 1, 'agenttime' => $time), array('uniacid' => $_W['uniacid'], 'id' => $member['id']));
            ...
            if (!empty($member['agentid'])) {
                ...
                if (!empty($parent) && ($parent['isagent'] == 1) && ($parent['status'] == 1)) {
                    $order_goods = pdo_fetchall(
                        'select g.id,g.title,og.total,og.price,og.realprice, og.optionname as optiontitle,g.noticeopenid,g.noticetype,og.commission1,og.commissions ' .
                        ' from ' . tablename('superdesk_shop_order_goods') . ' og ' . // TODO 标志 楼宇之窗 openid shop_order_goods 不处理
                        ' left join ' . tablename('superdesk_shop_goods') . ' g on g.id=og.goodsid ' .
                ...
                if ((intval($set['become']) == 4) && !empty($set['become_goodsid'])) {
                    if (empty($set['become_order'])) {
                        ...
                        if (in_array($set['become_goodsid'], array_keys($order_goods))) {
                            if (empty($member['partnerblack'])) {
                                pdo_update('superdesk_shop_member', array('partnerstatus' => $become_check, 'ispartner' => 1, 'partnertime' => ($become_check == 1 ? $time : 0)), array('uniacid' => $_W['uniacid'], 'id' => $member['id']));
                    ...
                    if (($set['become'] == 2) || ($set['become'] == 3)) {
                        if (empty($set['become_order'])) {
                            ...
                            if ($can) {
                                if (empty($member['partnerblack'])) {
                                    pdo_update('superdesk_shop_member', array('partnerstatus' => $become_check, 'ispartner' => 1, 'partnertime' => $time), array('uniacid' => $_W['uniacid'], 'id' => $member['id']));
        ...
        public function checkOrderFinish($orderid = '')
        {
                ...
                if (($set['become'] == '4') && !empty($set['become_goodsid'])) {
                    ...
                    if (($set['become'] == 2) || ($set['become'] == 3)) {
                        ...
                        if ($set['become'] == '2') {
                            $ordercount = pdo_fetchcolumn(
                                'select count(*) from ' . tablename('superdesk_shop_order') . // TODO 标志 楼宇之窗 openid shop_order 已处理
                                ' where openid=:openid and core_user=:core_user and status>=3 and uniacid=:uniacid limit 1',
                                array(
                                    ':uniacid'   => $_W['uniacid'],
                                    ':openid'    => $openid,
                                    ':core_user' => $core_user
            ...
            if (!empty($member['agentid'])) {
                ...
                if (!empty($parent) && ($parent['isagent'] == 1) && ($parent['status'] == 1)) {
                    $order_goods = pdo_fetchall(
                        'select g.id,g.title,og.total,og.realprice,og.price,og.optionname as optiontitle,g.noticeopenid,g.noticetype,og.commission1,og.commissions ' .
                        ' from ' . tablename('superdesk_shop_order_goods') . ' og ' . // TODO 标志 楼宇之窗 openid shop_order_goods 不处理
                        ' left join ' . tablename('superdesk_shop_goods') . ' g on g.id=og.goodsid ' .
                        ' where og.uniacid=:uniacid and og.orderid=:orderid ',
                        array(
            ...
            $this->upgradeLevelByOrder($openid, $core_user);
            ...
            if ($isagent) {
                ...
                if (!$plugin_author) {
                    ...
                    if (!empty($set['open'])) {
                        ...
                        if ($isauthor) {
                            $plugin_author->upgradeLevelByOrder($openid, $core_user);
                        } else {
                            $become_check = intval($set['become_check']);
                            ...
                            if ($set['become_order'] == '1') {
                                ...
                                if ($can) {
                                    if (empty($member['authorblack'])) {
                                        pdo_update('superdesk_shop_member', array('authorstatus' => $become_check, 'isauthor' => 1, 'authortime' => $time), array('uniacid' => $_W['uniacid'], 'id' => $member['id']));
                ...
                if ($ispartner) {
                    $plugin_globonus->upgradeLevelByOrder($openid, $core_user);
                    return NULL;
                }
        ...
        public function getShop($m)
        {
            ...
            $shop   = pdo_fetch('select * from ' . tablename('superdesk_shop_commission_shop') . ' where uniacid=:uniacid and mid=:mid limit 1', array(':uniacid' => $_W['uniacid'], ':mid' => $member['id']));
        ...
        public function getLevels($all = true, $default = false)
        {
            ...
            if ($all) {
                $levels = pdo_fetchall('select * from ' . tablename('superdesk_shop_commission_level') . ' where uniacid=:uniacid order by commission1 asc', array(':uniacid' => $_W['uniacid']));
            } else {
                $levels = pdo_fetchall('select * from ' . tablename('superdesk_shop_commission_level') . ' where uniacid=:uniacid and (ordermoney>0 or commissionmoney>0) order by commission1 asc', array(':uniacid' => $_W['uniacid']));
            }
        ...
        public function getLevel($openid)
        {
            ...
            $level = pdo_fetch('select * from ' . tablename('superdesk_shop_commission_level') . ' where uniacid=:uniacid and id=:id limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $member['agentlevel']));
            return $level;
        }
         ...
         * @param type $mid
         */
        public function upgradeLevelByOrder($openid, $core_user)// TODO 标志 楼宇之窗 openid shop_order 已处理
        {
            global $_W;
            ...
            if (($leveltype == 4) || ($leveltype == 5)) {
                ...
                $orders = pdo_fetch(
                    ...
                    ' left join  ' . tablename('superdesk_shop_order_goods') . ' og on og.orderid=o.id ' .// TODO 标志 楼宇之窗 openid shop_order_goods 已处理
                    ' where o.openid=:openid and o.core_user=:core_user and o.status>=3 and o.uniacid=:uniacid limit 1',
                    array(
                        ':uniacid'   => $_W['uniacid'],
                        ':openid'    => $openid,
                        ':core_user' => $core_user
                ...
                if ($leveltype == 4) {
                    $newlevel = pdo_fetch('select * from ' . tablename('superdesk_shop_commission_level') . ' where uniacid=:uniacid  and ' . $ordermoney . ' >= ordermoney and ordermoney>0  order by ordermoney desc limit 1', array(':uniacid' => $_W['uniacid']));
                    ...
                    if (!empty($oldlevel['id'])) {
                        ...
                        if ($newlevel['ordermoney'] < $oldlevel['ordermoney']) {
                            ...
                            if ($leveltype == 5) {
                                $newlevel = pdo_fetch('select * from ' . tablename('superdesk_shop_commission_level') . ' where uniacid=:uniacid  and ' . $ordercount . ' >= ordercount and ordercount>0  order by ordercount desc limit 1', array(':uniacid' => $_W['uniacid']));
                ...
                pdo_update('superdesk_shop_member', array('agentlevel' => $newlevel['id']), array('id' => $m['id']));
            ...
            if ((0 <= $leveltype) && ($leveltype <= 3)) {
                ...
                foreach ($agents as $agent) {
                    ...
                    pdo_update('superdesk_shop_member', array('agentlevel' => $newlevel['id']), array('id' => $agent['id']));
        ...
        public function upgradeLevelByAgent($openid)
        {
            ...
            if (($leveltype == 6) || ($leveltype == 8)) {
                ...
                foreach ($agents as $agent) {
                    ...
                    if ($leveltype == 6) {
                        $downs1    = pdo_fetchall('select id from ' . tablename('superdesk_shop_member') . ' where agentid=:agentid and uniacid=:uniacid ', array(':agentid' => $m['id'], ':uniacid' => $_W['uniacid']), 'id');
                        $downcount += count($downs1);

                        if (!empty($downs1)) {
                            $downs2    = pdo_fetchall('select id from ' . tablename('superdesk_shop_member') . ' where agentid in( ' . implode(',', array_keys($downs1)) . ') and uniacid=:uniacid', array(':uniacid' => $_W['uniacid']), 'id');
                            $downcount += count($downs2);

                            if (!empty($downs2)) {
                                $downs3    = pdo_fetchall('select id from ' . tablename('superdesk_shop_member') . ' where agentid in( ' . implode(',', array_keys($downs2)) . ') and uniacid=:uniacid', array(':uniacid' => $_W['uniacid']), 'id');
                        ...
                        $newlevel = pdo_fetch('select * from ' . tablename('superdesk_shop_commission_level') . ' where uniacid=:uniacid  and ' . $downcount . ' >= downcount and downcount>0  order by downcount desc limit 1', array(':uniacid' => $_W['uniacid']));
                    } else if ($leveltype == 8) {
            ...
            if ($leveltype == 7) {
                $downcount = pdo_fetchcolumn('select count(*) from ' . tablename('superdesk_shop_member') . ' where agentid=:agentid and uniacid=:uniacid ', array(':agentid' => $m['id'], ':uniacid' => $_W['uniacid']));
                $newlevel  = pdo_fetch('select * from ' . tablename('superdesk_shop_commission_level') . ' where uniacid=:uniacid  and ' . $downcount . ' >= downcount and downcount>0  order by downcount desc limit 1', array(':uniacid' => $_W['uniacid']));
            } else if ($leveltype == 9) {
         ...
         * @param type $mid
         */
        public function upgradeLevelByCommissionOK($openid, $core_user = 0)
        {
            global $_W;
            ...
            $newlevel        = pdo_fetch('select * from ' . tablename('superdesk_shop_commission_level') . ' where uniacid=:uniacid  and ' . $commissionmoney . ' >= commissionmoney and commissionmoney>0  order by commissionmoney desc limit 1', array(':uniacid' => $_W['uniacid']));
        ...
        public function sendMessage($openid = '', $data = array(), $message_type = '')
        {
            ...
            if (($message_type == TM_COMMISSION_UPGRADE) && empty($usernotice['commission_upgrade'])) {
                ...
                $message = str_replace('[旧一级营销比例]', $data['oldlevel']['commission1'] . '%', $message);
                $message = str_replace('[旧二级营销比例]', $data['oldlevel']['commission2'] . '%', $message);
                $message = str_replace('[旧三级营销比例]', $data['oldlevel']['commission3'] . '%', $message);
                $message = str_replace('[新等级]', $data['newlevel']['levelname'], $message);
                $message = str_replace('[新一级营销比例]', $data['newlevel']['commission1'] . '%', $message);
                $message = str_replace('[新二级营销比例]', $data['newlevel']['commission2'] . '%', $message);
                $message = str_replace('[新三级营销比例]', $data['newlevel']['commission3'] . '%', $message);
                $msg     = array(
                    'keyword1' => array('value' => (!empty($tm['commission_upgradetitle']) ? $tm['commission_upgradetitle'] : '营销等级升级通知'), 'color' => '#73a68d'),
                    'keyword2' => array('value' => $message, 'color' => '#73a68d')
                );
            ...
            if (($message_type == TM_COMMISSION_BECOME) && empty($usernotice['commission_become'])) {
                ...
                $msg     = array(
                    'keyword1' => array('value' => (!empty($tm['commission_becometitle']) ? $tm['commission_becometitle'] : '成为营销商通知'), 'color' => '#73a68d'),
                    'keyword2' => array('value' => $message, 'color' => '#73a68d')
                );
        ...
        {
            global $_W;
            return array('total1' => pdo_fetchcolumn('select count(*) from ' . tablename('superdesk_shop_commission_apply') . ' where status=1 and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'])), 'total2' => pdo_fetchcolumn('select count(*) from ' . tablename('superdesk_shop_commission_apply') . ' where status=2 and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'])), 'total3' => pdo_fetchcolumn('select count(*) from ' . tablename('superdesk_shop_commission_apply') . ' where status=3 and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'])), 'total_1' => pdo_fetchcolumn('select count(*) from ' . tablename('superdesk_shop_commission_apply') . ' where status=-1 and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'])));
        }
        ...
        protected function sendNotice($touser, $tm, $tag, $datas, $member, $msg)
        {
            ...
            if (!empty($tm['is_advanced']) && !empty($tm[$tag])) {
                $advanced_template = pdo_fetch('select * from ' . tablename('superdesk_shop_member_message_template') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $tm[$tag], ':uniacid' => $_W['uniacid']));
        ...
        {
            $set = $this->getSet();
            return array('[昵称]' => $member['nickname'], '[时间]' => date('Y-m-d H:i:s', time()), '[金额]' => (!empty($data['commission']) ? $data['commission'] : ''), '[提现方式]' => (!empty($data['type']) ? $data['type'] : ''), '[订单编号]' => (!empty($data['ordersn']) ? $data['ordersn'] : ''), '[订单金额]' => (!empty($data['price']) ? $data['price'] : ''), '[商品详情]' => (!empty($data['goods']) ? $data['goods'] : ''), '[旧等级]' => (!empty($data['oldlevel']['levelname']) ? $data['oldlevel']['levelname'] : ''), '[旧一级营销比例]' => (!empty($data['oldlevel']['commission1']) ? $data['oldlevel']['commission1'] . '%' : ''), '[旧二级营销比例]' => (!empty($data['oldlevel']['commission2']) ? $data['oldlevel']['commission2'] . '%' : ''), '[旧三级营销比例]' => (!empty($data['oldlevel']['commission3']) ? $data['oldlevel']['commission3'] . '%' : ''), '[新等级]' => (!empty($data['newlevel']['levelname']) ? $data['newlevel']['levelname'] : ''), '[新一级营销比例]' => (!empty($data['newlevel']['commission1']) ? $data['newlevel']['commission1'] . '%' : ''), '[新二级营销比例]' => (!empty($data['newlevel']['commission2']) ? $data['newlevel']['commission2'] . '%' : ''), '[新三级营销比例]' => (!empty($data['newlevel']['commission3']) ? $data['newlevel']['commission3'] . '%' : ''), '[下线]' => '[' . $set['texts']['down'] . ']');
        }
        ...
        public function getLastApply($mid, $type = -1)
        {
            ...
            $sql    = 'select type,alipay,bankname,bankcard from ' . tablename('superdesk_shop_commission_apply') . ' where mid=:mid and uniacid=:uniacid';
