...
class PosteraProcessor extends PluginProcessor
{
    ...
    private function responseScan($obj)
    {
        ...
        $poster = pdo_fetch(
            'select * ' .
            ' from ' . tablename('superdesk_shop_postera') .
            ' where ' .
            '       id=:id ' .
            '       and uniacid=:uniacid ' .
            ' limit 1',
    ...
    private function responseSubscribe($obj)
    {
        ...
        $log = pdo_fetch(
            'select * ' .
            ' from ' . tablename('superdesk_shop_postera_log') . // TODO 标志 楼宇之窗 openid shop_postera_log 已处理
            ' where ' .
            '       openid=:openid ' .
        ...
        if (empty($log) && ($openid != $qr['openid'])) {
            ...
            pdo_insert('superdesk_shop_postera_log', $log);// TODO 标志 楼宇之窗 openid shop_postera_log 已处理
            ...
            $reward_real = pdo_fetch(
                'select sum(reccredit) as reccredit_totle,sum(recmoney) as recmoney_totle,sum(reccouponnum) as reccouponnum_totle ' .
                ...
                ' limit 1',
                array(
                    ':from_openid' => $qr['openid'],// TODO 标志 楼宇之窗 openid shop_postera_log 待处理
                    ':posterid'    => $poster['id'],
                    ':uniacid'     => $_W['uniacid'],
            ...
            if (!empty($upgrade)) {
                pdo_update('superdesk_shop_postera_log', $upgrade, array('id' => $log['id']));// TODO 标志 楼宇之窗 openid shop_postera_log 不处理
            }
        }
