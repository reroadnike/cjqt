...
class Log_SuperdeskShopV2Page extends PcMobileLoginPage
{
    ...
    public function getlist()
    {
        ...
        if (!(empty($total))) {
            $sql =
                ' SELECT log.id,log.logno,log.goodsid,log.status,log.eno,log.paystatus,g.title,g.type,g.thumb,g.credit,g.money, ' .
                '        g.isverify,g.goodstype,log.addressid,log.storeid,g.goodstype,log.time_send,log.time_finish, ' .
                '        log.iscomment,op.title as optiontitle ' .
                ' FROM ' . tablename('superdesk_shop_creditshop_log') . ' log ' . // TODO 标志 楼宇之窗 openid shop_creditshop_log 已处理
                ' left join ' . tablename('superdesk_shop_creditshop_goods') . ' g on log.goodsid = g.id ' .
                ' left join ' . tablename('superdesk_shop_creditshop_option') . ' op on op.id = log.optionid ' .
                ' where 1 ' .
                $condition .
    ...
    public function detail()
    {
        ...
        if (!(empty($goods['isverify']))) {
            ...
            $verifytotal = pdo_fetchcolumn(
                'select count(1) ' .
                ' from ' . tablename('superdesk_shop_creditshop_verify') .
                ' where 1 ' .
                '       and logid = :id ' .
                '       and openid = :openid ' .
                '       and core_user = :core_user ' .
            ...
            if (empty($log['storeid'])) {
                ...
                $stores = pdo_fetchall(
                    ...
                    '       and uniacid=:uniacid ' .
                    '       and status=1 ' .
                    '       and `type` in(2,3)', // type	支持自提类型 1 自提 2 核销 3 自提与核销
                    array(
                        ':uniacid' => $_W['uniacid']
                    )
                );
    ...
    public function Receivepacket()
    {
        ...
        $params = array(
            ...
            'money'     => $money,
            'wishing'   => ($set['sendname'] ? $set['wishing'] : '红包领到手抽筋，别人加班你加薪!'),
            'act_name'  => '积分兑换红包',
            'remark'    => '积分兑换红包'
        );
        ...
        if (is_error($err)) {
            ...
            pdo_update("superdesk_shop_creditshop_goods", $updategoods, array('id' => $log['goodsid']));
        }
    ...
    public function express()
    {
        ...
        $goods = pdo_fetch(
            'select *  from ' . tablename('superdesk_shop_creditshop_goods') . '  where id=:id and uniacid=:uniacid ', array(':uniacid' => $_W['uniacid'], ':id' => $log['goodsid']));
    ...
    public function finish()
    {
        ...
        $logid = intval($_GPC['id']);
        $log   = pdo_fetch(
            'select * from ' . tablename('superdesk_shop_creditshop_log') . // TODO 标志 楼宇之窗 openid shop_creditshop_log 已处理
    ...
    public function paydispatch()
    {
        ...
        $member        = m('member')->getMember($_W['openid'], $_W['core_user']);
        $shop          = m('common')->getSysset('shop');
        $_W['uniacid'] = $_W['uniacid'];
        $paytype       = trim($_GPC['paytype']);
        $id            = intval($_GPC['id']);
        $addressid     = intval($_GPC['addressid']);
        $log           = pdo_fetch(
            'select * from ' . tablename('superdesk_shop_creditshop_log') . // TODO 标志 楼宇之窗 openid shop_creditshop_log 已处理
            ' where id=:id and openid=:openid and core_user=:core_user and uniacid=:uniacid limit 1',
            array(
                ':id'        => $id,
                ':openid'    => $_W['openid'],
                ':core_user' => $_W['core_user'],
        ...
        if ($paytype == 'wechat') {
            ...
            if (empty($dispatchno)) {
                ...
                pdo_update('superdesk_shop_creditshop_log', // TODO 标志 楼宇之窗 openid shop_creditshop_log 不处理
                    array("dispatchno" => $dispatchno, 'addressid' => $addressid),
                    array('id' => $log['id'])
                );
            }
            $params          = array();
    ...
    public function dispatch_complete()
    {
        ...
        if ($lastlog['dispatchstatus'] < 1) {
            ...
            pdo_update('superdesk_shop_creditshop_log', // TODO 标志 楼宇之窗 openid shop_creditshop_log 不处理
                $record,
                array('dispatchno' => $tid)
            );
            $creditlog = pdo_fetch(
                ' select id from ' . tablename('superdesk_shop_creditshop_log') . // TODO 标志 楼宇之窗 openid shop_creditshop_log 已处理
                ' where dispatchno=:dispatchno and openid=:openid and core_user=:core_user and dispatchstatus=1 and uniacid=:uniacid limit 1',
                array(
                    ':dispatchno' => $tid,
                    ':openid'     => $_W['openid'],
                    ':core_user'  => $_W['core_user'],
