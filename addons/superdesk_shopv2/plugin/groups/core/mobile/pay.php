...
class Pay_SuperdeskShopV2Page extends PluginMobileLoginPage
{
    public function main()
    {
        ...
        if (!empty($teamid)) {
            ...
            $num = pdo_fetchcolumn('select count(1) from ' . tablename('superdesk_shop_groups_order') . ' as o where teamid = :teamid and status > :status and uniacid = :uniacid ', array(':teamid' => $teamid, ':status' => 0, ':uniacid' => $uniacid));
        ...
        if (empty($log)) {
            ...
            pdo_insert('superdesk_shop_groups_paylog', $log);
            $plid = pdo_insertid();
        }
    ...
    public function complete()
    {
        ...
        if ($type == 'credit') {
            ...
            $credits = m('member')->getCredit($openid, $core_user,
                'credit2');
            ...
            $result = m('member')->setCredit($openid, $core_user,
                'credit2', -$fee, array($_W['member']['uid'], $_W['shopset']['shop']['name'] . '消费' . $fee));
