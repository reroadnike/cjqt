...
class Order_SuperdeskShopV2Page extends CommissionMobileLoginPage
{
    ...
    function get_list()
    {
        ...
        $member     = $this->model->getInfo($openid, array('ordercount0'), $core_user);
        $agentLevel = $this->model->getLevel($openid, $_W['core_user']);
        ...
        $ordercount = $member['ordercount0']; //营销订单数

        if ($level >= 1) {
            //一级下线
            $level1_memberids = pdo_fetchall('select id from ' . tablename('superdesk_shop_member') . ' where uniacid=:uniacid and agentid=:agentid', array(':uniacid' => $_W['uniacid'], ':agentid' => $member['id']), 'id');
            $level1_orders    = pdo_fetchall(
                ' select commission1,o.id,o.createtime,o.price,og.commissions ' .
                ...
                " where o.uniacid=:uniacid and o.agentid=:agentid {$condition} and og.status1>=0 and og.nocommission=0",
                array(
                    ':uniacid' => $_W['uniacid'],
                    ':agentid' => $member['id']
                )
        ...
        if ($level >= 2) {
            //二级下线
            if ($level1 > 0) {
                $level2_orders = pdo_fetchall('select commission2 ,o.id,o.createtime,o.price,og.commissions  ' .
                    ' from ' . tablename('superdesk_shop_order_goods') . ' og ' .// TODO 标志 楼宇之窗 openid shop_order_goods 不处理
                    ' left join  ' . tablename('superdesk_shop_order') . ' o on og.orderid=o.id ' . // TODO 标志 楼宇之窗 openid shop_order 不处理
                    " where o.uniacid=:uniacid and o.agentid in( " . implode(',', array_keys($member['level1_agentids'])) . ")  {$condition}  and og.status2>=0 and og.nocommission=0 ",
                    array(
                        ':uniacid' => $_W['uniacid']
                    )
                );
        ...
        if ($level >= 3) {
            if ($level2 > 0) {
                $level3_orders = pdo_fetchall(
                    'select commission3 ,o.id,o.createtime,o.price,og.commissions  from ' . tablename('superdesk_shop_order_goods') . ' og ' . // TODO 标志 楼宇之窗 openid shop_order_goods 不处理
                    ' left join  ' . tablename('superdesk_shop_order') . ' o on og.orderid=o.id ' . // TODO 标志 楼宇之窗 openid shop_order 不处理
                    ' where o.uniacid=:uniacid and o.agentid in( ' . implode(',', array_keys($member['level2_agentids'])) . ")  {$condition} and og.status3>=0 and og.nocommission=0",
                    array(
                        ':uniacid' => $_W['uniacid']
                    )
                );
        ...
        if (!empty($orderids)) {
            ...
            $list = pdo_fetchall(
                " select id,ordersn,openid,core_user,createtime,status " .
                " from " . tablename('superdesk_shop_order') . // TODO 标志 楼宇之窗 openid shop_order 已处理
                " where uniacid ={$_W['uniacid']} and id in ( " . implode(',', array_keys($orderids)) . ") " .
                " order by id desc"
            );
            ...
            foreach ($list as &$row) {
                ...
                if (!empty($this->set['openorderdetail'])) {
                    $goods = pdo_fetchall("SELECT og.id,og.goodsid,g.thumb,og.price,og.total,g.title,og.optionname,"
                        ...
                        . "og.content1,og.content2,og.content3 from " . tablename('superdesk_shop_order_goods') . " og"// TODO 标志 楼宇之窗 openid shop_order_goods 不处理
                        . " left join " . tablename('superdesk_shop_goods') . " g on g.id=og.goodsid  "
                        . " where og.orderid=:orderid and og.nocommission=0 and og.uniacid = :uniacid order by og.createtime  desc ",
                        array(
                            ':uniacid' => $_W['uniacid'],
                            ':orderid' => $row['id']
                        )
