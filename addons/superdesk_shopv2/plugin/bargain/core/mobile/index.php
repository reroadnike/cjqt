...
class Index_SuperdeskShopV2Page extends PluginMobileLoginPage
{
    public function main()
    {
        ...
        $share_res = pdo_fetch('SELECT * FROM ' . tablename('superdesk_shop_bargain_account') . 'WHERE id = :id', array(':id' => $_W['uniacid']));
        ...
        if ($_W['ispost']) {
            ...
            $res2 = pdo_fetchall(
                'SELECT * FROM' . tablename('superdesk_shop_bargain_goods') .
                ' WHERE ' .
    ...
    public function purchase()
    {
        ...
        $share_res = pdo_fetch(
            'SELECT * FROM' . tablename('superdesk_shop_bargain_account') .
            ' WHERE ' .
            '       id = :id',
            array(
                ':id' => $_W['uniacid']
        ...
        while ($i < count($act)) {
            ...
            $ewei_detail = pdo_fetch(
                'SELECT * ' .
                ' FROM ' . tablename('superdesk_shop_goods') .
                ' WHERE id = :id AND status = \'1\'',
                array(
                    ':id' => $goods[$i][0]['goods_id']
                )
            );
    ...
    public function bargain()
    {
        ...
        $res3 = pdo_fetchall(
            'SELECT * ' .
            ' FROM ' . tablename('superdesk_shop_bargain_record') .
            ' WHERE ' .
            '       actor_id = :actor_id ' .
            ' ORDER BY id DESC ' .
            ' LIMIT 0,10',
        ...
        $res4 = pdo_fetchall(
            ...
            ' FROM ' . tablename('superdesk_shop_bargain_actor') .
            ' WHERE ' .
            '       bargain_price <= \'' . $account_set['partin'] . '\' ' .
            '       AND account_id=:uniacid ' .
            ' ORDER BY bargain_price ASC ' .
            ' LIMIT 10',
            array(
                ':uniacid' => $_W['uniacid']
            )
        ...
        $myself_count = pdo_get(
            'superdesk_shop_bargain_record',
            array(
                'openid'   => $_W['openid'],
                'actor_id' => $res['id']
            ),
    ...
    public function join()
    {
        ...
        $user_info = m('member')->getMember($_W['openid'], $_W['core_user']);
        ...
        if (!empty($user_info['openid'])) {
            ...
            pdo_query(
                'UPDATE ' . tablename('superdesk_shop_bargain_goods') .
                ' SET act_times=act_times+1 ' .
                ' WHERE id= :id',
                array(
                    ':id' => $goods_id
                )
    ...
    public function cut($actor_id, $time3, $min_price, $each_time, $total_time, $max_price, $probability_json)
    {
        ...
        $info = m('member')->getMember($_W['openid'], $_W['core_user']);

        $min *= 100;
        $max *= 100;

        $record_res = pdo_fetchall(
            'SELECT * ' .
            ' FROM ' . tablename('superdesk_shop_bargain_record') .
            ' WHERE ' .
            '       actor_id=:actor_id ' .
        ...
        if ($temp_price <= $min_price) {
            ...
            $message_sql =
                'SELECT b.* ' .
                ' FROM ' . tablename('superdesk_shop_bargain_account') . ' a ' .
                ' INNER JOIN ' . tablename('superdesk_shop_member_message_template') . ' b ON a.end_message =b.id';
        ...
        $insert_data = array(
            'actor_id'      => $actor_id,
            'bargain_price' => $cut_price,
            'openid'        => $_W['openid'],
            'nickname'      => $info['nickname'],
            'head_image'    => $info['avatar'],
            'bargain_time'  => $time
        );
        ...
        $res_act = pdo_query(
            'UPDATE ' . tablename('superdesk_shop_bargain_actor') .
            ' SET now_price = now_price + :now_price ,update_time = :update_time , bargain_price = bargain_price + :bargain_price ' .
            ' WHERE id= :id',
            array(
                ':now_price'     => $cut_price,
                ':update_time'   => $time,
        ...
        if ($cut_price < 0) {
            ...
            $message_sql =
                'SELECT b.* ' .
                ' FROM ' . tablename('superdesk_shop_bargain_account') . ' a ' .
                ' INNER JOIN ' . tablename('superdesk_shop_member_message_template') . ' b ON a.message =b.id';
            ...
            pdo_query(
                'UPDATE ' . tablename('superdesk_shop_bargain_actor') .
                ' SET bargain_times = bargain_times + 1 ' .
                ' WHERE id = :id',
                array(
                    ':id' => $actor_id
                )
