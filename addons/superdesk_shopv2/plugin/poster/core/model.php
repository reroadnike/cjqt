...
{
    public function checkScan($openid = '', $core_user = 0)// TODO 标志 楼宇之窗 openid shop_poster_scan 已处理
    {
        global $_W;
        global $_GPC;
        ...
        $this->scanTime($openid, $parent['openid'], $poster, $core_user);
    }

    public function scanTime($openid, $from_openid, $poster, $core_user)
    {
        ...
        $scancount = pdo_fetchcolumn(
            'select count(*) ' .
            ' from ' . tablename('superdesk_shop_poster_scan') . // TODO 标志 楼宇之窗 openid shop_poster_scan 已处理
            ' where ' .
            '       openid=:openid ' .
        ...
        if ($scancount <= 0) {
            ...
            pdo_insert(
                'superdesk_shop_poster_scan', // TODO 标志 楼宇之窗 openid shop_poster_scan 已处理
                $scan
            );

            pdo_update(
                'superdesk_shop_poster',
                array(
     ...
     * @return array|string
     */
    public function createCommissionPoster($openid, $core_user = 0, $goodsid = 0, $type = 0)
    {
        global $_W;
...
//        socket_log(json_encode(array(':uniacid' => $_W['uniacid'], ':type' => $type)));
    ...
    public function getFixedTicket($poster, $member, $uniaccount)
    {
        ...
        $scene_str = md5('superdesk_shop_poster:' . $_W['uniacid'] . ':' . $member['openid'] . ':' . $poster['id']);
    ...
    public function getQR($poster, $member, $goodsid = 0)
    {
        ...
        if ($poster['type'] == 1) {
            ...
            $qr = pdo_fetch(
                'select * ' .
                ' from ' . tablename('superdesk_shop_poster_qr') . // TODO 标志 楼宇之窗 openid shop_poster_qr 已处理
                ' where ' .
                '       openid=:openid ' .
                '       and core_user=:core_user ' .
                '       and acid=:acid ' .
            ...
            if (empty($qr)) {
                ...
                pdo_insert('superdesk_shop_poster_qr', $qr);// TODO 标志 楼宇之窗 openid shop_poster_qr 已处理
                $qr['id'] = pdo_insertid();
            }
        ...
        if ($poster['type'] == 2) {
            ...
            $qr    = pdo_fetch(
                'select * from ' . tablename('superdesk_shop_poster_qr') . // TODO 标志 楼宇之窗 openid shop_poster_qr 已处理
                ' where openid=:openid and core_user=:core_user and acid=:acid and type=:type and goodsid=:goodsid limit 1',
                array(
                    ':openid'    => $member['openid'],
                    ':core_user' => $member['core_user'],
                    ':acid'      => $_W['acid'],
            ...
            if (empty($qr)) {
                $qr = array('acid' => $acid, 'openid' => $member['openid'], 'core_user' => $member['core_user'], 'type' => 3, 'goodsid' => $goodsid, 'qrimg' => $qrimg);
                pdo_insert('superdesk_shop_poster_qr', $qr);// TODO 标志 楼宇之窗 openid shop_poster_qr 已处理
                $qr['id'] = pdo_insertid();
            }
            ...
            $qr         = pdo_fetch(
                'select * from ' . tablename('superdesk_shop_poster_qr') . // TODO 标志 楼宇之窗 openid shop_poster_qr 已处理
                ' where openid=:openid and core_user=:core_user and acid=:acid and type=4 limit 1',
                array(
                    ':openid'    => $member['openid'],
                    ':core_user' => $member['core_user'],
                    ':acid'      => $acid
            ...
            if (empty($qr) || empty($qr['scenestr'])) {
                ...
                $ims_qrcode = array('uniacid' => $_W['uniacid'], 'acid' => $_W['acid'], 'scene_str' => $barcode['action_info']['scene']['scene_str'], 'model' => 2, 'name' => 'SUPERDESK_SHOPV2_POSTER_QRCODE', 'keyword' => 'SUPERDESK_SHOPV2_POSTER', 'expire' => 0, 'createtime' => time(), 'status' => 1, 'url' => $result['url'], 'ticket' => $result['ticket']);
                pdo_insert('qrcode', $ims_qrcode);
            }


            if (empty($qr)) {
                $qr = array('acid' => $acid, 'openid' => $member['openid'], 'core_user' => $member['core_user'], 'type' => 4, 'scenestr' => $barcode['action_info']['scene']['scene_str'], 'ticket' => $result['ticket'], 'qrimg' => $qrimg, 'url' => $result['url']);
                pdo_insert('superdesk_shop_poster_qr', $qr);// TODO 标志 楼宇之窗 openid shop_poster_qr 已处理
                $qr['id']            = pdo_insertid();
                $qr['current_qrimg'] = $qrimg;
    ...
    public function createPoster($poster, $member, $qr, $upload = true)
    {
        ...
        if (!empty($qr['goodsid'])) {
            $goods = pdo_fetch('select id,title,thumb,commission_thumb,marketprice,productprice from ' . tablename('superdesk_shop_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $qr['goodsid'], ':uniacid' => $_W['uniacid']));
        ...
        $img = $_W['siteroot'] . 'addons/superdesk_shopv2/data/poster/' . $_W['uniacid'] . '/' . $file;
    ...
    public function getQRByTicket($ticket = '')
    {
        ...
        $qrs = pdo_fetchall(
            'select * from ' . tablename('superdesk_shop_poster_qr') .// TODO 标志 楼宇之窗 openid shop_poster_qr 不处理
            ' where ticket=:ticket and acid=:acid and type=4 limit 1',
            array(
                ':ticket' => $ticket,
                ':acid'   => $_W['acid']
            )
