...
require SUPERDESK_SHOPV2_PLUGIN . 'enterprise/core/inc/common.php';

class EnterpriseWebPage extends PluginWebPage
{
    public $pluginname;
    public $model;
    public $plugintitle;
    ...
    public function __construct($_com = '', $_init = false)
    {
        ...
        $this->modulename  = 'superdesk_shopv2';
    ...
    private function init()
    {
        ...
        if (($_W['enterprise_isfounder'] != '1') && ($_W['routes'] != 'shop')) {

            $perm            = mcv($_W['routes']);
            $perm_type       = p('enterprise')->getLogTypes(true);
            ...
            $is_xxx = p('enterprise')->check_xxx($_W['routes']);
            if ($is_xxx) {
    ...
    public function template($filename = '', $type = TEMPLATE_INCLUDEPATH, $account = false)
    {
        ...
        $compile    = IA_ROOT . '/data/tpl/web/' . $_W['template'] . '/' . $name . '/plugin/' . $_W['plugin'] . '/' . $filename . '.tpl.php';
...
//        echo '<br/>';
        ...
        if (!is_file($source)) {
            ...
            $source  = $moduleroot . '/plugin/enterprise/template/web/manage/' . implode('/', $explode) . '.html';

            if (!is_file($source)) {
                $source = $moduleroot . '/plugin/enterprise/template/web/manage/' . implode('/', $explode) . '/index.html';
            }
        }
