...
class Package_SuperdeskShopV2Page extends MobilePage
{
    public function main()
    {
        ...
        if ($goodsid) {
            ...
            $packages_goods = pdo_fetchall(
                'SELECT id,pid FROM ' . tablename('superdesk_shop_package_goods') . "\n" .
                '                    WHERE uniacid = ' . $uniacid .
                ' and goodsid = ' . $goodsid .
                ' group by pid  ORDER BY id DESC'
            );

            foreach ($packages_goods as $key => $value) {

                $packages[$key] = pdo_fetch(
                    'SELECT id,title,thumb,price,goodsid FROM ' . tablename('superdesk_shop_package') . "\n" .
                    '                    WHERE uniacid = ' . $uniacid .
                    ' and id = ' . $value['pid'] .
                    ' and starttime <= ' . time() .
                    ' and endtime >= ' . time() .
                    ' and deleted = 0 and status = 1  ORDER BY id DESC'
                );
            }
            ...
            $packages = pdo_fetchall(
                'SELECT id,title,thumb,price FROM ' . tablename('superdesk_shop_package') . "\n" .
                '                    WHERE uniacid = ' . $uniacid .
                ' and starttime <= ' . time() .
                ' and endtime >= ' . time() .
                ' and deleted = 0 and status = 1  ORDER BY id DESC'
        ...
        foreach ($packages as $key => $value) {
            ...
            foreach ($goods as $k => $val) {
                ...
                $g = pdo_fetch(
                    'SELECT id,marketprice FROM ' . tablename('superdesk_shop_goods') . "\n" .
                    '                    WHERE uniacid = ' . $uniacid .
                    ' and id = ' . $val .
                    '  ORDER BY id DESC'
                );
    ...
    public function detail()
    {
        ...
        $package = pdo_fetch(
            'SELECT id,title,price,freight,share_title,share_icon,share_desc FROM ' . tablename('superdesk_shop_package') . "\n" .
            '                    WHERE uniacid = ' . $uniacid .
            ' and id = ' . $pid . ' ');

        $packgoods = array();
        $packgoods = pdo_fetchall(
            'SELECT id,title,thumb,marketprice,packageprice,`option`,goodsid FROM ' . tablename('superdesk_shop_package_goods') . "\n" .
            '                    WHERE uniacid = ' . $uniacid .
            ' and pid = ' . $pid .
            '  ORDER BY id DESC'
        );
        ...
        foreach ($packgoods as $key => $value) {
            ...
            if (0 < $option_array[0]) {
                $pgo = pdo_fetch(
                    'SELECT id,title,packageprice FROM ' . tablename('superdesk_shop_package_goods_option') . "\n" .
                    '                    WHERE uniacid = ' . $uniacid .
                    ' and pid = ' . $pid .
                    ' and goodsid = ' . $value['goodsid'] .
                    ' and optionid = ' . $option_array[0] . ' '
                );
    ...
    public function option()
    {
        ...
        $option = pdo_fetchall(
            'select optionid,title,goodsid,packageprice from ' . tablename('superdesk_shop_package_goods_option') .
            ' where pid = ' . $pid .
            ' and goodsid = ' . $goodsid .
            ' and uniacid = ' . $uniacid . ' '
        );
