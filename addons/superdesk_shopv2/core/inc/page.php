...
class Page extends WeModuleSite
{
    public function runTasks()
    {
        ...
        if (($lasttime + $interval) <= $current) {
            ...
            ihttp_request(SUPERDESK_SHOPV2_TASK_URL . 'order/receive.php', NULL, NULL, 1);
        }
        ...
        if (($lasttime + $interval) <= $current) {
            ...
            ihttp_request(SUPERDESK_SHOPV2_TASK_URL . 'order/close.php', NULL, NULL, 1);
        }

        //商户评分维度
        $lasttime = strtotime(m('cache')->getString('report','global'));
        $interval = intval(m('cache')->getString('report_time','global'));
        if(empty($interval)) {
            $interval = 60;
        }
        $interval *=60;
        ...
        if (($lasttime + $interval) <= $current) {
            ...
            ihttp_request(SUPERDESK_SHOPV2_TASK_URL . 'order/report.php', NULL, NULL, 1);
        }

        //商品评分
        $lasttime = strtotime(m('cache')->getString('goods','global'));
        $interval = intval(m('cache')->getString('goods_time','global'));
        if(empty($interval)) {
            $interval = 60;
        }
        $interval *=60;
        ...
        if (($lasttime + $interval) <= $current) {
            ...
            ihttp_request(SUPERDESK_SHOPV2_TASK_URL . 'order/goods.php', NULL, NULL, 1);
        }
        ...
        if (com('coupon')) {
            ...
            if (($lasttime + $interval) <= $current) {
                ...
                ihttp_request(SUPERDESK_SHOPV2_TASK_URL . 'coupon/back.php', NULL, NULL, 1);
            }
        }
        ...
        if (p('groups')) {
            ...
            if (($groups_order_lasttime + $groups_order_interval) <= $groups_order_current) {
                ...
                ihttp_request($_W['siteroot'] . 'addons/superdesk_shopv2/plugin/groups/task/order.php', NULL, NULL, 1);
            }
            ...
            if (($groups_team_lasttime + $groups_team_interval) <= $groups_team_current) {
                ...
                ihttp_request($_W['siteroot'] . 'addons/superdesk_shopv2/plugin/groups/task/refund.php', NULL, NULL, 1);
            }
            ...
            if (($groups_receive_lasttime + $groups_receive_interval) <= $groups_receive_current) {
                ...
                ihttp_request($_W['siteroot'] . 'addons/superdesk_shopv2/plugin/groups/task/receive.php', NULL, NULL, 1);
            }
        }
        exit('run finished.');
    }
    ...
    public function template($filename = '', $type = TEMPLATE_INCLUDEPATH, $account = false)
    {
        ...
        $name       = 'superdesk_shopv2';
        $moduleroot = IA_ROOT . '/addons/superdesk_shopv2';
