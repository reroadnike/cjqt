...
class Goods_trans_SuperdeskShopV2Page extends EnterpriseWebPage
{
	public function main()
	{
		...
		$sql =
			' SELECT g.id,g.title,g.thumb,g.viewcount,' .
			'        (select sum(og.total) from  ' . tablename('superdesk_shop_order_goods') . ' og ' .// TODO 标志 楼宇之窗 openid shop_order_goods 不处理
			'         left join ' . tablename('superdesk_shop_order') . ' o on og.orderid=o.id ' .// TODO 标志 楼宇之窗 openid shop_order 不处理
			'         where o.status>=1 and og.goodsid=g.id ' . $condition . ')  as buycount' .
			' from ' . tablename('superdesk_shop_goods') . ' g  ' .
			'where 1 ' . $condition1 .
			' order by buycount/g.viewcount ' . $orderby . '  ';
