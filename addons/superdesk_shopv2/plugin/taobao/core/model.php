...
class TaobaoModel extends PluginModel
{
    public function get_item_taobao($itemid = '', $taobaourl = '', $pcate = 0, $ccate = 0, $tcate = 0, $merchid = 0)
    {
        ...
        $g = pdo_fetch(
            ...
            ' where uniacid=:uniacid and merchid=:merchid and catch_id=:catch_id and catch_source=\'taobao\' ' .
            ' limit 1',
            array(
                ':uniacid'  => $_W['uniacid'],
                ':merchid'  => $merchid,
    ...
    public function get_item_jingdong($itemid = '', $jingdongurl = '', $pcate = 0, $ccate = 0, $tcate = 0, $merchid = 0)
    {
        ...
        $g = pdo_fetch(
            ...
            '       uniacid=:uniacid ' .
            '       and merchid=:merchid ' .
            '       and catch_id=:catch_id ' .
            '       and catch_source=\'jingdong\' ' .
            ' limit 1',
            array(
                ':uniacid'  => $_W['uniacid'],
                ':catch_id' => $itemid,
    ...
    public function save_taobao_goods($item = array(), $catch_url = '')
    {
        ...
        $data = array(
            ...
            'createtime'   => time(),
            'updatetime'   => time(),
            'hasoption'    => (0 < count($item['options']) ? 1 : 0),
            'status'       => 0,
            'deleted'      => 0,
            'buylevels'    => '',
            'showlevels'   => '',
        ...
        $goods = pdo_fetch(
            'select * from ' . tablename('superdesk_shop_goods') .
            ' where  catch_id=:catch_id and catch_source=\'taobao\' and uniacid=:uniacid and merchid=:merchid',
            array(':catch_id' => $item['itemId'], ':uniacid' => $_W['uniacid'], ':merchid' => $item['merchid'])
        );
        ...
        if (empty($goods)) {
            ...
            pdo_update('superdesk_shop_goods', $data, array('id' => $goodsid));
        }
        $goods_params = pdo_fetchall(
            'select * from ' . tablename('superdesk_shop_goods_param') .
        ...
        $displayorder = 0;
        foreach ($params as $p) {
            $oldp    = pdo_fetch('select * from ' . tablename('superdesk_shop_goods_param') . ' where goodsid=:goodsid and title=:title limit 1', array(':goodsid' => $goodsid, ':title' => $p['title']));
            $paramid = 0;
            $d       = array('uniacid' => $_W['uniacid'], 'goodsid' => $goodsid, 'title' => $p['title'], 'value' => $p['value'], 'displayorder' => $displayorder);
            if (empty($oldp)) {
                pdo_insert('superdesk_shop_goods_param', $d);
                $paramid = pdo_insertid();
            } else {
                pdo_update('superdesk_shop_goods_param', $d, array('id' => $oldp['id']));
                $paramid = $oldp['id'];
            }
            $paramids[] = $paramid;
            ++$displayorder;
        ...
        if (0 < count($paramids)) {
            pdo_query('delete from ' . tablename('superdesk_shop_goods_param') . ' where goodsid=:goodsid and id not in (' . implode(',', $paramids) . ')', array(':goodsid' => $goodsid));
        } else {
            pdo_query('delete from ' . tablename('superdesk_shop_goods_param') . ' where goodsid=:goodsid ', array(':goodsid' => $goodsid));
        }
        $specs        = $item['specs'];
        $specids      = array();
        $displayorder = 0;
        ...
        foreach ($specs as $spec) {
            $oldspec = pdo_fetch('select * from ' . tablename('superdesk_shop_goods_spec') . ' where goodsid=:goodsid and propId=:propId limit 1', array(':goodsid' => $goodsid, ':propId' => $spec['propId']));
            $specid  = 0;
            $d_spec  = array('uniacid' => $_W['uniacid'], 'goodsid' => $goodsid, 'title' => $spec['title'], 'displayorder' => $displayorder, 'propId' => $spec['propId']);
            if (empty($oldspec)) {
                pdo_insert('superdesk_shop_goods_spec', $d_spec);
                $specid = pdo_insertid();
            } else {
                pdo_update('superdesk_shop_goods_spec', $d_spec, array('id' => $oldspec['id']));
                $specid = $oldspec['id'];
            }
            $d_spec['id'] = $specid;
            $specids[]    = $specid;
            ...
            foreach ($spec_items as $spec_item) {
                ...
                $oldspecitem  = pdo_fetch('select * from ' . tablename('superdesk_shop_goods_spec_item') . ' where specid=:specid and valueId=:valueId limit 1', array(':specid' => $specid, ':valueId' => $spec_item['valueId']));
                $spec_item_id = 0;
                if (empty($oldspecitem)) {
                    pdo_insert('superdesk_shop_goods_spec_item', $d);
                    $spec_item_id = pdo_insertid();
                } else {
                    pdo_update('superdesk_shop_goods_spec_item', $d, array('id' => $oldspecitem['id']));
            ...
            if (0 < count($spec_itemids)) {
                pdo_query('delete from ' . tablename('superdesk_shop_goods_spec_item') . ' where specid=:specid and id not in (' . implode(',', $spec_itemids) . ')', array(':specid' => $specid));
            } else {
                pdo_query('delete from ' . tablename('superdesk_shop_goods_spec_item') . ' where specid=:specid ', array(':specid' => $specid));
            }
            pdo_update('superdesk_shop_goods_spec', array('content' => serialize($spec_itemids)), array('id' => $oldspec['id']));
        }
        if (0 < count($specids)) {
            pdo_query('delete from ' . tablename('superdesk_shop_goods_spec') . ' where goodsid=:goodsid and id not in (' . implode(',', $specids) . ')', array(':goodsid' => $goodsid));
        } else {
            pdo_query('delete from ' . tablename('superdesk_shop_goods_spec') . ' where goodsid=:goodsid ', array(':goodsid' => $goodsid));
        }
        $minprice = 0;
        ...
        foreach ($options as $o) {
            ...
            if (empty($oldoption)) {
                pdo_insert('superdesk_shop_goods_option', $do);
                $option_id = pdo_insertid();
            } else {
                pdo_update('superdesk_shop_goods_option', $do, array('id' => $oldoption['id']));
        ...
        if (0 < count($optionids)) {
            ...
            pdo_query(
                'delete from ' . tablename('superdesk_shop_goods_option') .
                ' where goodsid=:goodsid and id not in (' . implode(',', $optionids) . ')',
                array(':goodsid' => $goodsid)
            );
        } else {
        ...
        pdo_update('superdesk_shop_goods', $d, array('id' => $goodsid));

        if ($d['hasoption']) {
            $sql =
                'update ' . tablename('superdesk_shop_goods') . ' g ' .
                ' set' .
                '   g.minprice = (' .
                '               select min(marketprice) ' .
                '               from ' . tablename('superdesk_shop_goods_option') .
                '               where goodsid = ' . $goodsid . ' and marketprice > 0' .
                '               ),' .
                '   g.maxprice = (' .
            ...
            $sql =
                ...
                ' where id = ' . $goodsid . ' and hasoption=0;';
        }
    ...
    public function save_jingdong_goods($item = array(), $catch_url = '')
    {
        ...
        $goods = pdo_fetch(
            'select * from ' . tablename('superdesk_shop_goods') .
            ' where  catch_id=:catch_id and catch_source=\'jingdong\' and uniacid=:uniacid and merchid=:merchid',
            array(':catch_id' => $item['itemId'], ':uniacid' => $_W['uniacid'], ':merchid' => $item['merchid'])
        );
        ...
        foreach ($params as $p) {
            $oldp = pdo_fetch(
                'select * from ' . tablename('superdesk_shop_goods_param') .
                ' where goodsid=:goodsid and title=:title limit 1',
                array(':goodsid' => $goodsid, ':title' => $p['title'])
            );
        ...
        if (0 < count($paramids)) {
            ...
            pdo_query(
                'delete from ' . tablename('superdesk_shop_goods_param') .
                ' where goodsid=:goodsid and id not in (' . implode(',', $paramids) . ')',
                array(':goodsid' => $goodsid)
            );
        } else {
    ...
    public function save_1688_goods($item = array(), $catch_url = '')
    {
        ...
        $goods = pdo_fetch(
            'select * from ' . tablename('superdesk_shop_goods') .
            ' where  catch_id=:catch_id and catch_source=\'1688\' and uniacid=:uniacid and merchid=:merchid',
            array(
                ':catch_id' => $item['itemId'],
                ':uniacid'  => $_W['uniacid'],
                ':merchid'  => $item['merchid']
        ...
        if (0 < count($paramids)) {
            ...
            pdo_query('delete from ' . tablename('superdesk_shop_goods_param') .
                ' where goodsid=:goodsid and id not in (' . implode(',', $paramids) . ')',
                array(':goodsid' => $goodsid));
        } else {
        ...
        foreach ($specs as $spec) {
            $oldspec = pdo_fetch(
                ...
                ' where goodsid=:goodsid and propId=:propId limit 1',
                array(':goodsid' => $goodsid, ':propId' => $spec['propId'])
            );
            ...
            foreach ($spec_items as $spec_item) {
                ...
                $oldspecitem = pdo_fetch(
                    'select * from ' . tablename('superdesk_shop_goods_spec_item') .
                    ' where specid=:specid and valueId=:valueId limit 1',
                    array(
                        ':specid'  => $specid,
                        ':valueId' => $spec_item['valueId']
                    )
            ...
            if (0 < count($spec_itemids)) {
                ...
                pdo_query(
                    'delete from ' . tablename('superdesk_shop_goods_spec_item') .
                    ' where specid=:specid and id not in (' . implode(',', $spec_itemids) . ')',
                    array(':specid' => $specid)
                );
            } else {
            ...
            pdo_update('superdesk_shop_goods_spec',
                array('content' => serialize($spec_itemids)),
                array('id' => $oldspec['id'])
            );
        }
        ...
        if (0 < count($specids)) {
            pdo_query(
                'delete from ' . tablename('superdesk_shop_goods_spec') .
                ' where goodsid=:goodsid and id not in (' . implode(',', $specids) . ')',
                array(':goodsid' => $goodsid));
        } else {
            pdo_query(
                'delete from ' . tablename('superdesk_shop_goods_spec') .
