...
class Upgrade_SuperdeskShopV2Page extends SystemPage
{
    public function main()
    {
        ...
        $auth        = get_auth();
        $versionfile = IA_ROOT . '/addons/superdesk_shopv2/version.php';
        $updatedate  = date('Y-m-d H:i', filemtime($versionfile));

        $version = SUPERDESK_SHOPV2_VERSION;
        $release = SUPERDESK_SHOPV2_RELEASE;
        ...
        $resp = ihttp_request(SUPERDESK_SHOPV2_AUTH_URL,
            array(
                'ip'     => $ip,
                'id'     => $id,
                'code'   => $auth['code'],
    ...
    public function check()
    {
        ...
        $plugins = pdo_fetchall('select `identity` from ' . tablename('superdesk_shop_plugin'), array(), 'identity');
        ...
        $auth    = get_auth();
        $version = ((defined('SUPERDESK_SHOPV2_VERSION') ? SUPERDESK_SHOPV2_VERSION : '2.0.0'));
        $release = ((defined('SUPERDESK_SHOPV2_RELEASE') ? SUPERDESK_SHOPV2_RELEASE : '201605010000'));

        $resp = ihttp_post(SUPERDESK_SHOPV2_AUTH_URL . 'check',
            array(
                'ip'      => $auth['ip'],
                'id'      => $auth['id'],
                'code'    => $auth['code'],
            ...
            if ($result['status'] == 1) {
                ...
                if (!empty($upgrade['files'])) {
                    foreach ($upgrade['files'] as $file) {
                        $entry = SUPERDESK_SHOPV2_PATH . $file['path'];
                ...
                cache_write('cloud:modules:upgradev2', array(
                    'files'    => $files,
        ...
        if (is_file(SUPERDESK_SHOPV2_PATH . 'tmp')) {
            @unlink(SUPERDESK_SHOPV2_PATH . 'tmp');
        }
    ...
    public function process()
    {
        ...
        if ($action == 'file') {
            ...
            if (!empty($path)) {
                ...
                $resp = ihttp_post(
                    SUPERDESK_SHOPV2_AUTH_URL . 'download',
                    array(
                        'ip'     => $auth['ip'],
                        'id'     => $auth['id'],
                        'code'   => $auth['code'],
                ...
                if (is_array($ret)) {
                    ...
                    if (!is_dir(SUPERDESK_SHOPV2_PATH . $dirpath)) {
                        mkdirs(SUPERDESK_SHOPV2_PATH . $dirpath);
                        @chmod(SUPERDESK_SHOPV2_PATH . $dirpath, 511);
                    }

                    $content = base64_decode($ret['content']);
                    file_put_contents(SUPERDESK_SHOPV2_PATH . $path, $content);

                    if (isset($ret['path1'])) {
                        $path1    = $ret['path1'];
                        $dirpath1 = dirname($path1);

                        if (!is_dir(SUPERDESK_SHOPV2_PATH . $dirpath1)) {
                            mkdirs(SUPERDESK_SHOPV2_PATH . $dirpath1);
                            @chmod(SUPERDESK_SHOPV2_PATH . $dirpath1, 511);
                        }


                        $content1 = base64_decode($ret['content1']);
                        file_put_contents(SUPERDESK_SHOPV2_PATH . $path1, $content1);
                    }
                    ...
                    if (isset($ret['path2'])) {
                        ...
                        if (!is_dir(SUPERDESK_SHOPV2_PATH . $dirpath2)) {
                            mkdirs(SUPERDESK_SHOPV2_PATH . $dirpath2);
                            @chmod(SUPERDESK_SHOPV2_PATH . $dirpath2, 511);
                        }


                        $content2 = base64_decode($ret['content2']);
                        file_put_contents(SUPERDESK_SHOPV2_PATH . $path2, $content2);
                    }
        ...
        if ($action == 'upgrade') {
            ...
            if (!empty($update)) {
                $updatepath = SUPERDESK_SHOPV2_PATH . 'tmp/';
    ...
    protected function updateComplete($version, $release)
    {
        load()->func('file');
        file_put_contents(
            SUPERDESK_SHOPV2_PATH . 'version.php',
            '<?php if(!defined(\'IN_IA\')) {exit(\'Access Denied\');}if(!defined(\'SUPERDESK_SHOPV2_VERSION\')) {define(\'SUPERDESK_SHOPV2_VERSION\', \'' . $version . '\');}if(!defined(\'SUPERDESK_SHOPV2_RELEASE\')) {define(\'SUPERDESK_SHOPV2_RELEASE\', \'' . $release . '\');}');
        ...
        my_scandir(IA_ROOT . '/addons/superdesk_shopv2');

        foreach ($my_scenfiles as $file) {
            if (!strexists($file, '/superdesk_shopv2/data/') && !strexists($file, 'version.php')) {
                @touch($file, $time);
            }

        }

        rmdirs(IA_ROOT . '/addons/superdesk_shopv2/tmp');
    }

    public function checkversion()
    {
        file_put_contents(
            IA_ROOT . '/addons/superdesk_shopv2/version.php',
            '<?php if(!defined(\'IN_IA\')) {exit(\'Access Denied\');}if(!defined(\'SUPERDESK_SHOPV2_VERSION\')) {define(\'SUPERDESK_SHOPV2_VERSION\', \'2.0.0\');}if(!defined(\'SUPERDESK_SHOPV2_RELEASE\')) {define(\'SUPERDESK_SHOPV2_RELEASE\', \'201605010000\');}');
