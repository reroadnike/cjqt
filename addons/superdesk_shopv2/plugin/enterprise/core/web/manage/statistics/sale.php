...
class Sale_SuperdeskShopV2Page extends EnterpriseWebPage
{
			...
			while ($hour < 24) {
				...
				$dr = array(
					...
					'count' => pdo_fetchcolumn(
						' SELECT ifnull(' . $countfield . ',0) as cnt FROM ' . tablename('superdesk_shop_order') .// TODO 标志 楼宇之窗 openid shop_order 不处理
						' WHERE uniacid=:uniacid and enterprise_id=:enterprise_id and status>=1 and createtime >=:starttime and createtime <=:endtime',
						array(
							':uniacid' => $_W['uniacid'],
