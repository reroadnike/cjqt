define(['core', 'tpl'], function (core, tpl) {
    ...
    modal.getData = function (element) {
        ...
        cells.each(function () {
            var $this = $(this);
            var itemid = $this.data('itemid'), field = '#' + itemid, must = $this.data('must') == '1', type = $this.data('type'), name = $this.data('name'), tp_is_default = $this.data('tp_is_default'), key = $this.data('key');
            if (must) {
                if (type == 0 || type == 1) {
                    if ($(field).isEmpty()) {
