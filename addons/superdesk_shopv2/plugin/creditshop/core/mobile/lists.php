...
class Lists_SuperdeskShopV2Page extends PluginMobileLoginPage
{
    public function main()
    {
        ...
        if (!empty($cateid)) {
            $cate = pdo_fetch('select id,name from ' . tablename('superdesk_shop_creditshop_category') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $cateid, ':uniacid' => $uniacid));
        }
    ...
    public function getlist()
    {
        ...
        $sql   = 'SELECT COUNT(*) FROM ' . tablename('superdesk_shop_creditshop_goods') . ' where 1 ' . $condition;
        $total = pdo_fetchcolumn($sql, $params);
        $list  = array();
        if (!empty($total)) {
            $sql  = 'SELECT id,title,thumb,subtitle,type,credit,money FROM ' . tablename('superdesk_shop_creditshop_goods') . '  where 1 ' . $condition . ' ORDER BY displayorder desc,id DESC LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize;
