...
class Log_SuperdeskShopV2Page extends PluginWebPage
{
    public function main()
    {
        ...
        $sql =
            ' select ' .
            '       log.*, m.nickname,m.avatar,m.realname as mrealname,m.mobile as mmobile, g.title,g.thumb,g.thumb,g.credit,g.money,g.type as goodstype, ' .
            '        g.isverify,g.goodstype as iscoupon,s.storename,s.address as storeaddress ' .
            ' from ' . tablename('superdesk_shop_creditshop_log') . ' log ' . // TODO 标志 楼宇之窗 openid shop_creditshop_log 不处理
            '       left join ' . tablename('superdesk_shop_member') . ' m on m.openid = log.openid and m.uniacid=log.uniacid' .
            '       left join ' . tablename('superdesk_shop_member_address') . ' a on a.id = log.addressid' . // TODO 标志 楼宇之窗 openid superdesk_shop_member_address 不处理
            '       left join ' . tablename('superdesk_shop_store') . ' s on s.id = log.storeid' .
            '       left join ' . tablename('superdesk_shop_creditshop_goods') . ' g on g.id = log.goodsid' .
            ' where 1 ' .
            $condition .
        ...
        foreach ($list as &$row) {
            ...
            if (!empty($row['addressid'])) {
                $row['address'] = pdo_fetch('select realname,mobile,address,province,city,area from ' . tablename('superdesk_shop_member_address') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $row['addressid'], ':uniacid' => $_W['uniacid']));
            } else {
    ...
    public function detail()
    {
        ...
        if ($goods['isverify']) {
            if (!empty($log['storeid'])) {
                $store = pdo_fetch('select id,storename,address  from ' . tablename('superdesk_shop_store') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $log['storeid'], ':uniacid' => $_W['uniacid']));
            }
