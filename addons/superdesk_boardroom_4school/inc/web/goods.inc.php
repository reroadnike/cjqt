...
//$sql = 'SELECT * FROM ' . tablename('superdesk_boardroom_4school_s_category') . ' WHERE `uniacid` = :uniacid ORDER BY `parentid`, `displayorder` DESC';
...
if ($operation == 'post') {
    ...
    if (!empty($id)) {
        $item = pdo_fetch("SELECT * FROM " . tablename('superdesk_boardroom_4school_s_goods') . " WHERE id = :id", array(':id' => $id));
        if (empty($item)) {
            message('抱歉，商品不存在或是已经删除！', '', 'error');
        }
        $allspecs = pdo_fetchall("SELECT * FROM " . TABLENAME('superdesk_boardroom_4school_s_spec') . " WHERE goodsid=:id ORDER BY displayorder ASC", array(":id" => $id));
        foreach ($allspecs as &$s) {
            $s['items'] = pdo_fetchall("select * from " . tablename('superdesk_boardroom_4school_s_spec_item') . " where specid=:specid order by displayorder asc", array(":specid" => $s['id']));
        }
        unset($s);
        $params = pdo_fetchall("select * from " . tablename('superdesk_boardroom_4school_s_goods_param') . " where goodsid=:id order by displayorder asc", array(':id' => $id));
        ...
        //处理规格项
        $html = "";
        $options = pdo_fetchall("select * from " . tablename('superdesk_boardroom_4school_s_goods_option') . " where goodsid=:id order by id asc", array(':id' => $id));
        //排序好的specs
        $specs = array();
        //找出数据库存储的排列顺序
        if (count($options) > 0) {
    ...
    if (checksubmit('submit')) {
        ...
        if (empty($id)) {
            pdo_insert('superdesk_boardroom_4school_s_goods', $data);
            $id = pdo_insertid();
        } else {
            unset($data['createtime']);
            pdo_update('superdesk_boardroom_4school_s_goods', $data, array('id' => $id));
        }
        ...
        for ($k = 0; $k < $len; $k++) {
            ...
            if (!is_numeric($get_param_id)) {
                pdo_insert("superdesk_boardroom_4school_s_goods_param", $a);
                $param_id = pdo_insertid();
            } else {
                pdo_update("superdesk_boardroom_4school_s_goods_param", $a, array('id' => $get_param_id));
                $param_id = $get_param_id;
            }
            $paramids[] = $param_id;
        }
        if (count($paramids) > 0) {
            pdo_query("delete from " . tablename('superdesk_boardroom_4school_s_goods_param') . " where goodsid=$id and id not in ( " . implode(',', $paramids) . ")");
        } else {
            pdo_query("delete from " . tablename('superdesk_boardroom_4school_s_goods_param') . " where goodsid=$id");
        }

//        if ($totalstocks > 0) {
//            pdo_update("superdesk_boardroom_4school_s_goods", array("total" => $totalstocks), array("id" => $id));
//        }
        ...
        for ($k = 0; $k < $len; $k++) {
            ...
            if (is_numeric($get_spec_id)) {
                pdo_update("superdesk_boardroom_4school_s_spec", $a, array("id" => $get_spec_id));
                $spec_id = $get_spec_id;
            } else {
                pdo_insert("superdesk_boardroom_4school_s_spec", $a);
            ...
            for ($n = 0; $n < $itemlen; $n++) {
                ...
                if (is_numeric($get_item_id)) {
                    pdo_update("superdesk_boardroom_4school_s_spec_item", $d, array("id" => $get_item_id));
                    $item_id = $get_item_id;
                } else {
                    pdo_insert("superdesk_boardroom_4school_s_spec_item", $d);
            ...
            //删除其他的
            if (count($itemids) > 0) {
                pdo_query("delete from " . tablename('superdesk_boardroom_4school_s_spec_item') . " where uniacid={$_W['uniacid']} and specid=$spec_id and id not in (" . implode(",", $itemids) . ")");
            } else {
                pdo_query("delete from " . tablename('superdesk_boardroom_4school_s_spec_item') . " where uniacid={$_W['uniacid']} and specid=$spec_id");
            }
            //更新规格项id
            pdo_update("superdesk_boardroom_4school_s_spec", array("content" => serialize($itemids)), array("id" => $spec_id));
            $specids[] = $spec_id;
        }
        //删除其他的
        if (count($specids) > 0) {
            pdo_query("delete from " . tablename('superdesk_boardroom_4school_s_spec') . " where uniacid={$_W['uniacid']} and goodsid=$id and id not in (" . implode(",", $specids) . ")");
        } else {
            pdo_query("delete from " . tablename('superdesk_boardroom_4school_s_spec') . " where uniacid={$_W['uniacid']} and goodsid=$id");
        }
        //保存规格
        ...
        for ($k = 0; $k < $len; $k++) {
            ...
            if (empty($get_option_id)) {
                pdo_insert("superdesk_boardroom_4school_s_goods_option", $a);
                $option_id = pdo_insertid();
            } else {
                pdo_update("superdesk_boardroom_4school_s_goods_option", $a, array('id' => $get_option_id));
                $option_id = $get_option_id;
            }
            $optionids[] = $option_id;
        }
        if (count($optionids) > 0) {
            pdo_query("delete from " . tablename('superdesk_boardroom_4school_s_goods_option') . " where goodsid=$id and id not in ( " . implode(',', $optionids) . ")");
        } else {
            pdo_query("delete from " . tablename('superdesk_boardroom_4school_s_goods_option') . " where goodsid=$id");
        }
        //总库存
    ...
    if (!empty($total)) {
        $sql = 'SELECT * FROM ' . tablename('superdesk_boardroom_4school_s_goods') . $condition . ' ORDER BY `status` DESC, `displayorder` DESC,
    ...
    $id = intval($_GPC['id']);
    $row = pdo_fetch("SELECT id, thumb FROM " . tablename('superdesk_boardroom_4school_s_goods') . " WHERE id = :id", array(':id' => $id));
    if (empty($row)) {
...
//			}
//			pdo_delete('superdesk_boardroom_4school_s_goods', array('id' => $id));
    //修改成不直接删除，而设置deleted=1
    pdo_update("superdesk_boardroom_4school_s_goods", array("deleted" => 1), array('id' => $id));
    message('删除成功！', referer(), 'success');

} elseif ($operation == 'productdelete') {

    $id = intval($_GPC['id']);
    pdo_delete('superdesk_boardroom_4school_s_product', array('id' => $id));
