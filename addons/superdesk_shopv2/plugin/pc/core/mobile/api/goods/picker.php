...
class Picker_SuperdeskShopV2Page extends PcMobileLoginPage
{
    public function main()
    {
        ...
        $goods = pdo_fetch(
            ' select id,thumb,title,marketprice,total,maxbuy,minbuy,unit,hasoption,showtotal, ' .
            '        diyformid,diyformtype,diyfields,isdiscount,isdiscount_time,isdiscount_discounts, ' .
        ...
        if (!(empty($_W['shopset']['wap']['open'])) && !(empty($_W['shopset']['wap']['mustbind'])) && empty($member['mobileverify'])) {
            ...
            show_json(3,'WAP模式未打开?强制绑定手机&手机未验证?');
        }
