...
class Creditlog_SuperdeskShopV2Page extends PluginMobileLoginPage
{
    public function main()
    {
        ...
        $credit = intval(m('member')->getCredit($_W['openid'], $_W['core_user'], 'credit1'));
    ...
    public function getlist()
    {
        ...
        if (!(empty($total))) {
            ...
            $sql =
                ' SELECT log.id,log.goodsid,g.title,g.thumb,g.credit,g.type,g.money,log.createtime, log.status, g.thumb ' .
                ' FROM ' . tablename('superdesk_shop_creditshop_log') . ' log ' .// TODO 标志 楼宇之窗 openid shop_creditshop_log 已处理
