...
class Reg_SuperdeskShopV2Page extends PluginWebPage
{
    public function main()
    {
        ...
        $total = pdo_fetchcolumn('select count(*) from' . tablename('superdesk_shop_merch_reg') . '  where uniacid = :uniacid ' . $condition, $params);
    ...
    public function detail()
    {
        ...
        if ($_W['ispost']) {
            ...
            pdo_update('superdesk_shop_merch_reg', $item, array('id' => $item['id']));
            ...
            if ($status == 1) {
                ...
                if (empty($usercount)) {
                    ...
                    $user['regid']  = $item['id'];
                    $user['status'] = 0;
                    pdo_insert('superdesk_shop_merch_user', $user);
                    $userid = pdo_insertid();
                    pdo_update('superdesk_shop_merch_reg', $item, array('id' => $item['id']));
    ...
    public function delete()
    {
        ...
        foreach ($regs as $reg) {
            ...
            pdo_delete('superdesk_shop_merch_reg', array('id' => $reg['id']));
