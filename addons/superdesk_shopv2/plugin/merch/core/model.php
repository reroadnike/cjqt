...
class MerchModel extends PluginModel
{
    ...
    public function getCategory($data = array())
    {
        ...
        return pdo_fetchall(
            ' select ' . $res['field'] .
            ' from ' . tablename('superdesk_shop_merch_category') .
            $res['condition'],
            $res['params'],
            $res['column']
        );
    ...
    public function getCategorySwipe($data = array())
    {
        ...
        return pdo_fetchall(
            ' select ' . $res['field'] .
            ' from ' . tablename('superdesk_shop_merch_category_swipe') .
            $res['condition'],
            $res['params'],
            $res['column']
        );
    ...
    private function check($permtype = '')
    {
        ...
        $user = pdo_fetch(
            ...
            ' from ' . tablename('superdesk_shop_merch_account') . ' u ' .
            '       left join ' . tablename('superdesk_shop_merch_perm_role') . ' r on u.roleid = r.id ' .
            ' where u.id=:merchuid ' .
            ' limit 1 ',
            array(
                ':merchuid' => $uid
    ...
    public function log($type = '', $op = '')
    {
        ...
        pdo_insert('superdesk_shop_merch_perm_log', $log);
    }
    ...
    public function getUserTotals()
    {
        ...
        $totals = array(
            'reg0'  => pdo_fetchcolumn(
                ' select count(*) ' .
                ' from ' . tablename('superdesk_shop_merch_reg') .
                ' where uniacid=:uniacid ' .
                '       and status=0 ' .
                ' limit 1',
                array(
    ...
    public function getClearTotals()
    {
        ...
        $totals = array(
            'status0' => pdo_fetchcolumn(
                ' select count(*) ' .
                ' from ' . tablename('superdesk_shop_merch_clearing') .
                ' where uniacid=:uniacid ' .
                '       and status=0 ' .
                ' limit 1',
                array(
    ...
    public function getMerchOrderTotals($type = 0)
    {
        ...
        $sql =
            ...
            ' left join ' . tablename('superdesk_shop_merch_user') . ' u on u.id = o.merchid ' .
            ' where 1 ' .
            $condition . ' ';
    ...
    public function getMerchOrderTotalPrice($merchid)
    {
        ...
        $sql =
            ...
            ' from ' . tablename('superdesk_shop_merch_bill') .
            ' where 1 ' .
            $condition .
            ' and status=1';
    ...
    public function getMerchPrice($merchid, $flag = 0)
    {
        ...
        $sql =
            ...
            ' left join ' . tablename('superdesk_shop_order') . ' o on u.id=o.merchid' .// TODO 标志 楼宇之窗 openid shop_order 不处理
            ' where 1 ' .
            $condition .
            ' limit 1';

        $list             = pdo_fetch($sql, $params);
        $merchcouponprice = pdo_fetchcolumn(
            ' select sum(o.couponprice) ' .
            ' from ' . tablename('superdesk_shop_merch_user') . ' u ' .
            ' left join ' . tablename('superdesk_shop_order') . ' o on u.id=o.merchid' .// TODO 标志 楼宇之窗 openid shop_order 不处理
            ' where o.couponmerchid>0 ' . $condition .
            ' limit 1',
            $params
        );
    ...
    public function getMerchPriceList($merchid, $orderid = 0, $flag = 0)
    {
        ...
        $con =
            'o.id,u.merchname,u.payrate,o.price,o.goodsprice,o.dispatchprice,discountprice,' .
    ...
    public function getOneApply($id)
    {
        ...
        $sql  =
            ' select b.*,u.merchname,u.realname,u.mobile ' .
            ' from ' . tablename('superdesk_shop_merch_bill') . ' b ' .
            ' left join ' . tablename('superdesk_shop_merch_user') . ' u on b.merchid = u.id' .
            ' where 1 ' .
            $condition .
            ' Limit 1';
    ...
    public function setDefault($id, $type, $merchid = 0)
    {
        ...
        if (!empty($item)) {
            ...
            if ($type == 1) {
                plog(
                    'merch.exhelper.temp.express.setdefault',
            ...
            if ($type == 2) {
                plog(
                    'merch.exhelper.temp.invoice.setdefault',
    ...
    public function tempDelete($id, $type, $merchid = 0)
    {
        ...
        foreach ($items as $item) {
            ...
            if ($type == 1) {
                plog(
                    'merch.exhelper.temp.express.delete',
                    '删除 快递助手 快递单模板 ID: ' . $item['id'] . '， 模板名称: ' . $item['expressname'] . ' ');
            } else if ($type == 2) {
                plog(
                    'merch.exhelper.temp.invoice.delete',
