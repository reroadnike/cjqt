...
if ($_GPC['op'] == 'getTaxCode') {
    $taxCode = getTaxCode($id, $_W['merchid'], $item['jd_vop_sku']);
...
$similar_item = pdo_fetch(
    ' select * from ' . tablename('superdesk_shop_goods_similar') .
...
if (!empty($similar_item)) {
    $similar_goods = pdo_fetchall(
        ' select id,title,thumb from ' . tablename('superdesk_shop_goods') .
...
if ($_W['ispost']) {
    ...
    $data = array(
        ...
        'repair'             => intval($_GPC['repair']), 'seven' => intval($_GPC['seven']),
...
//        pdo_update('superdesk_shop_goods', $data, array('id' => $id));
        ...
        $this->_shopGoodsService->updateByColumn(
            $data,
            array('id' => $id)
        );
    ...
    if ($merchid != SUPERDESK_SHOPV2_JD_VOP_MERCHID) {
        ...
        $check_tax_code = pdo_fetch(' select * from ' . tablename('superdesk_shop_goods_exts') . 'where sku=:id', array(':id' => $id));
    ...
    if (0 < count($paramids)) {
        pdo_query('delete from ' . tablename('superdesk_shop_goods_param') . ' where goodsid=' . $id . ' and id not in ( ' . implode(',', $paramids) . ')');
    } else {
        pdo_query('delete from ' . tablename('superdesk_shop_goods_param') . ' where goodsid=' . $id);
    }
    ...
    while ($k < $len) {
        ...
        if (0 < count($itemids)) {
            pdo_query('delete from ' . tablename('superdesk_shop_goods_spec_item') . ' where uniacid=' . $_W['uniacid'] . ' and specid=' . $spec_id . ' and id not in (' . implode(',', $itemids) . ')');
        } else {
            pdo_query('delete from ' . tablename('superdesk_shop_goods_spec_item') . ' where uniacid=' . $_W['uniacid'] . ' and specid=' . $spec_id);
        }

        pdo_update('superdesk_shop_goods_spec', array('content' => serialize($itemids)), array('id' => $spec_id));
        $specids[] = $spec_id;
        ++$k;
    }

    if (0 < count($specids)) {
        pdo_query('delete from ' . tablename('superdesk_shop_goods_spec') . ' where uniacid=' . $_W['uniacid'] . ' and goodsid=' . $id . ' and id not in (' . implode(',', $specids) . ')');
    } else {
        pdo_query('delete from ' . tablename('superdesk_shop_goods_spec') . ' where uniacid=' . $_W['uniacid'] . ' and goodsid=' . $id);
    }
...
//    pdo_update('superdesk_shop_goods', array('isdiscount_discounts' => $is_discounts_json), array('id' => $id));
    $this->_shopGoodsService->updateByColumn(
        array('isdiscount_discounts' => $is_discounts_json),
        array('id' => $id)
    );

    if ((0 < count($optionids)) && ($data['hasoption'] !== 0)) {
        pdo_query('delete from ' . tablename('superdesk_shop_goods_option') . ' where goodsid=' . $id . ' and id not in ( ' . implode(',', $optionids) . ')');
        $sql = 'update ' . tablename('superdesk_shop_goods') . ' g set' . "\n" . '            g.minprice = (select min(marketprice) from ' . tablename('superdesk_shop_goods_option') . ' where goodsid = ' . $id . '),' . "\n" . '            g.maxprice = (select max(marketprice) from ' . tablename('superdesk_shop_goods_option') . ' where goodsid = ' . $id . ')' . "\n" . '            where g.id = ' . $id . ' and g.hasoption=1';
        pdo_query($sql);
    } else {
        pdo_query('delete from ' . tablename('superdesk_shop_goods_option') . ' where goodsid=' . $id);
        $sql = 'update ' . tablename('superdesk_shop_goods') . ' set minprice = marketprice,maxprice = marketprice where id = ' . $id . ' and hasoption=0;';
        pdo_query($sql);
    }
...
//        pdo_update('superdesk_shop_goods', array('total' => $totalstocks), array('id' => $id));
        $this->_shopGoodsService->updateByColumn(
            array('total' => $totalstocks),
            array('id' => $id)
        );
    }
    ...
    show_json(1, array(
        'url' => merchUrl('goods/edit', array('id' => $id, 'tab' => str_replace('#tab_', '', $_GPC['tab'])))
    ));
}
...
if (!empty($id)) {
    ...
    foreach ($allspecs as &$s) {
        ...
        $s['items'] = pdo_fetchall(
            ...
            ' from ' . tablename('superdesk_shop_goods_spec_item') . ' a ' .
            '       left join ' . tablename('superdesk_shop_virtual_type') . ' b on b.id=a.virtual ' .
...
{
    if ($merchid != SUPERDESK_SHOPV2_JD_VOP_MERCHID) {
        $goods_exts = pdo_fetch('select taxCode from ' . tablename('superdesk_shop_goods_exts') . 'where sku=:id', array(':id' => $id));
    } else {
        $goods_exts = pdo_fetch('select taxCode from ' . tablename('superdesk_jd_vop_product_exts') . 'where sku=:id', array(':id' => $sku));
    }
