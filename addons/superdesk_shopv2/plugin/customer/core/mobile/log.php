...
if ($_W['isajax']) {
    ...
    if ($operation == 'display') {
        ...
        $sql   =
            'SELECT ' .
            '       COUNT(*) ' .
            ' FROM ' . tablename('superdesk_shop_creditshop_log') . ' log ' .
            ' where 1 ' .
            $condition;// TODO 标志 楼宇之窗 openid shop_creditshop_log 已处理
        ...
        if (!empty($total)) {
            $sql  =
                'SELECT ' .
                '       log.id,log.goodsid,log.status,log.eno,log.paystatus,g.title,g.type,g.thumb,g.credit,g.money ' .
        ...
        if (!empty($log['addressid'])) {
            $address = pdo_fetch(
                ...
                ' from ' . tablename('superdesk_shop_member_address') . // TODO 标志 楼宇之窗 openid superdesk_shop_member_address 待处理
                ' where ' .
                '       id=:id ' .
                '       and openid=:openid ' .
                '       and uniacid=:uniacid ' .
