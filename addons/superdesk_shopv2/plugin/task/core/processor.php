...
require SUPERDESK_SHOPV2_INC . 'plugin_processor.php';
...
class TaskProcessor extends PluginProcessor
{
    ...
    private function responseScan($obj)
    {
        ...
        if ($member_info['isnew']) {
            if (!empty($join_info)) {
                $this->reward($member_info, $poster, $join_info, $qr, $openid, $qrmember);
            ...
            $scan_count = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('superdesk_shop_task_joiner') . ' where uniacid=:uniacid and task_user=:task_user and joiner_id=:joiner_id and join_id=:join_id ', $params);
    ...
    private function reward($member_info, $poster, $join_info, $qr, $openid, $core_user, $qrmember)
    {
        ...
        if (($join_info['needcount'] == $count) && ($join_info['is_reward'] == 0)) {
            ...
            pdo_insert('superdesk_shop_task_log', $reward_log);
            $log_id = pdo_insertid();
            ...
            pdo_insert('superdesk_shop_task_joiner', $scaner);
                ...
                if ($key == 'rec') {
                    ...
                    if (isset($val['credit']) && (0 < $val['credit'])) {
                        m('member')->setCredit($qr['openid'], $qr['core_user'],
                            'credit1', $val['credit'], array(0, '推荐扫码关注积分+' . $val['credit']));
                    }
                    ...
                    if (isset($val['money']) && (0 < $val['money']['num'])) {
                        ...
                        m('finance')->pay($qr['openid'], $qr['core_user'],
                            $val['money']['type'], $pay, '', '任务活动推荐奖励', false);
                    }
                    ...
                    if (isset($val['bribery']) && (0 < $val['bribery'])) {
                        ...
                        $err = m('common')->sendredpack($params, $wechat);// 红包
                    ...
                    if (0 < $val['credit']) {
                        m('member')->setCredit($openid, $core_user /*这个地方有问题*/,
                            'credit1', $val['credit'], array(0, '扫码关注积分+' . $val['credit']));
                    }
                    ...
                    if (0 < $val['money']['num']) {
                        ...
                        $res = m('finance')->pay($openid, $core_user, $val['money']['type'], $pay, '', '任务活动奖励', false);
                        if (is_error($res)) {
            ...
            foreach ($reward_data as $key => $val) {
                ...
                if ($key == 'sub') {
                    ...
                    if (0 < $val['money']['num']) {
                        ...
                        $res = m('finance')->pay($openid, $core_user,
                            $val['money']['type'], $pay, '', '任务活动奖励', false);
            ...
            $default_text = pdo_fetchcolumn(
                'SELECT `data` FROM ' . tablename('superdesk_shop_task_default') .
                ' WHERE uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'])
            );
                    ...
                    if ($default_text['is_completed'] == 1) {
                        if (!empty($default_text['completed'])) {
                            ...
                            foreach ($default_text['completed'] as $key => $val) {
                                $default_text['completed'][$key]['value'] = $this->model->notice_complain($val['value'], $qrmember, $poster, $member_info, 2);
                            }
                            if ($default_text['templateid']) {
