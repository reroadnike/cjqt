...
class Goods_SuperdeskShopV2Page extends EnterpriseWebPage
{
	public function main()
	{
		...
		$psize = 20;
		$condition = ' and og.uniacid=:uniacid and og.enterprise_id=:enterprise_id and o.status>=1';
		...
		$sql =
			' select og.price,og.total,o.createtime,o.ordersn,g.title,g.thumb,g.goodssn,op.goodssn as optiongoodssn ' .
			...
			' left join ' . tablename('superdesk_shop_goods_option') . ' op on op.id = og.optionid ' .
			' where 1 ' . $condition .
		...
		$total = pdo_fetchcolumn(
			' select  count(*) from ' . tablename('superdesk_shop_order_goods') . ' og ' .// TODO 标志 楼宇之窗 openid shop_order_goods 不处理
			' left join ' . tablename('superdesk_shop_order') . ' o on o.id = og.orderid ' . // TODO 标志 楼宇之窗 openid shop_order 不处理
			' left join ' . tablename('superdesk_shop_goods') . ' g on g.id = og.goodsid ' .
			' where 1 ' . $condition,
			$params
