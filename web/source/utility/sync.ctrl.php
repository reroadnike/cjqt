...
if (!empty($_W['uniacid'])) {
    ...
    if ($_W['account']['type'] == 1 && $_W['account']['level'] >= 3) {
        ...
        $data = pdo_fetchall(
            ' SELECT fanid, openid, acid, uid, uniacid ' .
            ' FROM ' . tablename('mc_mapping_fans') .
            " WHERE uniacid = :uniacid AND acid = :acid AND follow = 1 '.
            ' ORDER BY updatetime ASC, fanid DESC '.
            ' LIMIT 10",
            array(
                ':uniacid' => $_W['uniacid'],
                ':acid'    => $_W['acid']
...
//							$rec['avatar'] = rtrim($fan['headimgurl'], '0') . 132;
                            $rec['avatar'] = rtrim($fan['headimgurl'], '0');
                        }
                        if (!empty($rec)) {
