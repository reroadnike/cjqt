...
class Set_SuperdeskShopV2Page extends PluginWebPage
{
    public function main()
    {
        ...
        if ($_W['ispost']) {
            ...
            $data = array(
                'showmember'  => intval($_GPC['showmember']),
                'keyword'     => trim($_GPC['keyword']),
                'enter_title' => trim($_GPC['enter_title']),
                'enter_img'   => trim($_GPC['enter_img']),
            ...
            if (!empty($keyword)) {
                if ($keyword['name'] != 'superdesk_shopv2:qa') {
                    show_json(0, '关键字已存在！');
                }
            ...
            $cover_data = array(
                ...
                'title'       => $data['enter_title'],
                'description' => $data['enter_desc'],
                'thumb'       => $data['enter_img'],
                'url'         => mobileUrl('qa')
            );
            ...
            if (!empty($set)) {
                pdo_update('superdesk_shop_qa_set', $data, array('id' => $set['id']));
            } else {
                $data['uniacid'] = $_W['uniacid'];
                pdo_insert('superdesk_shop_qa_set', $data);
                $id = pdo_insertid();
            }
