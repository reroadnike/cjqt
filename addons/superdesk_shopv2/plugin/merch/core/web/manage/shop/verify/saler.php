...
class Saler_SuperdeskShopV2Page extends MerchWebPage
{
    ...
    public function main()
    {
        ...
        $sql = 'SELECT ' .
            '       s.*,m.nickname,m.avatar,m.mobile,m.realname,store.storename ' .
            ' FROM ' . tablename('superdesk_shop_merch_saler') . '  s ' .
            ' left join ' . tablename('superdesk_shop_member') . ' m on s.openid=m.openid and s.core_user=m.core_user and m.uniacid = s.uniacid ' . // TODO 标志 楼宇之窗 openid superdesk_shop_member 已处理
            ' left join ' . tablename('superdesk_shop_merch_store') . ' store on store.id=s.storeid ' .
            ' WHERE ' .
            $condition .
            ' ORDER BY id asc';
    ...
    protected function post()
    {
        ...
        if ($_W['ispost']) {
            ...
            $data = array(
                ...
                'core_user' => trim($_GPC['core_user']), // TODO core_user 界面缺失
            ...
            if (!empty($id)) {
                ...
                pdo_insert('superdesk_shop_merch_saler', $data);
                $id = pdo_insertid();
            ...
            show_json(1, array(
                'url' => merchUrl('shop/verify/saler')
            ));
        }
    ...
    public function query()
    {
        ...
        $ds = pdo_fetchall(
            ...
            ' FROM ' . tablename('superdesk_shop_merch_saler') . '  s ' .
            ' left join ' . tablename('superdesk_shop_member') . ' m on s.openid=m.openid and s.core_user=m.core_user ' .// TODO 标志 楼宇之窗 openid superdesk_shop_member 已处理
            ' left join ' . tablename('superdesk_shop_merch_store') . ' store on store.id=s.storeid ' .
            ' WHERE 1 ' .
            $condition .
            ' ORDER BY id asc',
