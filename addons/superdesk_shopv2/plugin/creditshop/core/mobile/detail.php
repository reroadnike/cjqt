...
class Detail_SuperdeskShopV2Page extends PluginMobileLoginPage
{
    ...
    public function pay($a = array(), $b = array())
    {
        ...
        if (0 < $goods['money']) {
            ...
            $lastlog = pdo_fetch(
                ...
                ' where goodsid=:goodsid and openid=:openid and core_user=:core_user  and status=0 and paystatus=1 and uniacid=:uniacid limit 1',
                array(
                    ':goodsid'   => $id,
                    ':openid'    => $_W['openid'],
                    ':core_user' => $_W['core_user'],
        ...
        pdo_insert('superdesk_shop_creditshop_log', $log);// TODO 标志 楼宇之窗 openid shop_creditshop_log 已处理

        $logid = pdo_insertid();

        if (!empty($log['realname']) && !empty($log['mobile'])) {
            $up = array('realname' => $log['realname'], 'mobile' => $log['mobile']);
            pdo_update('superdesk_shop_member', $up, array('id' => $member['id'], 'uniacid' => $_W['uniacid']));
            if (!empty($member['uid'])) {
                load()->model('mc');
    ...
    public function lottery()
    {
        ...
        pdo_update('superdesk_shop_creditshop_goods', array('joins' => $goods['joins'] + 1), array('id' => $id));
        ...
        if ((0 < $goods['money']) && empty($log['paystatus'])) {
            if ($log['paytype'] == 0) {
                m('member')->setCredit($_W['openid'], $_W['core_user'],
                    'credit2', -$goods['money'], '' . $_W['shopset']['trade']['credittext'] . '商城抽奖扣除余额度 ' . $goods['money']);
                $update['paystatus'] = 1;
            }
            if ($log['paytype'] == 1) {
                if (empty($log['paystatus'])) {
        ...
        if ((0 < $goods['credit']) && empty($log['creditpay'])) {
            m('member')->setCredit($_W['openid'], $_W['core_user'],
                'credit1', -$goods['credit'], '' . $_W['shopset']['trade']['credittext'] . '商城抽奖扣除' . $_W['shopset']['trade']['credittext'] . ' ' . $goods['credit']);
            $update['creditpay'] = 1;
        }
        ...
        pdo_update('superdesk_shop_creditshop_log', $update, array('id' => $logid));// TODO 标志 楼宇之窗 openid shop_creditshop_log 不处理
