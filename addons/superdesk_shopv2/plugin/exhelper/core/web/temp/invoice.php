...
class Invoice_SuperdeskShopV2Page extends PluginWebPage
{
    ...
    protected function post()
    {
        ...
        if (!empty($id)) {
            $item = pdo_fetch('select * from ' . tablename('superdesk_shop_exhelper_express') . ' where id=:id and type=:type and uniacid=:uniacid and merchid=0 limit 1', array(':id' => $id, ':type' => $type, ':uniacid' => $_W['uniacid']));
            if (!empty($item)) {
        ...
        if ($_W['ispost']) {
            ...
            if (!empty($id)) {
                pdo_update('superdesk_shop_exhelper_express', $data, array('id' => $id, 'merchid' => 0));
            ...
            if (!empty($data['isdefault'])) {
                pdo_update('superdesk_shop_exhelper_express', array('isdefault' => 0), array('type' => 2, 'uniacid' => $_W['uniacid'], 'merchid' => 0));
                pdo_update('superdesk_shop_exhelper_express', array('isdefault' => 1), array('type' => 2, 'id' => $id, 'merchid' => 0));
            }
