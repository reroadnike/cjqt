        ...
        if (div.length > 0) {
            ...
            batch.on("click",
                function() {
                    ...
                    var select_group_id = 0;
                    if($this.data('toggle') == 'batch-groups'){
                        select_group_id = $('#select_group_id').val();
                    }

                    if($this.data('toggle') == 'batch-redirect'){
                        location.href = href + '&ids=' + selecteds.join(',');
                        return
                    }
                                    ...
                                    var deferred = $.Deferred(),
                                        removeHandler = function(def) {
                                            ...
                                            return chks.parents("tr").fadeOut(function() {
                                                $(this).remove(),
                                                    c++,
                                                chks.length == c && def.resolve()
                                            }),
                                                def
                                        };
                                    $.when(removeHandler(deferred)).done(function() {
                                        batch.attr("disabled", 0 == $('tbody tr td:first-child [type="checkbox"]:checked', div).length),
                                        0 == $("table tbody tr", div).length && window.location.reload()
                                    })
                                } else {
                    ...
                    if ($this.data('confirm')) {
                        tip.confirm($this.data('confirm'), submit,
                            function() {
                ...
                $(document).on("click", '.table-responsive tbody [data-toggle="ajaxRemove"]',
                    function(e) {
                        e.preventDefault();
                        var obj = $(this),
                            ...
                            $.post(url).done(function(data) {
                                ...
                                if (data.status == 1) {
                                    ...
                                    tr.css({
                                        ...
                                        tr.fadeOut(function() {
                                            tr.remove(),
                                            0 == $(obj.closest('tbody')).find('tr').length && window.location.reload()
                                        })
                                } else {
                                    obj.button('reset'),
