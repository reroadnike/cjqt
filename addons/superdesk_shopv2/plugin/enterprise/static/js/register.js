define(['core', 'tpl', 'biz/plugin/diyform'], function (core, tpl, diyform) {
    ...
    modal.init = function (params) {
        ...
        $('.btn-submit').click(function () {
            ...
            if ($('#enterprise_name').isEmpty()) {
                FoxUI.toast.show('请填写企业名称!');
                return
            }
            if ($('#salecate').isEmpty()) {
            ...
            if ($(".diyform-container").length > 0) {
                diyformdata = diyform.getData('.page-enterprise-register .diyform-container');
                if (!diyformdata) {
                    return
                }
            ...
            core.json('enterprise/register', data, function (pjson) {
                if (pjson.status == 0) {
