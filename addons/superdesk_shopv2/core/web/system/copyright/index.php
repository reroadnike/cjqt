...
class Index_SuperdeskShopV2Page extends SystemPage
{
    public function main()
    {
        ...
        if (!empty($wechatid) && ($wechatid != -1)) {
            $copyrights = pdo_fetch(
                ' select * ' .
                ' from ' . tablename('superdesk_shop_system_copyright') .
                ' where uniacid=' . $wechatid .
                '       and ismanage=0 ' .
                'limit 1'
            );
        }
        ...
        if ($_W['ispost']) {
            ...
            foreach ($sets as $set) {
                ...
                if (($wechatid == $uniacid) || ($wechatid == -1)) {
                    ...
                    if (empty($cs)) {
                        pdo_insert(
                            'superdesk_shop_system_copyright',
                            array(
                                'uniacid'   => $uniacid,
                                'copyright' => $post,
            ...
            if ($wechatid == -1) {
                $global_copyrights = pdo_fetchcolumn(
                    ' select count(*) ' .
                    ' from ' . tablename('superdesk_shop_system_copyright') .
                    ' where uniacid=-1 ' .
                    '		and ismanage=0 ' .
