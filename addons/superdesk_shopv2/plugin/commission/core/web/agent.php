...
class Agent_SuperdeskShopV2Page extends PluginWebPage 
{
	public function main() 
	{
		...
		$sql = 'select dm.*,dm.nickname,dm.avatar,l.levelname,p.nickname as parentname,p.avatar as parentavatar from ' . tablename('superdesk_shop_member') . ' dm ' . ' left join ' . tablename('superdesk_shop_member') . ' p on p.id = dm.agentid ' . ' left join ' . tablename('superdesk_shop_commission_level') . ' l on l.id = dm.agentlevel' . ' left join ' . tablename('mc_mapping_fans') . 'f on f.openid=dm.openid and f.uniacid=' . $_W['uniacid'] . ' where dm.uniacid = ' . $_W['uniacid'] . ' and dm.isagent =1  ' . $condition . ' ORDER BY dm.agenttime desc';
		if (empty($_GPC['export'])) 
		{
			$sql .= ' limit ' . (($pindex - 1) * $psize) . ',' . $psize;
		}
		$list = pdo_fetchall($sql, $params);
		$total = pdo_fetchcolumn('select count(dm.id) from' . tablename('superdesk_shop_member') . ' dm  ' . ' left join ' . tablename('superdesk_shop_member') . ' p on p.id = dm.agentid ' . ' left join ' . tablename('mc_mapping_fans') . 'f on f.openid=dm.openid' . ' where dm.uniacid =' . $_W['uniacid'] . ' and dm.isagent =1 ' . $condition, $params);
		foreach ($list as &$row ) 
		{
		...
		if ($_GPC['export'] == '1') 
		{
			...
			plog('commission.agent.export', '导出营销商数据');
			foreach ($list as &$row ) 
			{
			...
			unset($row);
			$columns = array( array('title' => 'ID', 'field' => 'id', 'width' => 12), array('title' => '昵称', 'field' => 'nickname', 'width' => 12), array('title' => '姓名', 'field' => 'realname', 'width' => 12), array('title' => '手机号', 'field' => 'mobile', 'width' => 12), array('title' => '微信号', 'field' => 'weixin', 'width' => 12), array('title' => 'openid', 'field' => 'openid', 'width' => 24), array('title' => '推荐人', 'field' => 'parentname', 'width' => 12), array('title' => '营销商等级', 'field' => 'levelname', 'width' => 12), array('title' => '点击数', 'field' => 'clickcount', 'width' => 12), array('title' => '下线营销商总数', 'field' => 'levelcount', 'width' => 12), array('title' => '一级下线营销商数', 'field' => 'level1', 'width' => 12), array('title' => '二级下线营销商数', 'field' => 'level2', 'width' => 12), array('title' => '三级下线营销商数', 'field' => 'level3', 'width' => 12), array('title' => '累计佣金', 'field' => 'commission_total', 'width' => 12), array('title' => '打款佣金', 'field' => 'commission_pay', 'width' => 12), array('title' => '注册时间', 'field' => 'createtime', 'width' => 12), array('title' => '成为营销商时间', 'field' => 'createtime', 'width' => 12), array('title' => '审核状态', 'field' => 'createtime', 'width' => 12), array('title' => '是否关注', 'field' => 'followstr', 'width' => 12) );
			if (p('diyform')) 
			{
				$columns[] = array('title' => '营销商会员自定义信息', 'field' => 'member_diyformdata', 'width' => 36);
				$columns[] = array('title' => '营销商申请自定义信息', 'field' => 'agent_diyformdata', 'width' => 36);
			}
			m('excel')->export($list, array('title' => '营销商数据-' . date('Y-m-d-H-i', time()), 'columns' => $columns));
		}
	...
	public function delete() 
	{
		...
		{
			pdo_update('superdesk_shop_member', array('isagent' => 0, 'status' => 0), array('id' => $member['id']));
			plog('commission.agent.delete', '取消营销商资格 <br/>营销商信息:  ID: ' . $member['id'] . ' /  ' . $member['openid'] . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile']);
		}
	...
	public function user() 
	{
		...
		$level11 = pdo_fetchcolumn('select count(*) from ' . tablename('superdesk_shop_member') . ' where isagent=0 and agentid=:agentid and uniacid=:uniacid limit 1', array(':agentid' => $agentid, ':uniacid' => $_W['uniacid']));
		$condition = '';
		$params = array();
		if (empty($level)) 
		{
		...
		if ($hasagent) 
		{
			$total = pdo_fetchcolumn('select count(dm.id) from' . tablename('superdesk_shop_member') . ' dm ' . ' left join ' . tablename('superdesk_shop_member') . ' p on p.id = dm.agentid ' . ' left join ' . tablename('mc_mapping_fans') . 'f on f.openid=dm.openid' . ' where dm.uniacid =' . $_W['uniacid'] . '  ' . $condition, $params);
			$list = pdo_fetchall('select dm.*,p.nickname as parentname,p.avatar as parentavatar  from ' . tablename('superdesk_shop_member') . ' dm ' . ' left join ' . tablename('superdesk_shop_member') . ' p on p.id = dm.agentid ' . ' left join ' . tablename('mc_mapping_fans') . 'f on f.openid=dm.openid  and f.uniacid=' . $_W['uniacid'] . ' where dm.uniacid = ' . $_W['uniacid'] . '  ' . $condition . '   ORDER BY dm.agenttime desc limit ' . (($pindex - 1) * $psize) . ',' . $psize, $params);
			$pager = pagination($total, $pindex, $psize);
			foreach ($list as &$row ) 
			{
				$info = $this->model->getInfo($row['openid'], array('total', 'pay'));
	...
	public function check() 
	{
		...
		$members = pdo_fetchall('SELECT id,openid,agentid,nickname,realname,mobile,status FROM ' . tablename('superdesk_shop_member') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);
		$time = time();
		foreach ($members as $member ) 
		{
			...
			if ($status == 1) 
			{
				pdo_update('superdesk_shop_member', array('status' => 1, 'agenttime' => $time), array('id' => $member['id'], 'uniacid' => $_W['uniacid']));
				plog('commission.agent.check', '审核营销商 <br/>营销商信息:  ID: ' . $member['id'] . ' /  ' . $member['openid'] . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile']);
			...
			{
				pdo_update('superdesk_shop_member', array('status' => 0, 'agenttime' => 0), array('id' => $member['id'], 'uniacid' => $_W['uniacid']));
				plog('commission.agent.check', '取消审核 <br/>营销商信息:  ID: ' . $member['id'] . ' /  ' . $member['openid'] . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile']);
			}
		}
		show_json(1, array('url' => referer()));
	}
	public function agentblack() 
	{
		...
		$members = pdo_fetchall('SELECT id,openid,nickname,realname,mobile,agentblack FROM ' . tablename('superdesk_shop_member') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);
		foreach ($members as $member ) 
		{
			...
			if ($agentblack == 1) 
			{
				pdo_update('superdesk_shop_member', array('isagent' => 1, 'status' => 0, 'agentblack' => 1), array('id' => $member['id']));
				plog('commission.agent.agentblack', '设置黑名单 <br/>营销商信息:  ID: ' . $member['id'] . ' /  ' . $member['openid'] . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile']);
			}
			else 
			{
				pdo_update('superdesk_shop_member', array('isagent' => 1, 'status' => 1, 'agentblack' => 0), array('id' => $member['id']));
				plog('commission.agent.agentblack', '取消黑名单 <br/>营销商信息:  ID: ' . $member['id'] . ' /  ' . $member['openid'] . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile']);
			}
		}
		show_json(1, array('url' => referer()));
	}
