...
if (!empty($_W['account']['oauth']) && $_W['account']['oauth']['level'] == '4') {
    ...
    // WeUtility::logging("bootstrap.app.inc.php : @_SERVER[REQUEST_URI]   ", "http://" . $_SERVER[HTTP_HOST] . $_SERVER[REQUEST_URI]);
    // WeUtility::logging("bootstrap.app.inc.php : @_W[container]          ", $_W['container']);
    // WeUtility::logging("bootstrap.app.inc.php : @_GPC[logout]           ", $_GPC['logout']);
    // WeUtility::logging("bootstrap.app.inc.php : @_SESSION[openid]       ", $_SESSION['openid']);
    // WeUtility::logging("bootstrap.app.inc.php : @_SESSION[oauth_openid] ", $_SESSION['oauth_openid']);
    // WeUtility::logging("bootstrap.app.inc.php : @_W[openid]             ", $_W['openid']);
    // WeUtility::logging("bootstrap.app.inc.php : @_W[fans][openid]       ", $_W['fans']['openid']);

    // WeUtility::logging("bootstrap.app.inc.php : @controller", $controller);
	if (
        ($_W['container'] == 'wechat'
			&& !$_GPC['logout']
			&& empty($_W['openid'])
			&& ($controller != 'auth' || ($controller == 'auth' && !in_array($action, array('forward', 'oauth'))))
        )
			||
		($_W['container'] == 'wechat'
			&& !$_GPC['logout']
			&& empty($_SESSION['oauth_openid'])
			&& ($controller != 'auth')
        )
	) {
