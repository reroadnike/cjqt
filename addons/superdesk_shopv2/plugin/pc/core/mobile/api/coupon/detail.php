...
class Detail_SuperdeskShopV2Page extends PcMobileLoginPage
{
    public function main()
    {
        ...
        $coupon = pdo_fetch('select * from ' . tablename('superdesk_shop_coupon') . ' where id=:id and uniacid=:uniacid  limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
        if (empty($coupon)) {
            show_json(0, '暂无数据');
        }
        ...
        if ($coupon['limitgoodtype'] != 0) {
            ...
            $goods = pdo_fetchall('select `title` from ' . tablename('superdesk_shop_goods') . ' where uniacid=:uniacid ' . $where, array(':uniacid' => $_W['uniacid']), 'id');
        }
        if ($coupon['limitgoodcatetype'] != 0) {
            if (!empty($coupon['limitgoodcateids'])) {
                $where = 'and id in(' . $coupon['limitgoodcateids'] . ')';
            }
            $category = pdo_fetchall('select `name`  from ' . tablename('superdesk_shop_category') . ' where uniacid=:uniacid   ' . $where, array(':uniacid' => $_W['uniacid']), 'id');
        }
        ...
        if ($coupon['islimitlevel'] == 1) {
            ...
            if (!empty($coupon['limitmemberlevels']) || ($coupon['limitmemberlevels'] == '0')) {
                ...
                $level1 = pdo_fetchall('select * from ' . tablename('superdesk_shop_member_level') . ' where uniacid=:uniacid and  id in (' . $coupon['limitmemberlevels'] . ') ', array(':uniacid' => $_W['uniacid']));
                if (in_array($member['level'], $limitmemberlevels)) {
                    $pass = true;
                }
            }
            if ((!empty($coupon['limitagentlevels']) || ($coupon['limitagentlevels'] == '0')) && $hascommission) {
                $level2 = pdo_fetchall('select * from ' . tablename('superdesk_shop_commission_level') . ' where uniacid=:uniacid and id  in (' . $coupon['limitagentlevels'] . ') ', array(':uniacid' => $_W['uniacid']));
            ...
            if ((!empty($coupon['limitpartnerlevels']) || ($coupon['limitpartnerlevels'] == '0')) && $hasglobonus) {
                $level3 = pdo_fetchall('select * from ' . tablename('superdesk_shop_globonus_level') . ' where uniacid=:uniacid and  id in(' . $coupon['limitpartnerlevels'] . ') ', array(':uniacid' => $_W['uniacid']));
            ...
            if ((!empty($coupon['limitaagentlevels']) || ($coupon['limitaagentlevels'] == '0')) && $hasabonus) {
                $level4 = pdo_fetchall('select * from ' . tablename('superdesk_shop_abonus_level') . ' where uniacid=:uniacid and  id in (' . $coupon['limitaagentlevels'] . ') ', array(':uniacid' => $_W['uniacid']));
        ...
        if ($coupon['descnoset'] == '0') {
            if ($coupon['coupontype'] == '0') {
                $coupon['desc'] = $set['consumedesc'];
            } else {
                $coupon['desc'] = $set['rechargedesc'];
            }
        }

        show_json(1,
            compact(
                'coupon', 'pass', 'goods', 'category',
                'meblvname', 'level1',
                'hascommission', 'leveltitle2', 'limitagentlevels', 'commissionname', 'level2',
                'hasglobonus', 'leveltitle3', 'globonuname', 'level3',
                'hasabonus', 'leveltitle4', 'abonuname', 'level4'
            )
        );
    }
    ...
    public function pay($a = array(), $b = array())
    {
        ...
        if (0 < $coupon['money']) {
            pdo_delete('superdesk_shop_coupon_log', // TODO 标志 楼宇之窗 openid superdesk_shop_coupon_log 已处理
                array(
                    'couponid'  => $id,
                    'openid'    => $_W['openid'],
            ...
            $lastlog = pdo_fetch(
                'select * ' .
                ' from ' . tablename('superdesk_shop_coupon_log') . // TODO 标志 楼宇之窗 openid superdesk_shop_coupon_log 已处理
                ' where ' .
                '       couponid=:couponid ' .
                '       and openid=:openid ' .
        ...
        $log = array(
            ...
            'couponid'     => $id,
            'status'       => 0,
            'paystatus'    => (0 < $coupon['money'] ? 0 : -1),
            'creditstatus' => (0 < $coupon['credit'] ? 0 : -1),
            'createtime'   => time(),
            'getfrom'      => 1);
        ...
        if ($needpay) {
            ...
            if (!empty($coupon['usecredit2'])) {
                $money = m('member')->getCredit($_W['openid'], 'credit2');
                if ($coupon['money'] <= $money) {
                    $useweixin = false;
                }
            }
            pdo_update('superdesk_shop_coupon_log', // TODO 标志 楼宇之窗 openid superdesk_shop_coupon_log 不处理
                array(
    ...
    public function payresult($a = array())
    {
        ...
        $log = pdo_fetch(
            'select id,logno,status,paystatus,couponid ' .
