    ...
    public function complete()
    {
        ...
        if (is_h5app()) {
            ...
            if (!(m("finance")->isAlipayNotify($_GET))) {
                $log = pdo_fetch('SELECT * FROM ' . tablename('core_paylog') . ' WHERE `uniacid`=:uniacid AND `module`=:module AND `tid`=:tid limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'superdesk_shopv2', ':tid' => $tid));
                ...
                if (!(empty($log)) && !(empty($log['status']))) {
                    ...
                    if ($fromwechat) {
                        ...
                        $this->message(
                            array(
                                'message' => '请返回微信查看支付状态',
                                'title'   => '支付成功!', 'buttondisplay' => false
                            ),
                            NULL,
                            'success'
                        );
