...
class Comment_SuperdeskShopV2Page extends WebPage
{
	public function main()
	{
		...
		$list = pdo_fetchall(
			' SELECT  c.*, o.ordersn,g.title,g.thumb FROM ' . tablename('superdesk_shop_order_comment') . ' c  ' .// TODO 标志 楼宇之窗 openid superdesk_shop_order_comment 不处理
		...
		$total = pdo_fetchcolumn(
			'SELECT count(*) FROM ' . tablename('superdesk_shop_order_comment') . ' c  ' .// TODO 标志 楼宇之窗 openid superdesk_shop_order_comment 不处理
			' left join ' . tablename('superdesk_shop_goods') . ' g on c.goodsid = g.id  ' .
			' left join ' . tablename('superdesk_shop_order') . ' o on c.orderid = o.id  ' . // TODO 标志 楼宇之窗 openid shop_order 不处理
			' WHERE 1 ' . $condition . ' ',
			$params
	...
	public function delete()
	{
		...
		$items = pdo_fetchall(
			'SELECT id FROM ' . tablename('superdesk_shop_order_comment') . // TODO 标志 楼宇之窗 openid superdesk_shop_order_comment 不处理
		...
		foreach ($items as $item ) {
			...
			$goods = pdo_fetch('select id,thumb,title from ' . tablename('superdesk_shop_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $item['goodsid'], ':uniacid' => $_W['uniacid']));
	...
	protected function virtual()
	{
		...
		$id = intval($_GPC['id']);
		$item = pdo_fetch(
			'SELECT * FROM ' . tablename('superdesk_shop_order_comment') . // TODO 标志 楼宇之窗 openid superdesk_shop_order_comment 不处理
		...
		if ($_W['ispost']) {
			...
			$goods = set_medias(pdo_fetch('select id,thumb,title from ' . tablename('superdesk_shop_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $goodsid, ':uniacid' => $_W['uniacid'])), 'thumb');
			...
			if (!empty($id)) {
				...
				pdo_insert('superdesk_shop_order_comment', $data);// TODO 标志 楼宇之窗 openid superdesk_shop_order_comment 不处理
				$id = pdo_insertid();
		...
		$goods = pdo_fetch('select id,thumb,title from ' . tablename('superdesk_shop_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $goodsid, ':uniacid' => $_W['uniacid']));
	...
	public function post()
	{
		...
		$goods = pdo_fetch('select id,thumb,title from ' . tablename('superdesk_shop_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $item['goodsid'], ':uniacid' => $_W['uniacid']));
		$order = pdo_fetch(
			' select id,ordersn from ' . tablename('superdesk_shop_order') .// TODO 标志 楼宇之窗 openid shop_order 不处理
			' where id=:id and uniacid=:uniacid limit 1',
			array(
				':id' => $item['orderid'],
				':uniacid' => $_W['uniacid']
		...
		if ($_W['ispost']) {
			if ($type == 0) {
				...
				pdo_update('superdesk_shop_order_comment', $data, array('id' => $id));// TODO 标志 楼宇之窗 openid superdesk_shop_order_comment 不处理
				...
				$checked_array = array('审核通过', '待审核', '审核不通过');
				pdo_update('superdesk_shop_order_comment', $change_data, array('id' => $id));// TODO 标志 楼宇之窗 openid superdesk_shop_order_comment 不处理
