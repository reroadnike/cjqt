...
class Temp_SuperdeskShopV2Page extends MerchWebPage
{
    ...
    protected function post()
    {
        ...
        if ($_W['ispost']) {
            ...
            if (empty($id)) {
                ...
                pdo_insert('superdesk_shop_virtual_type', $insert);
                $id = pdo_insertid();

                mplog('virtual.temp.edit', '添加模板 ID: ' . $id);
            } else {

                pdo_update('superdesk_shop_virtual_type', $insert, array('id' => $id));

                mplog('virtual.temp.edit', '编辑模板 ID: ' . $id);
            }

            show_json(1, array(
                'url' => merchUrl('goods/virtual/temp')
            ));
        }
    ...
    public function delete()
    {
        ...
        foreach ($types as $type) {
            pdo_delete('superdesk_shop_virtual_type', array('id' => $type['id']));
            pdo_delete('superdesk_shop_virtual_data', array('typeid' => $type['id']));
