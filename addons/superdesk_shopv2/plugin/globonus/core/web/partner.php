...
class Partner_SuperdeskShopV2Page extends PluginWebPage
{
    public function main()
    {
        ...
        $sql =
            ...
            ' left join ' . tablename('superdesk_shop_globonus_level') . ' l on l.id = dm.partnerlevel' .
            ' where ' .
            '       dm.uniacid = ' . $_W['uniacid'] .
            '       and (dm.ispartner =1 or dm.ispartner=-1)  ' .
            $condition .
            ' ORDER BY dm.partnertime desc';
        ...
        foreach ($list as &$row) {
            ...
            $bonus = $this->model->getBonus($row['openid'], $row['core_user'], array('ok'));// TODO 标志 楼宇之窗 openid shop_globonus_billp 已处理
    ...
    public function delete()
    {
        ...
        foreach ($members as $member) {
            ...
            plog('globonus.partner.delete',
                '取消股东资格 <br/>股东信息:  ID: ' . $member['id'] . ' /  ' . $member['openid'] . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile']);
        }
    ...
    public function check()
    {
        ...
        $members = pdo_fetchall(
            'SELECT id,openid,nickname,realname,mobile,partnerstatus ' .
            ' FROM ' . tablename('superdesk_shop_member') .
            ' WHERE ' .
            '       id in( ' . $id . ' ) ' .
            '       AND uniacid=' . $_W['uniacid']
        ...
        foreach ($members as $member) {
            ...
            if ($status == 1) {
                ...
                plog('globonus.partner.check',
                    '审核股东 <br/>股东信息:  ID: ' . $member['id'] . ' /  ' . $member['openid'] . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile']);
                ...
                plog('globonus.partner.check',
                    '取消审核 <br/>股东信息:  ID: ' . $member['id'] . ' /  ' . $member['openid'] . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile']);
            }
        }
    ...
    public function partnerblack()
    {
        ...
        $members = pdo_fetchall(
            'SELECT id,openid,nickname,realname,mobile ' .
            ' FROM ' . tablename('superdesk_shop_member') .
            ' WHERE ' .
            '       id in( ' . $id . ' ) ' .
            '       AND uniacid=' . $_W['uniacid']
        ...
        foreach ($members as $member) {
            ...
            if ($partnerblack == 1) {
                ...
                plog('globonus.partner.partnerblack',
                    '设置黑名单 <br/>股东信息:  ID: ' . $member['id'] . ' /  ' . $member['openid'] . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile']);
                ...
                plog('globonus.partner.partnerblack',
                    '取消黑名单 <br/>股东信息:  ID: ' . $member['id'] . ' /  ' . $member['openid'] . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile']);
            }
        }
