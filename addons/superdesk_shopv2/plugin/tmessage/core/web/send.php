...
class Send_SuperdeskShopV2Page extends PluginWebPage
{
    public function main()
    {
        ...
        $list = pdo_fetchall(
            ...
            ' WHERE uniacid = \'' . $_W['uniacid'] . '\' ORDER BY level asc');

        $list2 = pdo_fetchall(
            'SELECT * ' .
            ' FROM ' . tablename('superdesk_shop_member_group') .
            ' WHERE uniacid = \'' . $_W['uniacid'] . '\' ORDER BY id asc');
    ...
    public function fetch()
    {
        ...
        pdo_update(
            ...
            array('sendtimes' => $send['sendtimes'] + 1),
            array('id' => $id)
        );
        ...
        if ($class1 == 1) {
            ...
            $member = pdo_fetchall(
                'SELECT openid ' .
                ' FROM ' . tablename('superdesk_shop_member') .
                ' WHERE uniacid = \'' . $_W['uniacid'] . '\' and isagent=1 and status=1 ' .
                $where,
                array(),
                'openid'
            );
