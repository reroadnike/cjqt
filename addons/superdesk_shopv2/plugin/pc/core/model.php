    ...
    public function packetmoney($goodsid)
    {
        ...
        if ($goods['packettype'] == 1) {
            ...
            if ($MoneyPackage['remainSize'] == 1) {
                ...
                return array(
                    "status" => 1,
                    "money"  => intval($MoneyPackage['remainMoney'] * 100) / 100
                );
            }
...
//                $money = $money;
    ...
    public function getGoods($id, $member, $optionid = 0)
    {
        ...
        if (empty($goods['canbuy'])) {
            ...
            if ($goods['hasoption'] && $optionid) {
                ...
                $option = pdo_fetch(
                    'select total,credit,money,title as optiontitle,weight ' .
    ...
    public function sendMessage($id = 0)
    {
        ...
        if (strexists($detailurl, '/addons/superdesk_shopv2/')) {
            $detailurl = str_replace('/addons/superdesk_shopv2/', '/', $detailurl);
        }
        ...
        if ($log['status'] == 2) {
            if (!(empty($type))) {
                ...
                $msg = array(
                    ...
                    'keyword3' => array('title' => '兑换时间', 'value' => date('Y-m-d', time()), 'color' => '#4a5077'),
                    'keyword4' => array('title' => '剩余积分', 'value' => $credit, 'color' => '#4a5077'),
                    'remark'   => array('value' => $remark, 'color' => '#4a5077')
                );
            ...
            if (strexists($detailurl1, '/addons/superdesk_shopv2/')) {
                $detailurl1 = str_replace('/addons/superdesk_shopv2/', '/', $detailurl1);
            }
            ...
            if ($goods['verifytype'] == 0) {
                ...
                pdo_update('superdesk_shop_creditshop_log', // TODO 标志 楼宇之窗 openid shop_creditshop_log 不处理
                    array(
                        'status'       => 3,
                        'usetime'      => time(),
                        'verifyopenid' => $openid, // TODO 标志 楼宇之窗 openid shop_creditshop_log 待处理
                        'time_finish'  => time()
                    ),
                    array(
                        'id' => $logid
                ...
                pdo_insert("superdesk_shop_creditshop_verify", $data);
