...
require SUPERDESK_SHOPV2_INC . 'com_processor.php';
...
class CouponProcessor extends ComProcessor
{
    ...
    public function respondText($obj)
    {
        ...
        $member = m('member')->getMember($openid,$core_user/*返回问题*/);

        $couponkey = $content;

        if (isset($_SESSION['superdesk_shop_coupon_key'])) {
            $couponkey = $_SESSION['superdesk_shop_coupon_key'];
        } else {
            $_SESSION['superdesk_shop_coupon_key'] = $content;
        }

        $coupon = pdo_fetch(
            'select ' .
            '       id,couponname,pwdkey2,pwdask,pwdsuc,pwdfail,pwdfull,pwdtimes,pwdurl,pwdwords,pwdown,pwdexit,pwdexitstr ' .
        ...
        if (empty($coupon)) {
            ...
            $obj->endContext();
            unset($_SESSION['superdesk_shop_coupon_key']);
            return $this->responseEmpty();
        }
        ...
        if (!$obj->inContext) {
            $guessok = pdo_fetch(
                ...
                '       and openid=:openid and pwdkey=:pwdkey and ok=1 and uniacid=:uniacid limit 1 ',
                array(
                    ':couponid' => $coupon['id'],
                    ':openid'   => $openid,
                    ':pwdkey'   => $coupon['pwdkey2'],
        ...
        $guess = pdo_fetch(
            'select id,times ' .
            ' from ' . tablename('superdesk_shop_coupon_guess') .
            ' where couponid=:couponid and openid=:openid and pwdkey=:pwdkey and uniacid=:uniacid ' .
            ' limit 1 ',
            array(
                ':couponid' => $coupon['id'],
                ':openid'   => $openid,
        ...
        if (empty($guess)) {
            ...
            pdo_insert(
                'superdesk_shop_coupon_guess',
                $guess
            );
        } else {

            pdo_update('superdesk_shop_coupon_guess',
                array(
                    'times' => $guess['times'] + 1,
                    'ok'    => ($ok ? 1 : 0)
                ),
