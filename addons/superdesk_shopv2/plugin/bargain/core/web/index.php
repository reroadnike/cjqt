...
class Index_SuperdeskShopV2Page extends PluginWebPage
{
    public function __construct()
    {
        ...
        $res = pdo_fetch(
            'SELECT * ' .
            ' FROM ' . tablename('superdesk_shop_bargain_account') .
            ' WHERE ' .
            '       id = :id',
            array(
                ':id' => $_W['uniacid']
        ...
        if (empty($res)) {
            ...
            pdo_insert(
                'superdesk_shop_bargain_account',
                array(
                    'id'        => $_W['uniacid'],
                    'mall_name' => '砍价商城'
                )
    ...
    public function main()
    {
        ...
        if (empty($_GPC['search'])) {
            ...
            $sql = 'SELECT goods.*,g.total,thumb,marketprice,sales,title FROM' . tablename('superdesk_shop_bargain_goods') . ' goods INNER JOIN' . tablename('superdesk_shop_goods') . ' g ON goods.goods_id = g.id WHERE g.total > 0 AND unix_timestamp(goods.end_time )>' . time() . ' AND unix_timestamp(goods.start_time )<' . time() . ' AND goods.status = \'0\' AND g.status = \'1\' AND g.uniacid = :uniacid AND g.deleted = \'0\' AND goods.account_id = :uniacid ORDER BY goods.id DESC LIMIT ' . (($page * $psize) - $psize) . ',' . $psize;

            $onSell = pdo_fetchall($sql, array(':uniacid' => $_W['uniacid']));

            $all_sql = 'SELECT COUNT(*) FROM' . tablename('superdesk_shop_bargain_goods') . ' goods INNER JOIN' . tablename('superdesk_shop_goods') . ' g ON goods.goods_id = g.id WHERE unix_timestamp(goods.end_time )>' . time() . ' AND g.total > 0 AND unix_timestamp(goods.start_time )>' . time() . ' AND goods.status = \'0\' AND g.status = \'1\' AND g.uniacid = :uniacid AND g.deleted = \'0\' AND goods.account_id = :uniacid';
    ...
    public function notstart()
    {
        ...
        if (empty($_GPC['search'])) {
            ...
            $sql = 'SELECT goods.*,g.total,thumb,marketprice,sales,title FROM' . tablename('superdesk_shop_bargain_goods') . ' goods INNER JOIN' . tablename('superdesk_shop_goods') . ' g ON goods.goods_id = g.id WHERE unix_timestamp(goods.start_time )>' . time() . ' AND goods.status = \'0\' AND g.status = \'1\' AND g.uniacid = :uniacid AND g.deleted = \'0\' AND goods.account_id = :uniacid ORDER BY goods.id DESC LIMIT ' . (($page * $psize) - $psize) . ',' . $psize;

            $onSell = pdo_fetchall($sql, array(':uniacid' => $_W['uniacid']));

            $all_sql = 'SELECT COUNT(*) FROM' . tablename('superdesk_shop_bargain_goods') . ' goods INNER JOIN' . tablename('superdesk_shop_goods') . ' g ON goods.goods_id = g.id WHERE unix_timestamp(goods.start_time )>' . time() . ' AND goods.status = \'0\' AND g.status = \'1\' AND g.uniacid = :uniacid AND g.deleted = \'0\' AND goods.account_id = :uniacid';
            ...
            $all_sql = 'SELECT COUNT(*) FROM' . tablename('superdesk_shop_bargain_goods') . ' goods INNER JOIN' . tablename('superdesk_shop_goods') . ' g ON goods.goods_id = g.id WHERE unix_timestamp(goods.start_time )>' . time() . ' AND goods.status = \'0\' AND g.status = \'1\' AND g.uniacid = :uniacid AND g.deleted = \'0\' AND g.title LIKE :keyword AND goods.account_id = :uniacid';
    ...
    public function complete()
    {
        ...
        $psize = 20;
        if (empty($_GPC['search'])) {
            $sql     = 'SELECT goods.*,g.total,thumb,marketprice,sales,title FROM' . tablename('superdesk_shop_bargain_goods') . ' goods INNER JOIN' . tablename('superdesk_shop_goods') . ' g ON goods.goods_id = g.id WHERE unix_timestamp(goods.end_time )<' . time() . ' AND goods.status = \'0\' AND g.status = \'1\' AND unix_timestamp(goods.start_time )<' . time() . ' AND g.uniacid = :uniacid AND g.deleted = \'0\' AND goods.account_id = :uniacid ORDER BY goods.id DESC LIMIT ' . (($page * $psize) - $psize) . ',' . $psize;
            $onSell  = pdo_fetchall($sql, array(':uniacid' => $_W['uniacid']));
            $all_sql = 'SELECT COUNT(*) FROM' . tablename('superdesk_shop_bargain_goods') . ' goods INNER JOIN' . tablename('superdesk_shop_goods') . ' g ON goods.goods_id = g.id WHERE unix_timestamp(goods.end_time )<' . time() . ' AND goods.status = \'0\' AND g.status = \'1\' AND g.uniacid = :uniacid AND g.deleted = \'0\' AND goods.account_id = :uniacid AND unix_timestamp(goods.start_time )<' . time();
            $all     = pdo_fetchcolumn($all_sql, array(':uniacid' => $_W['uniacid']));
        } else {
            $sql     = 'SELECT goods.*,g.total,thumb,marketprice,sales,title FROM' . tablename('superdesk_shop_bargain_goods') . ' goods INNER JOIN' . tablename('superdesk_shop_goods') . ' g ON goods.goods_id = g.id WHERE unix_timestamp(goods.end_time )<' . time() . ' AND goods.status = \'0\' AND g.status = \'1\' AND unix_timestamp(goods.start_time )<' . time() . ' AND g.uniacid = :uniacid AND g.deleted = \'0\' AND g.title LIKE :keyword AND goods.account_id = :uniacid ORDER BY goods.id DESC LIMIT ' . (($page * $psize) - $psize) . ',' . $psize;
            $onSell  = pdo_fetchall($sql, array(':uniacid' => $_W['uniacid'], ':keyword' => '%' . $_GPC['search'] . '%'));
    ...
    public function out()
    {
        ...
        $psize = 20;
        if (empty($_GPC['search'])) {
            $sql     = 'SELECT goods.*,g.total,thumb,marketprice,sales,title FROM' . tablename('superdesk_shop_bargain_goods') . ' goods INNER JOIN' . tablename('superdesk_shop_goods') . ' g ON goods.goods_id = g.id WHERE (g.status = \'0\' or g.deleted=\'1\') AND goods.status = \'0\' AND g.uniacid = :uniacid ORDER BY goods.id DESC LIMIT ' . (($page * $psize) - $psize) . ',' . $psize;
            $onSell  = pdo_fetchall($sql, array(':uniacid' => $_W['uniacid']));
            $all_sql = 'SELECT COUNT(*) FROM' . tablename('superdesk_shop_bargain_goods') . ' goods INNER JOIN' . tablename('superdesk_shop_goods') . ' g ON goods.goods_id = g.id WHERE (g.status = \'0\' or g.deleted=\'1\') AND goods.status = \'0\' AND g.uniacid = :uniacid';
            $all     = pdo_fetchcolumn($all_sql, array(':uniacid' => $_W['uniacid']));
        } else {
            $sql     = 'SELECT goods.*,g.total,thumb,marketprice,sales,title FROM' . tablename('superdesk_shop_bargain_goods') . ' goods INNER JOIN' . tablename('superdesk_shop_goods') . ' g ON goods.goods_id = g.id WHERE (g.status = \'0\' or g.deleted=\'1\') AND goods.status = \'0\' AND g.title LIKE :keyword AND g.uniacid = :uniacid ORDER BY goods.id DESC LIMIT ' . (($page * $psize) - $psize) . ',' . $psize;
            $onSell  = pdo_fetchall($sql, array(':uniacid' => $_W['uniacid'], ':keyword' => '%' . $_GPC['search'] . '%'));
            $all_sql = 'SELECT COUNT(*) FROM' . tablename('superdesk_shop_bargain_goods') . ' goods INNER JOIN' . tablename('superdesk_shop_goods') . ' g ON goods.goods_id = g.id WHERE (g.status = \'0\' or g.deleted=\'1\') AND goods.status = \'0\' AND g.uniacid = :uniacid AND g.title LIKE :keyword';
            $all     = pdo_fetchcolumn($all_sql, array(':uniacid' => $_W['uniacid'], ':keyword' => '%' . $_GPC['search'] . '%'));
        }
        $pager = pagination($all, $page, $psize);
        include $this->template();
    ...
    public function recycle()
    {
        ...
        if (empty($_GPC['search'])) {
            ...
            $sql =
                'SELECT goods.*,g.total,thumb,marketprice,sales,title ' .
                ' FROM' . tablename('superdesk_shop_bargain_goods') . ' goods ' .
                ' INNER JOIN' . tablename('superdesk_shop_goods') . ' g ON goods.goods_id = g.id ' .
                ' WHERE ' .
                '       goods.status = \'1\' ' .
    ...
    public function recover()
    {
        ...
        if (!empty($id)) {
            ...
            pdo_query(
                ...
                ' WHERE (status = \'0\' OR deleted=\'1\') AND uniacid = :uniacid',
                array(
                    ':uniacid' => $_W['uniacid']
                )
            );
    ...
    public function act()
    {
        ...
        $get_this_goods_sql = 'SELECT * FROM ' . tablename('superdesk_shop_goods') . ' WHERE id = :id';
        $this_goods         = pdo_fetch($get_this_goods_sql, array(':id' => $id));
        $temp               = pdo_get('superdesk_shop_goods', array('id' => $this_goods['id']), array('bargain'));
        if (!empty($_W['ispost'])) {
            $if_act_swi = false;
        }
        if (!empty($end_price) || ($end_price === 0)) {
            ...
            $res              = pdo_insert('superdesk_shop_bargain_goods', $data);
            $redirect_id      = pdo_insertid();

            if ($res) {

                $bargain_goods_id = pdo_insertid();
                $save_res         = pdo_update('superdesk_shop_goods', array('bargain' => $bargain_goods_id), array('id' => $this_goods['id']));
                if (empty($save_res)) {
                    show_json(0, '保存失败');
                } else {
                    show_json(1, '保存成功');
    ...
    public function react()
    {
        ...
        $user_set = json_decode($this_act['user_set'], true);
        ...
        if (!empty($end_price) || ($end_price === 0)) {
            ...
            $user_set_arr = array(
                'bargain_title'   => $bargain_share_title,
                'bargain_content' => $bargain_share_describe,
                'bargain_logo'    => $bargain_share_logo,
                'goods_title'     => $goods_share_title,
                'goods_content'   => $goods_share_describe,
                'goods_logo'      => $goods_share_logo
            );
