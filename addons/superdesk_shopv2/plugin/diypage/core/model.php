...
class DiypageModel extends PluginModel
{
    ...
    public function getPageList($type = 'allpage', $condition = NULL, $page = 0)
    {
        ...
        $list = pdo_fetchall(
            ' select id, `name`, `type`, createtime, lastedittime, keyword ' .
            ' from ' . tablename('superdesk_shop_diypage') .
            ' where uniacid=:uniacid '
            . $c . ' order by type desc, id desc ' . $limit,
            array(
                ':uniacid' => $_W['uniacid']
            )
        );
        ...
        if (!empty($page)) {
            ...
            if (!empty($page['data']['items']) && is_array($page['data']['items'])) {
                ...
                foreach ($page['data']['items'] as $itemid => &$item) {
                    ...
                    /* 商品组 */
                    if ($item['id'] == 'goods') {
                        if ($item['params']['goodsdata'] == '0') { /* 选择商品:手动选择 */
                            if (!empty($item['data']) && is_array($item['data'])) {
                                ...
                                if (!empty($goodsids) && is_array($goodsids)) {
                                    ...
                                    $goods        = pdo_fetchall(
                                        ' select id, title, thumb, minprice, total, showlevels, showgroups, bargain, productprice, merchid, tcate, costprice ' .
                                        ' from ' . tablename('superdesk_shop_goods') .
                                        ' where id in( ' . $newgoodsids . ' ) ' .
                                        '       and status=1 ' .
                                        '       and deleted=0 ' .
                                        '       and uniacid=:uniacid ' .
                        ...
                        } else if ($item['params']['goodsdata'] == '1') { /* 选择商品:选择分类 */
                        ...
                        } else if ($item['params']['goodsdata'] == '2') { /* 选择商品:选择分组 */
                    ...
                    if ($item['id'] == 'goods') {
                        ...
                        if ($item['params']['goodsdata'] == '1') {
                            ...
                            if (!empty($group) && !empty($group['goodsids'])) {
                                ...
                                $goods    = pdo_fetchall(
                                    ' select id, title, thumb, minprice, sales, salesreal, total, showlevels, showgroups, bargain, tcate, costprice, merchid ' .
                                    ' from ' . tablename('superdesk_shop_goods') .
    ...
    public function savePage($id, $data, $update = true)
    {
        ...
        if (!empty($id)) {
            ...
            pdo_update('superdesk_shop_diypage', $diypage, array('id' => $id, 'uniacid' => $_W['uniacid']));
            ...
            pdo_insert('superdesk_shop_diypage', $diypage);
            $id = pdo_insertid();
        ...
        if (!empty($keyword) && $update) {
            ...
            $rule = pdo_fetch(
                ...
                array(
                    ...
                    ':name'    => 'superdesk_shopv2:diypage:' . $id
                )
            );
    ...
    public function saveTemp($temp)
    {
        ...
        pdo_insert('superdesk_shop_diypage_template', $temp);
        ...
        if ($do == 'add') {
            ...
            if (!empty($tid)) {
                $template = pdo_fetch(
                    ...
                    ' where ' .
                    '       id=:id ' .
                    '       and (uniacid=:uniacid or uniacid=0) ' .
                    ' limit 1',
                    array(
                        ':uniacid' => $_W['uniacid'],
                        ':id'      => $tid
    ...
    public function getPageType($type = NULL)
    {
        $pagetype = array(
            ...
            4  => array('name' => '营销中心', 'pagetype' => 'sys', 'class' => 'warning'),
            5  => array('name' => '商品详情页', 'pagetype' => 'sys', 'class' => 'danger'),
            6  => array('name' => '商品列表页', 'pagetype' => 'sys', 'class' => 'info'),
    ...
    public function calculate($str = NULL, $member = false)
    {
        ...
        if (strexists($str, 'r=commission')) {
            ...
            if ($plugin_commissiom) {
                ...
                if (strexists($str, 'r=commission.log')) {
                    $this->commission['applycount'] = pdo_fetchcolumn(
                        'select count(id) ' .
                        ' from ' . tablename('superdesk_shop_commission_apply') .
                        ' where ' .
                        '       mid=:mid ' .
                        '       and uniacid=:uniacid ' .
                        ' limit 1',
        ...
        if (strexists($str, 'memberc') && p('commission')) {
            ...
            if (!empty($member)) {
                ...
                $member['commissionlevel'] = p('commission')->getLevel($_W['openid'], $_W['core_user']);

                $member['up'] = false;

                if (!empty($member['agentid'])) {
                    $member['up'] = m('member')->getMemberById($member['agentid']);
                }
        ...
        if (strexists($str, 'r=order') || strexists($str, 'r=member.cart') || strexists($str, 'r=coupon.my')) {
            ...
            if (strexists($str, 'status=0')) {
                ...
                $ordernum['status0'] = pdo_fetchcolumn(
                    'select count(*) ' .
            ...
            if (strexists($str, 'status=1')) {
                ...
                $ordernum['status1'] = pdo_fetchcolumn(
                    'select count(*) ' .
                    ' from ' . tablename('superdesk_shop_order') . // TODO 标志 楼宇之窗 openid shop_order 已处理
                    ' where ' .
                    $condition .
            ...
            if (strexists($str, 'status=2')) {
                ...
                $ordernum['status2'] = pdo_fetchcolumn(
                    'select count(*) ' .
                    ' from ' . tablename('superdesk_shop_order') . // TODO 标志 楼宇之窗 openid shop_order 已处理
                    ' where ' .
                    $condition .
            ...
            if (strexists($str, 'status=4')) {
                ...
                $ordernum['status4'] = pdo_fetchcolumn(
                    'select ' .
                    '       count(*) ' .
                    ' from ' . tablename('superdesk_shop_order') . // TODO 标志 楼宇之窗 openid shop_order 已处理
                    ' where ' .
            ...
            if (strexists($str, 'r=member.cart')) {
                ...
                $ordernum['cartnum'] = pdo_fetchcolumn(
                    'select ' .
                    '       ifnull(sum(total),0) ' .
            ...
            if (strexists($str, 'r=member.favorite')) {
                ...
                $ordernum['favorite'] = pdo_fetchcolumn(
                    'select ' .
                    '       count(*) ' .
            ...
            if (strexists($str, 'r=sale.coupon.my')) {
                ...
                $ordernum['coupon'] = pdo_fetchcolumn(
                    $sql,
                    array(
                        ':uniacid'   => $_W['uniacid'],
                        ':openid'    => $_W['openid'],
    ...
    public function judge($type = NULL, $str = NULL)
    {
        ...
        if ($type == 'dot') {
            ...
            } else if (strexists($str, 'r=member.cart')) {
                return $order['cartnum'];
            } else if (strexists($str, 'r=member.favorite')) {
                return $order['favorite'];
            } else if (strexists($str, 'r=sale.coupon.my')) {
                return $order['coupon'];
            } else {
                return 0;
            }
