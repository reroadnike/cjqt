...
class boardroom_4school_building_structuresModel
{

    public $table_name = "superdesk_boardroom_4school_building_structures";

    public $table_column_all = "id,uniacid,name,thumb,parentid,isrecommand,description,displayorder,enabled";
...
//        $pager = pagination($total, $page, $page_size);
    ...
    public function queryAllByCoreUser($where = array(), $page = 0, $page_size = 50)
    {
        ...
        $total = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename($this->table_name) . $where_sql, $params);
        $list = pdo_fetchall("SELECT * FROM " . tablename($this->table_name) . $where_sql . " ORDER BY parentid ASC, displayorder DESC LIMIT " . ($page - 1) * $page_size . ',' . $page_size, $params);
//        $pager = pagination($total, $page, $page_size);
