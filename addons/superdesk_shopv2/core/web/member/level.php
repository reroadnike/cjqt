...
class Level_SuperdeskShopV2Page extends WebPage 
{
	public function main() 
	{
		...
		$others = pdo_fetchall('SELECT * FROM ' . tablename('superdesk_shop_member_level') . ' WHERE 1 ' . $condition . ' ORDER BY level asc', $params);
	...
	protected function post() 
	{
		...
		$id = trim($_GPC['id']);
		$set = $_S;
		$setdata = pdo_fetch('select * from ' . tablename('superdesk_shop_sysset') . ' where uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid']));
		if ($id == 'default') 
		{
		...
		{
			$level = pdo_fetch('SELECT * FROM ' . tablename('superdesk_shop_member_level') . ' WHERE id=:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => intval($id)));
		}
		if ($_W['ispost']) 
		{
			...
			if (!empty($id)) 
			{
				if ($id == 'default') 
				{
					...
					if (empty($setdata)) 
					{
						pdo_insert('superdesk_shop_sysset', $data);
					}
					else 
					{
						pdo_update('superdesk_shop_sysset', $data, array('uniacid' => $_W['uniacid']));
					}
					$setdata = pdo_fetch('select * from ' . tablename('superdesk_shop_sysset') . ' where uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid']));
				...
				else 
				{
					...
					pdo_update('superdesk_shop_member_level', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
					plog('member.level.edit', '修改会员等级 ID: ' . $id . $updatecontent);
				}
			}
			else 
			{
				pdo_insert('superdesk_shop_member_level', $data);
				$id = pdo_insertid();
	...
	public function delete() 
	{
		...
		$items = pdo_fetchall('SELECT id,levelname FROM ' . tablename('superdesk_shop_member_level') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);
		foreach ($items as $item ) 
		{
			pdo_delete('superdesk_shop_member_level', array('id' => $item['id']));
	...
	public function enabled() 
	{
		...
		foreach ($items as $item ) 
		{
			pdo_update('superdesk_shop_member_level', array('enabled' => intval($_GPC['enabled'])), array('id' => $item['id']));
