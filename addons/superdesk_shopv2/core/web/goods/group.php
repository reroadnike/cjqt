...
class Group_SuperdeskShopV2Page extends WebPage
{
    ...
    protected function post()
    {
        ...
        if (!empty($id)) {
            ...
            if (!empty($item['goodsids'])) {
                $goods = pdo_fetchall(
                    ...
                    ' order by instr(\'' . $item['goodsids'] . '\',id)');
            }
        ...
        if ($_W['ispost']) {
            ...
            if (!empty($item)) {
                ...
                pdo_update('superdesk_shop_goods_group', $data, array('id' => $item['id']));
                ...
                pdo_insert('superdesk_shop_goods_group', $data);
                $id = pdo_insertid();
            ...
            show_json(1, array(
                'url' => webUrl('goods/group/edit', array('id' => $id))
            ));
        }
    ...
    public function excelImport(){
        ...
        $filename = time() . 'rd' . rand(1000,9999) . 'id' . $_W['uniacid'];
        $rows = m('excel')->importByPath('excelfile','general','goods',$filename);

        $num = count($rows);
        if($num > 2000){
            show_json(0,'最大只能导入2000条');
        }

        $sku_array = array_column($rows,'0');

        $id_array = array_column($rows,'1');

        $sku_array = array_filter($sku_array, function($v, $k) {
            return $v > 0;
        }, ARRAY_FILTER_USE_BOTH);

        $id_array = array_filter($id_array, function($v, $k) {
            return $v > 0;
        }, ARRAY_FILTER_USE_BOTH);
        ...
        $goodsList = pdo_fetchall(
            ' SELECT id,thumb,title FROM ' . tablename('superdesk_shop_goods') .
            ' WHERE ' . $where
        );
