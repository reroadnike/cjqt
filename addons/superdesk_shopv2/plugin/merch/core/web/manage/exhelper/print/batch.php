...
class Batch_SuperdeskShopV2Page extends MerchWebPage
{
    private $_orderModel;
    ...
    public function getdata()
    {
        ...
        if ($_W['ispost']) {
            ...
            $condition =
                ' o.uniacid = :uniacid ' .
                ' and o.deleted=0 and o.merchid=:merchid and o.isparent=0';
            ...
            if (!empty($_GPC['searchfield']) && !empty($_GPC['keyword'])) {
                ...
                if ($searchfield == 'ordersn') {
                    ...
                    $sqlcondition =
                        ' inner join ( select distinct og.orderid from ' . tablename('superdesk_shop_order_goods') . ' og ' .// TODO 标志 楼宇之窗 openid shop_order_goods 不处理
                        '              left join ' . tablename('superdesk_shop_goods') . ' g on g.id=og.goodsid ' .
                        '              where og.uniacid = \'' . $uniacid . '\' and og.merchid = \'' . $merchid . '\' and (locate(:keyword,g.title)>0)) gs on gs.orderid=o.id';
                } else if ($searchfield == 'goodssn') {
                    $sqlcondition =
                        ' inner join ( select distinct og.orderid from ' . tablename('superdesk_shop_order_goods') . ' og ' .// TODO 标志 楼宇之窗 openid shop_order_goods 不处理
                        '              left join ' . tablename('superdesk_shop_goods') . ' g on g.id=og.goodsid ' .
                        '              where og.uniacid = \'' . $uniacid . '\' and og.merchid = \'' . $merchid . '\' and (locate(:keyword,g.goodssn)>0)) gs on gs.orderid=o.id';
                }
            }

            $sql =
                ' select o.* ,a.realname ,m.nickname, d.dispatchname,m.nickname,r.status as refundstatus ' .
                ' from ' . tablename('superdesk_shop_order') . ' o' .// TODO 标志 楼宇之窗 openid shop_order 已处理
                ' left join ' . tablename('superdesk_shop_order_refund') . ' r on r.orderid=o.id and ifnull(r.status,-1)<>-1' .
            ...
            foreach ($orders as $i => $order) {
                ...
                $order_goods = pdo_fetchall(
                    ' select g.id,g.title,g.shorttitle,g.thumb,g.unit,g.goodssn,og.optionid,og.goodssn as option_goodssn, g.productsn, g.weight, ' .
                    '        og.productsn as option_productsn, og.total,og.price,og.optionname as optiontitle, og.realprice, ' .
                    '        og.printstate,og.printstate2,og.id as ordergoodid ' .
    ...
    public function getprintTemp()
    {
        ...
        if ($_W['ispost']) {
            ...
            $tempSender = pdo_fetch('SELECT * FROM ' . tablename('superdesk_shop_exhelper_senduser') . ' WHERE id=:id and uniacid=:uniacid and merchid=:merchid limit 1', array(':id' => $printUserId, ':uniacid' => $_W['uniacid'], ':merchid' => $merchid));

            $expTemp = pdo_fetch('SELECT * FROM ' . tablename('superdesk_shop_exhelper_express') . ' WHERE id=:id and uniacid=:uniacid and merchid=:merchid limit 1', array(':id' => $printTempId, ':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
    ...
    public function changestate()
    {
        ...
        if ($_W['ispost']) {
            ...
            foreach ($arr as $i => $data) {
                ...
                $ordergood   = pdo_fetch(
                    'SELECT id,goodsid,printstate,printstate2 FROM ' . tablename('superdesk_shop_order_goods') . // TODO 标志 楼宇之窗 openid shop_order_goods 不处理
                    ' WHERE goodsid=:goodsid and orderid=:orderid and uniacid=:uniacid and merchid=:merchid limit 1',
                    array(
                        ':orderid' => $orderid,
                        ':goodsid' => $ordergoodid,
                        ':uniacid' => $_W['uniacid'],
                ...
                if ($type == 1) {
                    ...
                    $orderprint = pdo_fetchcolumn(
                        'SELECT COUNT(*) FROM ' . tablename('superdesk_shop_order_goods') .// TODO 标志 楼宇之窗 openid shop_order_goods 不处理
                        ' WHERE orderid=:orderid and printstate=0 and uniacid= :uniacid and merchid=:merchid',
                        array(
                            ':orderid' => $orderid,
                            ':uniacid' => $_W['uniacid'],
                            ':merchid' => $merchid
                    ...
                    $orderprint = pdo_fetchcolumn(
                        'SELECT COUNT(*) FROM ' . tablename('superdesk_shop_order_goods') . // TODO 标志 楼宇之窗 openid shop_order_goods 不处理
                        ' WHERE orderid=:orderid and printstate2=0 and uniacid= :uniacid and merchid=:merchid',
                        array(
                            ':orderid' => $orderid,
                            ':uniacid' => $_W['uniacid'],
                            ':merchid' => $merchid
    ...
    public function getorderinfo()
    {
        ...
        if ($_W['ispost']) {
            ...
            $printTemp = pdo_fetch('SELECT id,type,expressname,express,expresscom FROM ' . tablename('superdesk_shop_exhelper_express') . ' WHERE id=:id and type=:type and uniacid=:uniacid and merchid=:merchid limit 1', array(':id' => $temp_express, ':type' => 1, ':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
            ...
            $orders = pdo_fetchall(
                ' SELECT id,ordersn,address,address_send,status,paytype,expresscom,expresssn,dispatchtype ' .
                ' FROM ' . tablename('superdesk_shop_order') . // TODO 标志 楼宇之窗 openid shop_order 不处理
                ' WHERE id in( ' . $in . ' ) and (status=1 or (paytype=3 and status=0)) and uniacid=:uniacid and merchid=:merchid and isparent=0 ' .
    ...
    public function dosend()
    {
        ...
        if ($_W['ispost']) {
            ...
            $orderinfo  = pdo_fetch(
                'SELECT * FROM ' . tablename('superdesk_shop_order') . // TODO 标志 楼宇之窗 openid shop_order 不处理
                ' WHERE id=:orderid and status>-1 and uniacid=:uniacid and merchid=:merchid limit 1',
                array(
                    ':orderid' => $orderid,
                    ':uniacid' => $_W['uniacid'],
                    ':merchid' => $merchid
            ...
            if (($orderinfo['status'] == 1) || (($orderinfo['status'] == 0) && ($orderinfo['paytype'] == 3))) {
                ...
                if (!empty($orderinfo['refundid'])) {
                    $refund = pdo_fetch('select * from ' . tablename('superdesk_shop_order_refund') . ' where id=:id limit 1', array(':id' => $orderinfo['refundid'], 'merchid' => $merchid));
                    if (!empty($refund)) {
                        pdo_update('superdesk_shop_order_refund', array('status' => -1), array('id' => $orderinfo['refundid']));
                        //mark kafka 为了kafka转成了model执行
                        $this->_orderModel->updateByColumn(
                            array(
                                'refundid' => 0
