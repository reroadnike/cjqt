...
$list = pdo_fetchall("SELECT * FROM " . tablename('superdesk_boardroom_4school_s_goods') . " WHERE uniacid = '{$_W['uniacid']}'  and deleted=0 AND status = '1' $condition ORDER BY $sortfield LIMIT " . ($pindex - 1) * $psize . ',' . $psize);
foreach ($list as &$r) {
    if ($r['istime'] == 1) {
...
unset($r);
$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('superdesk_boardroom_4school_s_goods') . " WHERE uniacid = '{$_W['uniacid']}'  and deleted=0  AND status = '1' $condition");
