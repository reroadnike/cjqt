...
class User_SuperdeskShopV2Page extends PluginWebPage
{
    ...
    public function main()
    {
        ...
        //' left join  ' . tablename('superdesk_shop_merch_account') . ' a on u.id = a.merchid ' . 用商户id会出现重复商户.因为该商户有多个帐号
        ...
        $sql =
            ...
            ' left join  ' . tablename('superdesk_shop_merch_account') . ' a on u.accountid = a.id ' .
    ...
    protected function post()
    {
        ...
        if ($_W['ispost']) {
            ...
            $data = array(
                ...
                'storename'    => trim($_GPC['storename']), //2019年5月8日 14:38:11 zjh 宇迪要求添加
            ...
            if (empty($item)) {
                ...
                pdo_insert('superdesk_shop_merch_user', $data);
                $id = pdo_insertid();

                $account['merchid'] = $id;
                pdo_insert('superdesk_shop_merch_account', $account);
                ...
                if (!empty($item['accountid'])) {
                    ...
                    pdo_update('superdesk_shop_merch_account', $account, array('id' => $item['accountid']));
                    ...
                    pdo_update('superdesk_shop_merch_user', array('accountid' => $accountid), array('id' => $id));
                }

                if($status == 2){
                    pdo_update('superdesk_shop_goods', array('status' => 0), array('merchid' => $id, 'uniacid' => $_W['uniacid'], 'status' => 1));
                }

                plog('merch.user.edit',
                    '编辑商户 ID: ' . $data['id'] .
                    ' 商户名: ' . $item['merchname'] . ' -> ' . $data['merchname'] .
            ...
            show_json(1, array(
                'url' => webUrl('merch/user', array('status' => $item['status']))
            ));
        }
    ...
    public function status()
    {
        ...
        foreach ($items as $item) {
            pdo_update(
                'superdesk_shop_merch_user',
                array('status' => intval($_GPC['status'])),
                array('id' => $item['id'])
            );

            if($_GPC['status'] == 2){
                pdo_update('superdesk_shop_goods', array('status' => 0), array('merchid' => $item['id'], 'uniacid' => $_W['uniacid'], 'status' => 1));
            }

            plog('merch.group.edit',
                (('修改商户分组账户状态' .
                    '<br/>ID: ' . $item['id'] .
                    '<br/>商户名称: ' . $item['merchname'] .
    ...
    public function delete()
    {
        ...
        foreach ($items as $item) {
            pdo_update('superdesk_shop_goods', $change_data, array('merchid' => $item['id'], 'uniacid' => $uniacid));
            pdo_delete('superdesk_shop_merch_account', array('merchid' => $item['id'], 'uniacid' => $uniacid));
            pdo_delete('superdesk_shop_merch_user', array('id' => $item['id'], 'uniacid' => $uniacid));
            plog('merch.user.delete',
                '删除`商户 ' .
                '<br/>商户:  ID: ' . $item['id'] .
                ' / 名称:   ' . $item['merchname']
            );
    ...
    public function setCanSee()
    {
        ...
        $merch = pdo_fetch(
            'select id,merchname,realname,mobile ' .
            ' from ' . tablename('superdesk_shop_merch_user') .
            ' WHERE ' .
            '       id=:id ' .
            '       AND uniacid=:uniacid',
        ...
        $is_default_see = intval($_GPC['is_default_see']);

        if (!empty($is_default_see)) {

            // TODO mark update_superdesk_shop_merch_user
            pdo_update('superdesk_shop_merch_user', array('is_default_see' => 1), array('id' => $merch['id']));

            $this->_plugin_merchService->deleteCacheMerchByEnterpriseId('default');

            plog('merch.user.edit', '设置默认对所有企业与客户可见 <br/>商户信息:  ID: ' . $merch['id'] . '/' . $merch['realname'] . '/' . $merch['mobile']);

        } else {

            // TODO mark update_superdesk_shop_merch_user

            pdo_update('superdesk_shop_merch_user', array('is_default_see' => 0), array('id' => $merch['id']));

            $this->_plugin_merchService->deleteCacheMerchByEnterpriseId('default');

            plog('merch.user.edit', '取消设置默认对所有企业与客户可见 <br/>商户信息:  ID: ' . $merch['id'] . '/' . $merch['realname'] . '/' . $merch['mobile']);
        }
