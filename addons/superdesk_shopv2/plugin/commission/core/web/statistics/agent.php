...
class Agent_SuperdeskShopV2Page extends PluginWebPage
{
    public function main()
    {
        ...
        if (!empty($_GPC['time']['start']) && !empty($_GPC['time']['end'])) {
            ...
            $condition .=
                ' AND dm.agenttime >= :starttime ' .
                ' AND dm.agenttime <= :endtime ';
        ...
        $sql =
            'select dm.*,dm.nickname,dm.avatar,l.levelname,p.nickname as parentname,p.avatar as parentavatar ' .
            ' from ' . tablename('superdesk_shop_member') . ' dm ' .
            ' left join ' . tablename('superdesk_shop_member') . ' p on p.id = dm.agentid ' .
            ' left join ' . tablename('superdesk_shop_commission_level') . ' l on l.id = dm.agentlevel' .
            ' left join ' . tablename('mc_mapping_fans') . 'f on f.openid=dm.openid and f.uniacid=' . $_W['uniacid'] .
            ' where dm.uniacid = ' . $_W['uniacid'] .
            ' and dm.isagent =1  ' .
            $condition .
        ...
        $total = pdo_fetchcolumn(
            ...
            ' where dm.uniacid =' . $_W['uniacid'] . ' and dm.isagent =1 ' .
            $condition,
            $params
        );
        ...
        $array = array(
            'total', 'pay',
            'ordercount0', 'ordermoney0',
            'ordercount', 'ordermoney',
            'ordercount3', 'ordermoney3'
        );
        ...
        foreach ($list as &$row) {
            ...
            if (p('diyform') && !empty($row['diycommissionfields']) && !empty($row['diycommissiondata'])) {
                ...
                $diyformdata_array = p('diyform')->getDatas(
                    iunserializer($row['diycommissionfields']),
        ...
        if ($_GPC['export'] == '1') {
            ...
            plog('commission.agent.export', '导出营销商统计数据');
            ...
            $columns = array(
                ...
                array('title' => '营销商等级', 'field' => 'levelname', 'width' => 12),
                array('title' => '点击数', 'field' => 'clickcount', 'width' => 12),
                array('title' => '下线营销商总数', 'field' => 'levelcount', 'width' => 12),
                array('title' => '一级下线营销商数', 'field' => 'level1', 'width' => 12),
                array('title' => '二级下线营销商数', 'field' => 'level2', 'width' => 12),
                array('title' => '三级下线营销商数', 'field' => 'level3', 'width' => 12),
                array('title' => '累计佣金', 'field' => 'commission_total', 'width' => 12),
                array('title' => '打款佣金', 'field' => 'commission_pay', 'width' => 12),
                array('title' => '营销金额', 'field' => 'ordermoney', 'width' => 12),
                array('title' => '营销订单数量', 'field' => 'level_ordercount', 'width' => 12),
                array('title' => '一级营销订单数量', 'field' => 'level1_ordercount', 'width' => 12),
                array('title' => '二级营销订单数量', 'field' => 'level2_ordercount', 'width' => 12),
                array('title' => '三级营销订单数量', 'field' => 'level3_ordercount', 'width' => 12),
                array('title' => '下级累计佣金', 'field' => 'level_commission_total', 'width' => 12),
                array('title' => '注册时间', 'field' => 'createtime', 'width' => 12),
                array('title' => '成为营销商时间', 'field' => 'createtime', 'width' => 12),
            ...
            m('excel')->export(
                $list,
                array(
                    'title' => '营销商统计数据-' . date('Y-m-d-H-i', time()),
                    'columns' => $columns
                )
            );
        }
    ...
    public function delete()
    {
        ...
        foreach ($members as $member) {
            ...
            plog(
                ...
                '取消营销商资格 <br/>营销商信息:  ID: ' . $member['id'] . ' /  ' . $member['openid'] . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile']
            );
        }
    ...
    public function user()
    {
        ...
        $level11 = pdo_fetchcolumn(
            'select count(*) '.
            ' from ' . tablename('superdesk_shop_member') .
            ' where isagent=0 and agentid=:agentid and uniacid=:uniacid limit 1',
            array(
                ':agentid' => $agentid,
                ':uniacid' => $_W['uniacid']
            )
        ...
        if ($hasagent) {
            ...
            $total = pdo_fetchcolumn(
                'select count(dm.id) from' . tablename('superdesk_shop_member') . ' dm ' .
                ' left join ' . tablename('superdesk_shop_member') . ' p on p.id = dm.agentid ' .
                ' left join ' . tablename('mc_mapping_fans') . 'f on f.openid=dm.openid' .
                ' where dm.uniacid =' . $_W['uniacid'] . ' ' .
                $condition,
            ...
            $list  = pdo_fetchall(
                'select dm.*,p.nickname as parentname,p.avatar as parentavatar  from ' . tablename('superdesk_shop_member') . ' dm ' .
                ' left join ' . tablename('superdesk_shop_member') . ' p on p.id = dm.agentid ' .
                ' left join ' . tablename('mc_mapping_fans') . 'f on f.openid=dm.openid  and f.uniacid=' . $_W['uniacid'] .
                ' where dm.uniacid = ' . $_W['uniacid'] . '  ' .
                $condition .
                ' ORDER BY dm.agenttime desc limit ' . (($pindex - 1) * $psize) . ',' . $psize,
                $params
            );
        ...
        if ($_GPC['export'] == 1) {
            ...
            if (p('diyform')) {
                $columns[] = array('title' => '营销商自定义信息', 'field' => 'agent_diyformdata', 'width' => 36);
            }
    ...
    public function query()
    {
        ...
        $ds = pdo_fetchall(
            'SELECT ' .
            '       id,avatar,nickname,openid,realname,mobile ' .
            ' FROM ' . tablename('superdesk_shop_member') .
            ' WHERE 1 ' .
            $condition .
    ...
    public function check()
    {
        ...
        $members = pdo_fetchall(
            'SELECT ' .
            '       id,openid,nickname,realname,mobile,status ' .
            ' FROM ' . tablename('superdesk_shop_member') .
            ' WHERE ' .
            '       id in( ' . $id . ' ) ' .
            '       AND uniacid=' . $_W['uniacid']
        ...
        foreach ($members as $member) {
            ...
            if ($status == 1) {
                ...
                plog(
                    'commission.agent.check',
                    '审核营销商 <br/>营销商信息:  ID: ' . $member['id'] . ' /  ' . $member['openid'] . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile']
                );
                ...
                plog(
                    'commission.agent.check',
                    '取消审核 <br/>营销商信息:  ID: ' . $member['id'] . ' /  ' . $member['openid'] . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile']
                );
            }
        }
    ...
    public function agentblack()
    {
        ...
        foreach ($members as $member) {
            ...
            if ($agentblack == 1) {
                ...
                plog(
                    'commission.agent.agentblack',
                    '设置黑名单 <br/>营销商信息:  ID: ' . $member['id'] . ' /  ' . $member['openid'] . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile']
                );
                ...
                plog(
                    'commission.agent.agentblack',
                    '取消黑名单 <br/>营销商信息:  ID: ' . $member['id'] . ' /  ' . $member['openid'] . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile']
                );
            }
        }
