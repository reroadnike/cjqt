...
class Import_log_SuperdeskShopV2Page extends EnterpriseWebPage
{
    public function main()
    {
        ...
        $list  = pdo_fetchall(
            'SELECT  log.* ,a.username FROM ' . tablename('superdesk_shop_enterprise_import_log') . ' log  ' .// TODO 标志 楼宇之窗 openid shop_enterprise_import_log 不处理
        ...
        $total = pdo_fetchcolumn(
            'SELECT count(*) FROM ' . tablename('superdesk_shop_enterprise_import_log') . ' log  ' .// TODO 标志 楼宇之窗 openid shop_enterprise_import_log 不处理
            ' left join ' . tablename('superdesk_shop_enterprise_account') . ' a on log.account_id = a.id  ' . // TODO 标志 楼宇之窗 openid shop_enterprise_account 不处理
            ' WHERE 1 ' . $condition . ' ',
            $params
        );
