...
class Level_SuperdeskShopV2Page extends PluginWebPage 
{
	public function main() 
	{
		...
		$others = pdo_fetchall('SELECT * FROM ' . tablename('superdesk_shop_commission_level') . ' WHERE uniacid = \'' . $_W['uniacid'] . '\' ORDER BY commission1 asc');
	...
	protected function post() 
	{
		...
		{
			$level = pdo_fetch('SELECT * FROM ' . tablename('superdesk_shop_commission_level') . ' WHERE id=:id and uniacid=:uniacid limit 1', array(':id' => intval($id), ':uniacid' => $_W['uniacid']));
		}
		if ($_W['ispost']) 
		{
			...
			if (!empty($id)) 
			{
				if ($id == 'default') 
				{
					...
					$this->updateSet($set);
					plog('commission.level.edit', '修改营销商默认等级' . $updatecontent);
				}
				else 
				{
					...
					pdo_update('superdesk_shop_commission_level', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
					plog('commission.level.edit', '修改营销商等级 ID: ' . $id . $updatecontent);
				}
			}
			else 
			{
				pdo_insert('superdesk_shop_commission_level', $data);
				$id = pdo_insertid();
				plog('commission.level.add', '添加营销商等级 ID: ' . $id);
			}
	...
	public function delete() 
	{
		...
		$items = pdo_fetchall('SELECT id,levelname FROM ' . tablename('superdesk_shop_commission_level') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);
		foreach ($items as $item ) 
		{
			pdo_delete('superdesk_shop_commission_level', array('id' => $item['id']));
			plog('commission.level.delete', '删除营销商等级 ID: ' . $id . ' 等级名称: ' . $level['levelname']);
		}
		show_json(1);
	}
}
