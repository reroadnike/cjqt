...
class Index_SuperdeskShopV2Page extends MobilePage
{
    ...
    public function gift()
    {
        ...
        if (!empty($giftgoodsid)) {
            foreach ($giftgoodsid as $key => $value) {
                $giftgoods[$key] = pdo_fetch(
                    'select id,status,title,thumb,marketprice ' .
    ...
    public function get_list()
    {
        ...
        $args = array(
            ...
            'by'            => trim($_GPC['by']),
            'priceMin'      => trim($_GPC['priceMin']),  //20180320新增价格筛选
            'priceMax'      => trim($_GPC['priceMax']),  //20180320新增价格筛选
            'switch_source' => trim($_GPC['switch_source']),  //20180411 数据源 elasticsearch mysql
        );
