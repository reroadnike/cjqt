...
class Log_SuperdeskShopV2Page extends PluginWebPage
{
	public function main()
	{
		...
		$list = pdo_fetchall(
			...
			' FROM ' . tablename('superdesk_shop_poster_log') . ' log ' . // TODO 标志 楼宇之窗 openid shop_poster_log 已处理
			...
			' left join ' . tablename('superdesk_shop_member') . ' m on m.openid = log.from_openid  and m.uniacid = log.uniacid' . // TODO 标志 楼宇之窗 openid shop_poster_log 待处理
			' WHERE 1 ' . $condition . ' ORDER BY log.createtime desc ' . '  LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize,
			$params
		);

		$total = pdo_fetchcolumn(
			'SELECT count(*)  FROM ' . tablename('superdesk_shop_poster_log') . ' log ' . // TODO 标志 楼宇之窗 openid shop_poster_log 已处理
			' left join ' . tablename('superdesk_shop_member') . ' m1 on m1.openid = log.openid and m1.core_user = log.core_user and m1.uniacid = log.uniacid ' .
			' left join ' . tablename('superdesk_shop_member') . ' m on m.openid = log.from_openid and m.uniacid = log.uniacid ' . // TODO 标志 楼宇之窗 openid shop_poster_log 待处理
			' where 1 ' . $condition . '  ',
			$params
		...
		foreach ($list as &$row ) {
			$row['times'] = pdo_fetchcolumn(
				...
				' where from_openid=:from_openid and posterid=:posterid and uniacid=:uniacid',
				array(
