...
class Log_SuperdeskShopV2Page extends WebPage
{
    protected function main($type = 0)
    {
        ...
        $sql =
            'select ' .
            '       log.id,' .
            '       m.id as mid, m.realname,m.avatar,m.weixin,' .
            '       log.logno,log.type,log.status,log.rechargetype,' .
            '       m.nickname,m.mobile,g.groupname,log.money,log.createtime,l.levelname,' .
            '       log.realmoney,log.deductionmoney,log.charge,log.remark,log.alipay,log.bankname,log.bankcard,log.realname as applyrealname,log.applytype ' .
            ' from ' . tablename('superdesk_shop_member_log') . ' log ' . // TODO 标志 楼宇之窗 openid superdesk_shop_member_log 待处理
            '       left join ' . tablename('superdesk_shop_member') . ' m on m.openid=log.openid and m.core_user=log.core_user ' . // TODO 标志 楼宇之窗 openid shop_member 已处理
        ...
        if ($_GPC['export'] == 1) {
            ...
            m('excel')->export(
                $list,
                array('title'   => ((empty($type) ? '会员充值数据-' : '会员提现记录')) . date('Y-m-d-H-i', time()),
                      'columns' => $columns
                )
            );
        }
    ...
    public function refund()
    {
        ...
        $current_credit = m('member')->getCredit($log['openid'], $log['core_user'], 'credit2');
        ...
        if ($log['rechargetype'] == 'wechat') {
            ...
            $result = m('finance')->pay($log['openid'], $log['core_user'],
                1,
                $log['money'] * 100,
                $out_refund_no,
                $set['name'] . '充值退款'
        ...
        m('member')->setCredit($log['openid'], $log['core_user'],
            'credit2', -$refundmoney, array(0, $set['name'] . '充值退款'));
        ...
        plog(
            'finance.log.refund',
            '充值退款 ID: ' . $log['id'] . ' 金额: ' . $log['money'] . ' <br/>会员信息:  ID: ' . $member['id'] . ' / ' . $member['openid'] . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile']
        );
    ...
    public function wechat()
    {
        ...
        $result = m('finance')->pay($log['openid'], $log['core_user'],
            1, $realmoeny * 100, $log['logno'], $set['name'] . '余额提现');
    ...
    public function manual()
    {
        ...
        $member = m('member')->getMember($log['openid'], $log['core_user']); // TODO 标志 楼宇之窗返回多 shop_member 待处理
    ...
    public function refuse()
    {
        ...
        $log = pdo_fetch(
            'select * ' .
            ' from ' . tablename('superdesk_shop_member_log') . // TODO 标志 楼宇之窗返回多 shop_member 不处理
            ' where ' .
            '       id=:id ' .
            '       and uniacid=:uniacid ' .
            ' limit 1',
        ...
        pdo_update(
            'superdesk_shop_member_log', // TODO 标志 楼宇之窗返回多 shop_member 不处理
            array(
                'status' => -1
            ),
            array(
        ...
        if (0 < $log['money']) {
            m('member')->setCredit($log['openid'], $log['core_user'],
                'credit2', $log['money'], array(0, $set['name'] . '余额提现退回'));
        }
        ...
        plog(
            'finance.log.refuse',
            '拒绝余额度提现 ID: ' . $log['id'] . ' 金额: ' . $log['money'] . ' <br/>会员信息:  ID: ' . $member['id'] . ' / ' . $member['openid'] . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile']
        );
