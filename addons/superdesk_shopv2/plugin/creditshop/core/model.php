...
if (!class_exists('CreditshopModel')) {
    ...
    class CreditshopModel extends PluginModel
    {
        public function getGoods($id, $member)
        {
            ...
            $goods = pdo_fetch('select * from ' . tablename('superdesk_shop_creditshop_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
            if (empty($goods)) {
            ...
            if (empty($goods['canbuy'])) {
                ...
                if (0 < $goods['total']) {
                    $logcount = pdo_fetchcolumn(
                        'select count(*)  from ' . tablename('superdesk_shop_creditshop_log') . // TODO 标志 楼宇之窗 openid shop_creditshop_log 不处理
                        '  where goodsid=:goodsid and status>=2  and uniacid=:uniacid  ',
                        array(
                            ':goodsid' => $id,
                            ':uniacid' => $_W['uniacid']
                        )
                    ...
                    if (($goods['joins'] < $logcount) || (($logcount < $goods['joins']) && empty($goods['type']))) {
                        pdo_update('superdesk_shop_creditshop_goods', array('joins' => $logcount), array('id' => $id));
                    }
                }
                if ($goods['canbuy']) {
                    if (0 < $goods['totalday']) {
                        $logcount = pdo_fetchcolumn(
                            'select count(*)  from ' . tablename('superdesk_shop_creditshop_log') . // TODO 标志 楼宇之窗 openid shop_creditshop_log 不处理
                            '  where goodsid=:goodsid and status>=2 and  date_format(from_UNIXTIME(`createtime`),\'%Y-%m-%d\') = date_format(now(),\'%Y-%m-%d\') and uniacid=:uniacid  ',
                            array(
                                ':goodsid' => $id,
                                ':uniacid' => $_W['uniacid']
                            )
                ...
                if ($goods['canbuy']) {
                    if (0 < $goods['chanceday']) {
                        $logcount = pdo_fetchcolumn(
                            'select count(*)  from ' . tablename('superdesk_shop_creditshop_log') . // TODO 标志 楼宇之窗 openid shop_creditshop_log 已处理
                            '  where goodsid=:goodsid and openid=:openid and core_user=:core_user and status>0 and  date_format(from_UNIXTIME(`createtime`),\'%Y-%m-%d\') = date_format(now(),\'%Y-%m-%d\') and uniacid=:uniacid  ',
                            array(
                                ':goodsid'   => $id,
                                ':uniacid'   => $_W['uniacid'],
                ...
                if ($goods['canbuy']) {
                    if (0 < $goods['chance']) {
                        $logcount = pdo_fetchcolumn(
                            'select count(*)  from ' . tablename('superdesk_shop_creditshop_log') . // TODO 标志 楼宇之窗 openid shop_creditshop_log 已处理
                            '  where goodsid=:goodsid and openid=:openid and core_user=:core_user and status>0 and  uniacid=:uniacid  ',
                            array(
                                ':goodsid'   => $id,
                                ':uniacid'   => $_W['uniacid'],
                                ':openid'    => $member['openid'],
                ...
                if ($goods['canbuy']) {
                    if (0 < $goods['usermaxbuy']) {
                        $logcount = pdo_fetchcolumn(
                            'select ifnull(sum(total),0)  from ' . tablename('superdesk_shop_creditshop_log') . // TODO 标志 楼宇之窗 openid shop_creditshop_log 已处理
                            '  where goodsid=:goodsid and openid=:openid and core_user=:core_user  and uniacid=:uniacid ',
                            array(
                                ':goodsid'   => $id,
                                ':uniacid'   => $_W['uniacid'],
                                ':openid'    => $member['openid'],
        ...
        public function createENO()
        {
            ...
            while (1) {
                $c = pdo_fetchcolumn(
                    'select count(*) from ' . tablename('superdesk_shop_creditshop_log') . // TODO 标志 楼宇之窗 openid shop_creditshop_log 不处理
                    ' where uniacid=:uniacid and eno=:eno limit 1',
                    array(
                        ':uniacid' => $_W['uniacid'],
                        ':eno'     => $eno
                    )
        ...
        public function sendMessage($id = 0)
        {
            ...
            $log = pdo_fetch(
                'select * from ' . tablename('superdesk_shop_creditshop_log') . // TODO 标志 楼宇之窗 openid shop_creditshop_log 不处理
            ...
            if (strexists($detailurl, '/addons/superdesk_shop/')) {
                $detailurl = str_replace('/addons/superdesk_shop/', '/', $detailurl);
            }
            if ($log['status'] == 2) {
                if (!empty($type)) {
                ...
                if (!empty($log['addressid'])) {
                    ...
                    if (!empty($address)) {
                        $info =
                            ' 收件人: ' . $address['realname'] .
                            ' 联系电话: ' . $address['mobile'] .
                            ' 收货地址: ' . $address['province'] . $address['city'] . $address['area'] . $address['town'] . ' ' . $address['address'];
                    }
                }
                ...
                if (strexists($detailurl1, '/addons/superdesk_shop/')) {
                    $detailurl1 = str_replace('/addons/superdesk_shop/', '/', $detailurl1);
                }
        ...
        public function createQrcode($logid = 0)
        {
            global $_W;
            global $_GPC;
            $path = IA_ROOT . '/addons/superdesk_shopv2/data/creditshop/' . $_W['uniacid'];
            if (!is_dir($path)) {
                load()->func('file');
                mkdirs($path);
            }
            ...
            return $_W['siteroot'] . '/addons/superdesk_shopv2/data/creditshop/' . $_W['uniacid'] . '/' . $file;
        }
