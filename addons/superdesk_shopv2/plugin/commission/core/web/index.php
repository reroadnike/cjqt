...
class Index_SuperdeskShopV2Page extends PluginWebPage 
{
	...
	public function set() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			plog('commission.set.edit', '修改基本设置<br>' . '营销内购 -- ' . $selfbuy . '<br>成为下线条件 -- ' . $become_child . '<br>成为营销商条件 -- ' . $become);
		...
		{
			$goods = pdo_fetch('select id,title,thumb from ' . tablename('superdesk_shop_goods') . ' where id=:id and uniacid=:uniacid limit 1 ', array(':id' => $data['become_goodsid'], ':uniacid' => $_W['uniacid']));
		}
		include $this->template();
	}
}
