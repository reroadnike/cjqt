...
class Goods_rank_SuperdeskShopV2Page extends EnterpriseWebPage
{
	public function main()
	{
		...
		$psize = 20;
		$params = array();
		$condition = ' and og.uniacid=' . $_W['uniacid'] . ' and og.enterprise_id=' . $_W['enterprise_id'];
		...
		$condition1 = ' and g.uniacid=:uniacid and g.enterprise_id=:enterprise_id';
		...
		$sql =
			' SELECT g.id,g.title,g.thumb,' .
			'        (select ifnull(sum(og.price),0) from  ' . tablename('superdesk_shop_order_goods') . ' og ' .// TODO 标志 楼宇之窗 openid shop_order_goods 不处理
			'         left join ' . tablename('superdesk_shop_order') . ' o on og.orderid=o.id ' .// TODO 标志 楼宇之窗 openid shop_order 不处理
			'         where o.status>=1 and og.goodsid=g.id ' . $condition . ')  as money,' .
			'        (select ifnull(sum(og.total),0) from  ' . tablename('superdesk_shop_order_goods') . ' og ' .// TODO 标志 楼宇之窗 openid shop_order_goods 不处理
			'         left join ' . tablename('superdesk_shop_order') . ' o on og.orderid=o.id ' .// TODO 标志 楼宇之窗 openid shop_order 不处理
			'         where o.status>=1 and og.goodsid=g.id ' . $condition . ') as count  ' .
		...
		$total = pdo_fetchcolumn('select  count(*) from ' . tablename('superdesk_shop_goods') . ' g ' . ' where 1 ' . $condition1 . ' ', $params1);
