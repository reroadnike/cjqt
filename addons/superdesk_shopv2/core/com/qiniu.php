...
class Qiniu_SuperdeskShopV2ComModel extends ComModel
{
    public function save($url, $config = NULL, $enforce = false)
    {
        ...
        if (strexists($url, '../addons/superdesk_shopv2')) {
            $url = str_replace('../addons/superdesk_shopv2', 'addons/superdesk_shopv2', $url);
        }
        ...
        if (!$outlinkEnforce) {
            ...
            } else if (strexists($url, 'addons/superdesk_shopv2')) {
        ...
        if ($outlinkEnforce) {
            ...
            $local = ATTACHMENT_ROOT . 'superdesk_shopv2_temp/';
