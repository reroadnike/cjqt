...
class boardroom_equipmentModel
{

    public $table_name = "superdesk_boardroom_4school_equipment";

    public $table_column_all = "id,name,images,updatetime,createtime,deleted,uniacid,";
...
//        $pager = pagination($total, $page, $page_size);
    ...
    public function queryByApi4AjaxTypehead($where = array(), $page = 0, $page_size = 50)
    {
        global $_GPC, $_W;//TIMESTAMP
        ...
        $name = isset($where['name']) ? intval($where['name']) : "" ;
        if (!empty($name)) {
        ...
        $total = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename($this->table_name) . $where_sql, $params);
        $list = pdo_fetchall("SELECT id as `value`,name as `text` FROM " . tablename($this->table_name) . $where_sql . " ORDER BY id ASC LIMIT " . ($page - 1) * $page_size . ',' . $page_size, $params);
