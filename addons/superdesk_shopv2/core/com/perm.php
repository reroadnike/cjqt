...
class Perm_SuperdeskShopV2ComModel extends ComModel
{
    ...
    public function allPerms()
    {
        if (empty($allPerms)) {
            ...
            $perms = array(
                ...
                'enterprise' => $this->perm_enterprise(),
                'category' => $this->perm_category(),
                'pc' => $this->perm_pc(),
            );
    ...
    {
        return $this->isopen('bargain')
        && $this->is_perm_plugin('bargain') ? array(
    ...
    protected function perm_order()
    {
        return array(
            ...
            'op'         => array(
                ...
                'transfer_member' => '客户转介-log',
                'send_examine_notice' => '提醒审核-log',
            ),
            'finance'    => array(
                'text'     => '财务跟踪',
                'edit'     => '编辑',
    ...
    protected function perm_enterprise()
    {
        return $this->isopen('enterprise')
        && $this->is_perm_plugin('enterprise') ? array(
            'text'       => m('plugin')->getName('enterprise'),
            'user'       => array(
                'text'   => '企业管理',
                'view'   => '查看详情',
                'add'    => '添加-log',
            ...
            'statistics' => array(
                ...
                'order.export' => '导出订单统计-log',
                'merch'        => '企业统计',
                'merch.export' => '导出企业统计-log',
            ),
        ) : array();
    }

    protected function perm_category()
    {
        return array(
            'text'     => '分类管理',
            'main'     => '浏览列表',
    ...
    private function check($permtype = '')
    {
        ...
        $user = pdo_fetch(
            ' select u.status as userstatus,r.status as rolestatus,u.perms2 as userperms,r.perms2 as roleperms,u.roleid ' .
            ' from ' . tablename('superdesk_shop_perm_user') . ' u ' .
            ' left join ' . tablename('superdesk_shop_perm_role') . ' r on u.roleid = r.id ' .
    ...
    public function check_plugin($pluginname = '')
    {
        ...
        $ac_perm = pdo_fetch('select  plugins from ' . tablename('superdesk_shop_perm_plugin') . ' where acid=:acid limit 1', array(':acid' => $acid));
    ...
    public function check_com($comname = '')
    {
        ...
        $allow   = true;
        $acid    = pdo_fetchcolumn('SELECT acid FROM ' . tablename('account_wechats') . ' WHERE `uniacid`=:uniacid LIMIT 1', array(':uniacid' => $_W['uniacid']));
        $ac_perm = pdo_fetch('select  coms from ' . tablename('superdesk_shop_perm_plugin') . ' where acid=:acid limit 1', array(':acid' => $acid));
        if (!empty($ac_perm)) {
    ...
    public function log($type = '', $op = '')
    {
        ...
        pdo_insert('superdesk_shop_perm_log', $log);
    ...
    {
        return $this->isopen('sns')
        && $this->is_perm_plugin('sns') ? array(
            ...
            'manage'   => array(
                'text'   => '版主管理', 'main' => '查看列表',
                'view'   => '查看详细', 'add' => '添加-log', 'edit' => '修改-log',
                'delete' => '删除-log',
            ),
            'board'    => array(
                'text' => '版块管理', 'main' => '查看列表', 'view' => '查看详细',
                'add'  => '添加-log', 'edit' => '修改-log', 'delete' => '删除-log',
                'xxx'  => array(
                    'status' => 'edit', 'displayorder' => 'edit',
                ),
            ),
            'posts'    => array(
                'text'    => '话题管理', 'main' => '查看', 'delete' => '删除-log',
                'delete1' => '彻底删除-log', 'check' => '审核-log',
                'best'    => '精华-log', 'top' => '置顶-log',
            ),
            'replys'   => array(
                'text'    => '评论管理', 'main' => '查看', 'delete' => '删除-log',
                'delete1' => '彻底删除-log', 'check' => '审核-log',
            ),
            'cover'    => array(
                'text' => '入口设置', 'main' => '查看', 'edit' => '修改-log',
            ),
            'notice'   => array(
                'text' => '通知设置', 'main' => '查看', 'edit' => '修改-log',
            ),
            'set'      => array(
                'text' => '基础设置', 'main' => '查看', 'edit' => '修改-log',
            ),
        ) : array();
    }
}