...
class Dispatch_SuperdeskShopV2Page extends WebPage
{
	public function main()
	{
		...
		$list = pdo_fetchall('SELECT * FROM ' . tablename('superdesk_shop_dispatch') . ' WHERE 1 ' . $condition . '  ORDER BY displayorder DESC limit ' . (($pindex - 1) * $psize) . ',' . $psize, $params);
		$total = pdo_fetchcolumn('SELECT count(*) FROM ' . tablename('superdesk_shop_dispatch') . ' WHERE 1 ' . $condition, $params);
	...
	protected function post()
	{
		...
		if ($_W['ispost']) {
			...
			if ($data['isdefault']) {
				pdo_update('superdesk_shop_dispatch', array('isdefault' => 0), array('uniacid' => $_W['uniacid'], 'merchid' => 0));
			}


			if (!empty($id)) {
				plog('shop.dispatch.edit', '修改配送方式 ID: ' . $id);
				pdo_update('superdesk_shop_dispatch', $data, array('id' => $id));
			}
			 else {
				pdo_insert('superdesk_shop_dispatch', $data);
				$id = pdo_insertid();
		...
		$dispatch = pdo_fetch('SELECT * FROM ' . tablename('superdesk_shop_dispatch') . ' WHERE id = \'' . $id . '\' and merchid=0 and uniacid = \'' . $_W['uniacid'] . '\'');
	...
	public function delete()
	{
		...
		$items = pdo_fetchall('SELECT id,dispatchname FROM ' . tablename('superdesk_shop_dispatch') . ' WHERE id in( ' . $id . ' ) AND merchid=0 AND uniacid=' . $_W['uniacid']);

		foreach ($items as $item ) {
			pdo_delete('superdesk_shop_dispatch', array('id' => $item['id']));
	...
	public function enabled()
	{
		...
		foreach ($items as $item ) {
			pdo_update('superdesk_shop_dispatch', array('enabled' => intval($_GPC['enabled'])), array('id' => $item['id']));
	...
	public function setdefault()
	{
		...
		foreach ($items as $item ) {
			pdo_update('superdesk_shop_dispatch', array('isdefault' => intval($_GPC['isdefault'])), array('id' => $item['id']));
	...
	public function displayorder()
	{
		...
		$item = pdo_fetchall('SELECT id,dispatchname FROM ' . tablename('superdesk_shop_dispatch') . ' WHERE id in( ' . $id . ' ) AND merchid=0 AND uniacid=' . $_W['uniacid']);

		if (!empty($item)) {
			pdo_update('superdesk_shop_dispatch', array('displayorder' => $displayorder), array('id' => $id));
