...
class GroupsModel extends PluginModel
{
    ...
    public function payResult($orderno, $type, $app = false)
    {
        ...
        $log = pdo_fetch(
            'SELECT * ' .
            ' FROM ' . tablename('superdesk_shop_groups_paylog') . "\n\t\t" .
            ' WHERE ' .
            '       uniacid=:uniacid ' .
            '       AND `module`=:module ' .
            '       AND `tid`=:tid ' .
        ...
        $result = m('member')->setCredit($openid,$core_user,
            'credit1', -$order['credit'], array($_W['member']['uid'], $_W['shopset']['shop']['name'] . '消费' . $order['credit'] . '积分'));
        ...
        pdo_update('superdesk_shop_groups_order', array('pay_type' => $type, 'status' => 1, 'paytime' => TIMESTAMP, 'starttime' => TIMESTAMP, 'apppay' => ($app ? 1 : 0)), array('orderno' => $orderno));
    ...
    public function getTotals()
    {
        ...
        $totals['all']     = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('superdesk_shop_groups_order') . ' o' . "\n\t\t\t" . ' WHERE o.uniacid = :uniacid ', $paras);
        $totals['status1'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('superdesk_shop_groups_order') . ' o' . "\n\t\t\t" . ' WHERE o.uniacid = :uniacid and o.status = 1 and (o.success = 1 or o.is_team = 0) ', $paras);
        $totals['status2'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('superdesk_shop_groups_order') . ' o' . "\n\t\t\t" . ' WHERE o.uniacid = :uniacid and o.status=2 ', $paras);
        $totals['status3'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('superdesk_shop_groups_order') . ' o' . "\n\t\t\t" . ' WHERE o.uniacid = :uniacid and o.status = 0 ', $paras);
        $totals['status4'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('superdesk_shop_groups_order') . ' o' . "\n\t\t\t" . ' WHERE o.uniacid = :uniacid and o.status = 3 ', $paras);
        $totals['status5'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('superdesk_shop_groups_order') . ' o' . "\n\t\t\t" . ' WHERE o.uniacid = :uniacid and o.status = -1 ', $paras);
        $totals['team1']   = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('superdesk_shop_groups_order') . ' o' . "\n\t\t\t" . ' WHERE o.uniacid = :uniacid and o.heads = 1 and o.paytime > 0 and is_team = 1 and o.success = 1 ', $paras);
        $totals['team2']   = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('superdesk_shop_groups_order') . ' o' . "\n\t\t\t" . ' WHERE o.uniacid = :uniacid and o.heads = 1 and o.paytime > 0 and is_team = 1 and o.success = 0 ', $paras);
        $totals['team3']   = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('superdesk_shop_groups_order') . ' o' . "\n\t\t\t" . ' WHERE o.uniacid = :uniacid and o.heads = 1 and o.paytime > 0 and is_team = 1 and o.success = -1 ', $paras);
        $totals['allteam'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('superdesk_shop_groups_order') . ' o' . "\n\t\t\t" . ' WHERE o.uniacid = :uniacid and o.heads = 1 and o.paytime > 0 and is_team = 1 ', $paras);
        $totals['refund1'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('superdesk_shop_groups_order_refund') . ' as ore' . "\n\t\t\t" . 'left join ' . tablename('superdesk_shop_groups_order') . ' as o on o.id = ore.orderid' . "\n\t\t\t" . 'right join ' . tablename('superdesk_shop_groups_goods') . ' as g on g.id = o.goodid' . "\n\t\t\t" . 'right join ' . tablename('superdesk_shop_member') . ' m on m.openid=o.openid and m.uniacid =  o.uniacid' . "\n\t\t\t" . 'left join ' . tablename('superdesk_shop_member_address') . ' a on a.id=ore.refundaddressid' . "\n\t\t\t" . 'right join ' . tablename('superdesk_shop_groups_category') . ' as c on c.id = g.category' . "\n\t\t\t" . 'WHERE ore.uniacid = :uniacid AND o.refundstate > 0 and o.refundid != 0 and ore.refundstatus >= 0 ', $paras);
        $totals['refund2'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('superdesk_shop_groups_order_refund') . ' as ore' . "\n\t\t\t" . 'left join ' . tablename('superdesk_shop_groups_order') . ' as o on o.id = ore.orderid' . "\n\t\t\t" . 'right join ' . tablename('superdesk_shop_groups_goods') . ' as g on g.id = o.goodid' . "\n\t\t\t" . 'right join ' . tablename('superdesk_shop_member') . ' m on m.openid=o.openid and m.uniacid =  o.uniacid' . "\n\t\t\t" . 'left join ' . tablename('superdesk_shop_member_address') . ' a on a.id=ore.refundaddressid' . "\n\t\t\t" . 'right join ' . tablename('superdesk_shop_groups_category') . ' as c on c.id = g.category' . "\n\t\t\t" . 'WHERE ore.uniacid = :uniacid AND (o.refundtime != 0 or ore.refundstatus < 0) ', $paras);
        $totals['verify1'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('superdesk_shop_groups_order') . ' as o' . "\n\t\t\t" . 'left join ' . tablename('superdesk_shop_groups_verify') . ' as v on v.orderid = o.id and v.uniacid=o.uniacid' . "\n\t\t\t" . 'left join ' . tablename('superdesk_shop_groups_goods') . ' as g on g.id = o.goodid' . "\n\t\t\t" . 'left join ' . tablename('superdesk_shop_member') . ' m on m.openid=o.openid and m.uniacid =  o.uniacid' . "\n\t\t\t" . 'left join ' . tablename('superdesk_shop_member_address') . ' a on a.id=o.addressid' . "\n\t\t\t" . 'right join ' . tablename('superdesk_shop_groups_category') . ' as c on c.id = g.category' . "\n\t\t\t" . 'left join ' . tablename('superdesk_shop_saler') . ' s on s.openid = v.verifier and s.uniacid=v.uniacid' . "\n\t\t\t" . 'left join ' . tablename('superdesk_shop_member') . ' sm on sm.openid = s.openid and sm.uniacid=s.uniacid' . "\n\t\t\t" . 'left join ' . tablename('superdesk_shop_store') . ' store on store.id = v.storeid and store.uniacid=o.uniacid' . "\n\t\t\t" . 'WHERE o.uniacid=:uniacid and o.isverify = 1 and o.status =  1 ', $paras);
        $totals['verify2'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('superdesk_shop_groups_order') . ' as o' . "\n\t\t\t" . 'left join ' . tablename('superdesk_shop_groups_verify') . ' as v on v.orderid = o.id and v.uniacid=o.uniacid' . "\n\t\t\t" . 'left join ' . tablename('superdesk_shop_groups_goods') . ' as g on g.id = o.goodid' . "\n\t\t\t" . 'left join ' . tablename('superdesk_shop_member') . ' m on m.openid=o.openid and m.uniacid =  o.uniacid' . "\n\t\t\t" . 'left join ' . tablename('superdesk_shop_member_address') . ' a on a.id=o.addressid' . "\n\t\t\t" . 'right join ' . tablename('superdesk_shop_groups_category') . ' as c on c.id = g.category' . "\n\t\t\t" . 'left join ' . tablename('superdesk_shop_saler') . ' s on s.openid = v.verifier and s.uniacid=v.uniacid' . "\n\t\t\t" . 'left join ' . tablename('superdesk_shop_member') . ' sm on sm.openid = s.openid and sm.uniacid=s.uniacid' . "\n\t\t\t" . 'left join ' . tablename('superdesk_shop_store') . ' store on store.id = v.storeid and store.uniacid=o.uniacid' . "\n\t\t\t" . 'WHERE o.uniacid=:uniacid and o.isverify = 1 and o.status = 3 ', $paras);
        $totals['verify3'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('superdesk_shop_groups_order') . ' as o' . "\n\t\t\t" . 'left join ' . tablename('superdesk_shop_groups_verify') . ' as v on v.orderid = o.id and v.uniacid=o.uniacid' . "\n\t\t\t" . 'left join ' . tablename('superdesk_shop_groups_goods') . ' as g on g.id = o.goodid' . "\n\t\t\t" . 'left join ' . tablename('superdesk_shop_member') . ' m on m.openid=o.openid and m.uniacid =  o.uniacid' . "\n\t\t\t" . 'left join ' . tablename('superdesk_shop_member_address') . ' a on a.id=o.addressid' . "\n\t\t\t" . 'right join ' . tablename('superdesk_shop_groups_category') . ' as c on c.id = g.category' . "\n\t\t\t" . 'left join ' . tablename('superdesk_shop_saler') . ' s on s.openid = v.verifier and s.uniacid=v.uniacid' . "\n\t\t\t" . 'left join ' . tablename('superdesk_shop_member') . ' sm on sm.openid = s.openid and sm.uniacid=s.uniacid' . "\n\t\t\t" . 'left join ' . tablename('superdesk_shop_store') . ' store on store.id = v.storeid and store.uniacid=o.uniacid' . "\n\t\t\t" . 'WHERE o.uniacid=:uniacid and o.isverify = 1 and o.status <= 0 ', $paras);
    ...
    public function groupsShare()
    {
        ...
        $share = pdo_fetch(
            'select ' .
            '       share_title,share_icon,share_desc,share_url ' .
    ...
    public function sendTeamMessage($orderid = '0', $delRefund = false)
    {
        ...
        $order_goods = pdo_fetch('select * from ' . tablename('superdesk_shop_groups_goods') . ' where uniacid=:uniacid and id=:id ', array(':uniacid' => $_W['uniacid'], ':id' => intval($order['goodid'])));
        ...
        if ($delRefund == true) {
            ...
            $order_refund = pdo_fetch('select * from ' . tablename('superdesk_shop_groups_order_refund') . ' where uniacid=:uniacid and id=:id ', array(':uniacid' => $_W['uniacid'], ':id' => intval($order['refundid'])));
            $refundtype   = '';
            ...
            if ($order['success'] == 1) {
                ...
                $order = pdo_fetchall('select * from ' . tablename('superdesk_shop_groups_order') . ' where teamid = :teamid and success = 1 and status = 1 ', array(':teamid' => $order['teamid']));
    ...
    public function sendGroupsNotice(array $params)
    {
        ...
        if ($tm['is_advanced']) {
            ...
            if (!empty($templateid)) {
                $advanced_template = pdo_fetch('select * from ' . tablename('superdesk_shop_member_message_template') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $templateid, ':uniacid' => $_W['uniacid']));
    ...
    public function allow($orderid, $times = 0, $verifycode = '', $openid = '')
    {
        ...
        $saler = pdo_fetch('select * from ' . tablename('superdesk_shop_saler') . ' where openid=:openid and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
        if (empty($saler)) {
            return error(-1, '无核销权限!');
        }
        $merchid = $saler['merchid'];
        $order   = pdo_fetch('select * from ' . tablename('superdesk_shop_groups_order') . ' where id=:id and uniacid=:uniacid  limit 1', array(':id' => $orderid, ':uniacid' => $uniacid));
        if (empty($order)) {
        ...
        if ($order['isverify']) {
            ...
            if ($order['verifytype'] == 0) {
                $verifynum = pdo_fetchcolumn('select COUNT(1) from ' . tablename('superdesk_shop_groups_verify') . ' where uniacid = :uniacid and orderid = :orderid ', array(':uniacid' => $uniacid, ':orderid' => $orderid));
            ...
            if (!empty($saler['storeid'])) {
                if (0 < $merchid) {
                    $store = pdo_fetch('select * from ' . tablename('superdesk_shop_merch_store') . ' where id=:id and uniacid=:uniacid and merchid = :merchid limit 1', array(':id' => $saler['storeid'], ':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
                } else {
                    $store = pdo_fetch('select * from ' . tablename('superdesk_shop_store') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $saler['storeid'], ':uniacid' => $_W['uniacid']));
                }
            }
        }
    ...
    public function verify($orderid = 0, $times = 0, $verifycode = '', $openid = '')
    {
        ...
        if ($order['isverify']) {
            if ($order['verifytype'] == 0) {
                pdo_update('superdesk_shop_groups_order', array('status' => 3, 'finishtime' => time(), 'sendtime' => $current_time), array('id' => $order['id']));
                ...
                pdo_insert('superdesk_shop_groups_verify', $data);
    ...
    public function tempData($type)
    {
        ...
        $sql   = 'SELECT id,expressname,expresscom,isdefault FROM ' . tablename('superdesk_shop_exhelper_express') . ' where  1 and ' . $condition . ' ORDER BY isdefault desc, id DESC LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize;
        $list  = pdo_fetchall($sql, $params);
        $total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('superdesk_shop_exhelper_express') . ' where 1 and ' . $condition, $params);
    ...
    {
        global $_W;
        $item = pdo_fetch('SELECT id,expressname,type FROM ' . tablename('superdesk_shop_exhelper_express') . ' WHERE id=:id and type=:type AND uniacid=:uniacid', array(':id' => $id, ':type' => $type, ':uniacid' => $_W['uniacid']));
        if (!empty($item)) {
            pdo_update('superdesk_shop_exhelper_express', array('isdefault' => 0), array('type' => $type, 'uniacid' => $_W['uniacid']));
            pdo_update('superdesk_shop_exhelper_express', array('isdefault' => 1), array('id' => $id));
            if ($type == 1) {
    ...
    {
        global $_W;
        $items = pdo_fetchall('SELECT id,expressname FROM ' . tablename('superdesk_shop_exhelper_express') . ' WHERE id in( ' . $id . ' ) and type=:type and uniacid=:uniacid ', array(':type' => $type, ':uniacid' => $_W['uniacid']));
        foreach ($items as $item) {
            pdo_delete('superdesk_shop_exhelper_express', array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
            if ($type == 1) {
    ...
    public function getTemp()
    {
        global $_W;
        global $_GPC;
        $temp_sender  = pdo_fetchall('SELECT id,isdefault,sendername,sendertel FROM ' . tablename('superdesk_shop_exhelper_senduser') . ' WHERE uniacid=:uniacid order by isdefault desc ', array(':uniacid' => $_W['uniacid']));
        $temp_express = pdo_fetchall('SELECT id,type,isdefault,expressname FROM ' . tablename('superdesk_shop_exhelper_express') . ' WHERE type=1 and uniacid=:uniacid order by isdefault desc ', array(':uniacid' => $_W['uniacid']));
        $temp_invoice = pdo_fetchall('SELECT id,type,isdefault,expressname FROM ' . tablename('superdesk_shop_exhelper_express') . ' WHERE type=2 and uniacid=:uniacid order by isdefault desc ', array(':uniacid' => $_W['uniacid']));
