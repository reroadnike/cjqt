...
class Sms_SuperdeskShopV2ComModel extends ComModel
{
...
//        die();
        ...
        if ($template['type'] == 'dayu') {
            ...
            include_once(SUPERDESK_SHOPV2_VENDOR . 'alidayu/api_client/AliDayuClient.class.php');
            $aliDayuClient = new AliDayuClient(
                $smsset['dayu_key'],
                $smsset['dayu_secret']
            );

//            $signName, $templateCode, $phoneNumbers, $templateParam = null, $outId = null
            $dayuResult = $aliDayuClient->sendSms(
                $template['smssign'], // 短信签名
                $template['smstplid'], // 正式短信模板编号
                '' . $mobile, // 短信接收者
...
//            var_dump($dayuResult);
            // error
//            object(stdClass)#1256 (3) {
//                        ["Message"]=>
//              string(19) "JSON参数不合法"
//                        ["RequestId"]=>
//              string(36) "0F65ED35-DCA0-42B0-902B-9BB47451FDA8"
//                        ["Code"]=>
//              string(22) "isv.INVALID_JSON_PARAM"
//            }
            // success
//            object(stdClass)#1256 (4) {
//            ["Message"]=>
//              string(2) "OK"
//                        ["RequestId"]=>
//              string(36) "1AF604AB-4295-4403-A0B1-B2F07916AA67"
//                        ["BizId"]=>
//              string(20) "939708514467218576^0"
//                        ["Code"]=>
//              string(2) "OK"
//            }
//            die();

            if (empty($dayuResult) || $dayuResult->Code != 'OK') {
                return array(
                    'status'  => 0,
                    'message' => '短信发送失败(/Message:' . $dayuResult->Message . '/Code: ' . $dayuResult->Code . '/RequestId: ' . $dayuResult->RequestId . ')'
                );
            }
        }
...
//        if ($template['type'] == 'dayu') {
//            include_once SUPERDESK_SHOPV2_VENDOR . 'dayu/TopSdk.php';
//
...
//                return array(
//                    'status'  => 0,
//                    'message' => '短信发送失败(' . $dayuResult['sub_msg'] . '/code: ' . $dayuResult['code'] . '/sub_code: ' . $dayuResult['sub_code'] . ')'
//                );
//            }
//        }
        ...
        if ($template['type'] == 'emay') {
            ...
            include_once SUPERDESK_SHOPV2_VENDOR . 'emay/SMSUtil.php';
            ...
            $emayClient = new SMSUtil(
                $smsset['emay_url'],
                $smsset['emay_sn'],
                $smsset['emay_pw'],
                $smsset['emay_sk'],
                array(
                    'proxyhost'     => $smsset['emay_phost'],
                    'proxyport'     => $smsset['pport'],
                    'proxyusername' => $smsset['puser'],
                    'proxypassword' => $smsset['ppw']
                ),
                $smsset['emay_out'],
                $smsset['emay_outresp']
            );
    ...
    public function sms_set()
    {
        ...
        return pdo_fetch(
            'SELECT * ' .
            ' FROM ' . tablename('superdesk_shop_sms_set') .
    ...
    public function sms_temp()
    {
        ...
        $list = pdo_fetchall(
            'SELECT id, `type`, `name` ' .
            ' FROM ' . tablename('superdesk_shop_sms') .
            ' WHERE ' .
            '       status=1 ' .
            '       and uniacid=:uniacid ',
            array(
    ...
    public function sms_num($type, $smsset = NULL)
    {
        ...
        if ($type == 'emay') {
            ...
            if (!empty($smsset['emay_warn']) && !empty($smsset['emay_mobile']) && ($num < $smsset['emay_warn']) && (($smsset['emay_warn_time'] + (60 * 60 * 24)) < time())) {
                ...
                $emayResult = $emayClient->send(
                    $smsset['emay_mobile'],
                    '【系统预警】' . '您的亿美软通SMS余额为:' . $num . '，低于预警值:' . $smsset['emay_warn'] . ' (24小时内仅通知一次)'
                );
                if (empty($emayResult)) {
                    pdo_update('superdesk_shop_sms_set', array('emay_warn_time' => time()), array('id' => $smsset['id']));
                }
            }
            return $num;
        }
