...
class Apply_SuperdeskShopV2Page extends PluginWebPage
{
    ...
    public function main()
    {
        ...
        $sql = 'select a.*, m.nickname,m.avatar,m.realname,m.mobile,l.levelname from ' . tablename('superdesk_shop_commission_apply') . ' a ' . ' left join ' . tablename('superdesk_shop_member') . ' m on m.id = a.mid' . ' left join ' . tablename('superdesk_shop_commission_level') . ' l on l.id = m.agentlevel' . ' where 1 ' . $condition . ' ORDER BY ' . $orderby . ' desc ';
        ...
        if ($_GPC['export'] == '1') {
            ...
            foreach ($list as &$row) {
                ...
                $orders = pdo_fetchall(
                    ' select id,agentid, ordersn,price,goodsprice,dispatchprice,createtime, paytype ' .
                ...
                foreach ($orders as &$order) {
                    ...
                    $goods = pdo_fetchall(
                        ' SELECT og.id,g.thumb,og.price,og.realprice, og.total,g.title,o.paytype,og.optionname,og.commission1,' .
                        '        og.commission2,og.commission3,og.commissions,og.status1,og.status2,og.status3,' .
                        '        og.content1,og.content2,og.content3 ' .
        ...
        $total = pdo_fetchcolumn('select count(a.id) from' . tablename('superdesk_shop_commission_apply') . ' a ' . ' left join ' . tablename('superdesk_shop_member') . ' m on m.uid = a.mid' . ' left join ' . tablename('superdesk_shop_commission_level') . ' l on l.id = m.agentlevel' . ' where 1 ' . $condition, $params);
    ...
    protected function applyData()
    {
        ...
        $apply = pdo_fetch('select * from ' . tablename('superdesk_shop_commission_apply') . ' where uniacid=:uniacid and id=:id limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $id));
        ...
        foreach ($list as &$row) {
            ...
            $goods = pdo_fetchall(
                ' SELECT og.id,g.thumb,og.price,og.realprice, og.total,g.title,o.paytype,og.optionname,og.commission1,og.commission2,og.commission3,og.commissions,' .
    ...
    public function check()
    {
        ...
        foreach ($ogids as $ogid) {
            $g = pdo_fetch(
                'SELECT ' .
                '       total,commission1,commission2,commission3,commissions ' .
                ' from ' . tablename('superdesk_shop_order_goods') . // TODO 标志 楼宇之窗 openid shop_order_goods 不处理
                ' where ' .
                '       id=:id ' .
                '       and uniacid = :uniacid ' .
        ...
        if ($isAllUncheck) {
            pdo_update('superdesk_shop_commission_apply', array('status' => -1, 'invalidtime' => $time), array('id' => $id, 'uniacid' => $_W['uniacid']));
        } else {
            pdo_update('superdesk_shop_commission_apply', array('status' => 2, 'checktime' => $time), array('id' => $id, 'uniacid' => $_W['uniacid']));
    ...
    public function cancel()
    {
        ...
        pdo_update('superdesk_shop_commission_apply', array('status' => 1, 'checktime' => 0, 'invalidtime' => 0), array('id' => $id, 'uniacid' => $_W['uniacid']));
    ...
    public function refuse()
    {
        ...
        pdo_update('superdesk_shop_commission_apply', array('status' => -2, 'checktime' => 0, 'invalidtime' => 0, 'refusetime' => time()), array('id' => $id, 'uniacid' => $_W['uniacid']));

        plog('commission.apply.refuse', '驳回申请 ID: ' . $id . ' 申请编号: ' . $apply['applyno'] . ' ');

        show_json(1, array(
            'url' => webUrl('commission/apply', array('status' => 0))
        ));
    }
    ...
    public function pay($params = array(), $mine = array())
    {
        ...
        if ($apply['type'] < 2) {
            ...
            $result = m('finance')->pay($member['openid'], $member['core_user'],
                $apply['type'], $pay, $apply['applyno'], $set['texts']['commission'] . '打款');
        ...
        $log = array(
            'uniacid'        => $_W['uniacid'], 'applyid' => $apply['id'],
            'mid'            => $member['id'],
        ...
        pdo_insert('superdesk_shop_commission_log', $log);
        ...
        $this->model->upgradeLevelByCommissionOK($member['openid'], $member['core_user']);

        if (p('globous')) {
            p('globous')->upgradeLevelByCommissionOK($member['openid'], $member['core_user']);
        }
        ...
        show_json(1, array(
            'url' => webUrl('commission/apply', array('status' => $apply['status']))
        ));
    }
    ...
    public function payed($params = array(), $mine = array())
    {
        ...
        pdo_update('superdesk_shop_commission_apply', array('status' => 3, 'paytime' => $time, 'commission_pay' => $totalpay, 'realmoney' => $realmoney, 'deductionmoney' => $deductionmoney), array('id' => $id, 'uniacid' => $_W['uniacid']));
    ...
    public function changecommission()
    {
        ...
        $order_goods_change = pdo_fetchall(
            ' select og.id,g.title,g.thumb,g.goodssn,og.goodssn as option_goodssn, g.productsn,og.productsn as option_productsn, og.total,og.price, ' .
            '        og.optionname as optiontitle, og.realprice,og.oldprice,og.commission1,og.commission2,og.commission3,og.commissions, ' .
