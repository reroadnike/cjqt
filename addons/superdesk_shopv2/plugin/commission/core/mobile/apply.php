...
class Apply_SuperdeskShopV2Page extends CommissionMobileLoginPage
{
    ...
    public function main()
    {
        ...
        $member = $this->model->getInfo($openid, array(), $core_user);
        ...
        $agentLevel    = $this->model->getLevel($openid,$core_user);
        $commission_ok = 0;
        $orderids      = array();
        if (1 <= $level) {
            $level1_orders = pdo_fetchall(
                ' select distinct o.id from ' . tablename('superdesk_shop_order') . ' o ' . // TODO 标志 楼宇之窗 openid shop_order 不处理
        ...
        if (2 <= $level) {
            if (0 < $member['level1']) {
                $level2_orders = pdo_fetchall(
                    ' select distinct o.id from ' . tablename('superdesk_shop_order') . ' o ' .// TODO 标志 楼宇之窗 openid shop_order 不处理
                    ' left join  ' . tablename('superdesk_shop_order_goods') . ' og on og.orderid=o.id ' .// TODO 标志 楼宇之窗 openid shop_order_goods 不处理
                    ' where o.agentid in( ' . implode(',', array_keys($member['level1_agentids'])) . ')  ' .
                    '        and o.status>=3  ' .
                    '        and og.status2=0 ' .
                    '        and og.nocommission=0 ' .
                    '        and (' . $time . ' - o.finishtime > ' . $day_times . ') ' .
        ...
        if (3 <= $level) {
            if (0 < $member['level2']) {
                $level3_orders = pdo_fetchall(
                    'select distinct o.id from ' . tablename('superdesk_shop_order') . ' o ' .// TODO 标志 楼宇之窗 openid shop_order 不处理
                    ' left join  ' . tablename('superdesk_shop_order_goods') . ' og on og.orderid=o.id ' .// TODO 标志 楼宇之窗 openid shop_order_goods 不处理
                    ' where o.agentid in( ' . implode(',', array_keys($member['level2_agentids'])) . ')  ' .
                    '        and o.status>=3  ' .
                    '        and  og.status3=0 ' .
                    '        and og.nocommission=0 ' .
                    '        and (' . $time . ' - o.finishtime > ' . $day_times . ') ' .
        ...
        foreach ($orderids as $o) {
            ...
            $goods = pdo_fetchall(
                ...
                array(':uniacid' => $_W['uniacid'], ':orderid' => $o['orderid']));
            foreach ($goods as $g) {
        ...
        if ($this->set['cashother'] == 1) {
            ...
            if ($this->set['cashcard'] == 1) {
                ...
                $banklist  = pdo_fetchall(
                    ' SELECT * '.
                    ' FROM ' . tablename('superdesk_shop_commission_bank') .
                    ' WHERE 1 ' . $condition .
        ...
        if ($_W['ispost']) {
            ...
            pdo_insert('superdesk_shop_commission_apply', $apply);
            ...
            $this->model->sendMessage($openid, $core_user, array('commission' => $mcommission, 'type' => $apply_type[$apply['type']]), TM_COMMISSION_APPLY);
