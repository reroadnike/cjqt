...
if (!class_exists('SnsModel')) {
    class SnsModel extends PluginModel
    {
        public function checkMember()
        {
            ...
            global $_GPC;
            if (!empty($_W['openid'])) {
                $member = pdo_fetch('select * from ' . tablename('superdesk_shop_sns_member') . ' where uniacid=:uniacid and openid=:openid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $_W['openid']));
                if (empty($member)) {
                    $member = array('uniacid' => $_W['uniacid'], 'openid' => $_W['openid'], 'createtime' => time());
                    pdo_insert('superdesk_shop_sns_member', $member);
                    return;
                }
        ...
        public function getMember($openid, $core_user)
        {
            global $_W;
            global $_GPC;
        ...
        public function getCategory($all = true)
        {
            ...
            return pdo_fetchall(
                'select * ' .
                ' from ' . tablename('superdesk_shop_sns_category') .
                ' where ' .
                '       uniacid=:uniacid ' . $condition .
                '       and enabled = 1 ' .
        ...
        public function getBoard($bid = '0')
        {
            ...
            return pdo_fetch(
                'select * ' .
                ' from ' . tablename('superdesk_shop_sns_board') .
                ' where ' .
                '       uniacid=:uniacid ' .
                '       and id=:id ' .
                ' limit 1',
        ...
        public function isManager($bid = 0, $openid = '')
        {
            ...
            $count = pdo_fetchcolumn(
                'select count(*) ' .
                ' from ' . tablename('superdesk_shop_sns_manage') .
                ' where ' .
                '       uniacid=:uniacid ' .
                '       and bid=:bid ' .
                '       and openid=:openid ' .
        ...
        public function getFollowCount($bid)
        {
            ...
            return pdo_fetchcolumn(
                'select count(*) ' .
                ' from ' . tablename('superdesk_shop_sns_board_follow') .
                ' where ' .
                '       uniacid=:uniacid ' .
                '       and bid=:bid ' .
                ' limit 1',
        ...
        public function getTops($bid = 0)
        {
            ...
            return pdo_fetchall(
                'select id,title,istop,isboardtop ' .
                ' from ' . tablename('superdesk_shop_sns_post') .
                ' where ' .
                '       uniacid=:uniacid ' .
                $condition .
                '       and pid=0 ' .
                '       and deleted=0 ' .
                ' order by istop desc,replytime desc',
                array(
                    ':uniacid' => $_W['uniacid']
                )
            );
        ...
        public function setCredit($openid = '', $core_user = 0, $bid = 0, $type = -1)
        {
            ...
            if (($type == SNS_CREDIT_POST) || ($type == SNS_CREDIT_DELETE_POST)) {
                ...
                $log    = '社区发表话题奖励积分: +' . $credit;

                if ($type == SNS_CREDIT_DELETE_POST) {
                    $credit = -$credit;
                    $log    = '社区被删除话题扣除积分: -' . abs($credit);
                }
            } else {
                ...
                if (($type == SNS_CREDIT_REPLY) || ($type == SNS_CREDIT_DELETE_REPLY)) {
                    ...
                    $log    = '社区发表评论奖励积分: +' . $credit;

                    if ($type == SNS_CREDIT_DELETE_REPLY) {
                        $log    = '社区被删除评论奖励积分: -' . abs($credit);
                        $credit = -$credit;
                    }
                    ...
                    if (($type == SNS_CREDIT_TOP) || ($type == SNS_CREDIT_TOP_CANCEL)) {
                        ...
                        $log    = '社区话题被全站置顶奖励积分: +' . $credit;

                        if ($type == SNS_CREDIT_TOP_CANCEL) {
                            $credit = -$credit;
                            $log    = '社区话题被取消全站置顶扣除积分: ' . abs($credit);
                        }
            ...
            if (0 < abs($credit)) {
                ...
                m('member')->setCredit($openid, $core_user, 'credit1', $credit, array(0, $log));
        ...
        public function getLevel($openid, $core_user = 0)
        {
            ...
            if (!empty($member) && !empty($member['sns_level'])) {
                ...
                $level = pdo_fetch(
                    'select * ' .
                    ' from ' . tablename('superdesk_shop_sns_level') .
                    ' where ' .
                    '       id=:id ' .
                    '       and uniacid=:uniacid ' .
                    ' limit 1',
        ...
        public function getLevels($all = true)
        {
            ...
            return pdo_fetchall('select * from ' . tablename('superdesk_shop_sns_level') . ' where uniacid=:uniacid' . $condition . ' order by id asc', array(':uniacid' => $_W['uniacid']));
        }

        public function replaceContent($content)
        {
            return str_replace("\n", '<br/>', preg_replace('/\\[EM(\\w+)\\]/', '<img src="../addons/superdesk_shopv2/plugin/sns/static/images/face/${1}.gif" class="emoji" />', $content));
        }
        ...
        public function getAvatar($avatar = '')
        {
            if (empty($avatar)) {
                $set    = $this->getSet();
                $avatar = ((empty($set['head']) ? '../addons/superdesk_shopv2/plugin/sns/static/images/head.jpg' : tomedia($set['head'])));
            }
            return $avatar;
        }
    }
