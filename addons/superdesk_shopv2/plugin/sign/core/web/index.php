...
class Index_SuperdeskShopV2Page extends PluginWebPage
{
    ...
    public function rule()
    {
        ...
        if ($_W['ispost']) {
            ...
            if (empty($set)) {
                ...
                pdo_insert('superdesk_shop_sign_set', $data);
            } else {
                pdo_update('superdesk_shop_sign_set', $data, array('id' => $set['id'], 'uniacid' => $_W['uniacid']));
            }
    ...
    public function records()
    {
        ...
        $list = pdo_fetchall(
            'select r.*, m.nickname, m.id as mid, m.avatar ' .
            ' from ' . tablename('superdesk_shop_sign_records') . 'r ' .
            ' left join ' . tablename('superdesk_shop_member') . 'm on r.openid=m.openid ' .
            ' where 1 ' .
            $condition .
    ...
    public function set()
    {
        ...
        if ($_W['ispost']) {
            ...
            if (!empty($keyword)) {
                if ($keyword['name'] != 'superdesk_shopv2:sign') {
                    show_json(0, '关键字已存在!');
                }
