...
class Verify_SuperdeskShopV2Page extends PluginWebPage
{
    private $_orderModel;
    ...
    public function fetch()
    {
        ...
        if (!empty($item['refundid'])) {
            $refund = pdo_fetch('select * from ' . tablename('superdesk_shop_order_refund') . ' where id=:id limit 1', array(':id' => $item['refundid']));
            if (!empty($refund)) {
                pdo_update('superdesk_shop_order_refund', array('status' => -1), array('id' => $item['refundid']));
                //mark kafka 为了kafka转成了model执行
                $this->_orderModel->updateByColumn(
                    array(
                        'refundstate' => 0
    ...
    public function detail()
    {
        ...
        $order      = pdo_fetch('SELECT o.*,g.title,g.category,g.groupsprice,g.singleprice,g.thumb,g.id as gid FROM ' . tablename('superdesk_shop_groups_order') . ' as o ' . 'left join ' . tablename('superdesk_shop_groups_goods') . ' as g on g.id = o.goodid ' . 'WHERE o.id = :orderid ', $params);
