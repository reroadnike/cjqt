...
class Goods_SuperdeskShopV2Page extends PluginMobileLoginPage
{
    public function main()
    {
        ...
        $uniacid                         = $_W['uniacid'];
        $id                              = intval($_GPC['id']);
        $groupsset                       = pdo_fetch('select description,groups_description,discount,headstype,headsmoney,headsdiscount from ' . tablename('superdesk_shop_groups_set') . "\r\n\t\t\t\t\t" . 'where uniacid = :uniacid ', array(':uniacid' => $uniacid));
        ...
        $goods['fightnum'] = pdo_fetchcolumn('select count(1) from ' . tablename('superdesk_shop_groups_order') . ' where goodid = :goodid and uniacid = :uniacid and deleted = 0 and is_team = 1 and status > 0 ', array(':goodid' => $goods['id'], ':uniacid' => $uniacid));
    ...
    public function openGroups()
    {
        ...
        $teams             = pdo_fetchall('select * from ' . tablename('superdesk_shop_groups_goods') . ' where deleted = 0 and status = 1 and uniacid = :uniacid order by sales desc limit 4', array(':uniacid' => $uniacid));
        foreach ($teams as $key => $value) {
            $value['fightnum'] = pdo_fetchcolumn('select count(1) from ' . tablename('superdesk_shop_groups_order') . ' where goodid = :goodid and uniacid = :uniacid and deleted = 0 and is_team = 1 and status > 0 ', array(':goodid' => $value['id'], ':uniacid' => $uniacid));
    ...
    public function fightGroups()
    {
        ...
        $goods['fightnum'] = $goods['teamnum'] + $goods['fightnum'];
        $goods             = set_medias($goods, 'thumb');
        $teams             = pdo_fetchall('select o.paytime,o.id,o.goodid,o.teamid,m.nickname,m.realname,m.mobile,m.avatar,g.endtime,g.groupnum from ' . tablename('superdesk_shop_groups_order') . ' as o' . "\r\n\t\t\t\t" . 'left join ' . tablename('superdesk_shop_member') . ' as m on m.openid=o.openid and m.uniacid =  o.uniacid' . "\r\n\t\t\t\t" . 'left join ' . tablename('superdesk_shop_groups_goods') . ' as g on g.id = o.goodid' . "\r\n\t\t\t\t" . 'where o.goodid = :goodid and o.uniacid = :uniacid and o.openid != :openid and o.deleted = 0 and o.heads = 1 and o.paytime > 0 and o.success = 0 limit 10 ', array(':goodid' => $goods['id'], ':uniacid' => $uniacid, ':openid' => $openid));
        foreach ($teams as $key => $value) {
            $num                   = pdo_fetchcolumn('select count(1) from ' . tablename('superdesk_shop_groups_order') . ' where uniacid = :uniacid and deleted = 0 and teamid = :teamid and status > 0 ', array(':teamid' => $value['teamid'], ':uniacid' => $uniacid));
