...
class Log_SuperdeskShopV2Page extends CommissionMobileLoginPage
{
    ...
    public function get_list()
    {
        ...
        $commissioncount = pdo_fetchcolumn('select sum(commission) from ' . tablename('superdesk_shop_commission_apply') . ' where mid=:mid and uniacid=:uniacid and status>-1 limit 1', array(':mid' => $member['id'], ':uniacid' => $_W['uniacid']));
        $list            = pdo_fetchall('select * from ' . tablename('superdesk_shop_commission_apply') . ' where 1 ' . $condition . ' order by id desc LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize, $params);
        $total           = pdo_fetchcolumn('select count(*) from ' . tablename('superdesk_shop_commission_apply') . ' where 1 ' . $condition, $params);
    ...
    public function detail()
    {
        ...
        $id     = intval($_GPC['id']);
        $member = m('member')->getMember($_W['openid'], $_W['core_user']);
        $apply  = pdo_fetch('select * from ' . tablename('superdesk_shop_commission_apply') . ' where id=:id and `mid`=:mid and uniacid=:uniacid limit 1', array(':id' => $id, ':mid' => $member['id'], ':uniacid' => $_W['uniacid']));
    ...
    public function orders()
    {
        ...
        $pindex  = max(1, intval($_GPC['page']));
        $psize   = 5;
        $apply   = pdo_fetch('select orderids from ' . tablename('superdesk_shop_commission_apply') . ' where id=:id and `mid`=:mid and uniacid=:uniacid limit 1', array(':id' => $id, ':mid' => $member['id'], ':uniacid' => $_W['uniacid']));
        ...
        $list = pdo_fetchall(
            ' select o.id,o.agentid, o.ordersn,o.price,o.goodsprice, o.dispatchprice,o.createtime, o.paytype ' .
        ...
        $total = pdo_fetchcolumn(
            ' select count(*) from ' . tablename('superdesk_shop_order') . ' o ' . // TODO 标志 楼宇之窗 openid shop_order 已处理
            ' left join ' . tablename('superdesk_shop_member') . ' m on o.openid = m.openid and o.core_user = m.core_user ' .
            ' where  o.id in ( ' . implode(',', $ids) . ' ) '
        );
        ...
        foreach ($list as &$row) {
            ...
            $goods = pdo_fetchall(
                'SELECT og.id,og.goodsid,g.thumb,og.price,og.total,g.title,og.optionname,' .
