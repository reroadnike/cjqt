...
class Index_SuperdeskShopV2Page extends PluginMobileLoginPage
{
    ...
    public function dosign()
    {
        ...
        if (!empty($date) && !empty($set['signold']) && (0 < $set['signold_price'])) {
            if (empty($set['signold_type'])) {
                ...
                m('member')->setCredit($_W['openid'], $_W['core_user'],
                    'credit2', -$set['signold_price'], $set['textcredit'] . $set['textsign'] . ': ' . $set['textsignold'] . '扣除' . $set['signold_price'] . $set['textmoney']);
            } else {
                if ($member['credit1'] < $set['signold_price']) {
                    show_json(0, $set['textsignold'] . '失败! 您的' . $set['textcredit'] . '不足, 无法' . $set['textsignold']);
                }
                m('member')->setCredit($_W['openid'], $_W['core_user'],
                    'credit1', -$set['signold_price'], $set['textcredit'] . $set['textsign'] . ': ' . $set['textsignold'] . '扣除' . $set['signold_price'] . $set['textcredit']);
            }
        }

        if (!empty($credit) && (0 < $credit)) {
            m('member')->setCredit($_W['openid'], $_W['core_user'],
                'credit1', +$credit, $set['textcredit'] . $set['textsign'] . ': ' . $message);
        }
        ...
        pdo_insert('superdesk_shop_sign_records', $arr);
    ...
    public function doreward()
    {
        ...
        $record = pdo_fetch(
            'select * ' .
            ' from ' . tablename('superdesk_shop_sign_records') .
            ' where ' .
            '       openid=:openid ' .
    ...
    public function getRecords()
    {
        ...
        $sql =
            'SELECT COUNT(*) ' .
            ' FROM ' . tablename('superdesk_shop_sign_records') . ' log ' .
            ' where 1 ' .
            $condition;
    ...
    public function getRank()
    {
        ...
        if (!empty($type)) {
            ...
            $sql =
                'SELECT COUNT(*) ' .
                ' FROM ' . tablename('superdesk_shop_sign_user') .
                ' where 1 ' .
                $conditioncol;
            ...
            if (!empty($total)) {
                ...
                $type = 'su.' . $type;
                $sql  =
                    ' SELECT su.*, sm.nickname, sm.avatar ' .
                    ' FROM ' . tablename('superdesk_shop_sign_user') . ' su ' .
                    '       left join ' . tablename('superdesk_shop_member') . ' sm on sm.openid=su.openid ' .
                    ' where 1 ' .
                    $condition .
                    ' ORDER BY ' . $type . ' DESC ' .
                    ' LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize;
