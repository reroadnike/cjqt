...
class Order_SuperdeskShopV2Page extends PluginWebPage
{
    public function main()
    {
        ...
        $list = pdo_fetchall(
            'SELECT ' .
            '       o.*,g.title,g.category,g.thumb,g.groupsprice,g.singleprice,g.price as gprice,g.goodssn,' .
        ...
        $total2 = pdo_fetchall(
            ...
            ' FROM ' . tablename('superdesk_shop_groups_order') . ' as o' .
            '       left join ' . tablename('superdesk_shop_groups_goods') . ' as g on g.id = o.goodid' .
            '       left join ' . tablename('superdesk_shop_member_address') . ' as a on a.id=o.addressid' . // TODO 标志 楼宇之窗 openid superdesk_shop_member_address 不处理
            '       right join ' . tablename('superdesk_shop_member') . ' as m on m.openid = o.openid and m.uniacid =  o.uniacid' .
            ' WHERE 1 ' .
            $condition .
            ' group by o.id ',
            $params
    ...
    public function detail()
    {
        ...
        $order   = pdo_fetch('SELECT o.*,g.title,g.category,g.groupsprice,g.singleprice,g.thumb,g.id as gid FROM ' . tablename('superdesk_shop_groups_order') . ' as o' . "\r\n\t\t\t\t" . 'left join ' . tablename('superdesk_shop_groups_goods') . ' as g on g.id = o.goodid' . "\r\n\t\t\t\t" . 'WHERE o.id = :orderid ', $params);
        $order   = set_medias($order, 'thumb');
        $member  = m('member')->getMember($order['openid'],$order['core_user']);// TODO 标志 m('member')->getMember true

        if ($order['verifytype'] == 0) {
            $verify = pdo_fetch('select * from ' . tablename('superdesk_shop_groups_verify') . ' where orderid = ' . $order['id'] . ' ');

            if (!empty($verify['verifier'])) {
                $saler              = m('member')->getMember($verify['verifier']);
                $saler['salername'] = pdo_fetchcolumn('select salername from ' . tablename('superdesk_shop_saler') . ' where openid=:openid and uniacid=:uniacid limit 1 ', array(':uniacid' => $_W['uniacid'], ':openid' => $verify['verifier']));
            }


            if (!empty($order['storeid'])) {
                $store = pdo_fetch('select * from ' . tablename('superdesk_shop_store') . ' where id=:storeid limit 1 ', array(':storeid' => $verify['storeid']));
            }

        } else if ($order['verifytype'] == 1) {
            $verifyinfo = pdo_fetchall('select v.*,sm.id as salerid,sm.nickname as salernickname,s.salername,store.storename from ' . tablename('superdesk_shop_groups_verify') . ' as v' . "\r\n\t\t\t\t\t" . 'left join ' . tablename('superdesk_shop_saler') . ' s on s.openid = v.verifier and s.uniacid = v.uniacid' . "\r\n\t\t\t\t\t" . 'left join ' . tablename('superdesk_shop_member') . ' sm on sm.openid = s.openid and sm.uniacid = s.uniacid' . "\r\n\t\t\t\t\t" . 'left join ' . tablename('superdesk_shop_store') . ' store on store.id = v.storeid and store.uniacid = v.uniacid' . "\r\n\t\t\t\t\t" . 'where v.uniacid = :uniacid and v.orderid = :orderid ', array(':orderid' => $orderid, ':uniacid' => $_W['uniacid']));
        }
    ...
    protected function opData()
    {
        ...
        global $_GPC;
        $id   = intval($_GPC['id']);
        $item = pdo_fetch('SELECT * FROM ' . tablename('superdesk_shop_groups_order') . ' WHERE id = :id and uniacid=:uniacid', array(':id' => $id, ':uniacid' => $_W['uniacid']));
    ...
    public function delete()
    {
        ...
        global $_GPC;
        $id   = intval($_GPC['id']);
        $item = pdo_fetch('SELECT id,name FROM ' . tablename('superdesk_shop_groups_order') . ' WHERE id = \'' . $id . '\' AND uniacid=' . $_W['uniacid'] . '');
        ...
        pdo_delete('superdesk_shop_groups_order', array('id' => $id));
    ...
    public function remarksaler()
    {
        ...
        if ($_W['ispost']) {
            pdo_update('superdesk_shop_groups_order', array('remark' => $_GPC['remark']), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
    ...
    public function changeaddress()
    {
        ...
        if ($_W['ispost']) {
            ...
            if (!empty($id)) {
                ...
                $item                      = pdo_fetch('SELECT id, orderno, address FROM ' . tablename('superdesk_shop_groups_order') . ' WHERE id = :id and uniacid=:uniacid', array(':id' => $id, ':uniacid' => $_W['uniacid']));
                ...
                pdo_update('superdesk_shop_groups_order', array('address' => $address_array), array('id' => $id, 'uniacid' => $_W['uniacid']));
    ...
    public function pay($a = array(), $b = array())
    {
        ...
        if (!empty($item['virtual']) && c('virtual')) {
            ...
            pdo_update('superdesk_shop_groups_order', array('status' => 1, 'pay_type' => 'other', 'paytime' => time(), 'starttime' => time()), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
    ...
    public function sendcancel()
    {
        ...
        if ($_W['ispost']) {
            ...
            pdo_update('superdesk_shop_groups_order',
                array('status' => 1, 'sendtime' => 0, 'remarksend' => $remark),
                array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
            plog('groups.order.sendcancel',
                '订单取消发货 ID: ' . $item['id'] .
                ' 订单号: ' . $item['orderno'] .
                ' 原因: ' . $remark);
            show_json(1);
    ...
    public function changeexpress()
    {
        ...
        if ($_W['ispost']) {
            ...
            if (!empty($expresssn)) {
                ...
                pdo_update('superdesk_shop_groups_order', $change_data, array('id' => $id, 'uniacid' => $_W['uniacid']));
    ...
    public function finish()
    {
        ...
        $opdata = $this->opData();
        extract($opdata);
        pdo_update('superdesk_shop_groups_order', array('status' => 3, 'finishtime' => time()), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
        p('groups')->sendTeamMessage($item['id']);
    ...
    public function enabled()
    {
        ...
        $items = pdo_fetchall('SELECT id,name FROM ' . tablename('superdesk_shop_groups_order') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);

        foreach ($items as $item) {
            pdo_update('superdesk_shop_groups_order', array('enabled' => intval($_GPC['enabled'])), array('id' => $item['id']));
    ...
    protected function selectOrderPrice($day = 0)
    {
        ...
        $sql     = 'select id,price,freight,starttime,openid,creditmoney from ' . tablename('superdesk_shop_groups_order') . ' where uniacid = :uniacid and status > 0 and paytime between :createtime1 and :createtime2 ';
    ...
    protected function selectteamPrice($success = 0)
    {
        ...
        $sql     = 'select id,price,freight,starttime from ' . tablename('superdesk_shop_groups_order') . ' where uniacid = :uniacid and paytime > 0 and is_team = 1 and heads = :heads and success = :success ';
    ...
    public function close()
    {
        ...
        if ($_W['ispost']) {
            ...
            if ((0 < $item['refundstate']) && !empty($item['refundid'])) {
                ...
                pdo_update('superdesk_shop_groups_order_refund', $change_refund, array('id' => $item['refundid'], 'uniacid' => $_W['uniacid']));
            }


            pdo_update('superdesk_shop_groups_order', array('status' => -1, 'refundstate' => 0, 'canceltime' => $time, 'remarkclose' => $_GPC['remark']), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
