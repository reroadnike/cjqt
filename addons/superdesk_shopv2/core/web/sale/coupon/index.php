...
class Index_SuperdeskShopV2Page extends ComWebPage
{
    ...
    public function main()
    {
        ...
        $sql  = 'SELECT * FROM ' . tablename('superdesk_shop_coupon') . ' ' . ' where  1 and ' . $condition . ' ORDER BY displayorder DESC,id DESC LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize;
        $list = pdo_fetchall($sql, $params);
        foreach ($list as &$row) {
            $row['gettotal'] = pdo_fetchcolumn(
                'select count(*) from ' . tablename('superdesk_shop_coupon_data') . // TODO 标志 楼宇之窗 openid shop_coupon_data 不处理
                ' where couponid=:couponid and uniacid=:uniacid limit 1',
                array(
                    ':couponid' => $row['id'],
                    ':uniacid'  => $_W['uniacid']
                )
            ...
            $row['usetotal'] = pdo_fetchcolumn(
                'select count(*) from ' . tablename('superdesk_shop_coupon_data') . // TODO 标志 楼宇之窗 openid shop_coupon_data 不处理
                ' where used = 1 and couponid=:couponid and uniacid=:uniacid limit 1',
                array(
                    ':couponid' => $row['id'],
                    ':uniacid'  => $_W['uniacid']
                )
            ...
            $row['pwdjoins'] = pdo_fetchcolumn(
                'select count(*) from ' . tablename('superdesk_shop_coupon_guess') .
                ' where couponid=:couponid and uniacid=:uniacid limit 1',
                array(
                    ':couponid' => $row['id'],
                    ':uniacid'  => $_W['uniacid']
            ...
            $row['pwdoks'] = pdo_fetchcolumn(
                'select count(*) from ' . tablename('superdesk_shop_coupon_guess') .
                ' where couponid=:couponid and uniacid=:uniacid and ok=1 limit 1',
                array(
                    ':couponid' => $row['id'],
                    ':uniacid'  => $_W['uniacid']
                )
        ...
        unset($row);
        $total    = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('superdesk_shop_coupon') . ' where 1 and ' . $condition, $params);
        $pager    = pagination($total, $pindex, $psize);
        $category = pdo_fetchall('select * from ' . tablename('superdesk_shop_coupon_category') . ' where uniacid=:uniacid and merchid=0 order by id desc', array(':uniacid' => $_W['uniacid']), 'id');
    ...
    protected function post()
    {
        ...
        if ($_W['ispost']) {
            ...
            if (!empty($id)) {
                if (!empty($data['pwdkey2'])) {
                    $pwdkey2 = pdo_fetch('SELECT pwdkey2 FROM ' . tablename('superdesk_shop_coupon') . ' WHERE id=:id and uniacid=:uniacid limit 1 ', array(':id' => $id, ':uniacid' => $_W['uniacid']));
                ...
                pdo_update('superdesk_shop_coupon', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
                ...
                pdo_insert('superdesk_shop_coupon', $data);
                $id = pdo_insertid();
                plog('sale.coupon.add', '添加优惠券 ID: ' . $id . '  <br/>优惠券名称: ' . $data['couponname']);
            }
            $key  = 'superdesk_shopv2:com:coupon:' . $id;
            $rule = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name  limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'superdesk_shopv2', ':name' => $key));
            if (!empty($data['pwdkey2'])) {
                if (empty($rule)) {
                    $rule_data = array('uniacid' => $_W['uniacid'], 'name' => $key, 'module' => 'superdesk_shopv2', 'displayorder' => 0, 'status' => $data['pwdopen']);
                    pdo_insert('rule', $rule_data);
                    $rid          = pdo_insertid();
                    $keyword_data = array('uniacid' => $_W['uniacid'], 'rid' => $rid, 'module' => 'superdesk_shopv2', 'content' => $data['pwdkey2'], 'type' => 1, 'displayorder' => 0, 'status' => $data['pwdopen']);
        ...
        $item = pdo_fetch('SELECT * FROM ' . tablename('superdesk_shop_coupon') . ' WHERE id =:id and uniacid=:uniacid and merchid=0 limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $id));
        if (empty($item)) {
            $starttime = time();
            ...
            if (($item['limitgoodtype'] == 1) || ($item['limitgoodtype'] == 2)) {
                if ($item['limitgoodids']) {
                    $goods = pdo_fetchall('SELECT id,title,thumb FROM ' . tablename('superdesk_shop_goods') . ' WHERE uniacid = :uniacid and id in (' . $item['limitgoodids'] . ') ', array(':uniacid' => $_W['uniacid']));
                }
            }
    ...
    public function delete()
    {
        ...
        $items = pdo_fetchall('SELECT id,couponname FROM ' . tablename('superdesk_shop_coupon') . ' WHERE id in( ' . $id . ' ) and merchid=0 AND uniacid=' . $_W['uniacid']);
        foreach ($items as $item) {
            pdo_delete('superdesk_shop_coupon', array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
            pdo_delete('superdesk_shop_coupon_data', array('couponid' => $item['id'], 'uniacid' => $_W['uniacid']));// TODO 标志 楼宇之窗 openid shop_coupon_data 不处理
    ...
    public function displayorder()
    {
        ...
        $items        = pdo_fetchall('SELECT id,couponname FROM ' . tablename('superdesk_shop_coupon') . ' WHERE id in( ' . $id . ' ) and merchid=0 AND uniacid=' . $_W['uniacid']);
        foreach ($items as $item) {
            pdo_update('superdesk_shop_coupon', array('displayorder' => $displayorder), array('id' => $id));
    ...
    public function query()
    {
        ...
        $time = time();
        $ds   = pdo_fetchall('SELECT * FROM ' . tablename('superdesk_shop_coupon') . '  WHERE 1 ' . $condition . ' ORDER BY id asc', $params);
        foreach ($ds as &$d) {
    ...
    public function querygoods()
    {
        ...
        $ds = pdo_fetchall('SELECT id,title,thumb FROM ' . tablename('superdesk_shop_goods') . ' WHERE 1 ' . $condition . ' order by createtime desc', $params);
    ...
    public function send()
    {
        ...
        if ($_W['ispost']) {
            ...
            if ($class1 == 1) {
                ...
                $ids = $_GPC['send_id'];
                $plog    = '发放优惠券 ID: ' . $couponid . ' 方式: 指定 OPENID 人数: ' . count($ids);
                ...
                if (!empty($_GPC['send_level'])) {
                    $levelname = pdo_fetchcolumn('select levelname from ' . tablename('superdesk_shop_member_level') . ' where id=:id limit 1', array(':id' => $_GPC['send_level']));
                } else {
                    $levelname = '全部';
                }
                ...
                if (!empty($_GPC['send_group'])) {
                    $groupname = pdo_fetchcolumn('select groupname from ' . tablename('superdesk_shop_member_group') . ' where id=:id limit 1', array(':id' => $_GPC['send_group']));
                } else {
                    $groupname = '全部分组';
                }
                ...
                if ($_GPC['send_agentlevel'] != '') {
                    $levelname = pdo_fetchcolumn('select levelname from ' . tablename('superdesk_shop_commission_level') . ' where id=:id limit 1', array(':id' => $_GPC['send_agentlevel']));
                } else {
                    $levelname = '全部';
                }
                ...
                $plog = '发放优惠券 ID: ' . $couponid . '  方式: 营销商-' . $levelname . ' 人数: ' . count($members);
                ...
                if ($_GPC['send_partnerlevels'] != '') {
                    $levelname = pdo_fetchcolumn('select levelname from ' . tablename('superdesk_shop_globonus_level') . ' where id=:id limit 1', array(':id' => $_GPC['send_partnerlevels']));
                } else {
                    $levelname = '全部';
                }
                ...
                $members = pdo_fetchall(
                    'SELECT id ' .
                    ' FROM ' . tablename('superdesk_shop_member') . // TODO 标志 楼宇之窗 openid shop_member 不处理
                    ' WHERE uniacid = \'' . $_W['uniacid'] . '\' and isaagent=1 and aagentstatus=1 ' .
                    $where,
                    array(),
                    'id'
                );
                ...
                if ($_GPC['send_aagentlevels'] != '') {
                    $levelname = pdo_fetchcolumn('select levelname from ' . tablename('superdesk_shop_abonus_level') . ' where id=:id limit 1', array(':id' => $_GPC['send_aagentlevels']));
                } else {
                    $levelname = '全部';
                }
...
//            }
            ...
            pdo_update('superdesk_shop_coupon', $upgrade, array('id' => $coupon['id']));
            ...
            foreach ($members as $m) {
                ...
                while ($i <= $send_total) {
                    ...
                    pdo_insert('superdesk_shop_coupon_log', $log);// TODO 标志 楼宇之窗 openid superdesk_shop_coupon_log 已处理
        ...
        $list    = pdo_fetchall('SELECT * FROM ' . tablename('superdesk_shop_member_level') . ' WHERE uniacid = \'' . $_W['uniacid'] . '\' ORDER BY level asc');
        $list2   = pdo_fetchall('SELECT * FROM ' . tablename('superdesk_shop_member_group') . ' WHERE uniacid = \'' . $_W['uniacid'] . '\' ORDER BY id asc');
        $coupons = pdo_fetchall('SELECT * FROM ' . tablename('superdesk_shop_coupon') . ' WHERE uniacid = \'' . $_W['uniacid'] . '\' ORDER BY id asc');
