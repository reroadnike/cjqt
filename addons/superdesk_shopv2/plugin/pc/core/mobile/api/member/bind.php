...
class Bind_SuperdeskShopV2Page extends PcMobileLoginPage
{
    protected $member;
    ...
    public function main()
    {
        ...
        if ($_W['ispost']) {
            ...
            $member2 = pdo_fetch(
                ' select * ' .
                ' from ' . tablename('superdesk_shop_member') .// TODO 标志 楼宇之窗 openid superdesk_shop_member 待处理
                ' where mobile=:mobile ' .
                '       and uniacid=:uniacid ' .
                '       and mobileverify=1 ' .
                ' limit 1',
            ...
            if (!(empty($bind))) {
                ...
                if (strexists($member2['openid'], 'sns_wx_')
                    || strexists($member2['openid'], 'sns_qq_')
                    || strexists($member2['openid'], 'wap_user_')
                ) {
            ...
            if (!(empty($member2))) {
                if (strexists($member2['openid'], 'sns_wx_')
                    ...
                    pdo_update("superdesk_shop_commission_apply", array("mid" => $member['id']), array('uniacid' => $_W['uniacid'], 'mid' => $member2['id']));

                    pdo_update('superdesk_shop_order', // TODO 标志 楼宇之窗 openid superdesk_shop_order 不处理
                        array(
                            'agentid' => $member['id']
                        ),
                        array(
                    ...
                    pdo_update("superdesk_shop_member", array("agentid" => $member['id']), array('agentid' => $member2['id']));

                    if (0 < $member2['credit1']) {
                        m('member')->setCredit($member['openid'], $member['core_user'],
                            'credit1', abs($member2['credit1']), 'WAP会员数据合并增加积分 +' . $member2['credit1']);
                    }

                    if (0 < $member2['credit2']) {
                        m('member')->setCredit($member['openid'], $member['core_user'],
                            'credit2', abs($member2['credit2']), 'WAP会员数据合并增加余额 +' . $member2['credit2']);
                    }
                    ...
                    pdo_update("superdesk_shop_member",
                        array("pwd" => '', "salt" => '', "mobileverify" => 0),
                        array("id" => $member2['id'], 'uniacid' => $_W['uniacid'])
                    );
        ...
        $ice_menu_array = array(
            array('menu_key' => 'info', 'menu_name' => '账户信息', 'menu_url' => mobileUrl('pc.member.info')),
            array('menu_key' => 'index', 'menu_name' => '绑定手机', 'menu_url' => mobileUrl('pc.member.bind'))
        );
