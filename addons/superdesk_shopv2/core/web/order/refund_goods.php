...
class Refund_goods_SuperdeskShopV2Page extends WebPage
{
    private $_orderModel;
    ...
    public function submit()
    {
        ...
        if ($_W['ispost']) {
            ...
            } else if ($refundstatus == 3) { //* 3 通过申请(需客户寄回商品)
            ...
            } else if ($refundstatus == 10) { //* 10 关闭申请(换货完成)
...
//                $order_goods_data['status']      = 3;
            ...
            } else if ($refundstatus == 1) { // 同意退款(无需客户发货直接退款)
...
//                paytype	支付类型 0为未支付 1为余额支付 2在线支付 3 企业月结 11 后台付款 21 微信支付 22 支付宝支付 23 银联支付

                if ($order['paytype'] == 1) { // 1为余额支付 如有余额抵扣： 会返回金额到商城用户余额
                ...
                } else { // 其他支付方式： 会返回到微信钱包(需商户平台余额充足)
                ...
                if (!empty($refundtype)) {
                    ...
                    //暂时屏蔽,不管余额抵扣
//                    if ($realprice < 0) {
//                        $order['deductcredit2'] = $refund['applyprice'];
//                    }
//
//
...
//                m('order')->setGiveBalance($order['id'], 2);
                ...
                if ($refund['orderprice'] == $refund['applyprice']) {
                    if (com('coupon') && !empty($order['couponid'])) {
                        com('coupon')->returnConsumeCoupon($order['id']);
                    }
...
//                    'select count(1) from ' . tablename('superdesk_shop_order_goods') . // TODO 标志 楼宇之窗 openid shop_order_goods 不处理
//                    ' where refundtime = 0 and orderid = :orderid',
...
//                }
    ...
    public function orderData($status, $st)
    {
        ...
        $condition = ' og.uniacid = :uniacid and o.deleted=0 and o.isparent=0 ';
        $uniacid   = $_W['uniacid'];
        $params    = $params1 = array(':uniacid' => $uniacid);
...
//                echo '<br/>';
        ...
        $sql =
            ...
            $sqlcondition .// TODO 标志 楼宇之窗 openid shop_order 待处理
            ' where ' .
            $condition .
            ' ' .
            $statuscondition;
...
//    * 5 等待客户收货
        ...
        if ($_GPC['export'] == 1) {
            ...
            m('excel')->export($list, array('title' => '订单数据-' . date('Y-m-d-H-i', time()), 'columns' => $columns));
        }


        $t = pdo_fetch(
            'SELECT ' .
            '       COUNT(*) as count, ifnull(sum(og.realprice),0) as sumprice ' .
            ' FROM ' . tablename('superdesk_shop_order_goods') . ' og ' .// TODO 标志 楼宇之窗 openid shop_order_goods 已处理
            ' left join ' . tablename('superdesk_shop_goods') . ' g on g.id=og.goodsid ' .
            ' left join ' . tablename('superdesk_shop_goods_option') . ' op on og.optionid = op.id ' .
            ' left join ' . tablename('superdesk_shop_order') . ' o on o.id=og.orderid ' .// TODO 标志 楼宇之窗 openid shop_order 已处理
