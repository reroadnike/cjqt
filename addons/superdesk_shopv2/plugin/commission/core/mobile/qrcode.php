...
class Qrcode_SuperdeskShopV2Page extends CommissionMobileLoginPage
{
...
//        $goodsid        = 210;
        ...
        if (!empty($goodsid)) {
            ...
            $goods = pdo_fetch(
                ...
                array(':uniacid' => $_W['uniacid'], ':id' => $goodsid));
...
//        var_dump($set['qrcode']);
        if ($_W['ispost']) {
            ...
            if ($share_goods) {
                if ($p) {
                    $img = $p->createCommissionPoster($_W['openid'], $goods['id'], 0, $_W['core_user']);
                }
                ...
                if ($p) {
                    $img = $p->createCommissionPoster($_W['openid'], 0, 4, $_W['core_user']);
                }

            } else if ($p) {

                $img = $p->createCommissionPoster($_W['openid'], 0, 0, $_W['core_user']);
...
//        var_dump($set['qrcode_content']);
    ...
    //获取个人推广二维码
    public function getSpreadQr()
    {
        global $_W, $_GET;
        $member = m('member')->getMember($_W['openid'], $_W['core_user']);
        $qrimg  = m('qrcode')->createQrcode(mobileUrl('commission', array('mid' => $member['id']), true), 8);

        include $this->template('commission/personal_qrcode');
    }
}
