...
class Notice_SuperdeskShopV2Page extends PluginWebPage
{
    public function main()
    {
        ...
        if ($_W['ispost']) {
            ...
            m('common')->updatePluginset(array(
                'enterprise' => array('tm' => $data)
            ));
            plog('enterprise.notice.edit', '修改通知设置');
            show_json(1);
        }


        $data   = m('common')->getPluginset('enterprise');
        $salers = array();
        ...
        if (!empty($data['tm']['applyopenid'])) {
            ...
            $applysalers = pdo_fetchall('select id,nickname,avatar,openid from ' . tablename('superdesk_shop_member') . ' where openid in (' . implode(',', $applyopenids) . ') and uniacid=' . $_W['uniacid']);
        }
