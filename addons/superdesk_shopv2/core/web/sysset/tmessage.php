...
class Tmessage_SuperdeskShopV2Page extends WebPage
{
    public function main()
    {
        ...
        $list  = pdo_fetchall('SELECT * FROM ' . tablename('superdesk_shop_member_message_template') . ' WHERE 1 ' . $condition . '  ORDER BY id asc limit ' . (($pindex - 1) * $psize) . ',' . $psize, $params);
        $total = pdo_fetchcolumn('SELECT count(*) FROM ' . tablename('superdesk_shop_member_message_template') . ' WHERE 1 ' . $condition, $params);
    ...
    protected function post()
    {
        ...
        if ($_W['ispost']) {
            ...
            if (empty($id)) {
                ...
                pdo_insert('superdesk_shop_member_message_template', $insert);
                $id = pdo_insertid();

                plog('sysset.tmessage.delete', '添加群发模板 ID: ' . $id . ' 标题: ' . $insert['title'] . ' ');
            } else {

                pdo_update('superdesk_shop_member_message_template', $insert, array('id' => $id));
    ...
    public function delete()
    {
        ...
        foreach ($items as $item) {
            ...
            pdo_delete('superdesk_shop_member_message_template', array('id' => $id, 'uniacid' => $_W['uniacid']));
