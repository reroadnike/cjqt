...
class Temp_SuperdeskShopV2Page extends PluginWebPage
{
    public function main()
    {
        ...
        $items = pdo_fetchall('SELECT * FROM ' . tablename('superdesk_shop_diyform_type') . ' WHERE uniacid=:uniacid and title like :name order by id desc limit ' . (($pindex - 1) * $psize) . ',' . $psize, array(':name' => '%' . $kw . '%', ':uniacid' => $_W['uniacid']));
        $total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('superdesk_shop_diyform_type') . ' WHERE uniacid=:uniacid and title like :name order by id desc ', array(':uniacid' => $_W['uniacid'], ':name' => '%' . $kw . '%'));
    ...
    protected function post()
    {
        ...
        if ($_W['ispost']) {
            ...
            if (empty($id)) {
                ...
                pdo_update('superdesk_shop_diyform_type', $insert, array('id' => $id));
    ...
    public function delete()
    {
        ...
        foreach ($types as $type) {
            ...
            if ($set['commission_diyform_open'] && ($set['commission_diyform'] == $id)) {
                $err .= '营销商申请资料正在使用该表单，请关闭后再进行删除。<br/>';
            }
            ...
            if (!empty($err)) {
                ...
                pdo_delete('superdesk_shop_diyform_type', array('id' => $id));
                pdo_delete('superdesk_shop_diyform_data', array('typeid' => $id));// TODO 标志 楼宇之窗 openid shop_diyform_data 不处理
        ...
        if (!empty($errmsg)) {
            show_json(0, array(
                'message' => $errmsg,
                'url'     => webUrl('diyform/temp')
            ));
        }

        show_json(1, array(
            'url' => webUrl('virtual/temp')
        ));
    }
