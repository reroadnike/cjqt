...
class Verify_SuperdeskShopV2Page extends PluginMobileLoginPage
{
    private $_orderModel;
    ...
    public function select()
    {
        ...
        $order = pdo_fetch(
            'select id,verifyinfo from ' . tablename('superdesk_shop_order') . // TODO 标志 楼宇之窗 openid shop_order 不处理
    ...
    public function check()
    {
        ...
        $uniacid = $_W['uniacid'];
        $orderid = intval($_GPC['id']);
        $order   = pdo_fetch('select id,status,isverify,verifytype,verifynum from ' . tablename('superdesk_shop_groups_order') . ' where id=:id and uniacid=:uniacid and openid=:openid limit 1', array(':id' => $orderid, ':uniacid' => $uniacid, ':openid' => $openid));
        if (empty($order)) {
            show_json(0);
        }
