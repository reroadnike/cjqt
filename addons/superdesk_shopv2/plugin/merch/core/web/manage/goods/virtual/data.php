...
class Data_SuperdeskShopV2Page extends MerchWebPage
{
    ...
    public function main()
    {
        ...
        $items = pdo_fetchall(
            ' SELECT d.*,m.avatar,m.nickname,m.realname,m.mobile  ' .
            ' FROM ' . tablename('superdesk_shop_virtual_data') . ' d ' .
            ' left join ' . tablename('superdesk_shop_member') . ' m on d.openid<>\'\' and m.openid = d.openid and m.core_user = d.core_user and m.uniacid= d.uniacid ' .// TODO 标志 楼宇之窗 openid superdesk_shop_member 已处理
            ' left join ' . tablename('superdesk_shop_order') . ' o on d.orderid<>0 and  o.id = d.orderid ' .// TODO 标志 楼宇之窗 openid shop_order 已处理
            ' where  1 ' . $condition .
            ...
            if (!empty($typeid)) {
                ...
                if (!empty($item['fields'])) {
                    ...
                    foreach ($tpids as $index => $id) {
                        ...
                        if (empty($datas)) {
                            ...
                            if (empty($keydata)) {
                                ...
                                pdo_insert('superdesk_shop_virtual_data', $insert);
                                pdo_update('superdesk_shop_virtual_type', 'alldata=alldata+1', array('id' => $item['id']));
                            } else {

                                pdo_update('superdesk_shop_virtual_data', $insert, array('id' => $keydata['id']));
                            }
                            ...
                            if (empty($keydata)) {
                                pdo_update('superdesk_shop_virtual_data', $insert, array('id' => $datas['id']));
                            } else {
                    ...
                    if (!empty($noinsert)) {
                        ...
                        show_json(1, array(
                            'message' => '部分数据保存成功！' . $tip,
                            'url'     => merchUrl('virtual/data', array('typeid' => $typeid))
                        ));

                    } else {

                        show_json(1, array(
                            'url' => merchUrl('goods/virtual/data', array('typeid' => $typeid))
                        ));
                    }
                }
    ...
    public function delete()
    {
        ...
        foreach ($types as $type) {
            ...
            pdo_delete('superdesk_shop_virtual_data', array('id' => $type['id']));
            pdo_update('superdesk_shop_virtual_type', 'alldata=alldata-1', array('id' => $type['id']));
    ...
    public function export()
    {
        ...
        $list = pdo_fetchall(
            ' SELECT d.*,o.carrier,m.avatar,m.nickname ' .
            ' FROM ' . tablename('superdesk_shop_virtual_data') . ' d ' .
            ' left join ' . tablename('superdesk_shop_member') . ' m on m.openid = d.openid on m.core_user = d.core_user and m.uniacid = d.uniacid ' .// TODO 标志 楼宇之窗 openid superdesk_shop_member 已处理
            ' left join ' . tablename('superdesk_shop_order') . ' o on o.id = d.orderid ' . // TODO 标志 楼宇之窗 openid shop_order 已处理
            ' where  1 ' .
            $condition .
    ...
    public function import()
    {
        ...
        foreach ($datas as $d) {
            ...
            if (empty($olddata)) {
                ...
                pdo_update('superdesk_shop_virtual_data', $d, array('id' => $olddata['id']));
            } else {
