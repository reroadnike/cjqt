...
class Comment_SuperdeskShopV2Page extends WebPage
{
    public function main()
    {
        ...
        //商户搜索
        if(!empty($_GPC['merch'])) {
            $sqlcondition .= ' LEFT JOIN ' . tablename('superdesk_shop_merch_user') . ' mu ON mu.`id` = c.`merchid` ';
            $condition .= 'AND mu.`id` = ' . $_GPC['merch'];
        }
        //关键词搜索
...
//        pdo_debug();
  ...
  /*      foreach ($items as $item) {
    ...
    public function view() {
        global $_GPC;
        $id = $_GPC['id'];
        $comment = pdo_get('superdesk_shop_order_comment',array('orderid'=>$id));
