...
if (!class_exists('ArticleModel')) {
    ...
    {
        public function doShare($aid/* 文章ID */, $shareid/* 分享者Mid */, $myid/* 点击者Mid(我)*/)
        {
            global $_W;
            global $_GPC;
            ...
            $profile = m('member')->getMemberById($shareid);
            $myinfo  = m('member')->getMemberById($myid);
            ...
            $my_click = pdo_fetchcolumn(
                'SELECT COUNT(*) ' .
                '   FROM ' . tablename('superdesk_shop_article_share') .
                ' WHERE ' .
                '       aid=:aid ' .
                '       and click_user=:click_user ' .
                '       and uniacid=:uniacid ',
                array(
                    ':aid'        => $article['id'],
                    ':click_user' => $myid,
            ...
            if ($article['article_rule_allnum'] <= $all_click) {
                ...
                $day_click = pdo_fetchcolumn(
                    'SELECT COUNT(*) ' .
                    ' FROM ' . tablename('superdesk_shop_article_share') .
                    ' WHERE ' .
                    '       aid=:aid ' .
                    '       and share_user=:share_user ' .
                    '       and click_date>:day_start ' .
                    '       and click_date<:day_end ' .
                    '       and uniacid=:uniacid ',
                    array(
                        ':aid'        => $article['id'],
                        ':share_user' => $shareid,
            ...
            $toto = pdo_fetchcolumn(
                'SELECT COUNT(*) ' .
                ' FROM ' . tablename('superdesk_shop_article_share') .
                ' WHERE ' .
                '       aid=:aid ' .
                '       and share_user=:click_user ' .
                '       and click_user=:share_user ' .
                '       and uniacid=:uniacid ',
                array(
                    ':aid'        => $article['id'],
                    ':share_user' => $shareid,
            ...
            pdo_insert('superdesk_shop_article_share', $insert);

            if (0 < $givecredit) {
                m('member')->setCredit($profile['openid'], $profile['core_user'],
                    'credit1', $givecredit, array(0, $shopset['name'] . ' 文章营销奖励积分'));
            }


            if (0 < $givemoney) {
                m('member')->setCredit($profile['openid'], $profile['core_user'],
                    'credit2', $givemoney, array(0, $shopset['name'] . ' 文章营销奖励余额'));
            }
        ...
        public function href_replace($lnk)
        {
            ...
            if (strexists($lnk, 'superdesk_shop') && !strexists($lnk, '&mid')) {
