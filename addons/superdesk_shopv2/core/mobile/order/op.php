...
class Op_SuperdeskShopV2Page extends MobileLoginPage
{
    private $_orderModel;
...
//        data:{"id":71,"remark":"我不想买了"}
        ...
        $order = pdo_fetch(
            ' select ' .
            '       id,ordersn,openid,core_user,status,deductcredit,deductcredit2,deductprice,couponid,isparent ' .
...
//            die(json_encode($result,JSON_PRETTY_PRINT|JSON_UNESCAPED_UNICODE));
    ...
    public function finish()
    {
        ...
        $order = pdo_fetch(
            ' select id,status,openid,core_user,couponid,refundstate,refundid,merchid ' .
            ' from ' . tablename('superdesk_shop_order') .// TODO 标志 楼宇之窗 openid shop_order 已处理
