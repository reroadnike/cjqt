...
class SignModel extends PluginModel
{
    public function getSet()
    {
        ...
        $set = pdo_fetch(
            'select * ' .
            ' from ' . tablename('superdesk_shop_sign_set') .
            ' where ' .
            '       uniacid=:uniacid ' .
            ' limit 1 ',
            array(
    ...
    public function getCalendar($year = NULL, $month = NULL, $week = true)
    {
        ...
        $records = pdo_fetchall(
            ...
            array(
                ':uniacid'   => $_W['uniacid'],
                ':openid'    => $_W['openid'],
                ':starttime' => $date['firsttime'],
                ':endtime'   => $date['lasttime']
            )
        );
    ...
    public function getAdvAward()
    {
        ...
        $records = pdo_fetchall('select * from ' . tablename('superdesk_shop_sign_records') . ' where openid=:openid and uniacid=:uniacid ' . $condition . ' order by `time` asc ', array(':uniacid' => $_W['uniacid'], ':openid' => $_W['openid']));
        if (!empty($records)) {
            foreach ($records as $item) {
    ...
    public function updateSign($signinfo)
    {
        ...
        $info = pdo_fetch('select id  from ' . tablename('superdesk_shop_sign_user') . ' where openid=:openid and uniacid=:uniacid limit 1 ', array(':openid' => $_W['openid'], ':uniacid' => $_W['uniacid']));
        $data = array('openid' => $_W['openid'], 'order' => $signinfo['order'], 'orderday' => $signinfo['orderday'], 'sum' => $signinfo['sum'], 'signdate' => date('Y-m'));
        if (empty($info)) {
            $data['uniacid'] = $_W['uniacid'];
            pdo_insert('superdesk_shop_sign_user', $data);
            return;
        }
        pdo_update('superdesk_shop_sign_user', $data, array('id' => $info['id']));
    }
}
