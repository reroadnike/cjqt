...
if ($do == 'delete') {
    ...
    if (empty($_id)) {
        message('非法 input ', referer(), 'error');
    }

    $count = pdo_fetchcolumn(
        ' SELECT COUNT(*) ' .
        ' FROM ' . tablename('activity_clerks') .
        ' WHERE uniacid = :uniacid AND storeid = :id',
        array(
            ':id'      => $_id,
            ':uniacid' => $_W['uniacid']
        )
    ...
    pdo_delete('activity_stores', array('id' => $_id, 'uniacid' => $_W['uniacid']));
