...
class sll_unitModel extends SuperdeskShopMemberBaseOutDbModel
{

    public $table_name = "sll_unit";

    public $table_column_all = "unit_id,unit_name,status";
...
//            $params['createtime'] = strtotime('now');
    ...
    public function saveOrUpdateByJdVop($params, $sku)
    {
        ...
        if (!$_is_exist) {
            ...
            // TODO
            $insert_data['unit_id']   = '';//  | NO | int(11) |
            $insert_data['unit_name'] = '';//  | YES | varchar(100) |
            $insert_data['status']    = '';// 0：废弃 1：使用  | YES | int(10) |
...
//        );
        ...
        $list  = $this->pdo_fetchall("SELECT * FROM " . $this->tablename($this->table_name) . $where_sql . " ORDER BY unit_id ASC LIMIT " . ($page - 1) * $page_size . ',' . $page_size, $params);
