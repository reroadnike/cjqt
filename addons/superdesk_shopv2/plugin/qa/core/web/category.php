...
class Category_SuperdeskShopV2Page extends PluginWebPage
{
    ...
    protected function post()
    {
        ...
        if ($_W['ispost']) {
            ...
            if (!empty($id)) {
                pdo_update('superdesk_shop_qa_category', $data, array('id' => $id));
                plog('qa.category.edit', '修改积分商城分类 ID: ' . $id);
            } else {
                pdo_insert('superdesk_shop_qa_category', $data);
                $id = pdo_insertid();
                plog('qa.category.add', '添加积分商城分类 ID: ' . $id);
            }

            show_json(1, array(
                'url' => webUrl('qa/category', array('op' => 'display'))
            ));
        }
    ...
    public function delete()
    {
        ...
        pdo_delete('superdesk_shop_qa_category', array('id' => $id));
    ...
    public function displayorder()
    {
        ...
        if (!empty($item)) {
            pdo_update('superdesk_shop_qa_category', array('displayorder' => $displayorder), array('id' => $id));
    ...
    public function enabled()
    {
        ...
        foreach ($items as $item) {
            pdo_update('superdesk_shop_qa_category', array('enabled' => intval($_GPC['enabled'])), array('id' => $item['id']));
    ...
    public function isrecommand()
    {
        ...
        foreach ($items as $item) {
            pdo_update('superdesk_shop_qa_category', array('isrecommand' => intval($_GPC['isrecommand'])), array('id' => $item['id']));
