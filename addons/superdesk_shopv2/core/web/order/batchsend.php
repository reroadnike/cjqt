...
class Batchsend_SuperdeskShopV2Page extends WebPage
{
	private $_orderModel;
	...
	public function main()
	{
		...
		if ($_W['ispost']) {
			...
			foreach ($rows as $rownum => $col ) {
				...
				$sql =
					' select id,status,refundid from ' . tablename('superdesk_shop_order') . // TODO 标志 楼宇之窗 openid shop_order 不处理
					' where ordersn=:ordersn and uniacid=:uniacid and isparent=0 and merchid=:merchid';
				...
				if (!empty($order)) {
					...
					if (!empty($order['refundid'])) {
						$refund = pdo_fetch('select id from ' . tablename('superdesk_shop_order_refund') . ' where id=:id limit 1', array(':id' => $order['refundid']));

						if (!empty($refund)) {
							$refund_flag = 1;
							pdo_update('superdesk_shop_order_refund', array('status' => -1, 'endtime' => $time), array('id' => $order['refundid']));
						}
