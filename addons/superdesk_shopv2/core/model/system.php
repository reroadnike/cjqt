...
class System_SuperdeskShopV2Model
{
    private $merch = false;
            ...
            foreach ($val['items'] as $child) {
                ...
                if (empty($child['items'])) {
                    ...
                    $menu_item_child = array(
                        'title' => $child['title'],
                        'route' => empty($child['route']) ? $child_route_default : $child['route']
                    );
        ...
        if ($this->merch) {
            ...
            if ($routes[0] == 'system') {
                ...
                if ($_W['role'] == 'founder') {
                    ...
                    $return_arr['menu_items'][] = 'line';
                    $return_arr['menu_items'][] = array('text' => '系统更新', 'href' => './index.php?c=account&a=display&', 'blank' => true);
                }
    ...
    public function init()
    {
        ...
        if (!$this->merch) {
            ...
            if ($this->cv('commission.apply.view1')) {
                ...
                $arr['commission1'] = pdo_fetchcolumn(
                    'select count(1) ' .
            ...
            if ($this->cv('commission.apply.view2')) {
                $arr['commission2'] = pdo_fetchcolumn(
                    'select count(1) ' .
                    ' from' . tablename('superdesk_shop_commission_apply') . ' a ' .
                    '       left join ' . tablename('superdesk_shop_member') . ' m on m.uid = a.mid' .// TODO 标志 楼宇之窗 openid shop_member 不处理
                    '       left join ' . tablename('superdesk_shop_commission_level') . ' l on l.id = m.agentlevel' .
            ...
            if ($this->cv('shop.comment')) {
                $arr['comment'] = pdo_fetchcolumn(
                    ...
                    ' LEFT JOIN ' . tablename('superdesk_shop_goods') . ' g ON g.id=c.goodsid ' .
                    ' WHERE ' .
                    '       (c.checked=1 OR c.replychecked=1) ' .
                    '       AND c.deleted=0 ' .
        ...
        if (!empty($arr['funbar']['open'])) {
            ...
            $funbardata = pdo_fetch(
                'select * ' .
                ' from ' . tablename('superdesk_shop_funbar') .
                ' where ' .
                '       uid=:uid ' .
                '       and uniacid=:uniacid ' .
                ' limit 1',
    ...
    protected function getOrderTotal($status = 0)
    {
        ...
        if ($status == 1) {
            ...
            $condition =
                'uniacid = :uniacid and isparent=0 and ismr=0 and ( status=1 or ( status=0 and paytype=3) ) and deleted=0';
    ...
    public function set_version($type = 0)
    {
        ...
        $set = pdo_fetch(
            'SELECT * ' .
            'FROM ' . tablename('superdesk_shop_version') .
            ' WHERE uid=:uid AND `type`=:type limit 1',
            array(
                ':type' => $type,
                ':uid'  => $uid
            )
