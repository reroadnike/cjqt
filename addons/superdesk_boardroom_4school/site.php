...
/**
 * Class Superdesk_boardroom_4schoolModuleSite
 */
class Superdesk_boardroom_4schoolModuleSite extends WeModuleSite
{
...
//        $_W['openid'] = "oX8KYwkxwNW6qzHF4cF-tGxYTcPg";
    ...
    //判断是否是操作员管理
    public function superdesk_core_user(){
        global $_W;

        include_once(MODULE_ROOT . '/../superdesk_core/model/users.class.php');
        $_usersModel = new usersModel();

        $user = $_usersModel->getOneByColumn(array('uid'=>$_W['uid']));
...
//        }
     ...
     * @return bool|string
     */
    public function superdesk_core_user_mobile($userMobile = '')
    {
        global $_GPC, $_W;
        ...
        $superdesk_user_info = false;

        if(empty($userMobile)){

            $key_openid2mobile = 'superdesk_core_' . 'openid2mobile' . ':' . $_W['uniacid'] . ":" . $_W['openid'];
            $userMobile = $_redis->get($key_openid2mobile);

            $key = 'superdesk_core_' . 'user_info' . ':' . $_W['uniacid'] . ":" . $userMobile;


            try {
                if ($_redis->isExists($key) == 1) {
                    $superdesk_user_info = $_redis->get($key);
                } else {
...
//            }
        ...
        }else{ // 当$userMobile不为空时，必定去取

            $key_openid2mobile = 'superdesk_core_' . 'openid2mobile' . ':' . $_W['uniacid'] . ":" . $_W['openid'];
            $_redis->set($key_openid2mobile, $userMobile);

            include_once(MODULE_ROOT . '/../superdesk_core/sdk/superdesk_core_sdk.php');
            $superdesk_core_sdk = new SuperDeskCoreSDK();

            $response = $superdesk_core_sdk->userInfo($userMobile);
            $response = json_decode($response,true);
//            print_r($response);
//            $response = $superdesk_core_sdk->objectToArray($response);
            ...
            //    "resultDesc": "success",
            //    "returnDTO": {
            //        "organizationCode": "TJ-TJSWDX",
            //        "organizationName": "天津市委党校",
            //        "status": "1",
            //        "userMobile": "13422832499",
            //        "userName": "雨",
            //        "userPhotoUrl": "http://wx.qlogo.cn/mmopen/WEca0hLfDBJ8YG5sPkdfLaPgbMHGubR1dKZwiaw2bJsvvy3Jt8ATZMyPLyVWVkHmYDlsOb0cNOurlqQZITTAeHHMvOlfJEVwy/0",
            //        "virtualCode": "TJSWDX",
            //        "virtualName": "天津市委党校项目部"
            //    }
            //}
            ...
            $key = 'superdesk_core_' . 'user_info' . ':' . $_W['uniacid'] . ":" . $userMobile;
            if (isset($response['resultCode']) && $response['resultCode'] == '200') {

//                var_dump($response['returnDTO']);
                $superdesk_user_info = $response['returnDTO'];
//                $_redis->setex($key, 3600, json_encode($superdesk_user_info, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT));


                $superdesk_user_info = json_encode($superdesk_user_info);
                $_redis->set($key, $superdesk_user_info);
                //microtime_format('Y-m-d H:i:s.x',microtime())



            } else {
                die("API返回:".$response['resultDesc']);
            }
        }

        return json_decode($superdesk_user_info,true);
    }
    ...
    public function getMenus()
    {
        ...
        <entry title="概览" do="overview" state="" direct="false"/>
        ...
        <entry title="楼宇管理" do="building_structures" state="" direct="false"/>
        <entry title="单元属性" do="building_attribute" state="" direct="false"/>
        ...
        */
        return array(
            ...
            array('title' => '概览', 'icon' => 'fa fa-puzzle-piece', 'url' => $this->createWebUrl('overview')),
            ...
            array('title' => '楼宇管理', 'icon' => 'fa fa-puzzle-piece', 'url' => $this->createWebUrl('building_structures')),
            array('title' => '单元属性', 'icon' => 'fa fa-puzzle-piece', 'url' => $this->createWebUrl('building_attribute')),
...
//        $table['day_time'] = $day_time;
    ...
    public function init_boardroom_situation($boardroom_id,$Ymd){
        ...
        while ($time_am[$i_am] < $_init_am_end) {
            ...
            $temp_am['lable_start']  = $this->dataEchoH_i($time_am[$i_am]);
            $temp_am['lable_end']  = $this->dataEchoH_i($time_am[$i_am+1]);
            $temp_am['out_trade_no'] = "";
            $result_am[] = $temp_am;
        ...
        while ($time_pm[$i_pm] < $_init_pm_end) {
            ...
            $temp_pm['lable_start']  = $this->dataEchoH_i($time_pm[$i_pm]);
            $temp_pm['lable_end']  = $this->dataEchoH_i($time_pm[$i_pm+1]);
            $temp_pm['out_trade_no'] = "";
            $result_pm[] = $temp_pm;
    ...
    public function set_boardroom_situation($boardroom_id,$Ymd,$value){
        ...
        $key = 'superdesk_boardroom_4school_' . 'appointment' . '_' . $_W['uniacid'];
...
//        '-1取消，0待审，1待审已付款，3已审'
        ...
        if ($_SESSION['superdesk_boardroom_pay_result'] != $params['tid']) {
            ...
            $pay_status = pdo_get('superdesk_boardroom_4school_s_order', array('id' => $params['tid']));
...
//            var_dump($situation_select['stbs']);exit(0);
            foreach ($situation_select['stbs'] as $index_key => $_situation_val){
                ...
                foreach($_situation_val['select_time_bar'] as $index => &$_situation){
                    ...
                    if($_situation['index'] >= 0 && $_situation['index'] <= 23){
                        ...
                        $situation_target['am'][$_situation['index']]['out_trade_no'] = $out_trade_no;
                    }
                    // 24-47
                    if($_situation['index'] >= 24 && $_situation['index'] <= 47){
                        $situation_target['pm'][$_situation['index']-24]['is_use'] = 1;
                        $situation_target['pm'][$_situation['index']-24]['out_trade_no'] = $out_trade_no;
                    }
                }
            ...
            /******************************************************* 附加服务 *********************************************************/
            $goods = pdo_fetchall("SELECT `goodsid`, `total`, `optionid` FROM " . tablename('superdesk_boardroom_4school_s_order_goods') . " WHERE `out_trade_no` = :out_trade_no",
                array(
                    ':out_trade_no' => $params['tid']
                )
            );
            if (!empty($goods)) {
                ...
                foreach ($goods as $row) {
                    $goodsInfo = pdo_fetch("SELECT `total`, `totalcnf`, `sales` FROM " . tablename('superdesk_boardroom_4school_s_goods') . " WHERE `id` = :id", array(':id' => $row['goodsid']));
                    $goodsupdate = array();
                    ...
                    if ($pay_status != 1) {
                        pdo_update('superdesk_boardroom_4school_s_goods', $goodsupdate, array('id' => $row['goodsid']));
                    }

                    $optionInfo = pdo_fetch("SELECT `stock` FROM " . tablename('superdesk_boardroom_4school_s_goods_option') . " WHERE `id` = :id",
                        array(
                            ':id' => $row['optionid']
                        )
                    );
                    ...
                    if ($goodsInfo['totalcnf'] == '1' && !empty($optionInfo['stock'])) {
                        ...
                        if ($pay_status != 1) {
                            pdo_update('superdesk_boardroom_4school_s_goods_option', $options, array('id' => $row['optionid']));
                        }
                    }
                }
            }

            pdo_update('superdesk_boardroom_4school_s_order', $data, array('out_trade_no' => $params['tid'], 'uniacid' => $_W['uniacid']));
            /******************************************************* 附加服务 *********************************************************/
...
//            $this->setOrderCredit($params['tid']);
            ...
            if (!empty($this->module['config']['noticeemail']) // 发送邮件
                ...
                $order = pdo_fetch("SELECT `out_trade_no`, `price`, `paytype`, `from_user`, `address`, `createtime` FROM " . tablename('superdesk_boardroom_4school_s_order') . " WHERE id = '{$params['tid']}'");
                $ordergoods = pdo_fetchall("SELECT goodsid, total FROM " . tablename('superdesk_boardroom_4school_s_order_goods') . " WHERE orderid = '{$params['tid']}'", array(), 'goodsid');
                $goods = pdo_fetchall("SELECT id, title, thumb, marketprice, unit, total FROM " . tablename('superdesk_boardroom_4school_s_goods') . " WHERE id IN ('" . implode("','", array_keys($ordergoods)) . "')");
//				$address = pdo_fetch("SELECT * FROM " . tablename('mc_member_address') . " WHERE id = :id", array(':id' => $order['addressid']));
    ...
    public function setOrderStock($id = '', $minus = true) {
        $goods = pdo_fetchall("SELECT g.id, g.title, g.thumb, g.unit, g.marketprice,g.total as goodstotal,o.total,o.optionid,g.sales FROM " . tablename('superdesk_boardroom_4school_s_order_goods') . " o left join " . tablename('superdesk_boardroom_4school_s_goods') . " g on o.goodsid=g.id "
            . " WHERE o.orderid='{$id}'");
        foreach ($goods as $item) {
            if ($minus) {
                //属性
                if (!empty($item['optionid'])) {
                    pdo_query("update " . tablename('superdesk_boardroom_4school_s_goods_option') . " set stock=stock-:stock where id=:id", array(":stock" => $item['total'], ":id" => $item['optionid']));
                }
                $data = array();
    ...
    public function getCartTotal() {
        global $_W;
        $cartotal = pdo_fetchcolumn("select sum(total) from " . tablename('superdesk_boardroom_4school_s_cart') . " where uniacid = '{$_W['uniacid']}' and from_user='{$_W['openid']}'");
    ...
    public function setOrderCredit($orderid, $add = true) {
        global $_W;
        $order = pdo_fetch("SELECT * FROM " . tablename('superdesk_boardroom_4school_s_order') . " WHERE id = :id AND uniacid = :uniacid  limit 1", array(':id' => $orderid, ':uniacid' => $_W['uniacid']));
        if (empty($order)) {
            return false;
        }
        $sql = 'SELECT `goodsid`, `total` FROM ' . tablename('superdesk_boardroom_4school_s_order_goods') . ' WHERE `orderid` = :orderid';
        $orderGoods = pdo_fetchall($sql, array(':orderid' => $orderid));
        if (!empty($orderGoods)) {
            $credit = 0.00;
            $sql = 'SELECT `credit` FROM ' . tablename('superdesk_boardroom_4school_s_goods') . ' WHERE `id` = :id';
