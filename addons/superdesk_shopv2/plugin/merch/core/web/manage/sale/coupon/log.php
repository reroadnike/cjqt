...
class Log_SuperdeskShopV2Page extends MerchWebPage
{
    ...
    public function main()
    {
        ...
        if (!empty($couponid)) {
            $coupon    = pdo_fetch('select * from ' . tablename('superdesk_shop_coupon') . ' where id=:id and uniacid=:uniacid and merchid=:merchid limit 1', array(':id' => $couponid, ':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));
        ...
        $sql =
            'SELECT d.*, c.coupontype,c.couponname,m.nickname,m.avatar,m.realname,m.mobile ' .
            ...
            ' left join ' . tablename('superdesk_shop_member') . ' m on m.openid = d.openid and m.core_user = d.core_user and m.uniacid = d.uniacid ' .// TODO 标志 楼宇之窗 openid superdesk_shop_member 已处理
            ' where  1 and ' .
            $condition .
