...
class Cart_SuperdeskShopV2Page extends PcMobileLoginPage
{
    public function main()
    {
        ...
        $sql =
            ...
            '        g.discounts,g.isdiscount_discounts,g.isdiscount,g.isdiscount_time,g.isnodiscount,g.merchsale,g.tcate, ' .
            '        o.title as optiontitle,o.specs,o.stock as optionstock,ifnull(o.marketprice, g.marketprice) as marketprice,ifnull(o.costprice, g.costprice) as costprice ' .
        ...
        foreach ($list as &$g) {
            ...
            $g['errors'] = 0;
            if ($g['status'] <= 0 || $g['checked'] != 0 || $g['stock'] <= 0 || $g['deleted'] != 0) {
                $error_list[] = $g;
                $g['errors']  = 1;
            }
        ...
        $count = pdo_fetchcolumn(
            ' select ' .
            '       count(*) ' .
            ' from ' . tablename('superdesk_shop_member_cart') . ' as f ' . // TODO 标志 楼宇之窗 openid superdesk_shop_member_cart 已处理
            ' where 1 ' .
            $condition,
            $params
        );
        ...
        if ($merch_plugin && $merch_data['is_openmerch']) {
            ...
            if (!empty($merch_user)) {
                ...
                foreach ($merch_user as $k => $v) {
                    $new_merch["$k"] = $v['merchname'];
                }

                $merch_user = $new_merch;
            }
        }
        ...
        $ice_menu_array = array(
            array(
                'menu_key'  => 'index',
                'menu_name' => '我的购物车',
                'menu_url'  => mobileUrl('pc.member.cart')
            )
        );

        show_json(1, compact('list', 'merch', 'merch_user', 'ischeckall', 'totalprice', 'total', 'count', 'error_list'));
    }
...
//                'optionid' => $optionid,
    ...
    public function add()
    {
        ...
        $goods = pdo_fetch(
            'select ' .
            '       id,marketprice,diyformid,diyformtype,diyfields,isverify,type,merchid,minbuy ' .
            ' from ' . tablename('superdesk_shop_goods') .
            ' where ' .
            '       id=:id ' .
            '       and uniacid=:uniacid ' .
        ...
        if (empty($member)) {
            show_json(0, array('message' => '请先登录'));
        }

        //2019年3月22日 18:19:54 zjh 最低购买校验
        if(!empty($goods['minbuy']) && $total < $goods['minbuy']){
            show_json(0, '最低购买'.$goods['minbuy']);
        }

        // 是否要绑定手机才能加购物车
//        if (
//            !(empty($_W['shopset']['wap']['open']))
...
//        }
    ...
    public function cartGoodsCount()
    {
        global $_W;
        global $_GPC;
