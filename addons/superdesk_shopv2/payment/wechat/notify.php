...
require IA_ROOT . '/addons/superdesk_shopv2/core/inc/functions.php';
require IA_ROOT . '/addons/superdesk_shopv2/core/inc/plugin_model.php';
require IA_ROOT . '/addons/superdesk_shopv2/core/inc/com_model.php';

//mark kafka 为了kafka转成了model执行
include_once(IA_ROOT . '/addons/superdesk_shopv2/model/order/order.class.php');

new SuperdeskShopWechatPay($get);
exit('fail');

class SuperdeskShopWechatPay
{
    public $get;
    public $type;
    public $total_fee;
    ...
    public function recharge()
    {
        ...
        $log = pdo_fetch(
            'SELECT * ' .
            ' FROM ' . tablename('superdesk_shop_member_log') . // TODO 标志 楼宇之窗 openid superdesk_shop_member_log 待处理 logno 有问题 out_trade_no
            ' WHERE ' .
            '       `uniacid`=:uniacid ' .
            '       and `logno`=:logno ' .
            ' limit 1',
        ...
        if ($OK) {
            ...
            m('member')->setCredit($log['openid'], $log['core_user'],
                'credit2', $log['money'], array(0, $shopset['name'] . '会员充值:wechatnotify:credit2:' . $log['money']));
    ...
    public function coupon()
    {
        ...
        $log   = pdo_fetch(
            'SELECT * FROM ' . tablename('superdesk_shop_coupon_log') . // TODO 标志 楼宇之窗 openid superdesk_shop_coupon_log 不处理
            ' WHERE `logno`=:logno and `uniacid`=:uniacid  limit 1',
            array(
                ':uniacid' => $_W['uniacid'],
                ':logno'   => $logno
        ...
        $coupon = pdo_fetchcolumn('select money from ' . tablename('superdesk_shop_coupon') . ' where id=:id limit 1', array(':id' => $log['couponid']));
    ...
    public function groups()
    {
        ...
        if ($this->is_jie) {
            pdo_update('superdesk_shop_groups_order', array('isborrow' => '1', 'borrowopenid' => $this->get['openid']), array('orderno' => $orderno, 'uniacid' => $_W['uniacid']));
        }
        if (p('groups')) {
    ...
    public function mr()
    {
        ...
        if (p('mr')) {
            $price = pdo_fetchcolumn('select payprice from ' . tablename('superdesk_shop_mr_order') . ' where ordersn=:ordersn limit 1', array(':ordersn' => $ordersn));
