...
class User_SuperdeskShopV2Page extends PluginWebPage 
{
	public function main() 
	{
		...
		$sql = 'select dm.id,dm.diycommissionfields,diycommissionfields,diymemberfields,diymemberdata,dm.nickname,dm.realname,dm.avatar,l.levelname from ' . tablename('superdesk_shop_member') . ' dm ' . ' left join ' . tablename('superdesk_shop_commission_level') . ' l on l.id = dm.agentlevel' . ' left join ' . tablename('mc_mapping_fans') . 'f on f.openid=dm.openid and f.uniacid=' . $_W['uniacid'] . ' where dm.uniacid = ' . $_W['uniacid'] . ' and dm.isagent =1  ' . $condition . ' ORDER BY dm.agenttime desc';
		...
		if (!empty($userlist)) 
		{
			foreach ($userlist as $k => $v ) 
			{
				...
				if ($hasagent) 
				{
					$child_sql = 'select dm.* from ' . tablename('superdesk_shop_member') . ' dm ' . ' left join ' . tablename('mc_mapping_fans') . 'f on f.openid=dm.openid  and f.uniacid=' . $_W['uniacid'] . ' where dm.uniacid = ' . $_W['uniacid'] . ' ' . $condition . '   ORDER BY dm.agenttime desc';
		...
		if ($_GPC['export'] == 1) 
		{
			...
			if (p('diyform')) 
			{
				...
				$columns[] = array('title' => '粉丝营销商申请自定义信息', 'field' => 'agent_diyformdata', 'width' => 36);
				$columns[] = array('title' => '推荐人会员自定义信息', 'field' => 'pmember_diyformdata', 'width' => 36);
				$columns[] = array('title' => '推荐人营销商申请自定义信息', 'field' => 'pagent_diyformdata', 'width' => 36);
			}
	...
	public function check() 
	{
		...
		$members = pdo_fetchall('SELECT id,openid,nickname,realname,mobile,status FROM ' . tablename('superdesk_shop_member') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);
		$time = time();
