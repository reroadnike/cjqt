...
class Send_SuperdeskShopV2Page extends WebPage
{
    public function main()
    {
        ...
        if (!empty($id)) {
            $send = pdo_fetch(
                'SELECT * FROM ' . tablename('superdesk_shop_member_message_template') .
        ...
        $list3 = pdo_fetchall('SELECT * FROM ' . tablename('superdesk_shop_commission_level') . ' WHERE uniacid = \'' . $_W['uniacid'] . '\' ORDER BY id asc');
    ...
    public function fetch()
    {
        ...
        pdo_update(
            'superdesk_shop_member_message_template',
            array(
        ...
        if ($class1 == 1) {
            ...
            if (!empty($value1)) {
                $groupname = pdo_fetchcolumn('select groupname from ' . tablename('superdesk_shop_member_group') . ' where id=:id limit 1', array(':id' => $value1));
            } else {
                $groupname = '全部分组';
            }
            ...
            $member  = pdo_fetchall(
                'SELECT openid FROM ' . tablename('superdesk_shop_member') . // TODO 标志 楼宇之窗 openid shop_member 不处理
                ' WHERE uniacid = \'' . $_W['uniacid'] . '\'',
                array(),
                'openid'
            );
            ...
            if (!empty($value1)) {
                $levelname = pdo_fetchcolumn('select levelname from ' . tablename('superdesk_shop_commission_level') . ' where id=:id limit 1', array(':id' => $value1));
            } else {
                $levelname = '全部营销商';
            }

            $typestr = '营销商-' . $levelname;
        }
    ...
    public function sendmessage()
    {
        ...
        $template = pdo_fetch('SELECT * FROM ' . tablename('superdesk_shop_member_message_template') . ' WHERE id=:id and uniacid=:uniacid ', array(':id' => $id, ':uniacid' => $_W['uniacid']));
        ...
        pdo_update('superdesk_shop_member_message_template', array('sendcount' => $template['sendcount'] + 1), array('id' => $id));
