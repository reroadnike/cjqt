...
//                echo '<br/>';
        ...
        if ($status !== '') {
            if ($status == '-1') {
                ...
                $statuscondition = ' AND ( o.status = 1 or (o.status=0 and o.paytype=3) )';// 有重复显示的数据 o.status=0 and o.paytype=3 (企业月结+未支付) o.status = 1 微信支付
            } else if ($status == '0') {
        ...
        switch (SUPERDESK_SHOPV2_MODE_USER) {
            case 1:// 1 超级前台
                ...
                $sqlcondition = ' left join ' . tablename('superdesk_core_virtualarchitecture') . ' core_enterprise on core_enterprise.id = o.member_enterprise_id ';
                $sqlcondition .= ' left join ' . tablename('superdesk_core_organization') . ' organization on organization.id = core_enterprise.organizationId ';
                break;
            case 2:// 2 福利商城
                $selectSql = ' core_enterprise.enterprise_name, ';

                $sqlcondition = ' left join ' . tablename('superdesk_shop_enterprise_user') . ' core_enterprise on core_enterprise.id = o.member_enterprise_id ';
                break;
        }
        ...
        $sql =
            ...
            ' left join ' . tablename('superdesk_shop_member') . ' m on m.core_user=o.core_user and m.uniacid =  o.uniacid ' .// TODO 标志 楼宇之窗 openid superdesk_shop_member 已处理
//            ' left join ' . tablename('superdesk_shop_member') . ' m on m.openid=o.openid and m.core_user=o.core_user and m.uniacid =  o.uniacid ' .// TODO 标志 楼宇之窗 openid superdesk_shop_member 已处理 //出现了换了openid的情况.这个情况还没处理故而先屏蔽
        ...
        if (empty($_GPC['orderBy']) || $_GPC['orderBy'] == 1) {
            $sql .= ' ORDER BY o.createtime DESC,o.status DESC  ';
        } else if ($_GPC['orderBy'] == 2) {
            $sql .= ' ORDER BY r.createtime DESC,o.status DESC  ';
        }
        ...
        socket_log("商户后台_订单_待发货_SQL:" . $sql . '_' . json_encode($paras, JSON_UNESCAPED_UNICODE));
        ...
        foreach ($list as &$value) {
            ...
            $order_goods = pdo_fetchall(
                ' select g.id,g.title,g.thumb,g.goodssn,og.goodssn as option_goodssn, g.productsn,og.productsn as option_productsn, ' .
                '       og.total,og.price,og.optionname as optiontitle, og.realprice,og.changeprice,og.oldprice,' .
                '       og.commission1,og.commission2,og.commission3,og.commissions,og.diyformdata,og.diyformfields,op.specs, ' .
                '       orf.reply ' .
                ...
                if ($value['agentid'] == $agentid) {
                    ...
                    if (0 < $agent['level2']) {
                        ...
                        $level2_commissions = pdo_fetchall(
                            'select commission2,commissions  ' .
                            ' from ' . tablename('superdesk_shop_order_goods') . ' og ' .// TODO 标志 楼宇之窗 openid shop_order_goods 不处理
                            ' left join  ' . tablename('superdesk_shop_order') . ' o on o.id = og.orderid ' . // TODO 标志 楼宇之窗 openid shop_order 不处理
                            ' where ' .
                            '       og.orderid=:orderid ' .
                            '       and  o.agentid in ( ' . implode(',', array_keys($agent['level1_agentids'])) . ')  ' .
                            '       and o.uniacid=:uniacid',
                            array(
                                ':orderid' => $value['id'],
                                ':uniacid' => $uniacid
                    ...
                    if (0 < $agent['level3']) {
                        ...
                        $level3_commissions = pdo_fetchall(
                            'select commission3,commissions ' .
                            ' from ' . tablename('superdesk_shop_order_goods') . ' og ' .// TODO 标志 楼宇之窗 openid shop_order_goods 不处理
                            ' left join  ' . tablename('superdesk_shop_order') . ' o on o.id = og.orderid ' . // TODO 标志 楼宇之窗 openid shop_order 不处理
                            ' where ' .
                            '       og.orderid=:orderid ' .
                            '       and o.agentid in ( ' . implode(',', array_keys($agent['level2_agentids'])) . ') ' .
                            '       and o.uniacid=:uniacid',
                            array(
                                ':orderid' => $value['id'],
                                ':uniacid' => $uniacid
            ...
            //添加采购经理审核状态显示 zjh 2018年4月23日 16:53:29
//            if ($value['paytypevalue'] == 3) {
//                $value['examine'] = m('examine')->getExamineOne($value['id']);
//            }
        }
...
//            ' left join ' . tablename('superdesk_shop_member') . ' m on m.openid=o.openid and m.core_user=o.core_user and m.uniacid =  o.uniacid' .// TODO 标志 楼宇之窗 openid superdesk_shop_member 已处理 //出现了换了openid的情况.这个情况还没处理故而先屏蔽
            ...
            ' left join ' . tablename('superdesk_shop_merch_saler') . ' s on s.openid = o.verifyopenid and s.uniacid=o.uniacid' .// TODO 标志 楼宇之窗 openid shop_order 不处理
            ' left join ' . tablename('superdesk_shop_member') . ' sm on sm.openid = s.openid and sm.core_user = s.core_user and sm.uniacid=s.uniacid' .// TODO 标志 楼宇之窗 openid superdesk_shop_member 已处理
            ' left join ' . tablename('superdesk_shop_order_examine') . ' oe on oe.orderid = o.id and o.paytype=3' .// TODO 标志 楼宇之窗 openid superdesk_shop_order_examine 已处理
            ' ' . $sqlcondition .
            ' WHERE ' .
    ...
    public function children()
    {
        ...
        if (!empty($_GPC['searchfield']) && !empty($_GPC['keyword'])) {
            ...
            $paras[':keyword'] = $_GPC['keyword'];
            if ($searchfield == 'ordersn') {
                $parentcondition .= ' AND locate(:keyword,o.ordersn)>0';
            } else if ($searchfield == 'jdorderid') {
                $parentcondition .= ' AND locate(:keyword,o.expresssn)>0';
            }
            ...
            $parent_order = pdo_fetch(
                ' select id,parentid from ' . tablename('superdesk_shop_order') . ' as o ' .// TODO 标志 楼宇之窗 openid shop_order 不处理
                ' where ' . $condition . $parentcondition, $paras);

            $parentid = $parent_order['parentid'] == 0 ? $parent_order['id'] : $parent_order['parentid'];
            if (empty($parentid)) {
...
//                ' left join ' . tablename('superdesk_shop_member') . ' m on m.openid=o.openid and m.core_user=o.core_user and m.uniacid =  o.uniacid ' .// TODO 标志 楼宇之窗 openid superdesk_shop_member 已处理 //出现了换了openid的情况.这个情况还没处理故而先屏蔽
                ...
                ' left join ' . tablename('superdesk_shop_member') . ' sm on sm.openid = s.openid and sm.core_user = s.core_user and sm.uniacid=s.uniacid' .// TODO 标志 楼宇之窗 openid superdesk_shop_member 已处理
                ' ' .
                $sqlcondition .// TODO 标志 楼宇之窗 openid shop_order 不处理
                ' where ' .
                $condition .
            ...
            foreach ($list as &$value) {
                ...
                $order_goods = pdo_fetchall(
                    ...
                    array(':uniacid' => $uniacid, ':orderid' => $value['id']));
                $goods       = '';
                ...
                if (!empty($agentid) && (0 < $level)) {
                    ...
                    if ($value['agentid'] == $agentid) {
                        $value['level']     = 1;
                        $level1_commissions = pdo_fetchall(
                            'select commission1,commissions  from ' . tablename('superdesk_shop_order_goods') . ' og ' .// TODO 标志 楼宇之窗 openid shop_order_goods 不处理
                            ' left join  ' . tablename('superdesk_shop_order') . ' o on o.id = og.orderid ' . // TODO 标志 楼宇之窗 openid shop_order 不处理
            ...
            $stores = pdo_fetchall('select id,storename from ' . tablename('superdesk_shop_merch_store') . ' where uniacid=:uniacid and merchid = :merchid', array(':uniacid' => $uniacid, ':merchid' => $_W['merchid']));
            $r_type = array('退货', '换货', '维修', '退货退款');
