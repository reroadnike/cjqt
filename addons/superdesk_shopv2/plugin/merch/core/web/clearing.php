...
class Clearing_SuperdeskShopV2Page extends PluginWebPage
{
    protected function main($status = 0)
    {
        ...
        $sql = 'select u.merchname,u.mobile,u.realname,u.logo,o.* from ' . tablename('superdesk_shop_merch_clearing') . ' o ' . ' left join ' . tablename('superdesk_shop_merch_user') . ' u on u.id=o.merchid' . ' where 1 ' . $condition . ' ORDER BY o.id DESC';
    ...
    protected function post()
    {
        ...
        if ($_W['ispost']) {
            ...
            if ($data['status'] == 2) {
                ...
                $merch_user = pdo_fetch('select * from ' . tablename('superdesk_shop_merch_user') . ' where uniacid=:uniacid and id=' . $merchid, array(':uniacid' => $_W['uniacid']));
            ...
            if ($id) {
                ...
                pdo_update('superdesk_shop_merch_clearing', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
            } else {
                pdo_insert('superdesk_shop_merch_clearing', $data);
                $id = pdo_insertid();
            }
        ...
        if ($id) {
            $item = pdo_fetch('SELECT * FROM ' . tablename('superdesk_shop_merch_clearing') . ' WHERE id=:id AND uniacid=:uniacid', array(':id' => $id, ':uniacid' => $_W['uniacid']));
            empty($item) && $this->message('未找到该账单', '', 'error');
            $merch = pdo_fetch('select id,merchname,logo,realname,mobile from ' . tablename('superdesk_shop_merch_user') . ' where id=:id and uniacid=:uniacid', array(':id' => $item['merchid'], ':uniacid' => $_W['uniacid']));
        } else if ($merchid) {
            $merch = pdo_fetch('select id,merchname,logo,realname,mobile from ' . tablename('superdesk_shop_merch_user') . ' where id=:id and uniacid=:uniacid', array(':id' => $merchid, ':uniacid' => $_W['uniacid']));
        }
