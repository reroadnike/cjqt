...
class Index_SuperdeskShopV2Page extends MerchWebPage
{
    ...
    public function main()
    {
        ...
        foreach ($list as &$row) {
            ...
            $row['pwdjoins'] = pdo_fetchcolumn(
                'select count(*) ' .
                ' from ' . tablename('superdesk_shop_coupon_guess') .
                ' where ' .
                '       couponid=:couponid ' .
                '       and uniacid=:uniacid ' .
                '       and merchid=:merchid ' .
    ...
    protected function post()
    {
        ...
        if ($_W['ispost']) {
            ...
            show_json(1, array(
                'url' => merchUrl('sale/coupon/edit', array('id' => $id, 'tab' => str_replace('#tab_', '', $_GPC['tab'])))
            ));
        }
    ...
    public function delete()
    {
        ...
        $items = pdo_fetchall('SELECT id,couponname FROM ' . tablename('superdesk_shop_coupon') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid'] . ' AND merchid=' . $_W['merchid']);

        foreach ($items as $item) {
            pdo_delete('superdesk_shop_coupon', array('id' => $item['id'], 'uniacid' => $_W['uniacid'], 'merchid' => $_W['merchid']));
            pdo_delete('superdesk_shop_coupon_data', // TODO 标志 楼宇之窗 openid shop_coupon_data 不处理
                array(
                    'couponid' => $item['id'],
                    'uniacid'  => $_W['uniacid'],
                    'merchid'  => $_W['merchid']
