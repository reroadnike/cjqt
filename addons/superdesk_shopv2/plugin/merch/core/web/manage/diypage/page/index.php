...
class Index_SuperdeskShopV2Page extends MerchWebPage
{
    ...
    public function create()
    {
        global $_W;
        global $_GPC;
        $tid_member     = pdo_fetchcolumn('select id from' . tablename('superdesk_shop_diypage_template') . ' where tplid=9 limit 1');
        $tid_commission = pdo_fetchcolumn('select id from' . tablename('superdesk_shop_diypage_template') . ' where tplid=10 limit 1');
        $tid_detail     = pdo_fetchcolumn('select id from' . tablename('superdesk_shop_diypage_template') . ' where tplid=11 limit 1');
