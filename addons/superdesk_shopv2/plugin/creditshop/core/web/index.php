...
class Index_SuperdeskShopV2Page extends PluginWebPage
{
    ...
    public function set()
    {
        ...
        if ($_W['ispost']) {
            ...
            if (!empty($keyword)) {
                if ($keyword['name'] != 'superdesk_shopv2:creditshop') {
                    show_json(0, '关键字已存在!');
                }
            ...
            $rule = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name  limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'superdesk_shopv2', ':name' => 'superdesk_shopv2:creditshop'));

            if (empty($rule)) {
                $rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'superdesk_shopv2:creditshop', 'module' => 'superdesk_shopv2', 'displayorder' => 0, 'status' => 1);
