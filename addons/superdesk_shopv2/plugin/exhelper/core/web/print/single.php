...
class Single_SuperdeskShopV2Page extends PluginWebPage
{
    private $_orderModel;
        ...
        if ($_W['ispost']) {
            ...
            if (!empty($_GPC['searchfield']) && !empty($_GPC['keyword'])) {
                ...
                if ($searchfield == 'ordersn') {
                    ...
                    $sqlcondition =
                        ...
                        '              where og.uniacid = \'' . $uniacid . '\' and og.merchid=0 and (locate(:keyword,g.title)>0)) gs on gs.orderid=o.id';
                } else if ($searchfield == 'goodssn') {
                    $sqlcondition =
                        ' inner join ( select distinct og.orderid from ' . tablename('superdesk_shop_order_goods') . ' og ' .// TODO 标志 楼宇之窗 openid shop_order_goods 不处理
                        '              left join ' . tablename('superdesk_shop_goods') . ' g on g.id=og.goodsid ' .
                        '              where og.uniacid = \'' . $uniacid . '\' and og.merchid=0 and (locate(:keyword,g.goodssn)>0)) gs on gs.orderid=o.id';
                }
            }
            ...
            foreach ($orders as $order) {
                ...
                if (!is_array($order_address)) {
                    ...
                    $addresskey =
                        $member_address['realname'] .
                        $member_address['mobile'] .
    ...
    public function getprintTemp()
    {
        ...
        if ($_W['ispost']) {
            ...
            $repDatas = array(
                $tempSender['sendername'],
                $tempSender['sendertel'],
                $tempSender['senderaddress'],
                $tempSender['sendersign'],
                date('Y-m-d H:i'),
                $tempSender['sendercode'],
                $tempSender['sendercity']
            );
    ...
    public function getorderinfo()
    {
        ...
        if ($_W['ispost']) {
            ...
            $orders = pdo_fetchall(
                ...
                ' WHERE id in( ' . $in . ' ) and (status=1 or (paytype=3 and status=0)) and uniacid=:uniacid and merchid=0 and isparent=0 ' .
    ...
    public function dosend()
    {
        ...
        if ($_W['ispost']) {
            ...
            $orderinfo = pdo_fetch(
                ...
                ' FROM ' . tablename('superdesk_shop_order') .// TODO 标志 楼宇之窗 openid shop_order 不处理
                ' WHERE id=:orderid ' .
                '       and status>-1 and uniacid=:uniacid and merchid=0 ' .
                ' limit 1',
                array(
                    ':orderid' => $orderid,
                    ':uniacid' => $_W['uniacid']
