...
require_once SUPERDESK_SHOPV2_INC . 'plugin_processor.php';
require_once SUPERDESK_SHOPV2_INC . 'receiver.php';
...
class PosterProcessor extends PluginProcessor
{
    ...
    private function responseScan($obj)
    {
        ...
        $poster = pdo_fetch('select * from ' . tablename('superdesk_shop_poster') . ' where type=4 and isdefault=1 and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid']));
        if (empty($poster)) {
            return $this->responseDefault($obj);
        }
        $member = m('member')->getMember($openid);
        $this->model->scanTime($openid, $qr['openid'], $poster, $member['core_user']);
    ...
    private function responseSubscribe($obj)
    {
        ...
        if ($member['isnew']) {
            pdo_update(
                ...
                array('follows' => $poster['follows'] + 1),
                array('id' => $poster['id'])
            );
        }

        $qrmember = m('member')->getMember($qr['openid'], $qr['core_user']);

        $log = pdo_fetch(
            'select * from ' . tablename('superdesk_shop_poster_log') .// TODO 标志 楼宇之窗 openid shop_poster_log 已处理
            ' where openid=:openid and core_user=:core_user and posterid=:posterid and uniacid=:uniacid limit 1',
            array(
                ':openid'    => $openid,
        ...
        if (empty($log) && ($openid != $qr['openid'])) {
            ...
            pdo_insert('superdesk_shop_poster_log', $log);// TODO 标志 楼宇之窗 openid shop_poster_log 已处理
            $log['id'] = pdo_insertid();
            ...
            if (0 < $poster['subcredit']) {
                m('member')->setCredit($openid, $core_user,
                    'credit1', $poster['subcredit'], array(0, '扫码关注积分+' . $poster['subcredit']));
            }
            ...
            if (0 < $poster['submoney']) {
                ...
                m('finance')->pay($openid, $core_user, $poster['paytype'], $pay, '', $subpaycontent, false);
            }

            $reward_totle = ((!empty($poster['reward_totle']) ? json_decode($poster['reward_totle'], true) : array()));
            $reward_real  = pdo_fetch(
                ' select ' .
                '       sum(reccredit) as reccredit_totle,sum(recmoney) as recmoney_totle,sum(reccouponnum) as reccouponnum_totle ' .
                ' from ' . tablename('superdesk_shop_poster_log') .// TODO 标志 楼宇之窗 openid shop_poster_log 不处理
                ' where ' .
                ...
                ' limit 1',
                array(
                    ':from_openid' => $qr['openid'],// TODO 标志 楼宇之窗 openid shop_poster_log 待处理
                    ':posterid'    => $poster['id'],
                    ':uniacid'     => $_W['uniacid'],
                    ':time1'       => strtotime(date('Y-m', time()) . '-1'),
                    ':time2'       => time()
                )
            );
            ...
            if (empty($reward_totle['reccredit_totle']) || (intval($reward_real['reccredit_totle']) < intval($reward_totle['reccredit_totle']))) {
                if (0 < $poster['reccredit']) {
                    m('member')->setCredit($qr['openid'], $qr['core_user'],
                        'credit1', $poster['reccredit'], array(0, '推荐扫码关注积分+' . $poster['reccredit']));
                }
            }
            ...
            if (empty($reward_totle['recmoney_totle']) || (floatval($reward_real['recmoney_totle']) < floatval($reward_totle['recmoney_totle']))) {
                ...
                if (0 < $poster['recmoney']) {
                    ...
                    m('finance')->pay($qr['openid'], $qr['core_user'],
                        $poster['paytype'], $pay, '', $recpaycontent, false);
                }
            }
            ...
            if (!empty($poster['subtext'])) {
                ...
                if (!empty($poster['templateid'])) {
                    m('message')->sendTplNotice(
                        $qr['openid'], $poster['templateid'],
                        array(
            ...
            if (!empty($upgrade)) {
                ...
                pdo_update(
                    'superdesk_shop_poster_log', // TODO 标志 楼宇之窗 openid shop_poster_log 不处理
                    $upgrade,
                    array(
                        'id' => $log['id']
                    )
                    ...
                    if (($qrmember['isagent'] == 1) && ($qrmember['status'] == 1)) {
                        ...
                        if (!empty($poster['bedown'])) {
                            ...
                            if (empty($member['agentid'])) {
                                ...
                                if (empty($member['fixagentid'])) {
                                    ...
                                    if ($author) {
                                        ...
                                        pdo_update('superdesk_shop_member', array('agentid' => $qrmember['id'], 'childtime' => $time, 'authorid' => $authorid), array('id' => $member['id']));
                                    } else {
                                        pdo_update('superdesk_shop_member', array('agentid' => $qrmember['id'], 'childtime' => $time), array('id' => $member['id']));
                                    }
                                    if ($author) {
                                        $author_set = $author->getSet();
                                        if (!empty($author_set['become']) && (($author_set['become'] == '2') || ($author_set['become'] == '5'))) {
                                            $can_author       = false;
                                            $getAgentsDownNum = $p->getAgentsDownNum($qrmember['openid'], $qrmember['core_user']);
                                            ...
                                            if ($can_author) {
                                                ...
                                                if (empty($member['authorblack'])) {
                                                    pdo_update('superdesk_shop_member', array('authorstatus' => $become_check, 'isauthor' => 1, 'authortime' => $time), array('uniacid' => $_W['uniacid'], 'id' => $qrmember['id']));
                                                    if ($become_check == 1) {
                            ...
                            if (!empty($poster['beagent'])) {
                                ...
                                pdo_update('superdesk_shop_member', array('isagent' => 1, 'status' => $become_check, 'agenttime' => $time), array('id' => $member['id']));
                                if ($become_check == 1) {
