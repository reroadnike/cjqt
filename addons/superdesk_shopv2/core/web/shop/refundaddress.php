...
class Refundaddress_SuperdeskShopV2Page extends WebPage
{
	public function main()
	{
		...
		$total = pdo_fetchcolumn(
			'SELECT count(1) FROM ' . tablename('superdesk_shop_refund_address') . // TODO 标志 楼宇之窗 openid shop_refund_address 不处理
			' WHERE 1 ' . $condition,
			$params
		);
	...
	protected function post()
	{
		...
		if ($_W['ispost']) {
			...
			if (!empty($id)) {
				...
				pdo_update('superdesk_shop_refund_address', $data, array('id' => $id));// TODO 标志 楼宇之窗 openid shop_refund_address 不处理
			}
			 else {
				pdo_insert('superdesk_shop_refund_address', $data);// TODO 标志 楼宇之窗 openid shop_refund_address 不处理
				$id = pdo_insertid();
		...
		if (!empty($id)) {
			$item = pdo_fetch(
				'SELECT * FROM ' . tablename('superdesk_shop_refund_address') . // TODO 标志 楼宇之窗 openid shop_refund_address 不处理
				' WHERE id = \'' . $id . '\' and merchid=0 and uniacid = \'' . $_W['uniacid'] . '\'');
	...
	public function delete()
	{
		...
		$items = pdo_fetchall(
			'SELECT id,title FROM ' . tablename('superdesk_shop_refund_address') . // TODO 标志 楼宇之窗 openid shop_refund_address 不处理
			' WHERE id in( ' . $id . ' ) AND merchid=0 AND uniacid=' . $_W['uniacid']);


		foreach ($items as $item ) {
			pdo_delete('superdesk_shop_refund_address', array('id' => $item['id']));// TODO 标志 楼宇之窗 openid shop_refund_address 不处理
	...
	public function setdefault()
	{
		...
		foreach ($items as $item ) {
			pdo_update('superdesk_shop_refund_address', // TODO 标志 楼宇之窗 openid shop_refund_address 不处理
				array('isdefault' => intval($_GPC['isdefault'])),
				array('id' => $item['id'])
			);
