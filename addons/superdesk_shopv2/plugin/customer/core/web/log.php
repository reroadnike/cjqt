...
if ($operation == 'display') {
    ...
    $list = pdo_fetchall(
        ' select ' .
        '       log.*, m.nickname,m.avatar,m.realname,m.mobile,g.title,g.thumb,g.thumb,g.credit,g.money,g.type as goodstype ' .
        ' from ' . tablename('superdesk_shop_creditshop_log') . ' log ' .// TODO 标志 楼宇之窗 openid shop_creditshop_log 已处理
        '       left join ' . tablename('superdesk_shop_member') . ' m on m.openid = log.openid and m.core_user = log.core_user and m.uniacid=log.uniacid' .
