...
class Refund_SuperdeskShopV2Page extends PluginWebPage
{
    protected function opData()
    {
        ...
        if (!empty($refundid)) {
            ...
            $refund           = pdo_fetch(
                'select * ' .
                ' from ' . tablename('superdesk_shop_groups_order_refund') .
                ' where ' .
                '       id=:id ' .
                ' limit 1',
                array(
    ...
    public function main()
    {
        ...
        try {
            ...
            if (!empty($searchtime)) {
                ...
                $condition .=
                    ' and ore.' . $searchtime . 'time > ' . strtotime($_GPC['time']['start']) .
                    ' and ore.' . $searchtime . 'time < ' . strtotime($_GPC['time']['end']) . ' ';
            ...
            $list = pdo_fetchall(
                'SELECT o.addressid,o.goodid,o.pay_type,o.price,o.creditmoney,o.is_team,o.refundid,ore.*,g.title,g.thumb,g.category,g.groupsprice,g.singleprice,c.name,g.thumb, ' .
                ...
                ' FROM ' . tablename('superdesk_shop_groups_order_refund') . ' as ore ' .
                ...
                '       left join ' . tablename('superdesk_shop_member_address') . ' a on a.id=ore.refundaddressid' . // TODO 标志 楼宇之窗 openid superdesk_shop_member_address 不处理
                '       right join ' . tablename('superdesk_shop_groups_category') . ' as c on c.id = g.category' .
                ' WHERE 1 ' .
                $condition .
                ' ORDER BY ore.applytime DESC ' .
                $page,
                $params
            );
    ...
    public function detail()
    {
        ...
        try {
            ...
            $refund = pdo_fetch(
                'SELECT ' .
                '       ore.*,' .
                '       o.price,o.credit,o.freight,o.paytime,o.pay_type,o.orderno,o.goodid,' .
                '       g.title,g.thumb,g.category,g.groupsprice ' .
                ' FROM ' . tablename('superdesk_shop_groups_order_refund') . ' as ore' .
                '       right join ' . tablename('superdesk_shop_groups_order') . ' as o on o.id = ore.orderid' .
    ...
    public function submit()
    {
        ...
        if ($_W['ispost']) {
            ...
            if ($item['status'] == 3) {
                pdo_update('superdesk_shop_groups_order', array('refundstate' => 0), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
            ...
            if (empty($refund['refundno'])) {
                ...
                pdo_update('superdesk_shop_groups_order_refund', array('refundno' => $refund['refundno']), array('id' => $refund['id']));
            }
            ...
            if ($refundstatus == 0) {
                ...
                if ($raid != 0) {
                    $raddress = pdo_fetch(
                        'select * from ' . tablename('superdesk_shop_refund_address') . // TODO 标志 楼宇之窗 openid shop_refund_address 不处理
                        ' where id=:id and uniacid=:uniacid limit 1',
                        array(':id' => $raid, ':uniacid' => $uniacid)
                    );
                } else {
                    $raddress = pdo_fetch(
                        'select * from ' . tablename('superdesk_shop_refund_address') . // TODO 标志 楼宇之窗 openid shop_refund_address 不处理
                ...
                if (empty($raddress)) {
                    $raddress = pdo_fetch(
                        'select * from ' . tablename('superdesk_shop_refund_address') . // TODO 标志 楼宇之窗 openid shop_refund_address 不处理
                        ' where uniacid=:uniacid order by id desc limit 1',
                ...
                pdo_update('superdesk_shop_groups_order_refund', $change_refund, array('id' => $item['refundid']));
                ...
                pdo_update('superdesk_shop_groups_order_refund', $refund_data, array('id' => $item['refundid'], 'uniacid' => $uniacid));

                $order_data                = array();
                $order_data['refundstate'] = 0;
                $order_data['status']      = 3;
                $order_data['refundtime']  = $time;
                pdo_update('superdesk_shop_groups_order', $order_data, array('id' => $item['id'], 'uniacid' => $uniacid));
                ...
                $order = pdo_fetch(
                    'SELECT id,orderno,credit,creditmoney,price,freight,status,pay_type,is_team,apppay ' .
                    ' FROM ' . tablename('superdesk_shop_groups_order') .
                    ' WHERE id = :orderid and uniacid=:uniacid',
                    array(
                        ':orderid' => $item['id'],
                        ':uniacid' => $uniacid
                    )
                ...
                if ($order['pay_type'] == 'credit') {
                    ...
                    m('member')->setCredit($item['openid'], $item['core_user'],
                        'credit2', $realprice, array(0, $shopset['name'] . '退款: ' . $realprice . '元 订单号: ' . $item['orderno']));
                    $result = true;
                    ...
                    if (empty($item['isborrow'])) {
                        $result = m('finance')->refund($item['openid'], $item['core_user'],
                            $ordersn, $refund['refundno'], $realprice * 100, $realprice * 100, (!empty($order['apppay']) ? true : false));
                    } else {
                        $result = m('finance')->refundBorrow($item['borrowopenid'], $item['core_user'],
                            $ordersn, $refund['refundno'], $realprice * 100, $realprice * 100, (!empty($order['apppay']) ? true : false));
                    }
                    ...
                    $result = m('finance')->pay($item['openid'], $item['core_user'],
                        1, $realprice * 100, $refund['refundno'], $shopset['name'] . '退款: ' . $realprice . '元 订单号: ' . $item['orderno']);
                ...
                if (0 < $item['credit']) {
                    m('member')->setCredit($item['openid'], $item['core_user'],
                        'credit1', $item['credit'], array('0', $shopset['name'] . '购物返还抵扣积分 积分: ' . $item['credit'] . ' 抵扣金额: ' . $item['creditmoney'] . ' 订单号: ' . $item['orderno']));
                }
                ...
                pdo_update('superdesk_shop_groups_order', array('refundstate' => 0, 'status' => -1, 'refundtime' => $time), array('id' => $item['id'], 'uniacid' => $uniacid));

                $sales = pdo_fetch('select id,sales,stock from ' . tablename('superdesk_shop_groups_goods') . ' where id = :id and uniacid = :uniacid ', array(':id' => $item['goodid'], ':uniacid' => $uniacid));

                pdo_update('superdesk_shop_groups_goods', array('sales' => $sales['sales'] - 1, 'stock' => $sales['stock'] + 1), array('id' => $sales['id'], 'uniacid' => $uniacid));

                plog('groups.refund', '订单退款 ID: ' . $item['id'] . ' 订单号: ' . $item['orderno']);

            } else if ($refundstatus == -1) {
                pdo_update('superdesk_shop_groups_order_refund', array('reply' => $refundcontent, 'refundstatus' => -1, 'endtime' => $time), array('id' => $item['refundid']));
                p('groups')->sendTeamMessage($item['id'], true);
                plog('groups.refund', '订单退款拒绝 ID: ' . $item['id'] . ' 订单号: ' . $item['orderno'] . ' 原因: ' . $refundcontent);
                pdo_update('superdesk_shop_groups_order', array('refundstate' => 0), array('id' => $item['id'], 'uniacid' => $uniacid));
