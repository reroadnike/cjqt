...
class Index_SuperdeskShopV2Page extends WebPage 
{
	public function main() 
	{
		...
		$gifts = pdo_fetchall('SELECT * FROM ' . tablename('superdesk_shop_gift') . "\n" . '                    WHERE 1 ' . $condition . ' ORDER BY displayorder DESC,id DESC LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize, $params);
		$total = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('superdesk_shop_gift') . ' WHERE 1 ' . $condition . ' ', $params);
	...
	protected function post() 
	{
		...
		$type = trim($_GPC['type']);
		$id = intval($_GPC['id']);
		$item = pdo_fetch('SELECT * FROM ' . tablename('superdesk_shop_gift') . ' WHERE uniacid = ' . $uniacid . ' and id = ' . $id . ' ');
		...
		if (!empty($item['goodsid'])) 
		{
			...
			if ($goodsid) 
			{
				...
				{
					$goods[$key] = pdo_fetch('select id,title,thumb from ' . tablename('superdesk_shop_goods') . ' where uniacid = ' . $uniacid . ' and status = 1 and id = ' . $value . ' and deleted = 0 ');
				}
			}
		}
		if (!empty($item['giftgoodsid'])) 
		{
			...
			if ($giftid) 
			{
				...
				{
					$gift[$key] = pdo_fetch('select id,title,thumb from ' . tablename('superdesk_shop_goods') . ' where uniacid = ' . $uniacid . ' and status = 2 and id = ' . $value . ' and deleted = 0 ');
				}
			}
		}
		if ($_W['ispost']) 
		{
			...
			if (empty($data['thumb'])) 
			{
				...
				{
					$giftgood = pdo_fetch('select id,title,thumb from ' . tablename('superdesk_shop_goods') . ' where uniacid = ' . $uniacid . ' and status = 2 and deleted = 0 limit 1 ');
				}
			...
			if (!empty($id)) 
			{
				pdo_update('superdesk_shop_gift', $data, array('id' => $id));
				plog('sale.gift.edit', '编辑赠品 ID: ' . $id . ' <br/>赠品名称: ' . $data['title']);
			}
			else 
			{
				pdo_insert('superdesk_shop_gift', $data);
				$id = pdo_insertid();
	...
	public function delete() 
	{
		...
		$items = pdo_fetchall('SELECT id,title FROM ' . tablename('superdesk_shop_gift') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);
		foreach ($items as $item ) 
		{
			pdo_update('superdesk_shop_gift', array('deleted' => 1, 'status' => 0), array('id' => $item['id']));
	...
	public function status() 
	{
		...
		foreach ($items as $item ) 
		{
			pdo_update('superdesk_shop_gift', array('status' => intval($_GPC['status'])), array('id' => $item['id']));
	...
	public function delete1() 
	{
		...
		foreach ($items as $item ) 
		{
			pdo_delete('superdesk_shop_gift', array('id' => $item['id']));
	...
	public function restore() 
	{
		...
		foreach ($items as $item ) 
		{
			pdo_update('superdesk_shop_gift', array('deleted' => 0), array('id' => $item['id']));
	...
	public function change() 
	{
		...
		$gift = pdo_fetch('select id from ' . tablename('superdesk_shop_gift') . ' where id=:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $id));
		if (empty($gift)) 
		{
			show_json(0, array('message' => '参数错误'));
		}
		pdo_update('superdesk_shop_gift', array($type => $value), array('id' => $id));
		show_json(1);
	}
