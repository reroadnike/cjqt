...
$order = pdo_fetch("SELECT * FROM " . tablename('superdesk_boardroom_4school_s_order') . " WHERE id = :id AND uniacid = :uniacid",
    array(
        ':id' => $orderid,
        ':uniacid' => $_W['uniacid']
    )
...
if (checksubmit('codsubmit')) {
    ...
    $ordergoods = pdo_fetchall("SELECT goodsid, total,optionid FROM " . tablename('superdesk_boardroom_4school_s_order_goods') . " WHERE orderid = '{$orderid}'", array(), 'goodsid');
    if (!empty($ordergoods)) {
        $goods = pdo_fetchall("SELECT id, title, thumb, marketprice, unit, total,credit FROM " . tablename('superdesk_boardroom_4school_s_goods') . " WHERE id IN ('" . implode("','", array_keys($ordergoods)) . "')");
    }
...
//				$address = pdo_fetch("SELECT * FROM " . tablename('mc_member_address') . " WHERE id = :id", array(':id' => $order['addressid']));
        ...
        if (!empty($goods)) {
            foreach ($goods as $row) {
                //属性
                $option = pdo_fetch("select title,marketprice,weight,stock from " . tablename("superdesk_boardroom_4school_s_goods_option") . " where id=:id limit 1", array(":id" => $ordergoods[$row['id']]['optionid']));
                if ($option) {
    ...
    pdo_update('superdesk_boardroom_4school_s_order', array('status' => '1', 'paytype' => '3'), array('id' => $orderid, 'uniacid' => $_W['uniacid']));
...
// 商品编号
$sql = 'SELECT `goodsid` FROM ' . tablename('superdesk_boardroom_4school_s_order_goods') . " WHERE `orderid` = :orderid";
$goodsId = pdo_fetchcolumn($sql, array(':orderid' => $orderid));

// 商品名称
$sql = 'SELECT `title` FROM ' . tablename('superdesk_boardroom_4school_s_goods') . " WHERE `id` = :id";
