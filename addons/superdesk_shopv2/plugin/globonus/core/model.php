    ...
    class GlobonusModel extends PluginModel
    {
        ...
        public function getLevels($all = true, $default = false)
        {
            ...
            if ($all) {
                ...
                $levels = pdo_fetchall(
                    'select * ' .
                    ' from ' . tablename('superdesk_shop_globonus_level') .
                    ' where ' .
                    '       uniacid=:uniacid ' .
                    ' order by bonus asc',
                    array(
                ...
                $levels = pdo_fetchall(
                    ...
                    ' where ' .
                    '       uniacid=:uniacid ' .
                    '       and (ordermoney>0 or commissionmoney>0 or bonusmoney>0) ' .
                    ' order by bonus asc',
                    array(
                        ':uniacid' => $_W['uniacid']
                    )
        ...
        public function getBonus($openid = '', $core_user = '', $params = array())// TODO 标志 楼宇之窗 openid shop_globonus_billp 已处理
        {
            global $_W;
        ...
        protected function replaceTemplate($str, $tag, $data, $member)
        {
            ...
            switch ($tag) {
                case 'become_advanced':
                    $arr['_obfuscate_W_aXtumXtF0'] = date('Y-m-d H:i:s', $data['partnertime']);
        ...
        public function getLevel($openid, $core_user = 0)
        {
            ...
            $level = pdo_fetch(
                ...
                ' from ' . tablename('superdesk_shop_globonus_level') .
                ' where ' .
                '       uniacid=:uniacid and id=:id limit 1',
                array(
                    ':uniacid' => $_W['uniacid'],
        ...
        public function upgradeLevelByOrder($openid, $core_user)
        {
            global $_W;
            ...
            if ((0 <= $leveltype) && ($leveltype <= 3)) {
                ...
                foreach ($agents as $agent) {
                    ...
                    $info = $this->getInfoById($agent['id'], array('ordercount3', 'ordermoney3', 'order13money', 'order13'));
                    ...
                    if ($leveltype == 0) {
                        ...
                        $newlevel = pdo_fetch(
                            'select id,uniacid,levelname,bonus,ordermoney,ordercount,commissionmoney,bonusmoney,downcount ' .
                            ' from ' . tablename('superdesk_shop_globonus_level') .
                            ' where ' .
                            '       uniacid=:uniacid ' .
                            '       and ' . $ordermoney . ' >= ordermoney ' .
         ...
         * @return mixed
         */
        public function getInfoById($mid, $options = NULL)
        {
            // 通过 MID得到member ;
        ...
        public function getInfo($openid, $core_user, $options = NULL)
        {
        ...
        public function upgradeLevelByAgent($mid)
        {
            ...
            $info = $this->getInfoById($m['id'], array());
            ...
            if (($leveltype == 6) || ($leveltype == 8)) {
                ...
                foreach ($agents as $agent) {
                    ...
                    $info = $this->getInfoById($agent['id'], array());
                    ...
                    if ($leveltype == 6) {
                        ...
                        if (!empty($downs1)) {
                            ...
                            $downs2 = pdo_fetchall(
                                ...
                                ' from ' . tablename('superdesk_shop_member') .
                                ' where ' .
                                '       agentid in( ' . implode(',', array_keys($downs1)) . ') ' .
                                '       and uniacid=:uniacid',
                                array(
                                    ':uniacid' => $_W['uniacid']
                                ),
                            ...
                            if (!empty($downs2)) {
                                ...
                                $downs3 = pdo_fetchall(
                                    ...
                                    ' from ' . tablename('superdesk_shop_member') .
                                    ' where ' .
                                    '       agentid in( ' . implode(',', array_keys($downs2)) . ') ' .
                                    '       and uniacid=:uniacid',
                                    array(
                                        ':uniacid' => $_W['uniacid']
                                    ),
                    ...
                    pdo_update('superdesk_shop_member', array('partnerlevel' => $newlevel['id']), array('id' => $agent['id']));
            ...
            pdo_update(
                'superdesk_shop_member',
                array('partnerlevel' => $newlevel['id']),
                array('id' => $m['id'])
            );
        ...
        public function upgradeLevelByCommissionOK($openid, $core_user = 0)
        {
            ...
            $info            = $this->getInfoById($m['id'], array('pay'));
        ...
        public function upgradeLevelByBonus($openid, $core_user)
        {
            global $_W;
            ...
            $bonusmoney = $this->getBonus($openid, $core_user, array('ok'));// TODO 标志 楼宇之窗 openid shop_globonus_billp 已处理
        ...
        public function getBonusData($year = 0, $month = 0, $week = 0, $openid = '')
        {
            ...
            $bill = pdo_fetch(
                'select * ' .
                ' from ' . tablename('superdesk_shop_globonus_bill') .
                ' where ' .
                '       uniacid=:uniacid ' .
                '       and `year`=:year ' .
                '       and `month`=:month ' .
            ...
            $partners = pdo_fetchall(
                'select m.id,m.openid,m.partnerlevel,l.bonus ' .
                ' from ' . tablename('superdesk_shop_member') . ' m ' .
                ' left join ' . tablename('superdesk_shop_globonus_level') . ' l on l.id = m.partnerlevel ' .
                ' where ' .
                '       m.uniacid=:uniacid ' .
                '       and m.ispartner=1 ' .
                '       and m.partnerstatus=1 ' .
                $pcondition,
                array(
                    ':uniacid' => $_W['uniacid']
                )
