...
class List_SuperdeskShopV2Page extends MerchWebPage
{
    ...
    protected function applyData($status, $st)
    {
        ...
        if (!empty($_GPC['timetype'])) {
            ...
            if (!empty($timetype)) {
                $condition .=
                    ' AND b.' . $timetype . ' >= :starttime ' .
                    ' AND b.' . $timetype . '  <= :endtime ';
        ...
        $total = pdo_fetchcolumn(
            'select count(b.id) from' . tablename('superdesk_shop_merch_bill') . ' b ' .
            ' left join ' . tablename('superdesk_shop_merch_user') . ' u on b.merchid = u.id' .
            ' where 1 ' .
            $condition,
            $params
    ...
    protected function post()
    {
        ...
        if ($_W['ispost']) {
            ...
            pdo_insert('superdesk_shop_merch_bill', $insert);
            $billid = pdo_insertid();
            ...
            foreach ($list as $k => $v) {
                ...
                pdo_insert('superdesk_shop_merch_billo', $insert_data);

                $change_order_data               = array();
                $change_order_data['merchapply'] = 1; // 商户提现
    ...
    public function getLastApply($merchid, $applytype = -1)
    {
        ...
        $sql =
            'select applytype,alipay,bankname,bankcard,applyrealname ' .
