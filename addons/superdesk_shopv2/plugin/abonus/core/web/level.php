...
class Level_SuperdeskShopV2Page extends PluginWebPage
{
    ...
    protected function post()
    {
        ...
        if ($_W['ispost']) {
            ...
            $data = array(
                ...
                'bonus1' => trim(trim($_GPC['bonus1']), '%'), 'bonus2' => trim(trim($_GPC['bonus2']), '%'), 'bonus3' => trim(trim($_GPC['bonus3']), '%'), 'ordermoney' => $_GPC['ordermoney'], 'ordercount' => intval($_GPC['ordercount']), 'bonusmoney' => trim($_GPC['bonusmoney'], '%'));
            ...
            if (!empty($id)) {
                if ($id == 'default') {
                    ...
                    pdo_update('superdesk_shop_abonus_level', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
                    plog('abonus.level.edit', '修改代理商等级 ID: ' . $id . $updatecontent);
                }
            } else {
                pdo_insert('superdesk_shop_abonus_level', $data);
                $id = pdo_insertid();
    ...
    public function delete()
    {
        ...
        $items = pdo_fetchall('SELECT id,levelname FROM ' . tablename('superdesk_shop_abonus_level') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);

        foreach ($items as $item) {
            pdo_delete('superdesk_shop_abonus_level', array('id' => $item['id']));
