...
class Register_SuperdeskShopV2Page extends PluginMobileLoginPage
{
    public function main()
    {
        global $_W;
        global $_GPC;
        $set = $_W['shopset']['enterprise'];

        if (empty($set['apply_openmobile'])) {
            $this->message('未开启企业入驻申请', '', 'error');
        }


        $reg  = pdo_fetch(
            'select * from ' . tablename('superdesk_shop_enterprise_reg') . // TODO 标志 楼宇之窗 openid shop_enterprise_reg 已处理
        ...
        if (!empty($reg['status'])) {
            $user = pdo_fetch('select * from ' . tablename('superdesk_shop_enterprise_user') . ' where openid=:openid and uniacid=:uniacid limit 1', array(':openid' => $_W['openid'], ':uniacid' => $_W['uniacid']));
        }
        ...
        if ($_W['ispost']) {
            ...
            if (empty($reg)) {
                $data['applytime'] = time();
                pdo_insert('superdesk_shop_enterprise_reg', $data);// TODO 标志 楼宇之窗 openid shop_enterprise_reg 已处理
            } else {
                pdo_update('superdesk_shop_enterprise_reg', $data, array('id' => $reg['id']));// TODO 标志 楼宇之窗 openid shop_enterprise_reg 已处理
            }

            $this->model->sendMessage(array('enterprise_name' => $data['enterprise_name'], 'salecate' => $data['salecate'], 'realname' => $data['realname'], 'mobile' => $data['mobile'], 'applytime' => time()), 'enterprise_apply');
            show_json(1);
        }
    ...
    public function notice()
    {
        global $_W;
        $set = $_W['shopset']['enterprise'];
        include $this->template('enterprise/register_notice');
    }
