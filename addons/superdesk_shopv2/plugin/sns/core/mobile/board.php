...
class Board_SuperdeskShopV2Page extends SnsMobilePage
{
    ...
    public function best()
    {
        ...
        $_W['shopshare'] = array(
            'title'  => (!empty($board['share_title']) ? $board['share_title'] : $board['title']),
            'imgUrl' => (!empty($board['share_icon']) ? tomedia($board['share_icon']) : tomedia($board['logo'])),
            'link'   => mobileUrl('sns', array(), true),
            'desc'   => (!empty($board['share_desc']) ? $board['share_desc'] : $board['desc'])
        );
    ...
    public function getlist()
    {
        ...
        foreach ($list as &$row) {
            ...
            $row['postcount']  = pdo_fetchcolumn('select count(*) from ' . tablename('superdesk_shop_sns_post') . ' where pid=:pid and deleted = 0 limit 1', array(':pid' => $row['id']));
    ...
    public function relate()
    {
        ...
        $sql       = 'select id,title,logo,`desc`  from ' . tablename('superdesk_shop_sns_board') . ' where 1 ' . $condition . ' order by displayorder desc';
        $list      = pdo_fetchall($sql, $params);
        foreach ($list as &$row) {
    ...
    public function get_boardlist()
    {
        ...
        $list = pdo_fetchall('SELECT sb.id,sb.title,sb.logo FROM ' . tablename('superdesk_shop_sns_board') . ' as sb' . "\n" . '                left join ' . tablename('superdesk_shop_sns_category') . ' as sc on sc.id = sb.cid' . "\n" . '                WHERE 1 ' . $condition . ' and sc.enabled = 1  ORDER BY sb.displayorder DESC limit ' . (($pindex - 1) * $psize) . ',' . $psize, $params);
        foreach ($list as &$row) {
            $row['postcount']   = $this->model->getPostCount($row['id']);
            $row['followcount'] = $this->model->getFollowCount($row['id']);
            $row['logo']        = tomedia($row['logo']);
        ...
        unset($row);
        $total = pdo_fetchcolumn('SELECT count(1) FROM ' . tablename('superdesk_shop_sns_board') . ' as sb WHERE 1 ' . $condition, $params);
        show_json(1, array('list' => $list, 'pagesize' => $psize, 'total' => $total));
    }
    ...
    public function follow()
    {
        ...
        $follow = pdo_fetch('select id from ' . tablename('superdesk_shop_sns_board_follow') . ' where bid=:bid and openid=:openid limit 1', array(':bid' => $bid, ':openid' => $_W['openid']));
        if (!empty($follow)) {
            pdo_delete('superdesk_shop_sns_board_follow', array('id' => $follow['id']));
            show_json(1, array('isfollow' => false));
            return;
        }
        $follow = array('uniacid' => $_W['uniacid'], 'bid' => $bid, 'openid' => $_W['openid'], 'createtime' => time());
        pdo_insert('superdesk_shop_sns_board_follow', $follow);
