...
class Member_increase_SuperdeskShopV2Page extends WebPage
{
    public function main()
    {
        ...
        if (!empty($days)) {
            ...
            while (0 <= $i) {
                ...
                $datas[] = array(
                    'date'   => $time,
                    'mcount' => pdo_fetchcolumn('select count(*) from ' . tablename('superdesk_shop_member') . ' where isagent=0  ' . $condition, $params),
                    'acount' => pdo_fetchcolumn('select count(*) from ' . tablename('superdesk_shop_member') . ' where isagent=1  ' . $condition, $params)
                );
