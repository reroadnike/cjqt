...
class Agent_SuperdeskShopV2Page extends PluginWebPage
{
    public function main()
    {
        ...
        if (!empty($_GPC['time']['start']) && !empty($_GPC['time']['end'])) {
            ...
            $condition .=
                ' AND dm.aagenttime >= :starttime ' .
                ' AND dm.aagenttime <= :endtime ';
        ...
        $sql =
            'select ' .
            '       dm.*,dm.nickname,dm.avatar,l.levelname,p.nickname as parentname,p.avatar as parentavatar ' .
            ' from ' . tablename('superdesk_shop_member') . ' dm ' .
            ' left join ' . tablename('superdesk_shop_member') . ' p on p.id = dm.agentid ' .
            ' left join ' . tablename('superdesk_shop_abonus_level') . ' l on l.id = dm.level' .
            ' where ' .
            '       dm.uniacid = ' . $_W['uniacid'] . ' and dm.isaagent =1  ' .
            $condition .
            ' ORDER BY dm.aagenttime desc';
        ...
        $total = pdo_fetchcolumn(
            ...
            ' from' . tablename('superdesk_shop_member') . ' dm  ' .
            ' left join ' . tablename('superdesk_shop_member') . ' p on p.id = dm.agentid ' .
            ' left join ' . tablename('superdesk_shop_abonus_level') . ' l on l.id = dm.aagentlevel' .
            ' where ' .
    ...
    public function query()
    {
        ...
        $ds = pdo_fetchall(
            'SELECT id,avatar,nickname,openid,core_user,realname,mobile ' .
            ' FROM ' . tablename('superdesk_shop_member') .
            ' WHERE 1 ' .
            $condition .
    ...
    public function check()
    {
        ...
        $members = pdo_fetchall(
            'SELECT ' .
            '       id,openid,nickname,realname,mobile,aagentstatus ' .
            ' FROM ' . tablename('superdesk_shop_member') .
            ' WHERE ' .
            '       id in( ' . $id . ' ) ' .
            '       AND uniacid=' . $_W['uniacid']
