...
class PosteraModel extends PluginModel
{
    ...
    public function getSceneID()
    {
        ...
        $start = 1;
        $end   = 2147483647;// mysql int(11) 最大
    ...
    public function getQR($poster, $member)
    {
        ...
        $qr = pdo_fetch(
            'select * ' .
            ' from ' . tablename('superdesk_shop_postera_qr') . // TODO 标志 楼宇之窗 openid shop_postera_qr 已处理
            ' where ' .
            '       openid=:openid ' .
            '       and acid=:acid ' .
        ...
        if (empty($qr)) {
            ...
            $ims_qrcode = array(
                ...
                'qrcid'      => $scene_id,
                'model'      => 0,
                'name'       => 'SUPERDESK_SHOPV2_POSTERA_QRCODE',
                'keyword'    => 'SUPERDESK_SHOPV2_POSTERA',
                'expire'     => $expire,
                'createtime' => time(),
                'status'     => 1,
                'url'        => $result['url'],
            ...
            pdo_insert('superdesk_shop_postera_qr', $qr);// TODO 标志 楼宇之窗 openid shop_postera_qr 已处理
            $qr['id'] = pdo_insertid();
    ...
    public function createPoster($poster, $member, $qr, $upload = true)
    {
        ...
        $path = IA_ROOT . '/addons/superdesk_shopv2/data/postera/' . $_W['uniacid'] . '/';
        ...
        if (!empty($qr['goodsid'])) {
            ...
            $goods = pdo_fetch(
                'select id,title,thumb,commission_thumb,marketprice,productprice ' .
        ...
        if (($qr['qrimg'] != $qr['current_qrimg']) || empty($qr['mediaid']) || empty($qr['createtime']) || ((($qr['createtime'] + (3600 * 24 * 3)) - 7200) < time())) {
            ...
            $qr['mediaid'] = $mediaid;
            pdo_update('superdesk_shop_postera_qr', // TODO 标志 楼宇之窗 openid shop_postera_qr 不处理
                array(
                    'mediaid'    => $mediaid,
                    'createtime' => time()
                ),
    ...
    public function getQRByTicket($ticket = '')
    {
        ...
        $qrs = pdo_fetchall(
            'select * ' .
            ' from ' . tablename('superdesk_shop_postera_qr') . // TODO 标志 楼宇之窗 openid shop_postera_qr 不处理
            ' where ' .
            '       ticket=:ticket ' .
            '       and acid=:acid ' .
            ' limit 1',
