...
if (!empty($id)) {
    $sql = 'SELECT `id`, `thumb`, `title`, `weight`, `marketprice`, `total`, `type`, `totalcnf`, `sales`, `unit`, `istime`, `timeend`, `usermaxbuy`
					FROM ' . tablename('superdesk_boardroom_s_goods') . ' WHERE `id` = :id';
    ...
    if (!empty($optionid)) {
        $option = pdo_fetch("select title,marketprice,weight,stock from " . tablename("superdesk_boardroom_s_goods_option") . " where id=:id limit 1", array(":id" => $optionid));
        if ($option) {
    ...
    // 检查用户最多购买数量
    $sql = 'SELECT SUM(`og`.`total`) AS `orderTotal` FROM ' . tablename('superdesk_boardroom_s_order_goods') . ' AS `og` JOIN ' . tablename('superdesk_boardroom_s_order') .
...
if (!$direct) {
    ...
    $list = pdo_fetchall("SELECT * FROM " . tablename('superdesk_boardroom_s_cart') . " WHERE  uniacid = '{$_W['uniacid']}' AND from_user = '{$_W['openid']}' {$condition}");
    if (!empty($list)) {
        foreach ($list as &$g) {
            $item = pdo_fetch("select id,thumb,title,weight,marketprice,total,type,totalcnf,sales,unit from " . tablename("superdesk_boardroom_s_goods") . " where id=:id limit 1", array(":id" => $g['goodsid']));
            //属性
