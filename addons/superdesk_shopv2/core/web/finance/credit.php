...
class Credit_SuperdeskShopV2Page extends WebPage
{
    protected function main($type = 'credit1')
    {
        ...
        $condition =
            ...
            ' and log.module=:module ' .
            ' and m.uniacid=:uniacid ' .
            ' and log.credittype=:credittype';
        $params    = array(
            ':uniacid'    => $_W['uniacid'],
        ...
        $sql =
            ' select ' .
            '       log.*,m.id as mid, m.realname,m.avatar,m.nickname,m.avatar, m.mobile, m.weixin,u.username ' .
            ' from ' . tablename('mc_credits_record') . ' log ' .
            '        left join ' . tablename('users') . ' u on log.operator<>0 and log.operator<>log.uid and  log.operator=u.uid' .
            '        left join ' . tablename('superdesk_shop_member') . ' m on m.uid=log.uid' . // TODO 标志 楼宇之窗 openid shop_member 不处理
