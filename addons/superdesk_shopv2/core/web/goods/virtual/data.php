...
class Data_SuperdeskShopV2Page extends ComWebPage
{
    ...
    public function main()
    {
        ...
        $psize          = 100;
        $type           = pdo_fetch('SELECT * FROM ' . tablename('superdesk_shop_virtual_type') . ' WHERE id=:id and uniacid=:uniacid and merchid=0', array(':id' => $typeid, ':uniacid' => $_W['uniacid']));
        ...
        $items = pdo_fetchall(
            'SELECT ' .
            '       d.*,o.carrier,m.avatar,m.nickname ' .
            ' FROM ' . tablename('superdesk_shop_virtual_data') . ' d ' .
            '       left join ' . tablename('superdesk_shop_member') . ' m on m.openid = d.openid and m.core_user = d.core_user and m.uniacid= d.uniacid and d.merchid=0' . // TODO 标志 楼宇之窗 openid shop_member 已处理
                ...
                if (!empty($item['fields'])) {
                    ...
                    if (!empty($noinsert)) {
                        ...
                        show_json(1, array(
                            ...
                            'url'     => webUrl('virtual/data', array('typeid' => $typeid))
                        ));

                    } else {

                        show_json(1, array(
                            'url' => webUrl('goods/virtual/data', array('typeid' => $typeid))
                        ));
                    }
                }
    ...
    public function export()
    {
        ...
        $list = pdo_fetchall(
            ...
            ' left join ' . tablename('superdesk_shop_member') . ' m on m.openid = d.openid and m.core_user = d.core_user and m.uniacid = d.uniacid and d.merchid=0' .// TODO 标志 楼宇之窗 openid shop_member 已处理
            ' left join ' . tablename('superdesk_shop_order') . ' o on o.id = d.orderid ' . // TODO 标志 楼宇之窗 openid shop_order 已处理
            ' where  1 ' . $condition . ' order by usetime desc',
            $params
        );
