...
class Increase_SuperdeskShopV2Page extends PluginWebPage
{
	public function main()
	{
		...
		if (!empty($days)) {
			...
			while (0 <= $i) {
				...
				$datas[] = array('date' => $time, 'acount' => pdo_fetchcolumn('select count(*) from ' . tablename('superdesk_shop_member') . ' where isagent=1 and status=1  ' . $condition, $params));
				--$i;
			}
		}
			...
			while ($d <= $lastday) {
				...
				$datas[] = array('date' => $d . '日', 'acount' => pdo_fetchcolumn('select count(*) from ' . tablename('superdesk_shop_member') . ' where isagent=1  ' . $condition, $params));
				++$d;
			}
		}
		 else if (!empty($year)) {
			...
			foreach ($months as $m ) {
				...
				$datas[] = array('date' => $m['data'] . '月', 'acount' => pdo_fetchcolumn('select count(*) from ' . tablename('superdesk_shop_member') . ' where isagent=1  ' . $condition, $params));
			}
		}
