...
//            $data['loading'] = save_media($data['loading']);
    ...
    public function notice()
    {
        ...
        //template_global_type 判断要推送哪个公众号

        $data = m('common')->getSysset($_GPC['template_global_type'], false);
...
//            var_dump($data);die;
            ...
            m('common')->updateSysset(array($_GPC['template_global_type'] => $data));
            m('common')->updateSysset(array('template_global_type' => $_GPC['template_global_type']));
        ...
        if (isset($data['openid'])) {
            if (!empty($data['openid'])) {
                ...
                $salers = pdo_fetchall(
                    ' select id,nickname,avatar,openid ' .
                    ' from ' . tablename('superdesk_shop_member') .
                    ' where openid in (' . implode(',', $openids) . ') ' .
                    '       and uniacid=' . $_W['uniacid']);
            }
        }
        $newtype = explode(',', $data['newtype']);
    ...
    {
        global $_W;
        $path = IA_ROOT . '/addons/superdesk_shopv2/cert';
        load()->func('file');
        mkdirs($path);
    ...
    public function payset()
    {
        ...
        if (!isset($sec['app_wechat'])) {
            $sec['app_wechat']              = array();
            $sec['app_wechat']['appid']     = '';
            $sec['app_wechat']['appsecret'] = '';
            $sec['app_wechat']['merchname'] = '';
            $sec['app_wechat']['merchid']   = '';
            $sec['app_wechat']['apikey']    = '';
        }
        // 微信支付(子商户版) start init
        if (!isset($sec['app_alipay'])) {
            $sec['app_alipay']               = array();
            $sec['app_wechat']['public_key'] = '';
        }
        if (!isset($sec['appid_sub'])) {
            $sec['appid_sub'] = '';
        }
        if (!isset($sec['sub_appid_sub'])) {
            $sec['sub_appid_sub'] = '';
        }
        if (!isset($sec['mchid_sub'])) {
            $sec['mchid_sub'] = '';
        }
        if (!isset($sec['sub_mchid_sub'])) {
            $sec['sub_mchid_sub'] = '';
        }
        // 微信支付(子商户版) end init
        ...
        // 借用微信支付(子商户版) start init
        if (!isset($sec['appid_jie_sub'])) {
            $sec['appid_jie_sub'] = '';
        }
        if (!isset($sec['sub_appid_jie_sub'])) {
            $sec['sub_appid_jie_sub'] = '';
        }
        if (!isset($sec['mchid_jie_sub'])) {
            $sec['mchid_jie_sub'] = '';
        }
        if (!isset($sec['sub_mchid_jie_sub'])) {
            $sec['sub_mchid_jie_sub'] = '';
        }
        if (!isset($sec['apikey_jie_sub'])) {
            $sec['apikey_jie_sub'] = '';
        }
        // 借用微信支付(子商户版) end init
        ...
        if ($_W['ispost']) {
            ...
            pdo_update('superdesk_shop_sysset', array('sec' => iserializer($sec)), array('uniacid' => $_W['uniacid']));
            ...
            $data['weixin_jie_buildwindow'] = intval($inputdata['weixin_jie_buildwindow']); //2019年2月13日 16:12:40 zjh 楼宇之窗借权支付,未完成,勿更新到线上
        ...
        $url = $_W['siteroot'] . 'addons/superdesk_shopv2/payment/wechat/notify.php';
        load()->func('communication');
        $resp = ihttp_get($url);
        include $this->template();
        exit();
    ...
    public function templat()
    {
        ...
        $styles = array();
        $dir    = IA_ROOT . '/addons/superdesk_shopv2/template/mobile/';
    ...
    public function wap()
    {
        ...
        $styles = array();
        $dir    = IA_ROOT . '/addons/superdesk_shopv2/template/account/';
        if ($handle = opendir($dir)) {
            while (($file = readdir($handle)) !== false) {
                if (($file != '..') && ($file != '.')) {
                    if (is_dir($dir . '/' . $file)) {
