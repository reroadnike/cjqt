...
class Index_SuperdeskShopV2Page extends PluginWebPage
{
    public function main()
    {
        ...
        $list = pdo_fetchall(
            'SELECT * '.
            ' FROM ' . tablename('superdesk_shop_task_poster') .
            ' WHERE 1 ' .
            $condition .
        ...
        unset($row);
        $total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('superdesk_shop_task_poster') . ' where 1 ' . $condition . ' ', $params);
        $pager = pagination($total, $pindex, $psize);
        foreach ($list as $key => $val) {
            $viewcount               = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('superdesk_shop_task_joiner') . ' where uniacid=:uniacid and task_id=:task_id and task_type=1', array(':uniacid' => $_W['uniacid'], ':task_id' => $val['id']));
    ...
    protected function post()
    {
        ...
        if ($_W['ispost']) {
            ...
            if (!empty($id)) {
                ...
                if (intval($_GPC['needcount']) < $item['needcount']) {
                    ...
                    $task_count = pdo_fetchcolumn(
                        'select COUNT(*) from ' . tablename('superdesk_shop_task_join') .
                        ' where uniacid=:uniacid and task_id=:task_id and task_type=1 and failtime>' . time(),
                        array(
                            ':uniacid' => $_W['uniacid'],
                            ':task_id' => $id
                        )
            ...
            if (($item['keyword'] != $data['keyword']) && !empty($keyword)) {
                if ($keyword['name'] != 'superdesk_shopv2:task:' . $id) {
                    show_json(0, '关键字已存在!');
                }
            }
            if (!empty($id)) {
                pdo_update('superdesk_shop_task_poster', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
                $updatesql = 'UPDATE ' . tablename('superdesk_shop_task_join') . ' SET  failtime = addtime+' . $data['days'] . ',needcount=:needcount WHERE uniacid = :uniacid AND task_id=:task_id AND task_type=1 AND failtime>' . time();
                pdo_query($updatesql, array(':needcount' => $data['needcount'], ':uniacid' => $_W['uniacid'], ':task_id' => $id));
                plog('task.edit', '修改活动海报 ID: ' . $id . '<br>' . (($data['isopen'] ? '允许非营销商生成自己的海报 -- 是<br>' : '允许非营销商生成自己的海报 -- 否<br>')) . (($data['bedown'] ? '扫码关注成为下线 -- 是<br>' : '扫码关注成为下线 -- 否<br>')) . (($data['beagent'] ? '扫码关注成为营销商 -- 是' : '扫码关注成为营销商 -- 否')));
            } else {
                pdo_insert('superdesk_shop_task_poster', $data);
                $id = pdo_insertid();
                plog('task.add', '修改活动海报 ID: ' . $id . '<br>' . (($data['isopen'] ? '允许非营销商生成自己的海报<br>' : '不允许非营销商生成自己的海报<br>')) . (($data['bedown'] ? '扫码关注成为下线 -- 是<br>' : '扫码关注成为下线 -- 否<br>')) . (($data['beagent'] ? '扫码关注成为营销商 -- 是' : '扫码关注成为营销商 -- 否')));
            }
            $rule         = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name  limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'superdesk_shopv2', ':name' => 'superdesk_shopv2:task:' . $id));
            $rule_data    = array('uniacid' => $_W['uniacid'], 'name' => 'superdesk_shopv2:task:' . $id, 'module' => 'superdesk_shopv2', 'displayorder' => 0, 'status' => $data['status']);
            $keyword_data = array('uniacid' => $_W['uniacid'], 'module' => 'superdesk_shopv2', 'content' => trim($data['keyword']), 'type' => 1, 'displayorder' => 0, 'status' => $data['status']);
            if (empty($rule)) {
            ...
            $ruleauto = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name  limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'superdesk_shopv2', ':name' => 'superdesk_shopv2:task:auto'));
            if (empty($ruleauto)) {
                $rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'superdesk_shopv2:task:auto', 'module' => 'superdesk_shopv2', 'displayorder' => 0, 'status' => 1);
                pdo_insert('rule', $rule_data);
                $rid          = pdo_insertid();
                $keyword_data = array('uniacid' => $_W['uniacid'], 'rid' => $rid, 'module' => 'superdesk_shopv2', 'content' => 'SUPERDESK_SHOPV2_TASK', 'type' => 1, 'displayorder' => 0, 'status' => 1);
                pdo_insert('rule_keyword', $keyword_data);
            }
        ...
        $default_text = pdo_fetchcolumn('SELECT `data` FROM ' . tablename('superdesk_shop_task_default') . ' WHERE uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid']));
    ...
    public function delete()
    {
        ...
        $posters = pdo_fetchall('SELECT id,title,keyword FROM ' . tablename('superdesk_shop_task_poster') . ' WHERE id in ( ' . $id . ' ) and uniacid=' . $_W['uniacid']);
        foreach ($posters as $poster) {
            ...
            pdo_update('superdesk_shop_task_poster', array('is_delete' => 1), array('id' => $poster['id'], 'uniacid' => $_W['uniacid']));
    ...
    public function clear()
    {
        ...
        global $_GPC;
        load()->func('file');
        @rmdirs(IA_ROOT . '/addons/superdesk_shopv2/data/task/poster/' . $_W['uniacid']);
        @rmdirs(IA_ROOT . '/addons/superdesk_shopv2/data/task/qrcode/' . $_W['uniacid']);
        $acid = pdo_fetchcolumn('SELECT acid FROM ' . tablename('account_wechats') . ' WHERE `uniacid`=:uniacid LIMIT 1', array(':uniacid' => $_W['uniacid']));
        pdo_update('superdesk_shop_task_poster_qr', array('mediaid' => ''), array('acid' => $acid));
    ...
    public function delreward()
    {
        ...
        if ($type == 1) {
            $item                = pdo_fetch('SELECT `id`,`reward_data` FROM ' . tablename('superdesk_shop_task_poster') . ' WHERE id =:id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
            ...
            pdo_update('superdesk_shop_task_poster', array('reward_data' => $item['reward_data']), array('id' => $id, 'uniacid' => $_W['uniacid']));
            $item['reward_data'] = unserialize($item['reward_data']);
            ...
            $item['reward_data'] = serialize($item['reward_data']);
            pdo_update(tablename('superdesk_shop_task_poster'), array('reward_data' => $item['reward_data']), array('id' => $id, 'uniacid' => $_W['uniacid']));
            $item['reward_data'] = unserialize($item['reward_data']);
