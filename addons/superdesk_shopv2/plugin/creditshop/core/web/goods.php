...
class Goods_SuperdeskShopV2Page extends PluginWebPage
{
    ...
    protected function post()
    {
        ...
        if (empty($item['goodstype'])) {
            $goods = pdo_fetch('SELECT * FROM ' . tablename('superdesk_shop_goods') . ' WHERE id =:id and status=1 and deleted=0 and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $item['goodsid']));
        } else {
            $coupon = pdo_fetch('SELECT * FROM ' . tablename('superdesk_shop_coupon') . ' WHERE id =:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $item['couponid']));
        }
        include $this->template();
    }
    ...
    public function delete()
    {
        ...
        foreach ($items as $item) {
            pdo_update('superdesk_shop_creditshop_goods', array('deleted' => 1), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
    ...
    public function property()
    {
        ...
        if (in_array($type, array('istop', 'isrecommand', 'status', 'displayorder'))) {
            ...
            pdo_update(
                'superdesk_shop_creditshop_goods',
                array(
                    $type => $value
                ),
                array(
    ...
    public function status()
    {
        ...
        pdo_query(
            'update ' . tablename('superdesk_shop_creditshop_goods') .
            ' set `status` = ' . $status .
            ' where '.
            '       uniacid=' . $_W['uniacid'] .
