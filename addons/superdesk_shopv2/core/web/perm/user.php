...
class User_SuperdeskShopV2Page extends WebPage
{
	public function main()
	{
		...
		$list = pdo_fetchall(
			' SELECT u.*,r.rolename,user.lastvisit '.
            ' FROM ' . tablename('superdesk_shop_perm_user') . ' u  ' .
			' left join ' . tablename('superdesk_shop_perm_role') . ' r on u.roleid =r.id  ' .
			' left join ' . tablename('users') . ' user on user.uid = u.uid  ' .
            ' WHERE 1 ' . $condition . ' ORDER BY id desc LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize, $params);
		$total = pdo_fetchcolumn(
			' SELECT count(*) '.
            ' FROM ' . tablename('superdesk_shop_perm_user') . ' u  ' . ' left join ' . tablename('superdesk_shop_perm_role') . ' r on u.roleid =r.id  ' .
            ' WHERE 1 ' . $condition . ' ', $params);
		$pager = pagination($total, $pindex, $psize);
		$roles = pdo_fetchall(
            ' select id,rolename '.
            ' from ' . tablename('superdesk_shop_perm_role') .
            ' where uniacid=:uniacid and deleted=0',
	...
	protected function post()
	{
		...
		$item = pdo_fetch(
            ' SELECT * '.
            ' FROM ' . tablename('superdesk_shop_perm_user') .
		...
		if ($_W['ispost']) {
			...
			if (!empty($item['id'])) {
				...
				pdo_update('superdesk_shop_perm_user', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
				plog('perm.user.edit',
                    '编辑操作员 ID: ' . $id .
                    ' 用户名: ' . $data['username'] . ' ');
			}
				...
				pdo_insert('superdesk_shop_perm_user', $data);
				$id = pdo_insertid();
	...
	public function delete()
	{
		...
		foreach ($items as $item ) {
			pdo_delete('superdesk_shop_perm_user', array('id' => $item['id']));
	...
	public function status()
	{
		...
		foreach ($items as $item ) {
			pdo_update('superdesk_shop_perm_user', array('status' => $status), array('id' => $item['id']));
			plog('perm.user.edit', 
                '修改操作员状态 ID: ' . $item['id'] . ' 操作员名称: ' . $item['username'] . ' 状态: ' . (($status == 0 ? '禁用' : '启用')));
		}
