...
class Order_SuperdeskShopV2Page extends WebPage
{
    protected function orderData($status, $st)
    {
        ...
        if (($condition != ' o.uniacid = :uniacid and o.ismr=0 and o.deleted=0 and o.isparent=0') || !empty($sqlcondition)) {
            $sql =
                ' select o.* , a.realname as arealname,a.mobile as amobile,a.province as aprovince ,a.city as acity , a.area as aarea,a.address as aaddress, ' .
                '        d.dispatchname,m.nickname,m.id as mid,m.realname as mrealname,m.mobile as mmobile,sm.id as salerid,sm.nickname as salernickname, ' .
                '        s.salername,r.rtype,r.status as rstatus,o.agentid ' .
                ...
                '       left join ' . tablename('superdesk_shop_member') . ' sm on sm.openid = s.openid and sm.uniacid=s.uniacid' . // TODO 标志 楼宇之窗 openid shop_order 待处理
                ' ' .
                $sqlcondition .
                ' where ' .
                $condition .
                ' ' .
                $statuscondition .
                ' GROUP BY o.id ORDER BY o.createtime DESC  ';
            ...
            if (!empty($list)) {
                ...
                $refundid_array     = pdo_fetchall('SELECT id,rtype,status as rstatus FROM ' . tablename('superdesk_shop_order_refund') . ' WHERE id IN (' . $refundid . ')', array(), 'id');
                $openid_array       = pdo_fetchall('SELECT openid,nickname,id as mid,realname as mrealname,mobile as mmobile FROM ' . tablename('superdesk_shop_member') . ' WHERE openid IN (' . $openid . ') AND uniacid=' . $_W['uniacid'], array(), 'openid');
                $addressid_array    = pdo_fetchall(
                    ' SELECT ' .
                    '       id,realname as arealname,mobile as amobile,province as aprovince ,city as acity , area as aarea, town as atown,address as aaddress ' .
                ...
                $dispatchid_array   = pdo_fetchall('SELECT id,dispatchname FROM ' . tablename('superdesk_shop_dispatch') . ' WHERE id IN (' . $dispatchid . ')', array(), 'id');
                $verifyopenid_array = pdo_fetchall('SELECT sm.id as salerid,sm.nickname as salernickname,sm.openid,s.salername FROM ' . tablename('superdesk_shop_saler') . ' s LEFT JOIN ' . tablename('superdesk_shop_member') . ' sm ON sm.openid = s.openid and sm.uniacid=s.uniacid WHERE s.openid IN (' . $verifyopenid . ')', array(), 'openid');
                foreach ($list as $key => &$value) {
        ...
        if (!empty($list)) {
            foreach ($list as &$value) {
                ...
                $order_goods = pdo_fetchall(
                    ' select g.id,g.title,g.thumb,g.goodssn,og.goodssn as option_goodssn, g.productsn,og.productsn as option_productsn, og.total,og.price, ' .
                    '        og.optionname as optiontitle, og.realprice,og.changeprice,og.oldprice, ' .
                    '        og.commission1,og.commission2,og.commission3,og.commissions, ' .
                    '        og.diyformdata,og.diyformfields,op.specs,g.merchid  ' .
        ...
        if ($_GPC['export'] == 1) {
            ...
            $columns = array(
                ...
                array('title' => '营销商id', 'field' => 'agent_id', 'width' => 12),
                array('title' => '营销商昵称', 'field' => 'agent_nickname', 'width' => 12),
                array('title' => '营销商姓名', 'field' => 'agent_realname', 'width' => 12),
                array('title' => '营销商手机号', 'field' => 'agent_mobile', 'width' => 12),
        ...
        if (($condition != ' o.uniacid = :uniacid and o.ismr=0 and o.deleted=0 and o.isparent=0') || !empty($sqlcondition)) {
            ...
            $t = pdo_fetch(
                ...
                ' FROM ' . tablename('superdesk_shop_order') . // TODO 标志 楼宇之窗 openid shop_order 不处理
                ' WHERE uniacid = :uniacid and ismr=0 and deleted=0 and isparent=0 ' . $status_condition,
                $paras
            );
        }
        $total      = $t['count'];
        ...
        $stores     = pdo_fetchall('select id,storename from ' . tablename('superdesk_shop_store') . ' where uniacid=:uniacid ', array(':uniacid' => $uniacid));
