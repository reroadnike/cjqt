...
class Common_SuperdeskShopV2Model
{
    ...
    public function updateSysset($values, $uniacid = 0)
    {
        ...
        if (empty($setdata)) {
            ...
            pdo_insert(
                'superdesk_shop_sysset',
                array(
                    'sets'    => iserializer($values),
                    'uniacid' => $uniacid
                )
            );
...
//            var_dump($sets);die;
    ...
    public function updatePluginset($values, $uniacid = 0)
    {
        ...
        if (empty($setdata)) {
            ...
            pdo_insert(
                'superdesk_shop_sysset',
                array(
                    'plugins' => iserializer($values),
                    'uniacid' => $uniacid
                )
            );

        } else {
            //由于可能原先有setdata但没有plugins.就会导致出错. zjh 2018年8月10日 17:16:00
            if (empty($setdata['plugins'])) {
                pdo_update(
                    'superdesk_shop_sysset',
                    array(
                        'plugins' => iserializer($values)
    ...
    public function alipay_build($params, $alipay = array(), $type = 0, $openid = '')
    {
        ...
        if (empty($type)) {
            $set['notify_url'] = $_W['siteroot'] . 'addons/superdesk_shopv2/payment/alipay/notify.php';
    ...
    public function AliPayBarcode($params, $config)
    {
        ...
        $config['notify_url']  = $_W['siteroot'] . 'addons/superdesk_shopv2/payment/alipay/notify.php';
    ...
    public function wechat_build($params, $wechat, $type = 0, $trade_type = 'JSAPI')
    {
        global $_W;
        ...
        if ($wechat['version'] == 1) {
            ...
            $package['device_info']      = 'superdesk_shopv2';
            $package['out_trade_no']     = $params['tid'];
            $package['total_fee']        = $params['fee'] * 100;
            $package['fee_type']         = '1';
            $package['notify_url']       = $_W['siteroot'] . 'addons/superdesk_shopv2/payment/wechat/notify.php';
    ...
    public function wechat_child_build($params, $wechat, $type = 0, $trade_type = 'JSAPI')
    {
        global $_W;
        load()->func('communication');
        $package                     = array();
        ...
        $package['nonce_str']        = random(32);
        $package['body']             = $params['title'];
        $package['device_info']      = ((isset($params['device_info']) ? 'superdesk_shopv2:' . $params['device_info'] : 'superdesk_shopv2'));
        ...
        $package['notify_url']  = ((empty($params['notify_url']) ? $_W['siteroot'] . 'addons/superdesk_shopv2/payment/wechat/notify.php' : $params['notify_url']));
        $package['trade_type']  = $trade_type;
    ...
    public function getAccount()
    {
        ...
        $acid = pdo_fetchcolumn('SELECT acid FROM ' . tablename('account_wechats') . ' WHERE `uniacid`=:uniacid LIMIT 1',
    ...
    public function createNO($table, $field, $prefix)
    {
        ...
        while (1) {
            $count = pdo_fetchcolumn(
                'select count(*) from ' . tablename('superdesk_shop_' . $table) .
                ' where ' . $field . '=:billno limit 1',
                array(
                    ':billno' => $billno
                )
            );
    ...
    public function paylog($log = '')
    {
        ...
        if (!empty($openpaylog)) {
            ...
            $path = IA_ROOT . '/addons/superdesk_shopv2/data/paylog/' . $_W['uniacid'] . '/' . date('Ymd');
    ...
    {
        $file     = IA_ROOT . '/addons/superdesk_shopv2/static/js/dist/area/Area.xml';
    ...
    {
        return pdo_fetchall('SELECT  a.uniacid,a.name FROM ' . tablename('superdesk_shop_sysset') . ' s  ' . ' left join ' . tablename('account_wechats') . ' a on a.uniacid = s.uniacid' . ' left join ' . tablename('account') . ' acc on acc.uniacid = a.uniacid where acc.isdeleted=0 GROUP BY uniacid');
    }
    ...
    public function getCopyright($ismanage = false)
    {
        ...
        if (!is_array($copyrights)) {
            $copyrights = pdo_fetchall('select *  from ' . tablename('superdesk_shop_system_copyright'));
