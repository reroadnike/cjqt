...
if (!class_exists('CommissionModel')) {
    ...
    class CommissionModel extends PluginModel
    {
        ...
        public function getSet($uniacid = 0)
        {
            ...
            $set['texts'] = array(
                'agent'             => (empty($set['texts']['agent']) ? '营销商' : $set['texts']['agent']),
                'shop'              => (empty($set['texts']['shop']) ? '小店' : $set['texts']['shop']),
                'myshop'            => (empty($set['texts']['myshop']) ? '我的小店' : $set['texts']['myshop']),
                'center'            => (empty($set['texts']['center']) ? '营销中心' : $set['texts']['center']),
                'become'            => (empty($set['texts']['become']) ? '成为营销商' : $set['texts']['become']),
                'withdraw'          => (empty($set['texts']['withdraw']) ? '提现' : $set['texts']['withdraw']),
                'commission'        => (empty($set['texts']['commission']) ? '佣金' : $set['texts']['commission']),
                'commission1'       => (empty($set['texts']['commission1']) ? '营销佣金' : $set['texts']['commission1']),
                ...
                'order'             => (empty($set['texts']['order']) ? '营销订单' : $set['texts']['order']),
        ...
        public function calculate($orderid = 0, $update = true)
        {
            ...
            $order  = pdo_fetch(
                ' select agentid,price,goodsprice,deductcredit2,discountprice,isdiscountprice,dispatchprice,changeprice,ispackage,packageid ' .
            ...
            $goods   = pdo_fetchall(
                ' select og.id,og.realprice,og.total,g.hasoption,og.goodsid,og.optionid,g.hascommission,g.nocommission, g.commission1_rate, ' .
                '        g.commission1_pay,g.commission2_rate,g.commission2_pay,g.commission3_rate,g.commission3_pay,g.commission,og.commissions ' .
                    ...
                    if (empty($cinfo['nocommission'])) {
                        ...
                        if (0 < $order['ispackage']) {
                            ...
                            if (!empty($cinfo['optionid'])) {
                                ...
                                $packoption = pdo_fetch('select commission1,commission2,commission3 from ' . tablename('superdesk_shop_package_goods_option') . "\r\n" . '                                            where uniacid = ' . $_W['uniacid'] . ' and pid = ' . $order['packageid'] . ' and optionid = ' . $cinfo['optionid'] . ' ');
                            } else {
                                $packoption = pdo_fetch('select commission1,commission2,commission3 from ' . tablename('superdesk_shop_package_goods') . "\r\n" . '                                            where uniacid = ' . $_W['uniacid'] . ' and pid = ' . $order['packageid'] . ' and goodsid = ' . $cinfo['goodsid'] . ' ');
                            }
                    ...
                    if ($update) {
                        ...
                        if (!empty($agentid)) {
                            ...
                            if (($m1['isagent'] == 1) && ($m1['status'] == 1)) {
                                ...
                                $l1 = $this->getLevel($m1['openid'], $m1['core_user']);
                                ...
                                if (!empty($m1['agentid'])) {
                                    ...
                                    $l2 = $this->getLevel($m2['openid'], $m2['core_user']);
                                    ...
                                    if (!empty($m2['agentid'])) {
                                        ...
                                        $l3 = $this->getLevel($m3['openid'], $m3['core_user']);
         ...
         * @return mixed
         */
        public function getInfo($openid, $core_user = 0, $options = NULL)
        {
            ...
            $agentLevel = $this->getLevel($openid, $core_user);
            ...
            if (1 <= $level) {
                ...
                if (in_array('total', $options)) {
                    ...
                    $level1_commissions = pdo_fetchall(
                        ' select og.commission1,og.commissions ' .
                        ' from ' . tablename('superdesk_shop_order_goods') . ' og ' .// TODO 标志 楼宇之窗 openid shop_order_goods 不处理
                        ' left join  ' . tablename('superdesk_shop_order') . ' o on o.id = og.orderid' . // TODO 标志 楼宇之窗 openid shop_order 不处理
                        ' where o.agentid=:agentid and o.status>=1 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0',
                        array(
                            ':uniacid' => $_W['uniacid'],
                            ':agentid' => $member['id']
                        )
                ...
                if (0 < $level1) {
                    ...
                    if (in_array('ordercount0', $options)) {
                        ...
                        $level2_ordercount = pdo_fetch(
                            ...
                            ' left join  ' . tablename('superdesk_shop_order_goods') . ' og on og.orderid=o.id ' .// TODO 标志 楼宇之窗 openid shop_order_goods 不处理
                            ' where ' .
                            '       o.agentid in( ' . implode(',', array_keys($level1_agentids)) . ')  ' .
                            '       and o.status>=0  ' .
                            '       and og.status2>=0 ' .
                            '       and og.nocommission=0 ' .
                            '       and o.uniacid=:uniacid ' .
                    ...
                    if (in_array('lock', $options)) {
                        ...
                        $level2_commissions1 = pdo_fetchall(
                            ...
                            ' where o.agentid in( ' . implode(',', array_keys($level1_agentids)) . ')  ' .
                            '       and (' . $time . ' - o.finishtime <= ' . $day_times . ') ' .
                            '       and og.status2=0 and o.status>=3 ' .
                            '       and og.nocommission=0 ' .
                            '       and o.uniacid=:uniacid ' .
                            '       and o.isparent=0',
                            array(
                    ...
                    $level2_agentids = pdo_fetchall(
                        'select id ' .
                        ' from ' . tablename('superdesk_shop_member') .
                        ' where ' .
                        '       agentid in( ' . implode(',', array_keys($level1_agentids)) . ') ' .
                        '       and isagent=1 and status=1 and uniacid=:uniacid',
                        array(
                            ':uniacid' => $_W['uniacid']
                        ),
                        'id'
                ...
                if (0 < $level2) {
                    ...
                    if (in_array('ordercount0', $options)) {
                        ...
                        $level3_ordercount = pdo_fetch(
                            ...
                            ' from ' . tablename('superdesk_shop_order') . ' o ' .// TODO 标志 楼宇之窗 openid shop_order 不处理
                            ' left join  ' . tablename('superdesk_shop_order_goods') . ' og on og.orderid=o.id ' .// TODO 标志 楼宇之窗 openid shop_order_goods 不处理
                            ' where o.agentid in( ' . implode(',', array_keys($level2_agentids)) . ') ' .
                            '       and o.status>=0 ' .
                            '       and og.status3>=0 ' .
                            '       and og.nocommission=0  ' .
                            '       and o.uniacid=:uniacid ' .
                    ...
                    if (in_array('total', $options)) {
                        ...
                        $level3_commissions = pdo_fetchall(
                            ' select og.commission3,og.commissions ' .
                            ' from ' . tablename('superdesk_shop_order_goods') . ' og ' .// TODO 标志 楼宇之窗 openid shop_order_goods 不处理
                            ' left join  ' . tablename('superdesk_shop_order') . ' o on o.id = og.orderid' .// TODO 标志 楼宇之窗 openid shop_order 不处理
                            ' where o.agentid in( ' . implode(',', array_keys($level2_agentids)) . ') ' .
                            '       and o.status>=1 ' .
                    ...
                    $level3_agentids = pdo_fetchall(
                        'select id ' .
                        ' from ' . tablename('superdesk_shop_member') .
                        ' where ' .
                        '       uniacid=:uniacid ' .
                        '       and agentid in( ' . implode(',', array_keys($level2_agentids)) . ') ' .
                        '       and isagent=1 ' .
                        '       and status=1',
                        array(
                            ':uniacid' => $_W['uniacid']
        ...
        public function getAgents($orderid = 0)
        {
            ...
            $order = pdo_fetch(
                ' select id,agentid,openid,core_user ' .
        ...
        public function getAgentsByMember($openid = '', $core_user = '', $num = 3)
        {
            global $_W;
            global $_GPC;
            ...
            $m = m('member')->getMember($openid, $core_user);// TODO 已处理

            if (!empty($m['agentid'])) {

                $m1 = m('member')->getMember($m['agentid']);// TODO 待处理
                ...
                if (!empty($m1) && ($m1['isagent'] == 1) && ($m1['status'] == 1) && (0 < $num)) {
                    ...
                    if (!empty($m1['agentid'])) {
                        ...
                        $m2 = m('member')->getMember($m1['agentid']);// TODO 待处理
                        ...
                        if (!empty($m2) && ($m2['isagent'] == 1) && ($m2['status'] == 1) && (1 < $num)) {
                            ...
                            if (!empty($m2['agentid'])) {
                                ...
                                $m3 = m('member')->getMember($m2['agentid']);// TODO 待处理
        ...
        public function getAgentsDownNum($openid = NULL, $core_user = 0)
        {
            global $_W;

            $openid    = ((isset($openid) ? $openid : $_W['openid']));
            $core_user = ((isset($core_user) ? $core_user : $_W['core_user']));

            $set    = $this->getSet();
            $member = $this->getInfo($openid, array(), $core_user);
            ...
            if ((2 <= $set['level']) && (0 < $levelcount1)) {
                ...
                $level2 = (int)pdo_fetchcolumn(
                    'select count(*) ' .
                    ' from ' . tablename('superdesk_shop_member') .
                    ' where ' .
                    '       agentid in( ' . implode(',', array_keys($member['level1_agentids'])) . ') ' .
                    '       and uniacid=:uniacid ' .
                    ' limit 1',
                    array(
                        ':uniacid' => $_W['uniacid']
            ...
            if ((3 <= $set['level']) && (0 < $levelcount2)) {
                ...
                $level3 = (int)pdo_fetchcolumn(
                    'select count(*) ' .
                    ' from ' . tablename('superdesk_shop_member') .
                    ' where ' .
                    '       agentid in( ' . implode(',', array_keys($member['level2_agentids'])) . ') ' .
                    '       and uniacid=:uniacid ' .
                    ' limit 1',
                    array(
                        ':uniacid' => $_W['uniacid']
        ...
        public function isAgent($openid, $core_user = 0)
        {
            if (empty($openid)) {
                return false;
            }
        ...
        public function getCommission($goods)
        {
            ...
            if ($goods['hascommission'] == 1) {
                ...
                $level   = $this->getLevel($_W['openid'], $_W['core_user']);
                $levelid = 'default';
                ...
                $openid = m('user')->getOpenid();// 待处理

                $level = $this->getLevel($openid, $_W['core_user']);
        ...
        public function createGoodsImage($goods)
        {
            ...
            $me = m('member')->getMember($_W['openid'], $_W['core_user']);
        ...
        public function checkAgent($openid = '', $core_user = 0)// TODO 标志 楼宇之窗 openid shop_order 已处理
        {
            global $_W;
            global $_GPC;
            ...
            if ($parent_is_agent) {
                ...
                if ($parent['openid'] != $openid) {
                    ...
                    if ($clickcount <= 0) {
                        ...
                        pdo_insert(
                            'superdesk_shop_commission_clickcount', // TODO 标志 楼宇之窗 openid shop_commission_clickcount 已处理
                            $click
                        );
                    ...
                    if (empty($become_child)) {
                        ...
                        if (empty($member['fixagentid'])) {
                            ...
                            if ($author) {
                                ...
                                pdo_update('superdesk_shop_member', array(
                                    'agentid'   => $parent['id'],
                                    'childtime' => $time,
                                    'authorid'  => $authorid
                                ), array(
                            ...
                            if ($author) {
                                ...
                                if (!empty($author_set['become']) && (($author_set['become'] == '2') || ($author_set['become'] == '5'))) {
                                    ...
                                    $getAgentsDownNum = $this->getAgentsDownNum($parent['openid'], $parent['core_user']);
        ...
        public function checkOrderConfirm($orderid = '0')
        {
            ...
            $order = pdo_fetch(
                ' select id,openid,core_user,ordersn,goodsprice,agentid,paytime ' .
                ' from ' . tablename('superdesk_shop_order') . // TODO 标志 楼宇之窗 openid shop_order 已处理
                ' where ' .
                '       id=:id ' .
                '       and status>=0 ' .
        ...
        public function checkOrderFinish($orderid = '')
        {
            ...
            $order = pdo_fetch(
                ' select id,openid, core_user, ordersn,goodsprice,agentid,finishtime ' .
                ' from ' . tablename('superdesk_shop_order') . // TODO 标志 楼宇之窗 openid shop_order 已处理
                ' where ' .
                '       id=:id ' .
                '       and status>=3 ' .
            ...
            if (!empty($member['agentid'])) {
                ...
                if (!empty($parent) && ($parent['isagent'] == 1) && ($parent['status'] == 1)) {
                    ...
                    $order_goods = pdo_fetchall(
                        ' select ' .
                        '       g.id,g.title,' .
                        '       og.total,og.realprice,og.price,og.optionname as optiontitle,g.noticeopenid,g.noticetype,og.commission1,og.commissions ' .
                        ' from ' . tablename('superdesk_shop_order_goods') . ' og ' . // TODO 标志 楼宇之窗 openid shop_order_goods 不处理
                        ' left join ' . tablename('superdesk_shop_goods') . ' g on g.id=og.goodsid ' .
                        ' where ' .
                        '       og.uniacid=:uniacid ' .
        ...
        public function getShop($m)
        {
            ...
            $shop = pdo_fetch(
                'select * ' .
                ' from ' . tablename('superdesk_shop_commission_shop') .
                ' where ' .
                '       uniacid=:uniacid ' .
                '       and mid=:mid ' .
                ' limit 1',
        ...
        public function getLevel($openid, $core_user = 0)
        {
            ...
            $shop_member = m('member')->getMember($openid, $core_user);

            if (empty($shop_member['agentlevel'])) {
                return false;
            }
        ...
        public function upgradeLevelByOrder($openid, $core_user = 0)
        {
            global $_W;
            ...
            // 0 没有绑定楼宇之窗 1 关联绑定楼宇之窗 开启了绑定后如果 core_user为0是不对的
            if (SUPERDESK_SHOPV2_IS_BUILD_WINDOW == 1 && empty($core_user)) {
                return false;
            }
            ...
            if (($leveltype == 4) || ($leveltype == 5)) {
                ...
                if (!empty($shop_member['agentnotupgrade'])) {
                    return;
                }

                $oldlevel = $this->getLevel($shop_member['openid'], $shop_member['core_user']);
                ...
                pdo_update('superdesk_shop_member', array('agentlevel' => $newlevel['id']), array('id' => $shop_member['id']));
                $this->sendMessage($shop_member['openid'], $shop_member['core_user'], array('nickname' => $shop_member['nickname'], 'oldlevel' => $oldlevel, 'newlevel' => $newlevel), TM_COMMISSION_UPGRADE);
                return;
            }
            ...
            if ((0 <= $leveltype) && ($leveltype <= 3)) {
                ...
                if (!empty($shop_member['agentid'])) {
                    $m1 = m('member')->getMember($shop_member['agentid']);
                    if (!empty($m1)) {
                        $agents[] = $m1;
                ...
                foreach ($agents as $agent) {
                    ...
                    $oldlevel = $this->getLevel($agent['openid'], $agent['core_user']);
                    ...
                    $this->sendMessage($agent['openid'], $agent['core_user'], array('nickname' => $agent['nickname'], 'oldlevel' => $oldlevel, 'newlevel' => $newlevel), TM_COMMISSION_UPGRADE);
                }
            }
        }

        public function upgradeLevelByAgent($openid, $core_user = 0)
        {
            global $_W;
            ...
            $info = $this->getInfo($shop_member['id'], array());

            if (($leveltype == 6) || ($leveltype == 8)) {

                $agents = array($shop_member);
                ...
                foreach ($agents as $agent) {
                    ...
                    if ($leveltype == 6) {
                        ...
                        $downs1    = pdo_fetchall('select id from ' . tablename('superdesk_shop_member') . ' where agentid=:agentid and uniacid=:uniacid ', array(':agentid' => $shop_member['id'], ':uniacid' => $_W['uniacid']), 'id');
            ...
            if ($leveltype == 7) {
                ...
                $downcount = pdo_fetchcolumn(
                    'select count(*) from ' . tablename('superdesk_shop_member') .
                    ' where agentid=:agentid and uniacid=:uniacid ',
                    array(':agentid' => $shop_member['id'], ':uniacid' => $_W['uniacid']));
                $newlevel  = pdo_fetch(
                    'select * from ' . tablename('superdesk_shop_commission_level') .
                    ' where uniacid=:uniacid  and ' . $downcount . ' >= downcount and downcount>0  order by downcount desc limit 1',
                    array(':uniacid' => $_W['uniacid']));
        ...
        public function upgradeLevelByCommissionOK($openid, $core_user = 0)
        {
            ...
            $info = $this->getInfo($shop_member['id'], array('pay'));
        ...
        public function sendMessage($openid = '', $core_user = '', $data = array(), $message_type = '')
        {
            global $_W;
            global $_GPC;
            $set        = $this->getSet();
            ...
            $member     = m('member')->getMember($openid, $core_user); // TODO 标志 m('member')->getMember true
            ...
            if (($message_type == TM_COMMISSION_AGENT_NEW) && empty($usernotice['commission_agent_new'])) {
                ...
                return $this->sendNotice($openid, $core_user, $tm, 'commission_agent_new_advanced', $data, $member, $msg);
            }
            if (($message_type == TM_COMMISSION_ORDER_PAY) && empty($usernotice['commission_order_pay'])) {
                $data['isagent'] = 0;
                if ($data['orderopenid'] == $openid) {
                    $agent           = $this->getAgentsByMember($openid, $core_user, 1);
                ...
                return $this->sendNotice($openid, $core_user, $tm, 'commission_order_pay_advanced', $data, $member, $msg);
            }
            if (($message_type == TM_COMMISSION_ORDER_FINISH) && empty($usernotice['commission_order_finish'])) {
                ...
                return $this->sendNotice($openid, $core_user, $tm, 'commission_order_finish_advanced', $data, $member, $msg);
            }
            if (($message_type == TM_COMMISSION_APPLY) && empty($usernotice['commission_apply'])) {
                ...
                return $this->sendNotice($openid, $core_user, $tm, 'commission_apply_advanced', $data, $member, $msg);
            }
            if (($message_type == TM_COMMISSION_CHECK) && empty($usernotice['commission_check'])) {
                ...
                return $this->sendNotice($openid, $core_user, $tm, 'commission_check_advanced', $data, $member, $msg);
            }
            if (($message_type == TM_COMMISSION_PAY) && empty($usernotice['commission_pay'])) {
                ...
                return $this->sendNotice($openid, $core_user, $tm, 'commission_pay_advanced', $data, $member, $msg);
            }
            if (($message_type == TM_COMMISSION_UPGRADE) && empty($usernotice['commission_upgrade'])) {
                ...
                $msg     = array('keyword1' => array('value' => (!empty($tm['commission_upgradetitle']) ? $tm['commission_upgradetitle'] : '营销等级升级通知'), 'color' => '#73a68d'), 'keyword2' => array('value' => $message, 'color' => '#73a68d'));
                return $this->sendNotice($openid, $core_user, $tm, 'commission_upgrade_advanced', $data, $member, $msg);
            }
            if (($message_type == TM_COMMISSION_BECOME) && empty($usernotice['commission_become'])) {
                ...
                $msg     = array('keyword1' => array('value' => (!empty($tm['commission_becometitle']) ? $tm['commission_becometitle'] : '成为营销商通知'), 'color' => '#73a68d'), 'keyword2' => array('value' => $message, 'color' => '#73a68d'));
                return $this->sendNotice($openid, $core_user, $tm, 'commission_become_advanced', $data, $member, $msg);
            }
        }
        ...
        protected function sendNotice($touser, $core_user, $tm, $tag, $datas, $member, $msg)
        {
            global $_W;
            if (!empty($tm['is_advanced']) && !empty($tm[$tag])) {
                ...
                if (!empty($advanced_template)) {
                    ...
                    $this->sendMoreAdvanced($touser, $core_user, $tm, $tag, $advanced_message, $url, $datas);
                }
            } else if (empty($msg['keyword2']['value'])) {
                return true;
            } else {
                $tag = str_replace('_advanced', '', $tag);
                $this->sendMore($touser, $core_user, $tm, $tag, $msg, $datas);
            }
            return true;
        }

        protected function sendMore($touser, $core_user, $tm, $tag, $msg, $datas)
        {
            $res                        = $this->getAgentsByMember($touser, $core_user, intval($tm[$tag . '_notice']));
            $set                        = $this->getSet();
            $msgbk                      = $msg;
        ...
        protected function sendMoreAdvanced($touser, $core_user, $tm, $tag, $msg, $url, $datas)
        {
            $res   = $this->getAgentsByMember($touser, $core_user, intval($tm[$tag . '_notice']));
            $set   = $this->getSet();
            $msgbk = $msg;
        ...
        protected function templateValue($member, $data)
        {
            ...
            return array(
                ...
                '[旧一级营销比例]' => (!empty($data['oldlevel']['commission1']) ? $data['oldlevel']['commission1'] . '%' : ''),
                '[旧二级营销比例]' => (!empty($data['oldlevel']['commission2']) ? $data['oldlevel']['commission2'] . '%' : ''),
                '[旧三级营销比例]' => (!empty($data['oldlevel']['commission3']) ? $data['oldlevel']['commission3'] . '%' : ''),
                '[新等级]'     => (!empty($data['newlevel']['levelname']) ? $data['newlevel']['levelname'] : ''),
                '[新一级营销比例]' => (!empty($data['newlevel']['commission1']) ? $data['newlevel']['commission1'] . '%' : ''),
                '[新二级营销比例]' => (!empty($data['newlevel']['commission2']) ? $data['newlevel']['commission2'] . '%' : ''),
                '[新三级营销比例]' => (!empty($data['newlevel']['commission3']) ? $data['newlevel']['commission3'] . '%' : ''),
                '[下线]'      => '[' . $set['texts']['down'] . ']'
            );
        }
        ...
        public function getLastApply($mid, $type = -1)
        {
            ...
            $sql    = 'select type,alipay,bankname,bankcard,realname from ' . tablename('superdesk_shop_commission_apply') . ' where mid=:mid and uniacid=:uniacid';
            if (-1 < $type) {
                $sql             .= ' and type=:type';
                $params[':type'] = $type;
            }
        ...
        public function getRepurchase($openid, $core_user, array $time)// TODO 标志 楼宇之窗 openid shop_order 已处理
        {
            global $_W;
            ...
            foreach ($time as $value) {
                ...
                if (!empty($repurchase_price)) {
                    $order_price = (double)pdo_fetchcolumn(
                        ' SELECT SUM(price) as price FROM ' . tablename('superdesk_shop_order') . // TODO 标志 楼宇之窗 openid shop_order 已处理
                        ' WHERE `uniacid`=:uniacid AND `openid`=:openid AND `core_user`=:core_user AND `status`>2 AND `createtime` BETWEEN :time1 AND :time2',
                        array(
                            ':uniacid'   => $_W['uniacid'],
                            ':openid'    => $openid,
                            ':core_user' => $core_user,
                    ...
                    $residue_price = (double)pdo_fetchcolumn(
                        'SELECT SUM(repurchase) FROM ' . tablename('superdesk_shop_commission_repurchase') .
