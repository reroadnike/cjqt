...
$order = pdo_fetch("SELECT * FROM " . tablename('superdesk_boardroom_4school_s_order') . " WHERE out_trade_no = :out_trade_no AND uniacid = :uniacid",
    array(
        ':out_trade_no' => $out_trade_no,
        ':uniacid' => $_W['uniacid']
    )
...
if (checksubmit('codsubmit')) {
    ...
    $ordergoods = pdo_fetchall("SELECT goodsid, total,optionid FROM " . tablename('superdesk_boardroom_4school_s_order_goods') . " WHERE orderid = '{$orderid}'",
        array(), 'goodsid'
    );
