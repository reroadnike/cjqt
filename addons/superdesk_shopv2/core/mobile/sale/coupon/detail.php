...
class Detail_SuperdeskShopV2Page extends MobileLoginPage
{
        ...
        if ($coupon['islimitlevel'] == 1) {
            ...
            if (!empty($coupon['limitmemberlevels']) || ($coupon['limitmemberlevels'] == '0')) {
                ...
                $level1 = pdo_fetchall(
                    ...
                    ' where ' .
                    '       uniacid=:uniacid ' .
                    '       and id in (' . $coupon['limitmemberlevels'] . ') ',
                    array(
                        ':uniacid' => $_W['uniacid']
                    )
                );
            ...
            if ((!empty($coupon['limitagentlevels']) || ($coupon['limitagentlevels'] == '0')) && $hascommission) {
                ...
                $level2 = pdo_fetchall(
                    ...
                    ' where ' .
                    '       uniacid=:uniacid ' .
                    '       and id in (' . $coupon['limitagentlevels'] . ') ',
                    array(
                        ':uniacid' => $_W['uniacid']
                    )
                );
            ...
            if ((!empty($coupon['limitpartnerlevels']) || ($coupon['limitpartnerlevels'] == '0')) && $hasglobonus) {
                ...
                $level3 = pdo_fetchall(
                    ...
                    ' where ' .
                    '       uniacid=:uniacid ' .
                    '       and id in(' . $coupon['limitpartnerlevels'] . ') ',
                    array(
                        ':uniacid' => $_W['uniacid']
                    )
                );
            ...
            if ((!empty($coupon['limitaagentlevels']) || ($coupon['limitaagentlevels'] == '0')) && $hasabonus) {
                ...
                $level4 = pdo_fetchall(
                    ...
                    ' where ' .
                    '       uniacid=:uniacid ' .
                    '       and id in (' . $coupon['limitaagentlevels'] . ') ',
                    array(
                        ':uniacid' => $_W['uniacid']
                    )
                );
    ...
    public function pay($a = array(), $b = array())
    {
        ...
        // 商户端
        // 领取设置 - 加入领券中心 - 说明 - 会员是否可以在领券中心直接领取或购买
        // 0 不可以 1 可以
        // 这个逻辑导致 : 加入领券中心 - 不可以时候 - 没有发放的coupon渠道的商户端,只是通过链接进去是无法领取的
        // 发放时会修正 gettype
        // 发放渠道有
        // 0 后台发放 1 领券中心 2 积分商城 3 超级海报 4 活动海报 5 口令优惠券
        // 还有就是商户端是没有发放界面的
        if (empty($coupon['gettype'])) {

            show_json(
                -1,
                '无法' . $coupon['gettypestr']
            );
        ...
        if (0 < $coupon['credit']) {
            $credit = m('member')->getCredit($_W['openid'], $_W['core_user'], 'credit1');
        ...
        if ($needpay) {
            ...
            if (!empty($coupon['usecredit2'])) {
                $money = m('member')->getCredit($_W['openid'], $_W['core_user'], 'credit2');
