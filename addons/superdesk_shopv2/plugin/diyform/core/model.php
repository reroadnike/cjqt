...
class DiyformModel extends PluginModel
{
    ...
    public function globalData()
    {
        ...
        return array(
            ...
            'category'            => pdo_fetchall(
                'select * ' .
                ' from ' . tablename('superdesk_shop_diyform_category') .
                ' where ' .
                '       uniacid=:uniacid ' .
                ' order by id desc',
                array(
    ...
    public function getLastData($type, $diymode, $diyformid, $cid, $fields, $member)
    {
        ...
        if (!empty($cid)) {
            ...
            $table_name = 'superdesk_shop_diyform_temp';// TODO 标志 楼宇之窗 openid shop_diyform_temp 已处理
            $sql        =
                'select * ' .
                ' from ' . tablename($table_name) .
                ' where ' .
                '       cid=:cid ' .
                '       and diyformid=:diyformid ' .
                '       and uniacid=:uniacid ' .
                '       and openid=:openid ' .
            ...
            if (empty($data)) {
                ...
                $table_name = 'superdesk_shop_order_goods';// TODO 标志 楼宇之窗 openid shop_order_goods 已处理
    ...
    public function getLastCartData($goodsid = 0)
    {
        ...
        if (!empty($cartdata)) {
            ...
            $f_data = $this->getDiyformData(
                $cartdata['diyformdata'],
                iunserializer($cartdata['diyformfields']),
                $member,
                1,
                1
            );
    ...
    public function addDataNum($id)
    {
        ...
        pdo_update('superdesk_shop_diyform_type', 'alldata=alldata+1', array('id' => $id));
    }
    ...
    public function getCountData($typeid, $type = 0)
    {
        ...
        $sql    =
            'select ' .
            '       count(*) ' .
            ' from ' . tablename('superdesk_shop_diyform_data') . // TODO 标志 楼宇之窗 openid shop_diyform_data 不处理
            ' where ' .
            '       typeid=:typeid ' .
            '       and uniacid=:uniacid';
        $params = array(
    ...
    public function getGoodsTemp($goodsid, $diyformid, $openid, $core_user, $type = 3)
    {
        global $_W;

        $sql    =
            'select * ' .
            ' from ' . tablename('superdesk_shop_diyform_temp') . // TODO 标志 楼宇之窗 openid shop_diyform_temp 已处理
            ' where ' .
            '       cid=:cid ' .
            '       and diyformid=:diyformid ' .
            '       and openid=:openid ' .
    ...
    public function getDiyformList()
    {
        ...
        $form_list = pdo_fetchall(
            'select * ' .
            ' from ' . tablename('superdesk_shop_diyform_type') .
            ' where ' .
            '       uniacid=:uniacid',
            array(
                ':uniacid' => $_W['uniacid']
    ...
    public function getOneDiyformTemp($id, $flag = 1)
    {
        ...
        $data = pdo_fetch(
            'select * ' .
            ' from ' . tablename('superdesk_shop_diyform_temp') . // TODO 标志 楼宇之窗 openid shop_diyform_temp 不处理
            ' where ' .
            '       id=:id ' .
            '       and uniacid=:uniacid ' .
            ' Limit 1',
