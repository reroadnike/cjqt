...
class Creditlog_SuperdeskShopV2Page extends PluginMobileLoginPage
{
    public function main()
    {
        ...
        $credit = intval(m('member')->getCredit($_W['openid'], 'credit1'));
    ...
    public function getlist()
    {
        ...
        $sql   = 'SELECT COUNT(*) FROM ' . tablename('superdesk_shop_creditshop_log') . ' log where 1 ' . $condition;// TODO 标志 楼宇之窗 openid shop_creditshop_log 已处理
        ...
        if (!empty($total)) {
            $sql =
                'SELECT log.id,log.goodsid,g.title,g.thumb,g.credit,g.type,g.money,log.createtime ' .
